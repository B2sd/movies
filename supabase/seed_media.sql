-- Generated by scripts/exportSupabaseSeed.mjs
-- Run supabase/schema.sql first.

insert into media_items (
  id,
  title_ru,
  title_original,
  type,
  year,
  poster_url,
  backdrop_url,
  description,
  genres,
  countries,
  tmdb_id,
  kinopoisk_id,
  imdb_id,
  kinopoisk_rating,
  imdb_rating,
  my_rating,
  my_review,
  watched_at,
  added_at,
  is_favorite,
  is_top,
  rewatch
)
values
(
  'media-1-1-0',
  '1+1',
  'Intouchables',
  'movie',
  2011,
  './posters/1-1-a5744f74.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a00000195040905dea964ff4bcf3e9c3df7/1344x756',
  'Пострадав в результате несчастного случая, богатый аристократ Филипп нанимает в помощники человека, который менее всего подходит для этой работы, – молодого жителя предместья Дрисса, только что освободившегося из тюрьмы. Несмотря на то, что Филипп прикован к инвалидному креслу, Дриссу удается привнести в размеренную жизнь аристократа дух приключений.',
  array['драма', 'комедия', 'биография']::text[],
  array['Франция']::text[],
  77338,
  535341,
  'tt1675434',
  8.8,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:00:00.000Z',
  false,
  false,
  false
),
(
  'media-джентльмены-1',
  'Джентльмены',
  'The Gentlemen',
  'movie',
  2019,
  './posters/джентльмены-db0b740a.jpg',
  'https://avatars.mds.yandex.net/get-ott/1648503/2a000001711b57abb795e9276957168f83e9/1344x756',
  'Один ушлый американец ещё со студенческих лет приторговывал наркотиками, а теперь придумал схему нелегального обогащения с использованием поместий обедневшей английской аристократии и очень неплохо на этом разбогател. Другой пронырливый журналист приходит к Рэю, правой руке американца, и предлагает тому купить киносценарий, в котором подробно описаны преступления его босса при участии других представителей лондонского криминального мира — партнёра-еврея, китайской диаспоры, чернокожих спортсменов и даже русского олигарха.',
  array['криминал', 'комедия', 'боевик']::text[],
  array[]::text[],
  522627,
  1143242,
  'tt8367814',
  8.7,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:00:01.000Z',
  false,
  false,
  false
),
(
  'media-гнев-человеческий-2',
  'Гнев человеческий',
  'Wrath of Man',
  'movie',
  2021,
  './posters/гнев-человеческий-63b96c15.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017b0699c684cc6e9767efb6d4b678/1344x756',
  'Грузовики лос-анджелесской инкассаторской компании Fortico Security часто подвергаются нападениям, и во время очередного ограбления погибают оба охранника. Через некоторое время в компанию устраивается крепкий немногословный британец Патрик Хилл. Он получает от босса прозвище Эйч и, впритык к необходимому минимуму пройдя тесты по фитнесу, стрельбе и вождению, отправляется на первое задание. Вскоре и его грузовик пытаются ограбить вооруженные налётчики, но Эйч в одиночку расправляется с целой бандой и становится героем. Кажется, слава и уважение коллег его совершенно не интересуют, ведь он преследует свои цели.',
  array['боевик', 'триллер']::text[],
  array[]::text[],
  637649,
  1318972,
  'tt11083552',
  7.7,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:00:02.000Z',
  false,
  false,
  false
),
(
  'media-брат-3',
  'Брат',
  'Брат',
  'movie',
  1997,
  './posters/брат-10351240.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000016127209a2837f7f55b295ef87323/1344x756',
  'Демобилизовавшись, Данила Багров возвращается в родной городок. Но скучная жизнь провинции его не устраивает, и он отправляется в Петербург, где, по слухам, уже несколько лет процветает его старший брат. Данила находит брата, но всё оказывается не так просто — брат работает наемным убийцей.',
  array['боевик', 'драма', 'криминал', 'музыка']::text[],
  array['Россия']::text[],
  20992,
  41519,
  'tt0118767',
  8.2,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:00:03.000Z',
  false,
  false,
  false
),
(
  'media-брат-2-4',
  'Брат 2',
  'Брат 2',
  'movie',
  2000,
  './posters/брат-2-c16b43fd.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000198a3d722f82233e91aad01acc8fa/1344x756',
  'Участвуя в программе на телевидении, Данила Багров встречает своих друзей по службе в Чечне. Одного из них внезапно убивают. Выясняется, что у того были неприятности из-за брата-хоккеиста в Америке. Данила должен разобраться. Он вылетает в Америку и за компанию берёт с собой старшего брата.',
  array['боевик', 'криминал']::text[],
  array['Россия', 'США']::text[],
  20993,
  41520,
  'tt0238883',
  8.1,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:00:04.000Z',
  false,
  false,
  false
),
(
  'media-волк-с-уолл-стрит-5',
  'Волк с Уолл-стрит',
  'The Wolf of Wall Street',
  'movie',
  2013,
  './posters/волк-с-уолл-стрит-8806e377.jpg',
  'https://avatars.mds.yandex.net/get-ott/13051577/2a00000197c16c99ec5791b09a7320e1b674/1344x756',
  '1987 год. Джордан Белфорт становится брокером в успешном инвестиционном банке. Вскоре банк закрывается после внезапного обвала индекса Доу-Джонса. По совету жены Терезы Джордан устраивается в небольшое заведение, занимающееся мелкими акциями. Его настойчивый стиль общения с клиентами и врождённая харизма быстро даёт свои плоды. Он знакомится с соседом по дому Донни, торговцем, который сразу находит общий язык с Джорданом и решает открыть с ним собственную фирму. В качестве сотрудников они нанимают нескольких друзей Белфорта, его отца Макса и называют компанию «Стрэттон Оукмонт». В свободное от работы время Джордан прожигает жизнь: лавирует от одной вечеринки к другой, вступает в сексуальные отношения с проститутками, употребляет множество наркотических препаратов, в том числе кокаин и кваалюд. Однажды наступает момент, когда быстрым обогащением Белфорта начинает интересоваться агент ФБР...',
  array['драма', 'комедия', 'криминал', 'биография']::text[],
  array['США']::text[],
  106646,
  462682,
  'tt0993846',
  7.8,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:00:05.000Z',
  false,
  false,
  false
),
(
  'media-игра-престолов-6',
  'Игра престолов',
  'Game of Thrones',
  'series',
  2011,
  './posters/игра-престолов-75c1d49d.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000016a9809ff1d0e4d9ababb6a2085e0/1344x756',
  'К концу подходит время благоденствия, и лето, длившееся почти десятилетие, угасает. Вокруг средоточия власти Семи королевств, Железного трона, зреет заговор, и в это непростое время король решает искать поддержки у друга юности Эддарда Старка. В мире, где все — от короля до наемника — рвутся к власти, плетут интриги и готовы вонзить нож в спину, есть место и благородству, состраданию и любви. Между тем никто не замечает пробуждение тьмы из легенд далеко на Севере — и лишь Стена защищает живых к югу от нее.',
  array['фэнтези', 'драма', 'приключения']::text[],
  array['США']::text[],
  1399,
  464963,
  'tt0944947',
  9,
  9.2,
  null,
  null,
  null,
  '2026-06-21T12:00:06.000Z',
  false,
  false,
  false
),
(
  'media-достать-ножи-7',
  'Достать ножи',
  'Knives Out',
  'movie',
  2019,
  './posters/достать-ножи-a50e3a73.jpg',
  'https://avatars.mds.yandex.net/get-ott/1648503/2a00000198a30ce65d3e21a89a074b3cf0fc/1344x756',
  'На следующее утро после празднования 85-летия известного автора криминальных романов Харлана Тромби виновника торжества находят мёртвым. Налицо — явное самоубийство, но полиция по протоколу опрашивает всех присутствующих в особняке членов семьи, хотя, в этом деле больше заинтересован частный детектив Бенуа Блан. Тем же утром он получил конверт с наличными от неизвестного и заказ на расследование смерти Харлана. Не нужно быть опытным следователем, чтобы понять, что все приукрашивают свои отношения с почившим главой семейства, но Блану достаётся настоящий подарок — медсестра покойного, которая физически не выносит ложь.',
  array['детектив', 'комедия', 'драма', 'криминал']::text[],
  array[]::text[],
  546554,
  1188529,
  'tt8946378',
  8.2,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:00:07.000Z',
  false,
  false,
  false
),
(
  'media-зеленая-книга-8',
  'Зеленая книга',
  'Green Book',
  'movie',
  2018,
  './posters/зеленая-книга-88d9d158.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a000001989ebae65cb92cd1ea74d24d906d/1344x756',
  '1960-е годы. После закрытия нью-йоркского ночного клуба на ремонт вышибала Тони по прозвищу Болтун ищет подработку на пару месяцев. Как раз в это время Дон Ширли — утонченный светский лев, богатый и талантливый чернокожий музыкант, исполняющий классическую музыку — собирается в турне по южным штатам, где ещё сильны расистские убеждения и царит сегрегация. Он нанимает Тони в качестве водителя, телохранителя и человека, способного решать текущие проблемы. У этих двоих так мало общего, и эта поездка навсегда изменит жизнь обоих.',
  array['биография', 'комедия', 'драма']::text[],
  array[]::text[],
  490132,
  1108577,
  'tt6966692',
  8.5,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:00:08.000Z',
  false,
  false,
  false
),
(
  'media-остров-проклятых-9',
  'Остров проклятых',
  'Shutter Island',
  'movie',
  2009,
  './posters/остров-проклятых-52166313.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017b3facc50cd2fab4e9aec3f590ad/1344x756',
  'Два американских судебных пристава отправляются на один из островов в штате Массачусетс, чтобы расследовать исчезновение пациентки клиники для умалишенных преступников. При проведении расследования им придется столкнуться с паутиной лжи, обрушившимся ураганом и смертельным бунтом обитателей клиники.',
  array['триллер', 'детектив', 'драма']::text[],
  array['США', 'Канада']::text[],
  null,
  397667,
  null,
  8.6,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:00:09.000Z',
  false,
  false,
  false
),
(
  'media-один-дома-10',
  'Один дома',
  'Home Alone',
  'movie',
  1990,
  './posters/один-дома-7805f81c.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017672cc1b8d4660336ae565af15fd/1344x756',
  'Американское семейство отправляется из Чикаго в Европу, но в спешке сборов бестолковые родители забывают дома... одного из своих детей. Юное создание, однако, не теряется и демонстрирует чудеса изобретательности. И когда в дом залезают грабители, им приходится не раз пожалеть о встрече с милым крошкой.',
  array['комедия', 'семейный']::text[],
  array[]::text[],
  771,
  8124,
  'tt0099785',
  8.3,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:00:10.000Z',
  false,
  false,
  false
),
(
  'media-один-дома-2-11',
  'Один дома 2',
  'Один дома 2',
  'movie',
  1992,
  './posters/один-дома-2-dab24c0e.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a000001612cb1fd548510a3a296ffde6bdd/1344x756',
  'Самый маленький герой Америки устраивает большой переполох в Нью-Йорке! Кевин МакКалистер вернулся! Но теперь он один не дома, а в Нью-Йорке и у него достаточно денег и кредитных карточек, чтобы превратить Большое Яблоко в собственную площадку для игр.

Но, как всегда, Кевину не суждено быть долго одному: его старые приятели ― жулики Гарри и Марв ― сбежали из тюрьмы, куда они попали стараниями Кевина. И надо же им было попасть именно в тот город, где Кевин планировал поразвлечься! Новые западни и ловушки уже ждут горе-бандитов.',
  array['комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  772,
  5928,
  'tt0104431',
  7.9,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:00:11.000Z',
  false,
  false,
  false
),
(
  'media-иван-васильевич-меняет-профессию-12',
  'Иван Васильевич меняет профессию',
  'Иван Васильевич меняет профессию',
  'movie',
  1973,
  './posters/иван-васильевич-меняет-профессию-e8a44982.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a000001616b8bee8c782e00a41555f383b9/1344x756',
  'Инженер-изобретатель Тимофеев сконструировал машину времени, которая соединила его квартиру с XVI веком — точнее, с палатами государя Ивана Грозного. Туда-то и попадают тёзка царя пенсионер-общественник Иван Васильевич Бунша и квартирный вор Жорж Милославский, а сам великий государь оказывается в квартире Тимофеева.',
  array['комедия', 'фантастика', 'приключения']::text[],
  array[]::text[],
  20803,
  42664,
  'tt0070233',
  8.8,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:00:12.000Z',
  false,
  false,
  false
),
(
  'media-операция-ы-и-другие-приключения-шурика-13',
  'Операция «Ы» и другие приключения Шурика',
  'Операция «Ы» и другие приключения Шурика',
  'movie',
  1965,
  './posters/операция-ы-и-другие-приключения-шурика-02837aa8.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017332fe43abb5852312cb62ce9851/1344x756',
  'Студент Шурик попадает в самые невероятные ситуации: сражается с хулиганом Верзилой, весьма оригинальным способом готовится к экзамену и предотвращает «ограбление века», на которое идёт троица бандитов — Балбес, Трус и Бывалый.',
  array['комедия', 'мелодрама', 'криминал']::text[],
  array[]::text[],
  20875,
  42782,
  'tt0059550',
  8.8,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:00:13.000Z',
  false,
  false,
  false
),
(
  'media-бойцовский-клуб-14',
  'Бойцовский клуб',
  'Fight Club',
  'movie',
  1999,
  './posters/бойцовский-клуб-37cbc4f5.jpg',
  'https://avatars.mds.yandex.net/get-ott/13051577/2a0000019a2f24c36d41f38513deaf87cecd/1344x756',
  'Сотрудник страховой компании страдает хронической бессонницей и отчаянно пытается вырваться из мучительно скучной жизни. Однажды в очередной командировке он встречает некоего Тайлера Дёрдена — харизматического торговца мылом с извращенной философией. Тайлер уверен, что самосовершенствование — удел слабых, а единственное, ради чего стоит жить, — саморазрушение.

Проходит немного времени, и вот уже новые друзья лупят друг друга почем зря на стоянке перед баром, и очищающий мордобой доставляет им высшее блаженство. Приобщая других мужчин к простым радостям физической жестокости, они основывают тайный Бойцовский клуб, который начинает пользоваться невероятной популярностью.',
  array['триллер', 'драма', 'криминал']::text[],
  array['США', 'Германия']::text[],
  550,
  361,
  'tt0137523',
  8.7,
  8.8,
  null,
  null,
  null,
  '2026-06-21T12:00:14.000Z',
  false,
  false,
  false
),
(
  'media-анора-15',
  'Анора',
  'Anora',
  'movie',
  2024,
  './posters/анора-56f5a404.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a000001957ee51f1799fefe469d3417e7f2/1344x756',
  'Бруклин. Стриптизерша Анора, предпочитающая имя Эни, знакомится с сыном русского олигарха Ваней, который проводит всё время в вечеринках и развлечениях. Вскоре парень выкупает время Эни на неделю, предлагает слетать в Вегас, а там молодые люди спонтанно женятся. Узнав об этом из газет, родители Вани приходят в ярость и срочно вылетают из России в США, а разобраться в ситуации велят нью-йоркскому армянину Торосу, который должен был присматривать за их сыном.',
  array['драма', 'мелодрама', 'комедия']::text[],
  array['США']::text[],
  null,
  5405057,
  'tt11044414',
  6.4,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:00:15.000Z',
  false,
  false,
  false
),
(
  'media-легенда-16',
  'Легенда',
  'Legend',
  'movie',
  2015,
  './posters/легенда-9b998f19.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000182f3ff694c60038e842e4aa0c0d0/1344x756',
  'Близнецы Реджи и Ронни Крэй — культовые фигуры преступного мира Великобритании 1960-х. Братья возглавляли самую влиятельную бандитскую группировку Ист-Энда. В их послужном списке были вооруженные грабежи, рэкет, поджоги, покушения, убийства и собственный ночной клуб, куда доезжали даже голливудские знаменитости. Среди их жертв — криминальные авторитеты Джек МакВитти и Джордж Корнелл.',
  array['криминал', 'триллер', 'драма']::text[],
  array[]::text[],
  276907,
  839954,
  'tt3569230',
  7.3,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:00:16.000Z',
  false,
  false,
  false
),
(
  'media-маша-и-медведь-17',
  'Маша и Медведь',
  'Маша и Медведь',
  'cartoon',
  2009,
  './posters/маша-и-медведь-08e61418.jpg',
  'https://avatars.mds.yandex.net/get-ott/13074011/2a00000198ed57083002c2e09c6068ea5c24/1344x756',
  'О дружбе и приключениях маленькой озорной девочки Маши и ее заботливого друга Медведя. Их дружба является олицетворением комичных взаимоотношений между ребенком, исследующим окружающий мир, и взрослым, поддерживающим его в этом увлекательном приключении.',
  array['мультфильм', 'комедия', 'детский']::text[],
  array[]::text[],
  46080,
  478491,
  'tt1884856',
  7.3,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:00:17.000Z',
  false,
  false,
  false
),
(
  'media-аватар-18',
  'Аватар',
  'Avatar',
  'movie',
  2009,
  './posters/аватар-fd59434c.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a000001616b87458162c9216ccd5144e94d/1344x756',
  'Бывший морпех Джейк Салли прикован к инвалидному креслу. Несмотря на немощное тело, Джейк в душе по-прежнему остается воином. Он получает задание совершить путешествие в несколько световых лет к базе землян на планете Пандора, где корпорации добывают редкий минерал, имеющий огромное значение для выхода Земли из энергетического кризиса.',
  array['фантастика', 'боевик', 'драма', 'приключения']::text[],
  array['США', 'Великобритания']::text[],
  19995,
  251733,
  'tt0499549',
  8,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:00:18.000Z',
  false,
  false,
  false
),
(
  'media-ну-погоди-19',
  'Ну, погоди!',
  'Ну, погоди!',
  'cartoon',
  1969,
  './posters/ну-погоди-8a6537a8.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000016513e6f85a22e34b6071666844b9/1344x756',
  'Веселые приключения неразлучной парочки - хулигана Волка и смышленого Зайца. Любимые с детства сцены погонь, ссор и примирений, шутки и мелодии.',
  array['мультфильм', 'комедия', 'приключения', 'детский']::text[],
  array[]::text[],
  6138,
  46483,
  'tt0234355',
  8.7,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:00:19.000Z',
  false,
  false,
  false
),
(
  'media-интерстеллар-20',
  'Интерстеллар',
  'Interstellar',
  'movie',
  2014,
  './posters/интерстеллар-ad03aff2.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a000001862c40fa5c5394735529ee7e7188/1344x756',
  'Когда засуха, пыльные бури и вымирание растений приводят человечество к продовольственному кризису, коллектив исследователей и учёных отправляется сквозь червоточину (которая предположительно соединяет области пространства-времени через большое расстояние) в путешествие, чтобы превзойти прежние ограничения для космических путешествий человека и найти планету с подходящими для человечества условиями.',
  array['фантастика', 'драма', 'приключения']::text[],
  array['США', 'Великобритания']::text[],
  157336,
  258687,
  'tt0816692',
  8.6,
  8.7,
  null,
  null,
  null,
  '2026-06-21T12:00:20.000Z',
  false,
  false,
  false
),
(
  'media-побег-из-шоушенка-21',
  'Побег из Шоушенка',
  'The Shawshank Redemption',
  'movie',
  1994,
  './posters/побег-из-шоушенка-34e9c273.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000016b03d1f5365474a90d26998e2a9f/1344x756',
  'Бухгалтер Энди Дюфрейн обвинён в убийстве собственной жены и её любовника. Оказавшись в тюрьме под названием Шоушенк, он сталкивается с жестокостью и беззаконием, царящими по обе стороны решётки. Каждый, кто попадает в эти стены, становится их рабом до конца жизни. Но Энди, обладающий живым умом и доброй душой, находит подход как к заключённым, так и к охранникам, добиваясь их особого к себе расположения.',
  array['драма', 'криминал']::text[],
  array['США']::text[],
  278,
  326,
  'tt0111161',
  9.1,
  9.3,
  null,
  null,
  null,
  '2026-06-21T12:00:21.000Z',
  false,
  false,
  false
),
(
  'media-зверополис-22',
  'Зверополис',
  'Zootopia',
  'cartoon',
  2016,
  './posters/зверополис-3a071c13.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000168f019d937828afe5aed1f710478/1344x756',
  'Добро пожаловать в Зверополис – современный город, населенный самыми разными животными, от огромных слонов до крошечных мышек. Зверополис разделен на районы, полностью повторяющие естественную среду обитания разных жителей – здесь есть и элитный район Площадь Сахары и неприветливый Тундратаун. В этом городе появляется новый офицер полиции, жизнерадостная зайчиха Джуди Хоппс, которая с первых дней работы понимает, как сложно быть маленькой и пушистой среди больших и сильных полицейских. Джуди хватается за первую же возможность проявить себя, несмотря на то, что ее партнером будет болтливый хитрый лис Ник Уайлд. Вдвоем им предстоит раскрыть сложное дело, от которого будет зависеть судьба всех обитателей Зверополиса.',
  array['мультфильм', 'боевик', 'комедия', 'криминал', 'приключения', 'семейный']::text[],
  array['США']::text[],
  269149,
  775276,
  'tt2948356',
  8.6,
  8,
  null,
  null,
  null,
  '2026-06-21T12:00:22.000Z',
  false,
  false,
  false
),
(
  'media-начало-23',
  'Начало',
  'Inception',
  'movie',
  2010,
  './posters/начало-1c619a23.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a00000178c5fc1db03b03dceda170357652/1344x756',
  'Кобб – талантливый вор, лучший из лучших в опасном искусстве извлечения: он крадет ценные секреты из глубин подсознания во время сна, когда человеческий разум наиболее уязвим. Редкие способности Кобба сделали его ценным игроком в привычном к предательству мире промышленного шпионажа, но они же превратили его в извечного беглеца и лишили всего, что он когда-либо любил. 

И вот у Кобба появляется шанс исправить ошибки. Его последнее дело может вернуть все назад, но для этого ему нужно совершить невозможное – инициацию. Вместо идеальной кражи Кобб и его команда спецов должны будут провернуть обратное. Теперь их задача – не украсть идею, а внедрить ее. Если у них получится, это и станет идеальным преступлением. 

Но никакое планирование или мастерство не могут подготовить команду к встрече с опасным противником, который, кажется, предугадывает каждый их ход. Врагом, увидеть которого мог бы лишь Кобб.',
  array['фантастика', 'боевик', 'триллер', 'драма', 'детектив']::text[],
  array['США', 'Великобритания']::text[],
  314714,
  447301,
  'tt1375666',
  8.7,
  8.8,
  null,
  null,
  null,
  '2026-06-21T12:00:23.000Z',
  false,
  false,
  false
),
(
  'media-зеленая-миля-24',
  'Зеленая миля',
  'The Green Mile',
  'movie',
  1999,
  './posters/зеленая-миля-e042fbb0.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000169e39ef77f588ccdfe574dae8227/1344x756',
  'Пол Эджкомб — начальник блока смертников в тюрьме «Холодная гора», каждый из узников которого однажды проходит «зеленую милю» по пути к месту казни. Пол повидал много заключённых и надзирателей за время работы. Однако гигант Джон Коффи, обвинённый в страшном преступлении, стал одним из самых необычных обитателей блока.',
  array['фэнтези', 'драма', 'криминал', 'детектив']::text[],
  array['США']::text[],
  497,
  435,
  'tt0120689',
  9.1,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:00:24.000Z',
  false,
  false,
  false
),
(
  'media-области-тьмы-25',
  'Области тьмы',
  'Limitless',
  'movie',
  2011,
  './posters/области-тьмы-1caca3dc.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a00000198a8dcc27d1b6b3d3b04ad49a783/1344x756',
  'Нью-йоркский писатель Эдди, желая преодолеть чёрную полосу в жизни, принимает засекреченный препарат под названием NZT. Таблетка выводит мозг парня на работу в нереальной мощности. Этот творческий наркотик меняет всю жизнь Эдди, за короткий срок он зарабатывает кучу денег, но скоро начинает страдать от зловещих побочных эффектов препарата. А когда пытается найти других NZT-гениев, чтобы понять, как можно справиться с этим пристрастием, он узнает страшную правду…',
  array['фантастика', 'триллер', 'детектив']::text[],
  array['США']::text[],
  51876,
  462606,
  'tt1219289',
  8,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:00:25.000Z',
  false,
  false,
  false
),
(
  'media-субстанция-26',
  'Субстанция',
  'The Substance',
  'movie',
  2024,
  './posters/субстанция-870c440c.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a0000019305ad148aa6309f251d8767acf7/1344x756',
  'Слава голливудской звезды Элизабет Спаркл осталась в прошлом, хотя она всё ещё ведёт фитнес-шоу на телевидении. Когда её передачу собираются перезапустить с ведущей помоложе, от отчаяния Элизабет решает принять инновационный препарат, создатели которого обещают невероятное преображение, но при соблюдении правила одной недели. Так на свет появляется молодая красотка Сью. Девушка начинает делать карьеру в шоу-бизнесе и вскоре решает, что семи дней ей маловато.',
  array['ужасы', 'драма']::text[],
  array[]::text[],
  933260,
  4860213,
  'tt17526714',
  6.3,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:00:26.000Z',
  false,
  false,
  false
),
(
  'media-слово-пацана-кровь-на-асфальте-27',
  'Слово пацана. Кровь на асфальте',
  'Слово пацана. Кровь на асфальте',
  'series',
  2023,
  './posters/слово-пацана-кровь-на-асфальте-28fb2383.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a00000194034ee83d8ddb3d473287c8094e/1344x756',
  'Казань, начало 1989 года. Пока родители борются за выживание, подростки сбиваются в уличные банды и бьются за асфальт. Буквально, чтобы контролировать всё, что стоит на «их» земле или передвигается по ней. Среди всеобщей нищеты — чёткие правила жизни, поддержка и слово пацана, которое сильнее клятвы.
14-летний Андрей из интеллигентной семьи, он учится в музыкальной школе и живёт с мамой и маленькой сестрёнкой. Однажды парня назначают подтянуть по английскому сверстника Марата — гопника из местной группировки. Пообщавшись с нормальным пацаном и решив, что ему надоело быть чушпаном, Андрей вступает в банду.',
  array['драма', 'криминал']::text[],
  array[]::text[],
  234763,
  5304403,
  'tt28429213',
  8.1,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:00:27.000Z',
  false,
  false,
  false
),
(
  'media-вышка-28',
  'Вышка',
  'Fall',
  'movie',
  2022,
  './posters/вышка-45d98da8.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000018383a57402bd749cdf4a8d130f2f/1344x756',
  'После гибели Дэна в результате падения со скалы его жена Бекки, ранее увлекавшаяся экстремальными развлечениями, впала в депрессию. Год спустя девушка всё ещё не может прийти в себя и регулярно заливает горе алкоголем, когда в её жизни внезапно появляется старая боевая подруга Хантер. Она предлагает почтить память Дэна, забравшись на телерадиомачту B67 и развеяв там его прах. Девушки отправляются к самому высокому сооружению Соединённых Штатов, даже не представляя, с чем им придётся столкнуться на его верхушке.',
  array['триллер']::text[],
  array[]::text[],
  985939,
  4542208,
  'tt15325794',
  6.8,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:00:28.000Z',
  false,
  false,
  false
),
(
  'media-джентльмены-удачи-29',
  'Джентльмены удачи',
  'Джентльмены удачи',
  'movie',
  1971,
  './posters/джентльмены-удачи-33f5b09d.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a000001616b8d3733de88938381c05fa48f/1344x756',
  'Заведующему детсадом Трошкину фатально не повезло: он оказался как две капли воды похож на бандита по кличке «Доцент», похитившего уникальный шлем Александра Македонского.

Милиция внедряет добряка Трошкина в воровскую среду - и ему ничего не остается, кроме как старательно изображать своего двойника-злодея, путая всех окружающих. Со временем он настолько блестяще входит в роль, что сам начинает порой приходить в ужас. Между тем, жизни его угрожает смертельная опасность...',
  array['комедия', 'драма', 'криминал', 'детектив']::text[],
  array[]::text[],
  20871,
  44386,
  'tt0068519',
  8.6,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:00:29.000Z',
  false,
  false,
  false
),
(
  'media-форрест-гамп-30',
  'Форрест Гамп',
  'Forrest Gump',
  'movie',
  1994,
  './posters/форрест-гамп-49cf66dc.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a00000196499920424538b6ed0b21735b8e/1344x756',
  'Сидя на автобусной остановке, Форрест Гамп — не очень умный, но добрый и открытый парень — рассказывает случайным встречным историю своей необыкновенной жизни.

С самого малолетства парень страдал от заболевания ног, соседские мальчишки дразнили его, но в один прекрасный день Форрест открыл в себе невероятные способности к бегу. Подруга детства Дженни всегда его поддерживала и защищала, но вскоре дороги их разошлись.',
  array['драма', 'мелодрама']::text[],
  array['США']::text[],
  13,
  448,
  'tt0109830',
  8.9,
  8.8,
  null,
  null,
  null,
  '2026-06-21T12:00:30.000Z',
  false,
  false,
  false
),
(
  'media-законопослушный-гражданин-31',
  'Законопослушный гражданин',
  'Law Abiding Citizen',
  'movie',
  2009,
  './posters/законопослушный-гражданин-33506423.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000160560b084db122bc04bcc2a69c31/1344x756',
  'Семью Клайда Шелтона убивают двое грабителей. Однако они не получают по заслугам — прокурор идет на сделку с преступниками. Тогда Клайд решает по-своему отомстить убийцам, а также представителям власти и юстиции США.',
  array['триллер', 'драма', 'криминал']::text[],
  array['США']::text[],
  22803,
  406408,
  'tt1197624',
  8,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:00:31.000Z',
  false,
  false,
  false
),
(
  'media-веном-32',
  'Веном',
  'Venom',
  'movie',
  2018,
  './posters/веном-15f53b4e.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000169e7b34b79b989dfa1f452acf75c/1344x756',
  'Что, если в один прекрасный день в тебя вселится симбиот, который наделит тебя сверхчеловеческими способностями? Вот только Веном — симбиот совсем недобрый, и договориться с ним невозможно. А нужно ли договариваться? Ведь в какой-то момент ты понимаешь, что быть плохим вовсе не так уж и плохо, так даже веселее. А в мире и так слишком много супергероев.',
  array['фантастика', 'боевик', 'ужасы']::text[],
  array[]::text[],
  335983,
  463634,
  'tt1270797',
  6.9,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:00:32.000Z',
  false,
  false,
  false
),
(
  'media-иллюзия-обмана-33',
  'Иллюзия обмана',
  'Now You See Me',
  'movie',
  2013,
  './posters/иллюзия-обмана-a418b403.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000160560c398022fe9efa3399a66b02/1344x756',
  'Команда лучших иллюзионистов мира проворачивает дерзкие ограбления прямо во время своих шоу, играя в кошки-мышки с агентами ФБР.',
  array['триллер', 'криминал', 'детектив']::text[],
  array['Франция', 'США']::text[],
  75656,
  522892,
  'tt1670345',
  7.7,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:00:33.000Z',
  false,
  false,
  false
),
(
  'media-атака-титанов-34',
  'Атака титанов',
  'Shingeki no kyojin',
  'anime',
  2013,
  './posters/атака-титанов-f311ed47.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a00000186a8b7a1951185c6175ed0f07fd0/1344x756',
  'Уже многие годы человечество ведёт борьбу с титанами — огромными существами, которые не обладают особым интеллектом, зато едят людей и получают от этого удовольствие. После продолжительной борьбы остатки человечества построили высокую стену, окружившую страну людей, через которую титаны пройти не могли. С тех пор прошло сто лет, люди мирно живут под защитой стены. Но однажды подростки Эрен и Микаса становятся свидетелями страшного события — участок стены разрушается супертитаном, появившимся прямо из воздуха. Титаны нападают на город, и дети в ужасе видят, как один из монстров заживо съедает мать Эрена. Мальчик клянётся, что убьёт всех титанов и отомстит за человечество.',
  array['аниме', 'мультфильм', 'фантастика', 'драма', 'боевик', 'фэнтези']::text[],
  array['Япония']::text[],
  null,
  749374,
  'tt2560140',
  8.7,
  9.1,
  null,
  null,
  null,
  '2026-06-21T12:00:34.000Z',
  false,
  false,
  false
),
(
  'media-бриллиантовая-рука-35',
  'Бриллиантовая рука',
  'Бриллиантовая рука',
  'movie',
  1968,
  './posters/бриллиантовая-рука-262eb319.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000016e8d9e10ce1e85a4f1f9d35e52be/1344x756',
  'В южном городке орудует шайка валютчиков, возглавляемая Шефом и его помощником Графом (в быту — Геной Козодоевым). Скромный советский служащий и примерный семьянин Семен Семенович Горбунков отправляется в зарубежный круиз на теплоходе, где также плывет Граф, который должен забрать бриллианты в одном из восточных городов и провезти их в загипсованной руке. Но из-за недоразумения вместо жулика на условленном месте падает ничего не подозревающий Семен Семенович, и драгоценный гипс накладывают ему.',
  array['комедия', 'криминал']::text[],
  array['СССР']::text[],
  null,
  46225,
  null,
  8.6,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:00:35.000Z',
  false,
  false,
  false
),
(
  'media-кавказская-пленница-или-новые-приключения-шурика-36',
  'Кавказская пленница, или Новые приключения Шурика',
  'Кавказская пленница, или Новые приключения Шурика',
  'movie',
  1966,
  './posters/кавказская-пленница-или-новые-приключения-шурика-ef298a36.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a000001616b8d7f5aeda7d7d189284f708c/1344x756',
  'Отправившись в одну из горных республик собирать фольклор, студент Шурик влюбляется в симпатичную Нину — спортсменку, отличницу, комсомолку и просто красавицу. Однако банда из трёх человек похищает девушку, чтобы насильно выдать замуж. Поняв, что происходит, Шурик отважно бросается освобождать кавказскую пленницу.',
  array['комедия', 'приключения', 'мелодрама', 'мюзикл']::text[],
  array['СССР']::text[],
  null,
  44745,
  null,
  8.5,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:00:36.000Z',
  false,
  false,
  false
),
(
  'media-леон-37',
  'Леон',
  'Léon',
  'movie',
  1994,
  './posters/леон-eea317b3.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001893576b73adc6564bfd0f54bb09f/1344x756',
  'Профессиональный убийца Леон неожиданно для себя самого решает помочь 12-летней соседке Матильде, семью которой убили коррумпированные полицейские.',
  array['триллер', 'драма', 'криминал']::text[],
  array['Франция']::text[],
  101,
  389,
  'tt0110413',
  8.7,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:00:37.000Z',
  false,
  false,
  false
),
(
  'media-паразиты-38',
  'Паразиты',
  'Gisaengchung',
  'movie',
  2019,
  './posters/паразиты-39e283f8.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017bd4880ba5fb76f8682846e528d9/1344x756',
  'Обычное корейское семейство Кимов жизнь не балует. Приходится жить в сыром грязном полуподвале, воровать интернет у соседей и перебиваться случайными подработками. Однажды друг сына семейства, уезжая на стажировку за границу, предлагает тому заменить его и поработать репетитором у старшеклассницы в богатой семье Пак. Подделав диплом о высшем образовании, парень отправляется в шикарный дизайнерский особняк и производит на хозяйку дома хорошее впечатление. Тут же ему в голову приходит необычный план по трудоустройству сестры.',
  array['драма', 'триллер', 'комедия']::text[],
  array[]::text[],
  496243,
  1043758,
  'tt6751668',
  8.1,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:00:38.000Z',
  false,
  false,
  false
),
(
  'media-москва-слезам-не-верит-39',
  'Москва слезам не верит',
  'Москва слезам не верит',
  'movie',
  1979,
  './posters/москва-слезам-не-верит-fda65a11.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000161adc652b03b55793b175d20154a/1344x756',
  'Москва, 1950-е годы. Три молодые провинциалки приезжают в Москву в поисках того, что ищут люди во всех столицах мира — любви, счастья и достатка. Антонина выходит замуж, растит детей, любит мужа. Людмиле Москва представляется лотереей, в которой она должна выиграть свое особенное счастье. Катерина же отчаянно влюбляется, но избранник ее оставляет. Однако она не опускает руки, в одиночку растит дочь и к тому же успевает делать блестящую карьеру. В 40 лет судьба дарит ей неожиданную встречу.',
  array['мелодрама', 'драма', 'комедия']::text[],
  array['СССР']::text[],
  null,
  46708,
  null,
  8.4,
  8,
  null,
  null,
  null,
  '2026-06-21T12:00:39.000Z',
  false,
  false,
  false
),
(
  'media-шрэк-40',
  'Шрэк',
  'Shrek',
  'cartoon',
  2001,
  './posters/шрэк-7311ee3f.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a000001706322c391c291c8b05829f08a14/1344x756',
  'Жил да был в сказочном государстве большой зеленый великан по имени Шрэк. Жил он в гордом одиночестве в лесу, на болоте, которое считал своим. Но однажды злобный коротышка — лорд Фаркуад, правитель волшебного королевства, безжалостно согнал на Шрэково болото всех сказочных обитателей.
И беспечной жизни зеленого великана пришел конец. Но лорд Фаркуад пообещал вернуть Шрэку болото, если великан добудет ему прекрасную принцессу Фиону, которая томится в неприступной башне, охраняемой огнедышащим драконом.',
  array['мультфильм', 'фэнтези', 'мелодрама', 'комедия', 'приключения', 'семейный']::text[],
  array[]::text[],
  808,
  430,
  'tt0126029',
  8.2,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:00:40.000Z',
  false,
  false,
  false
),
(
  'media-тачки-41',
  'Тачки',
  'Cars',
  'cartoon',
  2006,
  './posters/тачки-6194dae0.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000016928a36b0d65370954a65f765e0f/1344x756',
  'Неукротимый в своем желании всегда и во всем побеждать гоночный автомобиль «Молния» Маккуин вдруг обнаруживает, что сбился с пути и застрял в маленьком захолустном городке Радиатор-Спрингс, что находится где-то на трассе 66 в Калифорнии.

Участвуя в гонках на Кубок Поршня, где ему противостояли два очень опытных соперника, Маккуин совершенно не ожидал, что отныне ему придется общаться с персонажами совсем иного рода. Это, например, Салли — шикарный Порше 2002-го года выпуска, Док Хадсон — легковушка модели «Хадсон Хорнет», 1951-го года выпуска или Метр — ржавый грузовичок-эвакуатор. И все они помогают Маккуину понять, что в мире существуют некоторые более важные вещи, чем слава, призы и спонсоры…',
  array['мультфильм', 'комедия', 'приключения', 'семейный', 'спорт']::text[],
  array['США']::text[],
  920,
  61249,
  'tt0317219',
  7.5,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:00:41.000Z',
  false,
  false,
  false
),
(
  'media-мстители-война-бесконечности-42',
  'Мстители: Война бесконечности',
  'Мстители: Война бесконечности',
  'movie',
  2012,
  './posters/мстители-война-бесконечности-6601e7b3.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000168e7cbf1bf723879ea306ad8f363/1344x756',
  'Локи возвращается, и в этот раз он не один. Земля оказывается на грани порабощения, и только лучшие из лучших могут спасти человечество. Глава международной организации Щ. И. Т. Ник Фьюри собирает выдающихся защитников справедливости и добра. Под предводительством Капитана Америки Железный Человек, Тор, Невероятный Халк, Соколиный Глаз и Чёрная Вдова вступают в войну с захватчиком.',
  array['фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  24428,
  263531,
  'tt0848228',
  7.9,
  8,
  null,
  null,
  null,
  '2026-06-21T12:00:42.000Z',
  false,
  false,
  false
),
(
  'media-жил-был-пес-43',
  'Жил-был пёс',
  'Жил-был пёс',
  'cartoon',
  1982,
  './posters/жил-был-пес-0a1d23c5.jpg',
  './posters/жил-был-пес-0a1d23c5.jpg',
  'Жил-был пёс. Верно служил хозяевам, но по старости его выгнали со двора. В лесу он встречает такого же старого волка, и неожиданная дружба помогает обоим снова почувствовать себя нужными.',
  array['мультфильм', 'комедия', 'короткометражка', 'семейный']::text[],
  array['СССР']::text[],
  72215,
  45319,
  'tt0216434',
  9.2,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:00:43.000Z',
  false,
  false,
  false
),
(
  'media-кухня-44',
  'Кухня',
  'Кухня',
  'movie',
  2012,
  './posters/кухня-9de985e2.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a00000194fe69e46a7aaf9244d12fc65341/1344x756',
  '«Во все времена еда была рядом с людьми. Она искушает, вдохновляет, восхищает, отвращает. Но человек не может без еды, а значит тот, кто создает еду, может управлять миром...». Так думал Максим Лавров, когда шел осуществлять свою мечту – устраиваться поваром в один из самых дорогих ресторанов столицы.

Мечта сбылась, вот только на деле все вышло не так вкусно. Шеф-повар ресторана Виктор Баринов – настоящая звезда гастрономического бомонда, знающая как угодить взыскательной публике. Оборотная сторона этого идеального образа – злоупотребление алкоголем, страсть к азартным играм и невыносимый характер. В гневе Баринов страшен, в загуле – неукротим.

Арт-директор Виктория Гончарова – настоящая снежная королева, карьеристка с ледяным сердцем, однако именно с ней Макс проводит ночь до того, как пойти на новую работу. А вся команда поваров только и ждет промашки новичка, чтобы от души посмеяться.',
  array['комедия']::text[],
  array['Россия']::text[],
  45994,
  687595,
  'tt2930610',
  7.9,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:00:44.000Z',
  false,
  false,
  false
),
(
  'media-криминальное-чтиво-45',
  'Криминальное чтиво',
  'Pulp Fiction',
  'movie',
  1994,
  './posters/криминальное-чтиво-f54d63e9.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a0000019668936f7e30026fc6e441834868/1344x756',
  'Двое бандитов Винсент Вега и Джулс Винфилд ведут философские беседы в перерывах между разборками и решением проблем с должниками криминального босса Марселласа Уоллеса.
В первой истории Винсент проводит незабываемый вечер с женой Марселласа Мией. Во второй Марселлас покупает боксёра Бутча Кулиджа, чтобы тот сдал бой. В третьей истории Винсент и Джулс по нелепой случайности попадают в неприятности.',
  array['триллер', 'комедия', 'криминал']::text[],
  array['США']::text[],
  680,
  342,
  'tt0110912',
  8.6,
  8.8,
  null,
  null,
  null,
  '2026-06-21T12:00:45.000Z',
  false,
  false,
  false
),
(
  'media-три-кота-46',
  'Три кота',
  'Три кота',
  'cartoon',
  2015,
  './posters/три-кота-806ef7d0.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000019dbcd4fc498a1e0bc6847d4a3774/1920x1080',
  'Три кота — история о повседневной жизни, отношениях и приключениях трех маленьких любознательных котят Коржика, Компота и Карамельки. В их большой семье жизнь никогда не стоит на месте: вместе они учатся выражать эмоции, поддерживать друг друга и находить выход из любой, даже непростой на первый взгляд ситуации, при помощи фантазии и родительского совета.',
  array['мультфильм', 'детский', 'приключения']::text[],
  array['Россия']::text[],
  null,
  935898,
  'tt7038964',
  8.3,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:00:46.000Z',
  false,
  false,
  false
),
(
  'media-пятый-элемент-47',
  'Пятый элемент',
  'The Fifth Element',
  'movie',
  1997,
  './posters/пятый-элемент-740f5566.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017af8511b65af0939fdb611744bf3/1344x756',
  'Каждые пять тысяч лет открываются двери между измерениями, и тёмные силы стремятся нарушить существующую гармонию. 
XXIII век. Нью-йоркский таксист Корбен Даллас пытался спокойно отработать очередную смену, когда к нему в машину прямо с неба свалилась диковатая красотка и попросила о помощи. Не устояв перед наивными голубыми глазами девушки, Корбен ввязывается в неприятности.',
  array['фантастика', 'боевик', 'триллер', 'комедия']::text[],
  array['Франция']::text[],
  18,
  2656,
  'tt0119116',
  8.1,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:00:47.000Z',
  false,
  false,
  false
),
(
  'media-трудные-подростки-48',
  'Трудные подростки',
  'Трудные подростки',
  'series',
  2019,
  './posters/трудные-подростки-ad727fe2.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000018c01c18218f870bf93ea64489d5f/1344x756',
  'Некогда популярный футболист и капитан национальной сборной Антон Ковалёв, отсидев два года за пьяный дебош, выходит на свободу. Испорченная репутация не позволяет ему получить престижную работу. Воспитавший Антона глава центра для трудных подростков предлагает ему должность тренера, чтобы он подготовил непростых ребят к соревнованиям. Ковалёв соглашается, даже не представляя, насколько сложно ему будет найти общий язык с подопечными, и какое важное место они займут в его жизни.',
  array['драма', 'комедия', 'спорт']::text[],
  array[]::text[],
  95011,
  1311936,
  'tt11859946',
  8.2,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:00:48.000Z',
  false,
  false,
  false
),
(
  'media-мстители-финал-49',
  'Мстители: Финал',
  'Мстители: Финал',
  'movie',
  2012,
  './posters/мстители-финал-4ed16d5c.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000168e7cbf1bf723879ea306ad8f363/1344x756',
  'Локи возвращается, и в этот раз он не один. Земля оказывается на грани порабощения, и только лучшие из лучших могут спасти человечество. Глава международной организации Щ. И. Т. Ник Фьюри собирает выдающихся защитников справедливости и добра. Под предводительством Капитана Америки Железный Человек, Тор, Невероятный Халк, Соколиный Глаз и Чёрная Вдова вступают в войну с захватчиком.',
  array['фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  24428,
  263531,
  'tt0848228',
  7.9,
  8,
  null,
  null,
  null,
  '2026-06-21T12:00:49.000Z',
  false,
  false,
  false
),
(
  'media-поймай-меня-если-сможешь-50',
  'Поймай меня, если сможешь',
  'Catch Me If You Can',
  'movie',
  2002,
  './posters/поймай-меня-если-сможешь-2c9c6d8f.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001612718d2d2418d95f8b2ecbda3ec/1344x756',
  'Фрэнк Эбегнейл успел поработать врачом, адвокатом и пилотом на пассажирской авиалинии – и все это до достижения полного совершеннолетия в 21 год. Мастер в обмане и жульничестве, он также обладал искусством подделки документов, что в конечном счете принесло ему миллионы долларов, которые он получил по фальшивым чекам.

Агент ФБР Карл Хэнрэтти отдал бы все, чтобы схватить Фрэнка и привлечь к ответственности за свои деяния, но Фрэнк всегда опережает его на шаг, заставляя продолжать погоню.',
  array['драма', 'криминал', 'биография']::text[],
  array['США', 'Канада']::text[],
  640,
  324,
  'tt0264464',
  8.5,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:00:50.000Z',
  false,
  false,
  false
),
(
  'media-очень-странные-дела-51',
  'Очень странные дела',
  'Stranger Things',
  'series',
  2016,
  './posters/очень-странные-дела-8af49e8b.jpg',
  'https://image.tmdb.org/t/p/w1280/8zbAoryWbtH0DKdev8abFAjdufy.jpg',
  '1980-е годы, тихий провинциальный американский городок. Благоприятное течение местной жизни нарушает загадочное исчезновение подростка по имени Уилл. Выяснить обстоятельства дела полны решимости родные мальчика и местный шериф, также события затрагивают лучшего друга Уилла – Майка. Он начинает собственное расследование. Майк уверен, что близок к разгадке, и теперь ему предстоит оказаться в эпицентре ожесточенной битвы потусторонних сил.',
  array['ужасы', 'фантастика', 'фэнтези', 'триллер', 'драма', 'детектив']::text[],
  array[]::text[],
  66732,
  915196,
  'tt4574334',
  8.4,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:00:51.000Z',
  false,
  false,
  false
),
(
  'media-дюна-52',
  'Дюна',
  'Dune: Part One',
  'movie',
  2021,
  './posters/дюна-298242b7.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017c9a7befa597b53c1700dcaa89b7/1344x756',
  'Наследник знаменитого дома Атрейдесов Пол отправляется вместе с семьей на одну из самых опасных планет во Вселенной — Арракис. Здесь нет ничего, кроме песка, палящего солнца, гигантских чудовищ и основной причины межгалактических конфликтов — невероятно ценного ресурса, который называется меланж. В результате захвата власти Пол вынужден бежать и скрываться, и это становится началом его эпического путешествия. Враждебный мир Арракиса приготовил для него множество тяжелых испытаний, но только тот, кто готов взглянуть в глаза своему страху, достоин стать избранным.',
  array['фантастика', 'боевик', 'драма', 'приключения']::text[],
  array['США', 'Канада', 'Венгрия']::text[],
  null,
  409424,
  'tt1160419',
  7.7,
  8,
  null,
  null,
  null,
  '2026-06-21T12:00:52.000Z',
  false,
  false,
  false
),
(
  'media-дюна-часть-вторая-53',
  'Дюна: Часть вторая',
  'Dune: Part Two',
  'movie',
  2024,
  './posters/дюна-часть-вторая-17816116.jpg',
  './posters/дюна-часть-вторая-17816116.jpg',
  'Герцог Пол Атрейдес присоединяется к фрименам, чтобы стать Муад Дибом, одновременно пытаясь остановить наступление войны',
  array['фантастика', 'боевик', 'драма', 'приключения']::text[],
  array['США', 'Канада', 'ОАЭ', 'Венгрия', 'Италия', 'Новая Зеландия', 'Иордания', 'Гамбия']::text[],
  693134,
  4540126,
  'tt15239678',
  8.2,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:00:53.000Z',
  false,
  false,
  false
),
(
  'media-во-все-тяжкие-54',
  'Во все тяжкие',
  'Breaking Bad',
  'series',
  2008,
  './posters/во-все-тяжкие-6072e20c.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017802c1ad72babf5fca7bb369de8e/1344x756',
  'Школьный учитель химии Уолтер Уайт узнаёт, что болен раком лёгких. Учитывая сложное финансовое состояние дел семьи, а также перспективы, Уолтер решает заняться изготовлением метамфетамина. Для этого он привлекает своего бывшего ученика Джесси Пинкмана, когда-то исключённого из школы при активном содействии Уайта. Пинкман сам занимался варкой мета, но накануне, в ходе рейда УБН, он лишился подельника и лаборатории.',
  array['криминал', 'драма', 'триллер']::text[],
  array['США']::text[],
  null,
  404900,
  'tt0903747',
  8.9,
  9.5,
  null,
  null,
  null,
  '2026-06-21T12:00:54.000Z',
  false,
  false,
  false
),
(
  'media-рататуй-55',
  'Рататуй',
  'Ratatouille',
  'cartoon',
  2007,
  './posters/рататуй-48e9ddf9.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000168f009d0c401435b974efdf63842/1344x756',
  'Крыс Реми обладает уникальным вкусом. Он готов рисковать собственной жизнью, чтобы посмотреть любимое кулинарное шоу и раздобыть какую-нибудь приправку или просто свежий продукт. Реми живет со своими сородичами, которые его не понимают и не принимают его увлечения кулинарией. Когда Реми случайно попадает на кухню шикарного ресторана, он решает воспользоваться выпавшим ему шансом и проверить свои навыки. 

На эту же кухню попадает и юный Лингвини. Всё, на что он может рассчитывать — это должность уборщика. Но он тоже получает свой шанс.',
  array['мультфильм', 'фэнтези', 'комедия', 'семейный']::text[],
  array['США']::text[],
  2062,
  89514,
  'tt0382932',
  8,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:00:55.000Z',
  false,
  false,
  false
),
(
  'media-джанго-освобожденный-56',
  'Джанго освобожденный',
  'Django Unchained',
  'movie',
  2012,
  './posters/джанго-освобожденный-50829df8.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000016c65ca93df91c6b03e406ea2104e/1344x756',
  'Шульц — эксцентричный охотник за головами, который выслеживает и отстреливает самых опасных преступников. Он освобождает раба по имени Джанго, поскольку тот может помочь ему в поисках трёх бандитов. Джанго знает этих парней в лицо, ведь у него с ними свои счёты.',
  array['драма', 'вестерн', 'приключения', 'комедия']::text[],
  array['США']::text[],
  68718,
  586397,
  'tt1853728',
  8.2,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:00:56.000Z',
  false,
  false,
  false
),
(
  'media-чернобыль-57',
  'Чернобыль',
  'Chernobyl',
  'series',
  2019,
  './posters/чернобыль-40b84d9d.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000016a9186ce706e697325b13b5a5d0c/1344x756',
  '26 апреля 1988 года химик Валерий Легасов, надёжно спрятав шесть аудиокассет со своими воспоминаниями, вешается у себя в квартире.

26 апреля 1986 года в 1:23:45 во время проведения эксперимента по безопасности на Чернобыльской АЭС происходит взрыв реактора и пожар. Поднятые по тревоге пожарные без спецзащиты прибывают на место аварии, не подозревая, что оказались в эпицентре крупнейшей в истории человечества техногенной катастрофы. Руководство ЧАЭС уверяет Кремль, что ситуация под контролем, и радиационный фон в норме, но по настоянию академика Легасова его вместе с зампредседателя Совета Министров Борисом Щербиной отправляют разобраться в происходящем на месте.',
  array['драма', 'история']::text[],
  array[]::text[],
  87108,
  1227803,
  'tt7366338',
  8.8,
  9.3,
  null,
  null,
  null,
  '2026-06-21T12:00:57.000Z',
  false,
  false,
  false
),
(
  'media-драйв-58',
  'Драйв',
  'Drive',
  'movie',
  2011,
  './posters/драйв-f905b192.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000019929c096fce1b11f15dd90725f97/1344x756',
  'В порт Сан-Франциско из Гонконга прибывает грузовое судно, с загадочным пассажиром по имени Тоби Вонг на борту. Он бежит от корпорации Леунг, в поисках свободы в США. Однако Тоби преследует наемник Вик Мэдисон со своей командой по заданию мистера Лао, главы Леунг.',
  array['фантастика', 'боевик', 'комедия', 'приключения']::text[],
  array['США']::text[],
  64690,
  276598,
  'tt0780504',
  7.4,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:00:58.000Z',
  false,
  false,
  false
),
(
  'media-тайна-коко-59',
  'Тайна Коко',
  'Coco',
  'cartoon',
  2017,
  './posters/тайна-коко-6a80b3e5.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a00000168eff6f4136b915c4555f04f597c/1344x756',
  '12-летний Мигель живёт в мексиканской деревушке в семье сапожников и тайно мечтает стать музыкантом. Тайно, потому что в его семье музыка считается проклятием. Когда-то его прапрадед оставил жену, прапрабабку Мигеля, ради мечты, которая теперь не даёт спокойно жить и его праправнуку. С тех пор музыкальная тема в семье стала табу. Мигель обнаруживает, что между ним и его любимым певцом Эрнесто де ла Крусом, ныне покойным, существует некая связь. Паренёк отправляется к своему кумиру в Страну Мёртвых, где встречает души предков. Мигель знакомится там с духом-скелетом по имени Гектор, который становится его проводником. Вдвоём они отправляются на поиски де ла Круса.',
  array['мультфильм', 'фэнтези', 'комедия', 'приключения', 'семейный', 'музыка']::text[],
  array[]::text[],
  354912,
  679486,
  'tt2380307',
  8.7,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:00:59.000Z',
  false,
  false,
  false
),
(
  'media-темный-рыцарь-60',
  'Темный рыцарь',
  'The Dark Knight',
  'movie',
  2008,
  './posters/темный-рыцарь-60c0463e.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a000001670d5f5d21a5ed5475c6a524aa8a/1344x756',
  'Бэтмен поднимает ставки в войне с криминалом. С помощью лейтенанта Джима Гордона и прокурора Харви Дента он намерен очистить улицы Готэма от преступности. Сотрудничество оказывается эффективным, но скоро они обнаружат себя посреди хаоса, развязанного восходящим криминальным гением, известным напуганным горожанам под именем Джокер.',
  array['фантастика', 'боевик', 'триллер', 'криминал', 'драма']::text[],
  array['США', 'Великобритания']::text[],
  155,
  111543,
  'tt0468569',
  8.5,
  9.1,
  null,
  null,
  null,
  '2026-06-21T12:01:00.000Z',
  false,
  false,
  false
),
(
  'media-стражи-галактики-61',
  'Стражи Галактики',
  'Guardians of the Galaxy',
  'movie',
  2014,
  './posters/стражи-галактики-5afcf49c.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a00000168e5ccf51bd49e47747b40f6e487/1344x756',
  'Отважному путешественнику Питеру Квиллу попадает в руки таинственный артефакт, принадлежащий могущественному и безжалостному злодею Ронану, строящему коварные планы по захвату Вселенной. Питер оказывается в центре межгалактической охоты, где жертва — он сам.

Единственный способ спасти свою жизнь — объединиться с четверкой нелюдимых изгоев: воинственным енотом по кличке Ракета, человекоподобным деревом Грутом, смертельно опасной Гаморой и одержимым жаждой мести Драксом, также известным как Разрушитель. Когда Квилл понимает, какой силой обладает украденный артефакт и какую опасность он представляет для вселенной, одиночка пойдет на все, чтобы сплотить случайных союзников для решающей битвы за судьбу галактики.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Великобритания']::text[],
  118340,
  689066,
  'tt2015381',
  7.8,
  8,
  null,
  null,
  null,
  '2026-06-21T12:01:01.000Z',
  false,
  false,
  false
),
(
  'media-гладиатор-62',
  'Гладиатор',
  'Gladiator',
  'movie',
  2000,
  './posters/гладиатор-07971349.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000016128aa76c17a22aa251619a76717/1344x756',
  'Римская империя. Бесстрашного и благородного генерала Максимуса боготворят солдаты, а старый император Марк Аврелий безгранично доверяет ему и относится как к сыну. Однако опытный воин, готовый сразиться с любым противником в честном бою, оказывается бессильным перед коварными придворными интригами. Коммод, сын Марка Аврелия, убивает отца, который планировал сделать преемником не его, а Максимуса, и захватывает власть. Решив избавиться от опасного соперника, который к тому же отказывается присягнуть ему на верность, Коммод отдаёт приказ убить Максимуса и всю его семью. Чудом выжив, но не сумев спасти близких, Максимус попадает в плен к работорговцу, который продаёт его организатору гладиаторских боёв Проксимо. Так легендарный полководец становится гладиатором. Но вскоре ему представится шанс встретиться со своим смертельным врагом лицом к лицу.',
  array['боевик', 'драма']::text[],
  array['США', 'Великобритания']::text[],
  98,
  474,
  'tt0172495',
  8.6,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:01:02.000Z',
  false,
  false,
  false
),
(
  'media-мастер-63',
  'Мастер',
  'A Working Man',
  'movie',
  2025,
  './posters/мастер-262064f4.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000196a5670c57eb87d5baaadc80690c/1344x756',
  'Бывший британский спецназовец Левон Кейд оставил войну в прошлом и теперь работает бригадиром на стройке. Он всегда поможет в трудную минуту и не допустит несправедливости, за что его уважают коллеги. Однажды у близких ему людей бандиты похищают 19-летнюю дочь. Полиция бездействует, и Левон по просьбе родителей решает самостоятельно найти девушку и вернуть её домой.',
  array['боевик', 'криминал']::text[],
  array[]::text[],
  1197306,
  5450038,
  'tt9150192',
  7,
  5.7,
  null,
  null,
  null,
  '2026-06-21T12:01:03.000Z',
  false,
  false,
  false
),
(
  'media-исчезнувшая-64',
  'Исчезнувшая',
  'Gone Girl',
  'movie',
  2014,
  './posters/исчезнувшая-c9632904.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a00000198f1345cc5bf734f171e5847a726/1344x756',
  'Всё было готово для празднования пятилетия супружеской жизни, когда вдруг необъяснимо пропала виновница торжества. Остались следы борьбы в доме, кровь, которую явно пытались стереть, и цепочка подсказок в игре «охота за сокровищами» - жена ежегодно устраивала её для своего обожаемого мужа. И похоже, что эти подсказки дают шанс пролить свет на судьбу исчезнувшей.',
  array['триллер', 'драма', 'криминал', 'детектив']::text[],
  array['США']::text[],
  210577,
  692861,
  'tt2267998',
  7.9,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:01:04.000Z',
  false,
  false,
  false
),
(
  'media-терминатор-65',
  'Терминатор',
  'The Terminator',
  'movie',
  1984,
  './posters/терминатор-629f45f5.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a00000171792af1cb4c52988f912996f3b8/1344x756',
  'История противостояния солдата Кайла Риза и киборга-терминатора, прибывших в 1984 год из пост-апокалиптического будущего, где миром правят машины-убийцы, а человечество находится на грани вымирания.',
  array['фантастика', 'боевик', 'триллер']::text[],
  array[]::text[],
  218,
  507,
  'tt0088247',
  8,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:01:05.000Z',
  false,
  false,
  false
),
(
  'media-терминатор-2-судный-день-66',
  'Терминатор 2: Судный день',
  'Terminator 2: Judgment Day',
  'movie',
  1991,
  './posters/терминатор-2-судный-день-670f469e.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a00000171792bbdad319e2bddd92e719481/1344x756',
  'Прошло более десяти лет с тех пор, как киборг из 2029 года пытался уничтожить Сару Коннор — женщину, чей будущий сын выиграет войну человечества против машин.

Теперь у Сары родился сын Джон и время, когда он поведёт за собой выживших людей на борьбу с машинами, неумолимо приближается. Именно в этот момент из постапокалиптического будущего прибывает новый терминатор — практически неуязвимая модель T-1000, способная принимать любое обличье. Цель нового терминатора уже не Сара, а уничтожение молодого Джона Коннора.

Однако шансы Джона на спасение существенно повышаются, когда на помощь приходит перепрограммированный сопротивлением терминатор предыдущего поколения. Оба киборга вступают в смертельный бой, от исхода которого зависит судьба человечества.',
  array['фантастика', 'боевик', 'триллер']::text[],
  array['США', 'Франция']::text[],
  280,
  444,
  'tt0103064',
  8.3,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:01:06.000Z',
  false,
  false,
  false
),
(
  'media-хатико-самый-верный-друг-67',
  'Хатико: Самый верный друг',
  'Hachi: A Dog''s Tale',
  'movie',
  2008,
  './posters/хатико-самый-верный-друг-f5a3dbef.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a00000161285620f9a23cfb48206e0b7b72/1344x756',
  'Однажды, возвращаясь с работы, профессор колледжа нашел на вокзале симпатичного щенка породы акита-ину. Профессор и Хатико стали верными друзьями. Каждый день пес провожал и встречал хозяина на вокзале.',
  array['драма', 'семейный', 'биография']::text[],
  array['США', 'Великобритания']::text[],
  null,
  387556,
  null,
  8.4,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:01:07.000Z',
  false,
  false,
  false
),
(
  'media-доктор-стрэндж-68',
  'Доктор Стрэндж',
  'Doctor Strange',
  'movie',
  2016,
  './posters/доктор-стрэндж-0da255a4.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000169022ed31a24454b60208e71dc88/1344x756',
  'Страшная автокатастрофа поставила крест на карьере успешного нейрохирурга Доктора Стрэнджа. Отчаявшись, он отправляется в путешествие в поисках исцеления и открывает в себе невероятные способности к трансформации пространства и времени. Теперь он — связующее звено между параллельными измерениями, а его миссия — защищать жителей Земли и противодействовать злу, какое бы обличие оно ни принимало.',
  array['фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array[]::text[],
  284052,
  409600,
  'tt1211837',
  7.5,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:01:08.000Z',
  false,
  false,
  false
),
(
  'media-мстители-69',
  'Мстители',
  'The Avengers',
  'movie',
  2012,
  './posters/мстители-6edcb376.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000168e7cbf1bf723879ea306ad8f363/1344x756',
  'Локи возвращается, и в этот раз он не один. Земля оказывается на грани порабощения, и только лучшие из лучших могут спасти человечество. Глава международной организации Щ. И. Т. Ник Фьюри собирает выдающихся защитников справедливости и добра. Под предводительством Капитана Америки Железный Человек, Тор, Невероятный Халк, Соколиный Глаз и Чёрная Вдова вступают в войну с захватчиком.',
  array['фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  24428,
  263531,
  'tt0848228',
  7.9,
  8,
  null,
  null,
  null,
  '2026-06-21T12:01:09.000Z',
  false,
  false,
  false
),
(
  'media-валл-и-70',
  'ВАЛЛ·И',
  'WALL·E',
  'cartoon',
  2008,
  './posters/валл-и-41eed133.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a00000168f0107c4e9c235881c7b3e4bd25/1344x756',
  'Робот ВАЛЛ·И из года в год прилежно трудится на опустевшей Земле, очищая нашу планету от гор мусора, которые оставили после себя улетевшие в космос люди. Он и не представляет, что совсем скоро произойдут невероятные события, благодаря которым он встретит друзей, поднимется к звездам и даже сумеет изменить к лучшему своих бывших хозяев, совсем позабывших родную Землю.',
  array['мультфильм', 'фантастика', 'приключения', 'семейный']::text[],
  array['США']::text[],
  10681,
  279102,
  'tt0910970',
  8.3,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:01:10.000Z',
  false,
  false,
  false
),
(
  'media-трое-из-простоквашино-71',
  'Трое из Простоквашино',
  'Трое из Простоквашино',
  'cartoon',
  1978,
  './posters/трое-из-простоквашино-ee2c140d.jpg',
  'https://avatars.mds.yandex.net/get-ott/12808873/2a0000019680dcba1fb03431ae548b02276c/1344x756',
  'Жил-был мальчик по прозвищу Дядя Федор. Он очень любил животных, но его родители и слышать ничего не хотели о том, чтобы в их квартире завелся кто-то хвостатый и лохматый. И пришлось Дяде Федору со своими новыми приятелями - котом Матроскиным и верным песиком Шариком - отправиться в деревню Простоквашино.',
  array['мультфильм', 'детский', 'короткометражка']::text[],
  array[]::text[],
  77294,
  42326,
  'tt0189160',
  8.8,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:01:11.000Z',
  false,
  false,
  false
),
(
  'media-король-лев-72',
  'Король Лев',
  'The Lion King',
  'cartoon',
  1994,
  './posters/король-лев-b056136f.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000169053916b35362cab3991b0fc116/1344x756',
  'У величественного Короля-Льва Муфасы рождается наследник по имени Симба. Уже в детстве любознательный малыш становится жертвой интриг своего завистливого дяди Шрама, мечтающего о власти.
Симба познаёт горе утраты, предательство и изгнание, но в конце концов обретает верных друзей и находит любимую. Закалённый испытаниями, он в нелёгкой борьбе отвоёвывает своё законное место в «Круге жизни», осознав, что значит быть настоящим Королём. ',
  array['мультфильм', 'мюзикл', 'драма', 'приключения', 'семейный']::text[],
  array['США']::text[],
  8587,
  2360,
  'tt0110357',
  8.8,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:01:12.000Z',
  false,
  false,
  false
),
(
  'media-дэдпул-73',
  'Дэдпул',
  'Deadpool',
  'movie',
  2016,
  './posters/дэдпул-55d87ff9.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a000001626d3bfd64104ee81e873a01e8ce/1344x756',
  'Уэйд Уилсон — наёмник. Будучи побочным продуктом программы вооружённых сил под названием «Оружие X», Уилсон приобрёл невероятную силу, проворство и способность к исцелению. Но страшной ценой: его клеточная структура постоянно меняется, а здравомыслие сомнительно. Всё, чего хочет Уилсон, — держаться на плаву в социальной выгребной яме. Но течение в ней слишком быстрое.',
  array['фантастика', 'боевик', 'комедия', 'приключения']::text[],
  array['США', 'Канада']::text[],
  293660,
  462360,
  'tt1431045',
  7.8,
  8,
  null,
  null,
  null,
  '2026-06-21T12:01:13.000Z',
  false,
  false,
  false
),
(
  'media-унесенные-призраками-74',
  'Унесённые призраками',
  'Sen to Chihiro no kamikakushi',
  'anime',
  2001,
  './posters/унесенные-призраками-36a3611b.jpg',
  './posters/унесенные-призраками-36a3611b.jpg',
  'Тихиро с мамой и папой переезжает в новый дом. Заблудившись по дороге, они оказываются в странном пустынном городе, где ',
  array['аниме', 'мультфильм', 'фэнтези', 'приключения', 'семейный']::text[],
  array['Япония']::text[],
  129,
  370,
  'tt0245429',
  8.6,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:01:14.000Z',
  false,
  false,
  false
),
(
  'media-бесславные-ублюдки-75',
  'Бесславные ублюдки',
  'Inglourious Basterds',
  'movie',
  2009,
  './posters/бесславные-ублюдки-f35cdcc3.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000016128d77c99cc26600ad7a3952eb9/1344x756',
  'Вторая мировая война. В оккупированной немцами Франции группа американских солдат-евреев наводит страх на нацистов, жестоко убивая и скальпируя солдат.',
  array['боевик', 'драма', 'комедия', 'военный']::text[],
  array['США', 'Германия']::text[],
  16869,
  9691,
  'tt0361748',
  7.9,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:01:15.000Z',
  false,
  false,
  false
),
(
  'media-железный-человек-76',
  'Железный человек',
  'Iron Man',
  'movie',
  2008,
  './posters/железный-человек-d3ca481f.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000168e5bb656ca199318b2a17deab5a/1344x756',
  'Миллиардер-изобретатель Тони Старк попадает в плен к Афганским террористам, которые пытаются заставить его создать оружие массового поражения. В тайне от своих захватчиков Старк конструирует высокотехнологичную киберброню, которая помогает  ему сбежать. Однако по возвращении в США он узнаёт, что в совете директоров его фирмы плетётся заговор, чреватый страшными последствиями. Используя своё последнее изобретение, Старк пытается решить проблемы своей компании радикально...',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  1726,
  61237,
  'tt0371746',
  7.9,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:01:16.000Z',
  false,
  false,
  false
),
(
  'media-молодежка-77',
  'Молодёжка',
  'Молодежка',
  'series',
  2013,
  './posters/молодежка-360510f8.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a00000192ec5b89615b39ef892d02808ab3/1344x756',
  '«Медведи» никогда не были лидерами своей лиги. Но теперь все будет по-другому — за команду берется новый тренер Сергей Макеев. Когда-то он блеснул в НХЛ, но после травмы завершил карьеру игрока. Перед новоиспеченным тренером стоит невыполнимая задача — выиграть чемпионат и превратить команду любителей в настоящих профи. Но вот только как это сделать, ведь у его игроков в их 15-17 лет есть еще так много интересных дел. Да и в клубе не все рады приходу слишком активного новичка…',
  array['драма', 'спорт']::text[],
  array['Россия']::text[],
  276105,
  732568,
  'tt3778126',
  7.7,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:01:17.000Z',
  false,
  false,
  false
),
(
  'media-смешарики-78',
  'Смешарики',
  'Смешарики',
  'movie',
  2003,
  './posters/смешарики-c50d4806.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017158fef89cbd13a99b16c4181d97/1344x756',
  'Истории о дружбе и приключениях круглых героев. Весёлые и музыкальные, неожиданные и мечтательные, домашние и авантюрные. Целый мир в одной уютной ромашковой долине.',
  array['мультфильм', 'детский', 'комедия', 'семейный']::text[],
  array[]::text[],
  6589,
  256124,
  'tt0853174',
  7.8,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:01:18.000Z',
  false,
  false,
  false
),
(
  'media-гравити-фолз-79',
  'Гравити Фолз',
  'Gravity Falls',
  'series',
  2012,
  './posters/гравити-фолз-6f8b1ae5.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000170cdf1b6ca23676008b2ab962969/1344x756',
  'Приключения двойняшек Диппера и Мэйбл, которые проводят летние каникулы у двоюродного дедушки в городке под названием Гравити Фолз.',
  array['мультфильм', 'фэнтези', 'комедия', 'детектив', 'приключения', 'семейный']::text[],
  array['США']::text[],
  40075,
  591929,
  'tt1865718',
  8.8,
  8.9,
  null,
  null,
  null,
  '2026-06-21T12:01:19.000Z',
  false,
  false,
  false
),
(
  'media-ford-против-ferrari-80',
  'Ford против Ferrari',
  'Ford v Ferrari',
  'movie',
  2019,
  './posters/ford-против-ferrari-557b71c5.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000016eac2e2af37848539220c7c25704/1344x756',
  'В начале 1960-х Генри Форд II принимает решение улучшить имидж компании и сменить курс на производство более модных автомобилей. После неудавшейся попытки купить практически банкрота Ferrari американцы решают бросить вызов итальянским конкурентам на трассе и выиграть престижную гонку 24 часа Ле-Мана. Чтобы создать подходящую машину, компания нанимает автоконструктора Кэррола Шэлби, а тот отказывается работать без выдающегося, но, как считается, трудного в общении гонщика Кена Майлза. Вместе они принимаются за разработку впоследствии знаменитого спорткара Ford GT40.',
  array['биография', 'спорт', 'драма', 'боевик']::text[],
  array[]::text[],
  359724,
  835086,
  'tt1950186',
  8.2,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:01:20.000Z',
  false,
  false,
  false
),
(
  'media-назад-в-будущее-81',
  'Назад в будущее',
  'Back to the Future',
  'movie',
  1985,
  './posters/назад-в-будущее-5ae8c45c.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000016128aa9721be7e9d22f4bb2be635/1344x756',
  'Подросток Марти с помощью машины времени, сооружённой его другом-профессором доком Брауном, попадает из 80-х в далекие 50-е. Там он встречается со своими будущими родителями, ещё подростками, и другом-профессором, совсем молодым.',
  array['фантастика', 'комедия', 'приключения']::text[],
  array[]::text[],
  105,
  476,
  'tt0088763',
  8.6,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:01:21.000Z',
  false,
  false,
  false
),
(
  'media-как-приручить-дракона-82',
  'Как приручить дракона',
  'How to Train Your Dragon',
  'movie',
  2010,
  './posters/как-приручить-дракона-8bb5602b.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017d72bf184d92abb48d1731aec0e6/1344x756',
  'Подростку Иккингу не слишком близки традиции его героического племени, много лет ведущего войну с драконами. Парень неожиданно заводит дружбу с драконом Беззубиком, который поможет ему и другим викингам увидеть мир с совершенно другой стороны.',
  array['мультфильм', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  10191,
  280172,
  'tt0892769',
  8.2,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:01:22.000Z',
  false,
  false,
  false
),
(
  'media-счастливое-число-слевина-83',
  'Счастливое число Слевина',
  'Lucky Number Slevin',
  'movie',
  2005,
  './posters/счастливое-число-слевина-2150eee8.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000160561222eff07f4343dac39f4bf6/1344x756',
  'Слевину не везет — дом опечатан, девушка ушла к другому. Его друг Ник уезжает из Нью-Йорка и предлагает Слевину пожить в пустой квартире. Тем временем, крупный криминальный авторитет по прозвищу Босс хочет рассчитаться со своим бывшим партнером за убийство сына и в отместку заказать его наследника.',
  array['триллер', 'драма', 'криминал']::text[],
  array['США', 'Канада', 'Великобритания', 'Германия']::text[],
  null,
  86326,
  null,
  8.2,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:01:23.000Z',
  false,
  false,
  false
),
(
  'media-шерлок-84',
  'Шерлок',
  'Sherlock',
  'series',
  2010,
  './posters/шерлок-c351e705.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a00000170aa632ff1e84afcf180b7c85c31/1344x756',
  'События разворачиваются в наши дни. Он прошел Афганистан, остался инвалидом. По возвращении в родные края встречается с загадочным, но своеобразным гениальным человеком. Тот в поиске соседа по квартире. Лондон, 2010 год. Происходят необъяснимые убийства. Скотланд-Ярд без понятия, за что хвататься. Существует лишь один человек, который в силах разрешить проблемы и найти ответы на сложные вопросы.',
  array['триллер', 'драма', 'криминал', 'детектив']::text[],
  array['Великобритания', 'США']::text[],
  19885,
  502838,
  'tt1475582',
  8.9,
  9,
  null,
  null,
  null,
  '2026-06-21T12:01:24.000Z',
  false,
  false,
  false
),
(
  'media-дьявол-носит-prada-85',
  'Дьявол носит Prada',
  'The Devil Wears Prada',
  'movie',
  2006,
  './posters/дьявол-носит-prada-aa82945a.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a00000178d13745854b9b8498213ed8151a/1344x756',
  'Мечтающая стать журналисткой провинциальная девушка Энди по окончании университета получает должность помощницы всесильной Миранды Пристли, деспотичного редактора одного из крупнейших нью-йоркских журналов мод. Энди всегда мечтала о такой работе, не зная, с каким нервным напряжением это будет связано…',
  array['драма', 'мелодрама', 'комедия']::text[],
  array['США', 'Франция']::text[],
  350,
  104992,
  'tt0458352',
  7.5,
  7,
  null,
  null,
  null,
  '2026-06-21T12:01:25.000Z',
  false,
  false,
  false
),
(
  'media-головоломка-86',
  'Головоломка',
  'Inside Out',
  'cartoon',
  2015,
  './posters/головоломка-47ddbfc2.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000168effca5541c59e90bb0faa21ccc/1344x756',
  'В голове у обычной 11-летней школьницы Райли живут пять базовых эмоций: Радость, Грусть, Страх, Гнев и Отвращение. Каждый день они помогают девочке справляться с проблемами, руководя всеми её поступками. Вдруг выясняется, что Райли с родителями предстоит переезд из небольшого уютного городка в шумный и людный мегаполис. Каждая из эмоций считает, что именно она лучше прочих знает, что нужно делать в этой непростой ситуации, и в голове у девочки наступает полный хаос.',
  array['мультфильм', 'семейный', 'комедия']::text[],
  array['США']::text[],
  150540,
  645118,
  'tt2096673',
  8,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:01:26.000Z',
  false,
  false,
  false
),
(
  'media-марсианин-87',
  'Марсианин',
  'The Martian',
  'movie',
  2015,
  './posters/марсианин-3934e7ca.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017c4ca02777225edc8cd84bcd77b3/1344x756',
  'Марсианская миссия «Арес-3» в процессе работы была вынуждена экстренно покинуть планету из-за надвигающейся песчаной бури. Инженер и биолог Марк Уотни получил повреждение скафандра во время песчаной бури. Сотрудники миссии, посчитав его погибшим, эвакуировались с планеты, оставив Марка одного.

Очнувшись, Уотни обнаруживает, что связь с Землёй отсутствует, но при этом полностью функционирует жилой модуль. Главный герой начинает искать способ продержаться на имеющихся запасах еды, витаминов, воды и воздуха ещё 4 года до прилёта следующей миссии «Арес-4».',
  array['фантастика', 'приключения']::text[],
  array['США', 'Великобритания']::text[],
  286217,
  841700,
  'tt3659388',
  7.7,
  8,
  null,
  null,
  null,
  '2026-06-21T12:01:27.000Z',
  false,
  false,
  false
),
(
  'media-однажды-в-голливуде-88',
  'Однажды в Голливуде',
  'What Just Happened',
  'movie',
  2008,
  './posters/однажды-в-голливуде-95a27949.jpg',
  './posters/однажды-в-голливуде-95a27949.jpg',
  'Лента рассказывает о нелегких буднях крупного продюсера, пытающегося закончить съемки многострадальной ленты. Голливуд – место не для слабаков, и в этом ему в очередной раз придется убедиться на собственной шкуре…',
  array['драма', 'комедия']::text[],
  array['США']::text[],
  null,
  195205,
  'tt0486674',
  5.9,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:01:28.000Z',
  false,
  false,
  false
),
(
  'media-престиж-89',
  'Престиж',
  'The Prestige',
  'movie',
  2006,
  './posters/престиж-8fbb3adf.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a000001670d5f7343a577e1025acfaacfd0/1344x756',
  'Роберт и Альфред - фокусники-иллюзионисты, которые на рубеже XIX и XX веков соперничали друг с другом в Лондоне. С годами их дружеская конкуренция на профессиональной почве перерастает в настоящую войну.

Они готовы на все, чтобы выведать друг у друга секреты фантастических трюков и сорвать их исполнение. Непримиримая вражда, вспыхнувшая между ними, начинает угрожать жизни окружающих их людей…',
  array['фантастика', 'триллер', 'драма', 'детектив']::text[],
  array['США', 'Великобритания']::text[],
  1124,
  195334,
  'tt0482571',
  8.5,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:01:29.000Z',
  false,
  false,
  false
),
(
  'media-бесстыжие-90',
  'Бесстыжие',
  'Shameless',
  'series',
  2011,
  './posters/бесстыжие-9689d05e.jpg',
  './posters/бесстыжие-9689d05e.jpg',
  'О взбалмошной многодетной семье Галлагеров и их соседях, которые веселятся, попадают в самые невероятные ситуации и пыта',
  array['драма', 'комедия']::text[],
  array['США']::text[],
  34307,
  571335,
  'tt1586680',
  8.6,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:01:30.000Z',
  false,
  false,
  false
),
(
  'media-такси-91',
  'Такси',
  'Taxi',
  'movie',
  1998,
  './posters/такси-ad66a927.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001837e6995edc21df6137eb02d2ad5/1344x756',
  'Таксист Даниэль помешан на быстрой езде. Как ураган проносится он по извилистым улицам Марселя на мощном ревущем «Пежо», пугая прохожих и приводя в ужас пассажиров. Начинающий следователь Эмильен вынуждает его помогать в поимке банды грабителей банков, каждый раз ускользающих от полиции на неуловимых «Мерседесах».',
  array['боевик', 'комедия', 'криминал']::text[],
  array['Франция']::text[],
  2330,
  14349,
  'tt0152930',
  7.9,
  7,
  null,
  null,
  null,
  '2026-06-21T12:01:31.000Z',
  false,
  false,
  false
),
(
  'media-такси-2-92',
  'Такси 2',
  'Taxi 2',
  'movie',
  2000,
  './posters/такси-2-de37c2b3.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000016b2c54c6a65e64b1585f126b5a49/1344x756',
  'Во Францию прибывает министр обороны Японии. Цель его визита - ознакомиться с французским опытом борьбы с терроризмом и подписать «контракт века» о взаимном сотрудничестве.

Неожиданно во время показательных выступлений французской полиции министра обороны похищают якудза, желающая сорвать заключение наиважнейшего контракта. Даниэль и Эмильен отправляются на поиски высокого гостя. В дело вступает уже хорошо знакомое нам такси.',
  array['боевик', 'комедия', 'криминал']::text[],
  array['Франция']::text[],
  2332,
  854,
  'tt0183869',
  7.6,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:01:32.000Z',
  false,
  false,
  false
),
(
  'media-такси-3-93',
  'Такси 3',
  'Taxi 3',
  'movie',
  2003,
  './posters/такси-3-888dad07.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000016b5584f2cf67d98f608863512cb1/1344x756',
  'В канун Рождества банда Санта-Клаусов совершает ограбления банков в Марселе. Ловить преступников предстоит таксисту Даниелю и полицейскому Эмильену. Но те вытворяют такое, что вреда от них может быть больше, чем от самих грабителей.',
  array['боевик', 'комедия', 'криминал']::text[],
  array['Франция']::text[],
  null,
  14339,
  'tt0295721',
  7.2,
  5.8,
  null,
  null,
  null,
  '2026-06-21T12:01:33.000Z',
  false,
  false,
  false
),
(
  'media-такси-4-94',
  'Такси 4',
  'Taxi 4',
  'movie',
  2007,
  './posters/такси-4-f6ca51de.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017ca20c9863ff94d01feb156e5ef8/1344x756',
  'Преступника № 1 транспортируют из Бельгии в Конго через Марсель. Всего несколько часов самый опасный человек в мире, упрятанный в скафандр и железную клетку, должен пробыть в полицейском участке комиссара Жибера, а охранять преступника доверили Эмильену.',
  array['боевик', 'комедия', 'криминал']::text[],
  array['Франция']::text[],
  null,
  258478,
  'tt0804540',
  6.8,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:01:34.000Z',
  false,
  false,
  false
),
(
  'media-круэлла-95',
  'Круэлла',
  'Cruella',
  'cartoon',
  2021,
  './posters/круэлла-ea710b81.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017b781d5e5388d7e6b517483ca856/1344x756',
  'Великобритания, 1960-е годы. Эстелла была необычным ребёнком, и особенно трудно ей было мириться со всякого рода несправедливостью. Вылетев из очередной школы, она с мамой отправляется в Лондон. По дороге они заезжают в особняк известной модельерши по имени Баронесса, где в результате ужасного несчастного случая мама погибает. Добравшись до Лондона, Эстелла знакомится с двумя мальчишками — уличными мошенниками Джаспером и Хорасом.

10 лет спустя та же компания промышляет на улицах британской столицы мелким воровством, но Эстелла никак не может оставить мечту сделать карьеру в мире моды. Хитростью устроившись в фешенебельный универмаг, девушка привлекает внимание Баронессы, и та берёт её к себе в штат дизайнеров.',
  array['комедия', 'криминал', 'драма']::text[],
  array[]::text[],
  337404,
  804662,
  'tt3228774',
  7.6,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:01:35.000Z',
  false,
  false,
  false
),
(
  'media-большой-куш-96',
  'Большой куш',
  'Snatch',
  'movie',
  2000,
  './posters/большой-куш-04df5b00.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a00000182cec22671613737070692f08e38/1344x756',
  'Фрэнки Четыре Пальца должен был переправить краденый алмаз из Англии в США своему боссу Эви, но, сделав ставку на подпольный боксерский поединок, он попал в круговорот весьма нежелательных событий. Вокруг него и его груза разворачивается сложная интрига с участием множества колоритных персонажей лондонского дна — русского гангстера, троих незадачливых грабителей, хитрого боксера и угрюмого громилы грозного мафиози. Каждый норовит в одиночку сорвать большой куш.',
  array['криминал', 'комедия']::text[],
  array['Великобритания', 'США']::text[],
  107,
  526,
  'tt0208092',
  8.5,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:01:36.000Z',
  false,
  false,
  false
),
(
  'media-душа-97',
  'Душа',
  'Soul',
  'cartoon',
  2020,
  './posters/душа-8bf7803f.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017914287f74f119b3885c69bb1de0/1344x756',
  'Уже немолодой школьный учитель музыки Джо Гарднер всю жизнь мечтал выступать на сцене в составе джазового ансамбля. Однажды он успешно проходит прослушивание у легендарной саксофонистки и, возвращаясь домой вне себя от счастья, падает в люк и умирает. Теперь у Джо одна дорога — в Великое После, но он сбегает с идущего в вечность эскалатора и случайно попадает в Великое До. Тут новенькие души обретают себя, и у будущих людей зарождаются увлечения, мечты и интересы. Джо становится наставником упрямой души 22, которая уже много веков не может найти свою искру и отправиться на Землю.',
  array['мультфильм', 'фэнтези', 'комедия', 'приключения', 'семейный', 'музыка']::text[],
  array[]::text[],
  508442,
  775273,
  'tt2948372',
  8.4,
  8,
  null,
  null,
  null,
  '2026-06-21T12:01:37.000Z',
  false,
  false,
  false
),
(
  'media-шоу-трумана-98',
  'Шоу Трумана',
  'The Truman Show',
  'movie',
  1998,
  './posters/шоу-трумана-507c98d5.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000016128a6e86a61ab0ba35ef1273d86/1344x756',
  'Представьте себе, что вы вдруг начинаете понимать, что все вокруг вас — декорации, а люди — актеры, притворяющиеся теми, кем они вам кажутся. Весь ваш мир оказывается большим телесериалом, где вы исполняете главную роль, даже не подозревая об этом. Вся ваша жизнь — результат работы автора телешоу, которое вот уже тридцать лет смотрит вся планета, начиная с момента вашего рождения.
 В такой ситуации оказался Труман, главный герой картины. Будет ли он продолжать жить в безопасном мире, где, как он теперь знает, у него практически нет свободы выбора, или все-таки выйдет из «игры» и станет сам хозяином своей судьбы, в которой его ждет не запланированная сценарием девушка?',
  array['драма']::text[],
  array['США']::text[],
  37165,
  4541,
  'tt0120382',
  8.3,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:01:38.000Z',
  false,
  false,
  false
),
(
  'media-барбоскины-99',
  'Барбоскины',
  'Барбоскины',
  'cartoon',
  2011,
  './posters/барбоскины-44672205.jpg',
  'https://avatars.mds.yandex.net/get-ott/1648503/2a00000191c401b747c780c187d27ab90e68/1344x756',
  'Мама, папа и… пятеро щенят. У Барбоскиных все, как у людей. Папа свободное время проводит за компьютером, дочери увлекаются модой и слушают Жанну Киску, а сыновья играют в футбол и мечтают о Нобелевской премии.',
  array['мультфильм', 'детский']::text[],
  array[]::text[],
  71970,
  592212,
  'tt3968624',
  6.9,
  5.3,
  null,
  null,
  null,
  '2026-06-21T12:01:39.000Z',
  false,
  false,
  false
),
(
  'media-главный-герой-100',
  'Главный герой',
  'Free Guy',
  'movie',
  2021,
  './posters/главный-герой-493f263e.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017bd0bb4ee0941aec3aa1ccd7ea5c/1344x756',
  'Парень по имени Парень счастлив. Он живет в лучшем в мире городе Городе, работает на лучшей в мире работе в Банке и дружит с охранником по имени Приятель. И его совершенно не волнует, что Банк грабят по нескольку раз на дню, а улицы Города напоминают зону военных действий. Единственное, чего Парню не хватает для полного счастья — идеальной девушки, к которой у него имеется точный список требований. И вот однажды он видит на улице красотку, точь-в-точь как в его мечтах. Эта встреча изменит не только нашего главного героя, но и перевернёт весь известный ему мир.',
  array['фантастика', 'боевик', 'комедия', 'мелодрама']::text[],
  array[]::text[],
  550988,
  1199100,
  'tt6264654',
  7.4,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:01:40.000Z',
  false,
  false,
  false
),
(
  'media-первому-игроку-приготовиться-101',
  'Первому игроку приготовиться',
  'Ready Player One',
  'movie',
  2018,
  './posters/первому-игроку-приготовиться-9f7d842f.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a00000167652493c916e6a5e7fb94a03a6b/1344x756',
  '2045 год. Мир погружается в хаос, и люди ищут спасения в виртуальной вселенной игры OASIS. Её создатель, гениальный и эксцентричный Джеймс Холлидей, оставляет завещание: его колоссальное состояние получит игрок, первым обнаруживший в игре цифровое «пасхальное яйцо». На поиски бросается весь мир. Обычный парень Уэйд Уоттс решает принять участие в состязании и бросается в головокружительную погоню за сокровищами по фантастическому миру, полному загадок, опасностей и невероятных открытий.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array[]::text[],
  333339,
  538225,
  'tt1677720',
  7.4,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:01:41.000Z',
  false,
  false,
  false
),
(
  'media-сверхъестественное-102',
  'Сверхъестественное',
  'Supernatural',
  'series',
  2005,
  './posters/сверхъестественное-1e7eac2d.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a00000175c2d3c9c93a06850be2b698ec36/1344x756',
  'Сериал рассказывает о приключениях братьев Сэма и Дина Винчестеров, которые путешествуют по Соединённым Штатам на чёрном автомобиле Chevrolet Impala 1967 года, расследуют паранормальные явления, многие из которых основаны на американских городских легендах и фольклоре, и сражаются с порождениями зла, такими как демоны и призраки.',
  array['ужасы', 'фэнтези', 'триллер', 'драма', 'детектив']::text[],
  array['США']::text[],
  1622,
  178707,
  'tt0460681',
  8.3,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:01:42.000Z',
  false,
  false,
  false
),
(
  'media-волчонок-103',
  'Волчонок',
  'Teen Wolf',
  'series',
  2011,
  './posters/волчонок-40d07052.jpg',
  './posters/волчонок-40d07052.jpg',
  'Старшеклассник Скотт МакКолл после укуса неизвестного зверя становится оборотнем и пытается совместить обычную школьную жизнь с новыми опасными способностями.',
  array['фэнтези', 'боевик', 'триллер', 'драма']::text[],
  array['США']::text[],
  34524,
  492613,
  'tt1567432',
  8,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:01:43.000Z',
  false,
  false,
  false
),
(
  'media-бригада-104',
  'Бригада',
  'Бригада',
  'series',
  2002,
  './posters/бригада-e6e4304b.jpg',
  'https://avatars.mds.yandex.net/get-ott/17667793/2a0000019ce79657c7bdfc02efb6c84a7845/1344x756',
  'Это история четырех друзей детства, обычных московских парней, Саши Белого, Космоса, Пчелы и Фила, выросших в одном дворе. Друзья решили немного подзаработать, но незапланированное убийство вмиг перемешало все задуманное, поставив на кон их жизни.

Ставка слишком высока, но отступать некуда. Теперь парни прокладывают себе дорогу в криминальном мире и волею судеб превращаются в одну из самых сплоченных и влиятельных группировок…',
  array['драма', 'криминал']::text[],
  array['Россия']::text[],
  30877,
  77039,
  'tt0337898',
  8.1,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:01:44.000Z',
  false,
  false,
  false
),
(
  'media-время-105',
  'Время',
  'In Time',
  'movie',
  2011,
  './posters/время-ceb6dd4a.jpg',
  'https://avatars.mds.yandex.net/get-ott/13051577/2a0000019985856f7a7ea2b3d94d09db59b6/1344x756',
  'Добро пожаловать в мир, где время стало единственной и самой твердой валютой, где люди генетически запрограммированы так, что в 25 лет перестают стареть. Правда, последующие годы стоят денег. И вот богатые становятся практически бессмертными, а бедные обречены сражаться за жизнь. Уилл, бунтарь из гетто, несправедливо обвинен в убийстве с целью грабежа времени и теперь вынужден, захватив заложницу, пуститься в бега.',
  array['фантастика', 'триллер', 'драма', 'мелодрама', 'криминал']::text[],
  array['США']::text[],
  49530,
  517988,
  'tt1637688',
  7.3,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:01:45.000Z',
  false,
  false,
  false
),
(
  'media-винни-пух-106',
  'Винни Пух',
  'Винни Пух',
  'cartoon',
  1969,
  './posters/винни-пух-62f0eab3.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a000001725a8e6cf54e7e06f8b359c6d406/1344x756',
  'Первый фильм из серии о забавном медвежонке и его друзьях. Фильм снят по сказке Алана Милна. Глава, в которой происходит знакомство с Винни-Пухом и подозрительными пчелами.',
  array['мультфильм', 'детский', 'короткометражка']::text[],
  array['СССР']::text[],
  null,
  45779,
  null,
  8.8,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:01:46.000Z',
  false,
  false,
  false
),
(
  'media-движение-вверх-107',
  'Движение вверх',
  'Движение вверх',
  'movie',
  2017,
  './posters/движение-вверх-06aa6086.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a000001638cb5aa75f0a9d6132790193256/1344x756',
  'Есть победы, которые меняют ход истории. Победы духа, победы страны, победы всего мира. Таким триумфом стали легендарные «три секунды» - выигрыш сборной СССР по баскетболу на роковой мюнхенской Олимпиаде 1972 г. Впервые за 36 лет была повержена «непобедимая» команда США. Никто даже помыслить не мог о том, что это возможно – обыграть великолепных непогрешимых американцев на Олимпийских играх! Никто, кроме советских баскетболистов (русских и грузин, украинцев и казахов, белорусов и литовцев).

Когда проигрыш означал поражение страны, когда нужно было выходить и бороться в раскаленной обстановке из-за произошедшего теракта, великий тренер сборной СССР был готов на все, лишь бы помочь своим подопечным разбить американский миф о непотопляемой команде мечты. Ведь он знал, что создал самую сильную сборную на планете, и в начале заставил поверить в это своих игроков, а затем весь мир.',
  array['спорт', 'драма']::text[],
  array[]::text[],
  444218,
  840817,
  'tt5987402',
  7.6,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:01:47.000Z',
  false,
  false,
  false
),
(
  'media-фиксики-108',
  'Фиксики',
  'Фиксики',
  'cartoon',
  2010,
  './posters/фиксики-9b277d48.jpg',
  'https://avatars.mds.yandex.net/get-ott/17702706/2a0000019c42783eab56dba11a2eab07f174/1344x756',
  '«А кто такие фиксики — большой-большой секрет!» — так поётся в песенке про фиксиков. До недавнего времени люди почти ничего не знали об этих маленьких человечках, которые живут внутри машин и приборов. Фиксики ухаживают за техникой изнутри, чистят её, смазывают, устраняют мелкие поломки. Они — умелые и добросовестные мастера. Фиксики привыкли скрываться от людей, но есть они повсюду: в компьютерах, холодильниках, телевизорах... Каждый помнит о случаях, когда неработающий прибор начинал вдруг работать, если по нему слегка стукнуть. Всё просто: внутри проснулся фиксик и всё наладил. А теперь об этих таинственных трудолюбивых человечках можно посмотреть мультфильмы и познакомиться с ними поближе...',
  array['мультфильм', 'детский']::text[],
  array[]::text[],
  46105,
  581653,
  'tt3886188',
  7.5,
  6.2,
  null,
  null,
  null,
  '2026-06-21T12:01:48.000Z',
  false,
  false,
  false
),
(
  'media-люди-в-черном-109',
  'Люди в чёрном',
  'Men in Black',
  'movie',
  1997,
  './posters/люди-в-черном-4ca756f2.jpg',
  './posters/люди-в-черном-4ca756f2.jpg',
  'Они - самый большой секрет Земли. Они работают на неофициальное правительственное агентство, регулирующее деятельность инопланетян на Земле.
Они - это лучшая, последняя и единственная линия защиты Земли от отбросов вселенной. Их работа секретна, их оружие совершенно, им нет равных, они не оставляют следов. Они - это Люди в черном.',
  array['фантастика', 'комедия']::text[],
  array['США']::text[],
  607,
  1091,
  'tt0119654',
  8,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:01:49.000Z',
  false,
  false,
  false
),
(
  'media-люди-в-черном-2-110',
  'Люди в черном 2',
  'Men in Black II',
  'movie',
  2002,
  './posters/люди-в-черном-2-7d1c08f3.jpg',
  './posters/люди-в-черном-2-7d1c08f3.jpg',
  'Много лет назад лучший агент организации «Люди в черном» Кей получил задание особой важности. Ему поручили надёжно спрят',
  array['фантастика', 'боевик', 'комедия', 'детектив', 'приключения']::text[],
  array['США']::text[],
  null,
  6379,
  'tt0120912',
  7.3,
  6.2,
  null,
  null,
  null,
  '2026-06-21T12:01:50.000Z',
  false,
  false,
  false
),
(
  'media-люди-в-черном-3-111',
  'Люди в чёрном 3',
  'Men in Black³',
  'movie',
  2012,
  './posters/люди-в-черном-3-1dbc0f49.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a000001717926b158aa40974da9d5cb1db5/1344x756',
  'Агент Джей узнает, что никакого защитного галактического щита, способного отразить иноземный удар, не существует. Агент Кей не построил его, потому что был убит в 1969 году. Агент Джей должен совершить путешествие во времени, чтобы спасти Землю и жизнь напарника.',
  array['фантастика', 'боевик', 'комедия', 'приключения']::text[],
  array[]::text[],
  41154,
  455773,
  'tt1409024',
  7.2,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:01:51.000Z',
  false,
  false,
  false
),
(
  'media-укрощение-строптивого-112',
  'Укрощение строптивого',
  'Il bisbetico domato',
  'movie',
  1980,
  './posters/укрощение-строптивого-c06f58f0.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017af26e3d6b4a31026b056e0476e5/1344x756',
  'Категорически не приемлющий женского общества грубоватый фермер вполне счастлив и доволен своей холостяцкой жизнью. Но неожиданно появившаяся в его жизни женщина пытается изменить его взгляды на жизнь и очаровать его.',
  array['мелодрама', 'комедия']::text[],
  array[]::text[],
  10986,
  63912,
  'tt0080439',
  8.4,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:01:52.000Z',
  false,
  false,
  false
),
(
  'media-туз-113',
  'Туз',
  'Asso',
  'movie',
  1981,
  './posters/туз-6bee99ec.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000017b1192544a6d321f36dfda9814da/1344x756',
  'Профессиональный карточный шулер по кличке Туз накануне свадьбы объявляет о своем решении завязать. Но неожиданно оказывается втянутым в крупную игру с Марсельцем. Молодая жена отпускает его на эту игру. Отчаянно блефуя, он срывает банк, но игра продолжается.
На него начинается охота и его убивает наемный убийца, после чего Туз превращается в... привидение, которое не может видеть никто, кроме его жены. Обретя таким трагическим образом новые возможности, он старается пристроить свою вдову и отомстить...',
  array['фэнтези', 'комедия', 'криминал']::text[],
  array['Италия']::text[],
  null,
  136947,
  'tt0082034',
  7.9,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:01:53.000Z',
  false,
  false,
  false
),
(
  'media-семь-114',
  'Семь',
  'Se7en',
  'movie',
  1995,
  './posters/семь-4d8e4d1f.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000016c206db8b4754e38b88565eefd28/1344x756',
  'Детектив Уильям Сомерсет — ветеран уголовного розыска, мечтающий уйти на пенсию и уехать подальше от города и его грешных обитателей. За семь дней до пенсии на Сомерсета сваливаются две неприятности: молодой напарник Миллс и особо изощренное убийство. Острый ум опытного следователя сразу определяет, что за этим преступлением, скорее всего, последуют другие.',
  array['триллер', 'драма', 'криминал', 'детектив']::text[],
  array['США']::text[],
  807,
  377,
  'tt0114369',
  8.3,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:01:54.000Z',
  false,
  false,
  false
),
(
  'media-холодное-сердце-115',
  'Холодное сердце',
  'Frozen',
  'cartoon',
  2013,
  './posters/холодное-сердце-74865f13.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000169050d4a0ee264a7f984df93a535/1344x756',
  'Когда сбывается древнее предсказание, и королевство погружается в объятия вечной зимы, трое бесстрашных героев — принцесса Анна, отважный Кристофф и его верный олень Свен — отправляются в горы, чтобы найти сестру Анны Эльзу, которая может снять со страны ледяное заклятие.',
  array['мультфильм', 'мюзикл', 'фэнтези', 'комедия', 'мелодрама', 'приключения', 'семейный']::text[],
  array['США']::text[],
  109445,
  493208,
  'tt2294629',
  7.8,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:01:55.000Z',
  false,
  false,
  false
),
(
  'media-константин-повелитель-тьмы-116',
  'Константин: Повелитель тьмы',
  'Constantine',
  'movie',
  2005,
  './posters/константин-повелитель-тьмы-efa031aa.jpg',
  './posters/константин-повелитель-тьмы-efa031aa.jpg',
  'Джону Константину удалось не только побывать в аду, но и вернуться обратно. Родившись с неугодным самому себе талантом —',
  array['фэнтези', 'ужасы', 'боевик', 'детектив']::text[],
  array['США', 'Германия']::text[],
  561,
  3793,
  'tt0360486',
  7.9,
  7,
  null,
  null,
  null,
  '2026-06-21T12:01:56.000Z',
  false,
  false,
  false
),
(
  'media-жмурки-117',
  'Жмурки',
  'Жмурки',
  'movie',
  2005,
  './posters/жмурки-ac2b58f2.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000161277a8439149fdffd67031572d3/1344x756',
  'Нижний Новгород, 90-е. Два мелких бандита работают на местного криминального авторитета Сергея Михайловича. Однажды Серега и Саймон получают от шефа очередное поручение — привезти химика, специалиста по белому порошку. Парни проваливают задание, однако Сергей Михайлович дает им шанс исправить ситуацию. Для этого бандитам нужно съездить в одну контору и обменять чемодан денег на чемодан наркотиков. Но на выходе парней поджидают конкуренты — Корон, Бала и Баклажан, которых местный «оборотень в погонах» нанял, чтобы кинуть мафиози Михалыча.',
  array['комедия', 'криминал']::text[],
  array[]::text[],
  20994,
  84830,
  'tt0420982',
  7.3,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:01:57.000Z',
  false,
  false,
  false
),
(
  'media-человек-паук-118',
  'Человек-паук',
  'Spider-Man',
  'movie',
  2002,
  './posters/человек-паук-e92472a2.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000169e7571b0993232db73c3cd072f8/1344x756',
  'Питер Паркер – обыкновенный школьник. Однажды он отправился с классом на экскурсию, где его кусает странный паук-мутант. Через время парень почувствовал в себе нечеловеческую силу и ловкость в движении, а главное – умение лазать по стенам и метать стальную паутину. Свои способности он направляет на защиту слабых. Так Питер становится настоящим супергероем по имени Человек-паук, который помогает людям и борется с преступностью. Но там, где есть супергерой, рано или поздно всегда объявляется и суперзлодей...',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  557,
  838,
  'tt0145487',
  7.5,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:01:58.000Z',
  false,
  false,
  false
),
(
  'media-омерзительная-восьмерка-119',
  'Омерзительная восьмерка',
  'The Hateful Eight',
  'movie',
  2015,
  './posters/омерзительная-восьмерка-faa35e35.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000160561058cd605f216bc391b84bee/1344x756',
  'США после Гражданской войны. Легендарный охотник за головами Джон Рут по кличке Вешатель конвоирует заключенную. По пути к ним прибиваются еще несколько путешественников. Снежная буря вынуждает компанию искать укрытие в лавке на отшибе, где уже расположилось весьма пёстрое общество: генерал конфедератов, мексиканец, ковбой… И один из них — не тот, за кого себя выдает.',
  array['триллер', 'драма', 'криминал', 'детектив', 'вестерн']::text[],
  array['США']::text[],
  273248,
  819101,
  'tt3460252',
  7.9,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:01:59.000Z',
  false,
  false,
  false
),
(
  'media-лига-справедливости-зака-снайдера-120',
  'Лига справедливости Зака Снайдера',
  'Zack Snyder''s Justice League',
  'movie',
  2021,
  './posters/лига-справедливости-зака-снайдера-c875e0e9.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000178a166d251170b6aab6f8c1d99c1/1344x756',
  'Вдохновившись самопожертвованием Супермена, Брюс Уэйн вновь обретает веру в человечество. Он заручается поддержкой новой союзницы Дианы Принс, чтобы сразиться с ещё более могущественным противником. Бэтмен и Чудо-женщина набирают команду сверхлюдей для борьбы с пробудившейся угрозой.',
  array['фантастика', 'боевик', 'фэнтези']::text[],
  array['США']::text[],
  null,
  1387021,
  'tt12361974',
  7.8,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:02:00.000Z',
  false,
  false,
  false
),
(
  'media-чарли-и-шоколадная-фабрика-121',
  'Чарли и шоколадная фабрика',
  'Charlie and the Chocolate Factory',
  'movie',
  2005,
  './posters/чарли-и-шоколадная-фабрика-8a4fbbe8.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a000001670d5ff261b74e0cfd81306129ba/1344x756',
  'Какие чудеса ждут вас на фабрике Вилли Вонки? Только представьте: травяные луга из сладкого мятного сахара в Шоколадной Комнате ... Можно проплыть по Шоколадной реке на розовой сахарной лодке ... Или поставить эксперименты в Комнате изобретений с леденцами, которые никогда не тают ... Понаблюдать за дрессированными белками в Ореховой Комнате или отправиться в стеклянном лифте в Телевизионную Комнату. Вы найдете слишком много смешного, чуть таинственного и настолько захватывающего в этом путешествии, что оно станет настолько же приятным и сладким для вас, как восхитительная сладкая палочка с розовой сливочной помадкой от Вилли Вонки.',
  array['фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  118,
  38905,
  'tt0367594',
  7.6,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:02:01.000Z',
  false,
  false,
  false
),
(
  'media-дивергент-122',
  'Дивергент',
  'Divergent',
  'movie',
  2014,
  './posters/дивергент-534db769.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000160560cfa75f9bd5f3b656d21135b/1344x756',
  'В антиутопическом Чикаго будущего общество разделено на пять фракций: «Эрудиция», «Дружелюбие», «Искренность», «Отречение» и «Бесстрашие». Каждый житель, достигший 16 лет, должен пройти тест, чтобы определить, к какой фракции он принадлежит.
Юная Беатрис оказывается угрозой для всей сложившейся системы, когда тесты выявляют в ней дивергента — человека, которого невозможно однозначно определить в одну из фракций. Способные мыслить независимо и не питающие особого уважения к правительству, дивергенты одним своим существованием дискредитируют принципы, на которых строится общество. И теперь Беатрис — одна из таких людей, живущих вне закона и борющихся с системой, которая намерена любой ценой от них избавиться.',
  array['фантастика', 'детектив', 'боевик', 'мелодрама']::text[],
  array[]::text[],
  157350,
  582101,
  'tt1840309',
  6.8,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:02:02.000Z',
  false,
  false,
  false
),
(
  'media-живая-сталь-123',
  'Живая сталь',
  'Real Steel',
  'movie',
  2011,
  './posters/живая-сталь-19e5b892.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000016939d2473e569d9318325211ea3a/1344x756',
  'События фильма происходят в будущем, где бокс запрещен за негуманностью и заменен боями 2000-фунтовых роботов, управляемых людьми. Бывший боксер, а теперь промоутер, переметнувшийся в Робобокс, решает, что наконец нашел своего чемпиона, когда ему попадается выбракованный, но очень способный робот. Одновременно на жизненном пути героя возникает 11-летний парень, оказывающийся его сыном. И по мере того, как машина пробивает свой путь к вершине, обретшие друг друга отец и сын учатся дружить.',
  array['фантастика', 'боевик', 'семейный']::text[],
  array['США', 'Индия']::text[],
  39254,
  88198,
  'tt0433035',
  7.7,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:02:03.000Z',
  false,
  false,
  false
),
(
  'media-молчание-ягнят-124',
  'Молчание ягнят',
  'The Silence of the Lambs',
  'movie',
  1991,
  './posters/молчание-ягнят-f37c348c.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017e7850049178b7615c726cc86cce/1344x756',
  'Психопат похищает и убивает молодых женщин по всему Среднему Западу. ФБР, уверенное, что все преступления совершены одним и тем же человеком, поручает агенту Клариссе Старлинг встретиться с заключенным-маньяком Ганнибалом Лектером, который мог бы помочь составить психологический портрет убийцы. Сам Лектер отбывает наказание за убийства и каннибализм. Он согласен помочь Клариссе лишь в том случае, если она попотчует его больное воображение подробностями своей личной жизни.',
  array['триллер', 'детектив', 'криминал', 'драма', 'ужасы']::text[],
  array[]::text[],
  274,
  345,
  'tt0102926',
  8.3,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:02:04.000Z',
  false,
  false,
  false
),
(
  'media-маска-125',
  'Маска',
  'The Mask',
  'movie',
  1994,
  './posters/маска-e0e82bc4.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a000001677e8cfc8b723227c0ff7a5b6447/1344x756',
  'Скромный и застенчивый служащий банка чувствует себя неуверенно с красивыми девушками и вообще рядом с людьми. Волей судьбы к нему попадает волшебная маска, и Стенли Ипкис приобретает способность превращаться в неуязвимое мультяшное существо с озорным характером.',
  array['фэнтези', 'комедия']::text[],
  array['США']::text[],
  854,
  6039,
  'tt0110475',
  8,
  7,
  null,
  null,
  null,
  '2026-06-21T12:02:05.000Z',
  false,
  false,
  false
),
(
  'media-грань-будущего-126',
  'Грань будущего',
  'Edge of Tomorrow',
  'movie',
  2014,
  './posters/грань-будущего-ffa7b80d.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000167f9d9905e6a787582c6b9ccf31c/1344x756',
  'В недалёком будущем раса инопланетян вторгается на Землю. Никакая армия человечества не может им противостоять. Майор Уильям Кейдж умирает в бою, но случается невероятное — он оказывается во временной петле. Раз за разом Кейдж попадает в один и тот же бой, сражается и умирает снова и снова. И каждое повторяющееся сражение приближает его к разгадке того, как победить врага.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Канада']::text[],
  137113,
  505851,
  'tt1631867',
  8,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:02:06.000Z',
  false,
  false,
  false
),
(
  'media-острые-козырьки-127',
  'Острые козырьки',
  'Peaky Blinders',
  'series',
  2013,
  './posters/острые-козырьки-89b51c56.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a0000017217306930108b66a949784c91cf/1344x756',
  'Британский сериал о криминальном мире Бирмингема 20-х годов прошлого века, в котором многолюдная семья Шелби стала одной из самых жестоких и влиятельных гангстерских банд послевоенного времени. Фирменным знаком группировки, промышлявшей грабежами и азартными играми, стали зашитые в козырьки лезвия.',
  array['драма', 'криминал']::text[],
  array['Великобритания']::text[],
  60574,
  716587,
  'tt2442560',
  8.5,
  8.7,
  null,
  null,
  null,
  '2026-06-21T12:02:07.000Z',
  false,
  false,
  false
),
(
  'media-довод-128',
  'Довод',
  'Tenet',
  'movie',
  2020,
  './posters/довод-54e2f644.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017651ff3b92609d7472755f2f81c1/1344x756',
  'После теракта в киевском оперном театре агент ЦРУ объединяется с британской разведкой, чтобы противостоять русскому олигарху, который сколотил состояние на торговле оружием. Для этого агенты используют инверсию времени — технологию будущего, позволяющую времени идти вспять.',
  array['фантастика', 'боевик']::text[],
  array[]::text[],
  577922,
  1236063,
  'tt6723592',
  7.5,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:02:08.000Z',
  false,
  false,
  false
),
(
  'media-брюс-всемогущий-129',
  'Брюс Всемогущий',
  'Bruce Almighty',
  'movie',
  2003,
  './posters/брюс-всемогущий-f7fe19fa.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a0000017f546087b7c8a305e378180e9ccd/1344x756',
  'Пессимист Брюс Нолан — популярный ведущий теленовостей. Его ничего не интересует: ни девушка Грейс, ни собственные репортажи. Но однажды Брюса увольняют. Разочарованный герой бросает вызов Богу и неожиданно получает ответ Всевышнего. Бог предлагает Брюсу поменяться с ним местами на неделю и посмотреть, сможет ли Брюс сделать мир хоть чуточку лучше.',
  array['фэнтези', 'драма', 'комедия']::text[],
  array['США']::text[],
  310,
  39577,
  'tt0315327',
  7.7,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:02:09.000Z',
  false,
  false,
  false
),
(
  'media-корпорация-монстров-130',
  'Корпорация монстров',
  'Monsters, Inc.',
  'cartoon',
  2001,
  './posters/корпорация-монстров-6a3ed75a.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a00000168f007bc238bd63c163318bf2870/1344x756',
  'Склизкий гад в сливном бачке, мохнатый зверь, похожий на чудовище из «Аленького цветочка», гигантские мокрицы под кроватью — все они существуют на самом деле. Все, что им нужно — пугать детей, потому что из детских криков они получают электричество.
Полнометражный мультфильм рассказывает о кризисах в мире монстров, их жизни. Но однажды вся мирная жизнь монстров оказывается под угрозой: в их мир попадает ребенок. А с детьми столько хлопот, что они могут довести даже монстров.',
  array['мультфильм', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  585,
  458,
  'tt0198781',
  8.1,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:02:10.000Z',
  false,
  false,
  false
),
(
  'media-человек-паук-вдали-от-дома-131',
  'Человек-паук: Вдали от дома',
  'Человек-паук: Вдали от дома',
  'movie',
  2002,
  './posters/человек-паук-вдали-от-дома-07061745.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000169e7571b0993232db73c3cd072f8/1344x756',
  'Питер Паркер – обыкновенный школьник. Однажды он отправился с классом на экскурсию, где его кусает странный паук-мутант. Через время парень почувствовал в себе нечеловеческую силу и ловкость в движении, а главное – умение лазать по стенам и метать стальную паутину. Свои способности он направляет на защиту слабых. Так Питер становится настоящим супергероем по имени Человек-паук, который помогает людям и борется с преступностью. Но там, где есть супергерой, рано или поздно всегда объявляется и суперзлодей...',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  557,
  838,
  'tt0145487',
  7.5,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:02:11.000Z',
  false,
  false,
  false
),
(
  'media-джон-уик-132',
  'Джон Уик',
  'John Wick',
  'movie',
  2014,
  './posters/джон-уик-e26de99e.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000162251f4534d9ae8fa869a2b5ebc0/1344x756',
  'Джон Уик - на первый взгляд, самый обычный среднестатистический американец, который ведет спокойную мирную жизнь. Однако мало кто знает, что он был наёмным убийцей, причём одним из лучших профессионалов в своём деле.
После того как сынок главы бандитской группы со своими приятелями угоняет его любимый «Мустанг» 1969 года выпуска, при этом убив его собаку Дейзи, которая была подарком недавно почившей супруги, Джон вынужден вернуться к своему прошлому. Теперь Уик начинает охоту за теми, кто имел неосторожность перейти ему дорогу, и он готов на всё, чтобы отомстить.',
  array['боевик', 'триллер', 'криминал']::text[],
  array[]::text[],
  245891,
  762738,
  'tt2911666',
  7,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:02:12.000Z',
  false,
  false,
  false
),
(
  'media-джон-уик-2-133',
  'Джон Уик 2',
  'John Wick: Chapter Two',
  'movie',
  2017,
  './posters/джон-уик-2-b7e4b65c.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000160561280228b15cdda87a5905536/1344x756',
  'Когда бывший коллега Джона решает взять под свой контроль таинственную гильдию убийц, Уик вынужден выйти из отставки. Ведомый кровавой клятвой Джон отправляется в Рим, где ему придется сразиться с одними из самых опасных киллеров в мире.',
  array['боевик', 'триллер', 'криминал']::text[],
  array[]::text[],
  324552,
  885658,
  'tt4425200',
  7.2,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:02:13.000Z',
  false,
  false,
  false
),
(
  'media-джон-уик-3-134',
  'Джон Уик 3',
  'John Wick: Chapter 3 - Parabellum',
  'movie',
  2019,
  './posters/джон-уик-3-68e32704.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000016c207a598bb3e8d494b8c9af08e9/1344x756',
  'Суперкиллер Джон Уик после нарушения кодекса тайной гильдии ассасинов получает статус изгоя — экскомьюникадо. За его голову назначена цена в 14 миллионов долларов, и армия самых жестоких профессиональных убийц со всего мира открывает на него кровавую охоту.',
  array['боевик', 'триллер', 'криминал']::text[],
  array[]::text[],
  458156,
  1009536,
  'tt6146586',
  7.1,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:02:14.000Z',
  false,
  false,
  false
),
(
  'media-джон-уик-4-135',
  'Джон Уик 4',
  'John Wick: Chapter 4',
  'movie',
  2023,
  './posters/джон-уик-4-baeea4e3.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a00000188533e9ba4bd77a13a9e3073ac19/1344x756',
  'Оставляя за собой горы трупов, Джон Уик продолжает скрываться от всевозможных наёмных убийц, и теперь охоту на него возглавляет молодой и честолюбивый член Правления по имени Маркиз. Два управляющих отелями «Континенталь» в Нью-Йорке и Осаке, решившие по старой дружбе помочь своенравному киллеру, уже жестоко за это поплатились, но внезапно Джон узнаёт способ выбраться из этой, казалось бы, безвыходной ситуации.',
  array['боевик', 'триллер', 'криминал']::text[],
  array[]::text[],
  603692,
  1267348,
  'tt10366206',
  7.6,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:02:15.000Z',
  false,
  false,
  false
),
(
  'media-12-лет-рабства-136',
  '12 лет рабства',
  '12 Years a Slave',
  'movie',
  2013,
  './posters/12-лет-рабства-4ac2ae7e.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a000001612860e5704763081509f7f76c5f/1344x756',
  'США после Гражданской войны. Семьянин Соломон Нортап живёт и работает в штате Нью-Йорк, когда однажды ему предлагают привлекательную работу в Вашингтоне. По прибытии Соломона похищают, и он становится рабом на долгие 12 лет.',
  array['драма', 'биография', 'история']::text[],
  array['США', 'Великобритания']::text[],
  76203,
  606646,
  'tt2024544',
  7.7,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:02:16.000Z',
  false,
  false,
  false
),
(
  'media-я-легенда-137',
  'Я легенда',
  'I Am Legend',
  'movie',
  2007,
  './posters/я-легенда-36d4aaf5.jpg',
  './posters/я-легенда-36d4aaf5.jpg',
  'Неизвестный вирус унёс жизни половины населения земного шара, а остальную половину превратил в вампиров. Единственный уцелевший человек с иммунитетом к заболеванию ночами держит осаду упырей, а днем пытается найти противоядие и докопаться до причин эпидемии.',
  array['фантастика', 'триллер', 'драма']::text[],
  array['США']::text[],
  506482,
  195524,
  'tt0480249',
  7.9,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:02:17.000Z',
  false,
  false,
  false
),
(
  'media-трасса-60-138',
  'Трасса 60',
  'Interstate 60',
  'movie',
  2001,
  './posters/трасса-60-210794fe.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000161288be3c3654fb80ee3b767ea82/1344x756',
  'В легендах и мифах есть персонажи, главная задача которых – исполнять желания. У арабов это - джинны, у ирландцев – лепреконы, у китайцев – драконы, у европейцев – феи и лесные духи. А в Америке есть некто О. Ж. Грант – довольно странный и забавный человек, который тоже может исполнить любое желание. Но будьте осторожны, он очень проказлив! Вот его-то и встречает однажды Нил Оливер.

Нил вполне доволен своей жизнью: у него обеспеченные родители, симпатичная невеста и впереди блестящая карьера юриста. Но с недавних пор по ночам ему стала сниться загадочная девушка, которую он никак не может выбросить из головы.

Чудаковатый Грант приглашает Нила совершить поездку по таинственной автостраде 60, которой нет ни на одной карте США. И Нил бросает всё и пускается в самое невероятное и рискованное путешествие в своей жизни, решив во что бы то ни стало разыскать незнакомку из своих снов.',
  array['фантастика', 'фэнтези', 'драма', 'комедия', 'детектив', 'приключения']::text[],
  array['Канада', 'США']::text[],
  20312,
  3563,
  'tt0165832',
  8,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:02:18.000Z',
  false,
  false,
  false
),
(
  'media-тор-рагнарек-139',
  'Тор: Рагнарек',
  'Тор: Рагнарек',
  'movie',
  2011,
  './posters/тор-рагнарек-622aa725.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000016904e109ece54d43ffc6c8ad3d8e/1344x756',
  'Эпическое приключение происходит как на нашей планете Земля, так и в королевстве богов Асгарде. В центре истории - Могучий Тор, сильный, но высокомерный воин, чьи безрассудные поступки возрождают древнюю войну в Асгарде. Тора отправляют в ссылку на Землю, лишают сил и заставляют жить среди обычных людей в качестве наказания...',
  array['фантастика', 'боевик', 'фэнтези', 'приключения']::text[],
  array[]::text[],
  10195,
  258941,
  'tt0800369',
  7.1,
  7,
  null,
  null,
  null,
  '2026-06-21T12:02:19.000Z',
  false,
  false,
  false
),
(
  'media-загадочная-история-бенджамина-баттона-140',
  'Загадочная история Бенджамина Баттона',
  'The Curious Case of Benjamin Button',
  'movie',
  2008,
  './posters/загадочная-история-бенджамина-баттона-c37e88b5.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000167f06424acc2c99e698e4a7c5b03/1344x756',
  'Фильм о мужчине, который родился в возрасте 80 лет, а затем… начал молодеть. Этот человек, как и каждый из нас, не мог остановить время. Его путь в ХХI век, берущий свое начало в Новом Орлеане в 1918 году в самом конце Первой Мировой войны, будет столь необычен, что вряд ли мог иметь место в жизни кого-либо другого. Фильм повествует о судьбе уникального человека, о людях и событиях, что ждут его впереди, о любви, которую он обретет и потеряет, о радостях жизни и грусти потерь и о том, что остается с нами вне времени.',
  array['фантастика', 'драма']::text[],
  array['США']::text[],
  4922,
  81555,
  'tt0421715',
  8.1,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:02:20.000Z',
  false,
  false,
  false
),
(
  'media-оно-141',
  'Оно',
  'It',
  'movie',
  2017,
  './posters/оно-ff842c4e.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a00000186a3241d4fd20773f84bc958faf1/1344x756',
  'Когда в городке Дерри штата Мэн начинают пропадать дети, несколько ребят сталкиваются со своими величайшими страхами — не только с группой школьных хулиганов, но со злобным клоуном Пеннивайзом, список жертв которого уходит вглубь веков.',
  array['ужасы', 'фэнтези', 'драма', 'детектив']::text[],
  array[]::text[],
  346364,
  453397,
  'tt1396484',
  7.3,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:02:21.000Z',
  false,
  false,
  false
),
(
  'media-отряд-самоубийц-142',
  'Отряд самоубийц',
  'Suicide Squad',
  'movie',
  2016,
  './posters/отряд-самоубийц-4b00a88b.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a00000186a7de81c611eab624c211e9dd75/1344x756',
  'Правительство решает дать команде суперзлодеев шанс на искупление. Подвох в том, что их отправляют на выполнение миссии, где они, вероятнее всего, погибнут.',
  array['фантастика', 'боевик', 'фэнтези', 'приключения']::text[],
  array[]::text[],
  297761,
  468522,
  'tt1386697',
  6.1,
  5.9,
  null,
  null,
  null,
  '2026-06-21T12:02:22.000Z',
  false,
  false,
  false
),
(
  'media-стражи-галактики-часть-2-143',
  'Стражи Галактики. Часть 2',
  'Guardians of the Galaxy Vol. 2',
  'movie',
  2017,
  './posters/стражи-галактики-часть-2-a96fd30d.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a00000169be26710f97eb7693ac59e64c61/1344x756',
  'Все в сборе: землянин Питер Квилл (Звездный Лорд), молчаливый громила Дракс, зеленокожая наемница Гамора, живое дерево Грут и говорящий енот. Герои не изменяют себе и с завидной регулярностью продолжают попадать в немыслимые ситуации, выпутываясь из них почти без ущерба (а иногда даже с пользой) для окружающих. На этот раз им предстоит раскрыть одну из самых главных тайн во всей Галактике: кто же на самом деле отец Питера Квилла?',
  array['фантастика', 'боевик', 'приключения', 'комедия']::text[],
  array[]::text[],
  283995,
  841263,
  'tt3896198',
  7.8,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:02:23.000Z',
  false,
  false,
  false
),
(
  'media-день-сурка-144',
  'День сурка',
  'Groundhog Day',
  'movie',
  1993,
  './posters/день-сурка-b2b03fba.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a000001723259f8f3d37aaf587f67ad2c59/1344x756',
  'Телевизионный комментатор Фил Коннорс каждый год приезжает в маленький городок в штате Пенсильвания на празднование Дня сурка. Но на этот раз веселье рискует зайти слишком далеко. Время сыграло с ним злую шутку: оно взяло да и остановилось.

Теперь на календаре Фила чернеет одна и та же дата - 2 февраля, из которой он никак не может выбраться. Неунывающий ведущий пытается извлечь выгоду из своего комичного положения: впереди у него уйма времени и безмятежное предсказуемое будущее.

Отныне с ним не случится ничего плохого... и ничего хорошего. У Фила осталась одна заветная мечта, простая и незамысловатая - 3 февраля...',
  array['фэнтези', 'мелодрама', 'комедия']::text[],
  array['США']::text[],
  137,
  527,
  'tt0107048',
  8.1,
  8,
  null,
  null,
  null,
  '2026-06-21T12:02:24.000Z',
  false,
  false,
  false
),
(
  'media-эффект-бабочки-145',
  'Эффект бабочки',
  'The Butterfly Effect',
  'movie',
  2003,
  './posters/эффект-бабочки-44704e32.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000016c20686f23f6a4db35ac5c5a345c/1344x756',
  'Эван перенял от своего отца-психопата, ныне запертого в доме для умалишённых, странную болезнь — он не помнит некоторых эпизодов своей жизни, причем в эти моменты происходили довольно странные, а то и ужасные события. Поступив в колледж, Эван делает удивительное открытие. Читая дневники, которые писал в детстве по совету врача, он может возвращаться в детство и менять будущее.',
  array['фантастика', 'триллер']::text[],
  array['США', 'Канада']::text[],
  1954,
  5167,
  'tt0289879',
  8.2,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:02:25.000Z',
  false,
  false,
  false
),
(
  'media-шрэк-2-146',
  'Шрэк 2',
  'Shrek 2',
  'cartoon',
  2004,
  './posters/шрэк-2-4ff6bcd4.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a00000162250f54b25b9040586f825d3852/1344x756',
  'Шрэк и Фиона возвращаются после медового месяца и находят письмо от родителей Фионы с приглашением на ужин. Однако те не подозревают, что их дочь тоже стала огром! Вместе с Осликом счастливая пара отправляется в путешествие, полное неожиданностей, и попадает в круговорот событий, во время которых приобретает множество друзей…',
  array['мультфильм', 'фэнтези', 'мелодрама', 'комедия', 'приключения', 'семейный']::text[],
  array[]::text[],
  809,
  5273,
  'tt0298148',
  8,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:02:26.000Z',
  false,
  false,
  false
),
(
  'media-kingsman-секретная-служба-147',
  'Kingsman: Секретная служба',
  'Kingsman: The Secret Service',
  'movie',
  2015,
  './posters/kingsman-секретная-служба-02cd820a.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017c286371b298cc99cf039c0af138/orig',
  'Эггси прошел службу в морской пехоте и имеет высокий интеллект. Он мог бы добиться многого, но выбрал другой путь и стал мелким преступником. Однажды он знакомится с Гарри Хартом, которому его отец когда-то спас жизнь. Этот человек решает сделать жизнь Эггси лучше и открыть для него новые возможности. 
Гарри рассказывает, что является агентом секретной независимой организации, которая стоит на защите мира, и предлагает парню пройти обучение и стать новым членом их команды.',
  array['боевик', 'комедия', 'криминал', 'приключения']::text[],
  array['Великобритания', 'США']::text[],
  207703,
  749540,
  'tt2802144',
  7.6,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:02:27.000Z',
  false,
  false,
  false
),
(
  'media-доктор-хаус-148',
  'Доктор Хаус',
  'House, M.D.',
  'series',
  2004,
  './posters/доктор-хаус-2ac1678b.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000016b94647cff0c079b6516bdcb1572/1344x756',
  'Сериал рассказывает о команде врачей, которые должны правильно поставить диагноз пациенту и спасти его. Возглавляет команду доктор Грегори Хаус, который ходит с тростью после того, как его мышечный инфаркт в правой ноге слишком поздно правильно диагностировали. Как врач Хаус просто гений, но сам не отличается проникновенностью в общении с больными и с удовольствием избегает их, если только есть возможность. Он сам всё время проводит в борьбе с собственной болью, а трость в его руке только подчеркивает его жесткую, ядовитую манеру общения. Порой его поведение можно назвать почти бесчеловечным, и при этом он прекрасный врач, обладающий нетипичным умом и безупречным инстинктом, что снискало ему глубокое уважение. Будучи инфекционистом, он ещё и замечательный диагност, который любит разгадывать медицинские загадки, чтобы спасти кому-то жизнь. Если бы все было по его воле, то Хаус лечил бы больных не выходя из своего кабинета.',
  array['драма', 'детектив']::text[],
  array['США']::text[],
  1408,
  178710,
  'tt0412142',
  8.8,
  8.7,
  null,
  null,
  null,
  '2026-06-21T12:02:28.000Z',
  false,
  false,
  false
),
(
  'media-игры-разума-149',
  'Игры разума',
  'A Beautiful Mind',
  'movie',
  2001,
  './posters/игры-разума-98b4d979.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a0000017d9e483d912fe90e90dfa138232f/1344x756',
  'От всемирной известности до греховных глубин — все это познал на своей шкуре Джон Форбс Нэш-младший. Математический гений, он на заре своей карьеры сделал титаническую работу в области теории игр, которая перевернула этот раздел математики и практически принесла ему международную известность.

Однако буквально в то же время заносчивый и пользующийся успехом у женщин Нэш получает удар судьбы, который переворачивает уже его собственную жизнь.',
  array['драма', 'биография']::text[],
  array['США']::text[],
  453,
  530,
  'tt0268978',
  8.6,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:02:29.000Z',
  false,
  false,
  false
),
(
  'media-темный-рыцарь-возрождение-легенды-150',
  'Темный рыцарь: Возрождение легенды',
  'Темный рыцарь: Возрождение легенды',
  'movie',
  2008,
  './posters/темный-рыцарь-возрождение-легенды-eaea5bc3.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a000001670d5f5d21a5ed5475c6a524aa8a/1344x756',
  'Восемь лет назад Бэтмен растворился в ночи, превратившись из героя в беглеца. Приняв на себя вину за смерть прокурора Харви Дента, он пожертвовал всем. Вместе с комиссаром Гордоном они решили, что так будет лучше для всех. Пока преступность была раздавлена антикриминальным актом Дента, ложь действовала.

Тем не менее, еще опаснее становится появление нового врага Бэйна, чье лицо закрыто маской.  Он разворачивает в Готэме чудовищную деятельность, и это вынуждает Брюса Уэйна выйти из импровизированного изгнания. Однако даже надев свой костюм, Бэтмен может проиграть Бэйну. Но с появлением хитрой воровки по прозвищу Женщина — Кошка с загадочным прошлым всё меняется.',
  array['фантастика', 'боевик', 'триллер', 'драма', 'криминал']::text[],
  array['США', 'Великобритания']::text[],
  155,
  111543,
  'tt0468569',
  8.2,
  9.1,
  null,
  null,
  null,
  '2026-06-21T12:02:30.000Z',
  false,
  false,
  false
),
(
  'media-по-соображениям-совести-151',
  'По соображениям совести',
  'Hacksaw Ridge',
  'movie',
  2016,
  './posters/по-соображениям-совести-2c81a0c9.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a000001692aaa0c92df1141279044f97454/1344x756',
  'Медик американской армии времён Второй мировой войны Десмонд Досс, который служил во время битвы за Окинаву, отказывается убивать людей и становится первым идейным уклонистом в американской истории, удостоенным Медали Почёта.',
  array['биография', 'военный', 'драма', 'история']::text[],
  array[]::text[],
  324786,
  649917,
  'tt2119532',
  8.2,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:02:31.000Z',
  false,
  false,
  false
),
(
  'media-мадагаскар-152',
  'Мадагаскар',
  'Madagascar',
  'cartoon',
  2005,
  './posters/мадагаскар-78ea489f.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a000001622513fc545f99da445c66008268/1344x756',
  'Четверо изнеженных животных из Центрального зоопарка в Нью-Йорке - лев Алекс, зебра Марти, жираф Мелман и гиппопотамиха Глория - решаются на побег. Оказавшись после кораблекрушения на экзотическом острове Мадагаскар, населенном лемурами и поедающими их фоссами, путешественники с ужасом понимают, что с городскими привычками им придется распрощаться. 

Здесь нет людей, любимых клеток, вкусной кормежки. Начинается шоу «Последний герой» только с животными - нашим друзьям нужно выжить в непривычной среде обитания, где на каждом шагу подстерегает опасность...',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  953,
  6006,
  'tt0351283',
  7.9,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:02:32.000Z',
  false,
  false,
  false
),
(
  'media-ходячий-замок-153',
  'Ходячий замок',
  'Howl no Ugoku Shiro',
  'anime',
  2004,
  './posters/ходячий-замок-0e0e4546.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017394799e6893b002f04de0fc6408/1344x756',
  'Злая ведьма заточила 18-летнюю Софи в тело старухи. Девушка-бабушка бежит из города куда глаза глядят и встречает удивительный дом на ножках, где знакомится с могущественным волшебником Хаулом и демоном Кальцифером. Кальцифер должен служить Хаулу по договору, условия которого он не может разглашать. Девушка и демон решают помочь друг другу избавиться от злых чар.',
  array['аниме', 'мультфильм', 'фантастика', 'фэнтези', 'мелодрама', 'приключения']::text[],
  array['Япония']::text[],
  4935,
  49684,
  'tt0347149',
  8.3,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:02:33.000Z',
  false,
  false,
  false
),
(
  'media-дэдпул-2-154',
  'Дэдпул 2',
  'Deadpool 2',
  'movie',
  2018,
  './posters/дэдпул-2-a60a174d.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a00000163e00b45484a100a2828f89bcac9/1344x756',
  'Единственный и неповторимый болтливый наемник вернулся! Ещё более масштабный, ещё более разрушительный и даже ещё более голозадый чем прежде! Когда в его жизнь врывается суперсолдат с убийственной миссией, Дэдпул вынужден задуматься о дружбе, семье и о том, что на самом деле значит быть героем, попутно надирая 50 оттенков задниц. Потому что иногда, чтобы делать хорошие вещи, нужно использовать грязные приёмчики.',
  array['фантастика', 'боевик', 'комедия', 'приключения']::text[],
  array[]::text[],
  383498,
  961715,
  'tt5463162',
  7.4,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:02:34.000Z',
  false,
  false,
  false
),
(
  'media-гадкий-я-155',
  'Гадкий я',
  'Despicable Me',
  'cartoon',
  2010,
  './posters/гадкий-я-0c260d9b.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a000001612892e487ae08e26b9822e714b4/1344x756',
  'Гадкий снаружи, но добрый внутри Грю намерен, тем не менее, закрепить за собой статус главного архизлодея в мире, для чего он решает выкрасть Луну при помощи созданной им армии миньонов. Дело осложняют конкуренты, вставляющие высокотехнические палки в колеса, и семейные обстоятельства в виде трех сироток, о которых Грю вынужден заботиться.',
  array['мультфильм', 'комедия', 'семейный']::text[],
  array['США']::text[],
  20352,
  432724,
  'tt1323594',
  7.8,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:02:35.000Z',
  false,
  false,
  false
),
(
  'media-гадкий-я-2-156',
  'Гадкий я 2',
  'Despicable Me 2',
  'cartoon',
  2013,
  './posters/гадкий-я-2-6993ebe2.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001616b9334d241a9fd1a283fe21ba1/1344x756',
  'В то время как Грю, бывший суперзлодей, приспосабливается к семейной жизни и пытается честным трудом зарабатывать в сфере бизнеса, тайные лаборатории Арктики оказываются украденными. Анти-Злодейская лига решает, что она нуждается в помощи Грю и его новобранцев в расследовании.',
  array['мультфильм', 'комедия', 'семейный']::text[],
  array['США']::text[],
  93456,
  550910,
  'tt1690953',
  7.6,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:02:36.000Z',
  false,
  false,
  false
),
(
  'media-адвокат-дьявола-157',
  'Адвокат дьявола',
  'The Devil''s Advocate',
  'movie',
  1997,
  './posters/адвокат-дьявола-b6eb9b4b.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a000001685b512db7e76753c21944ab8a26/1344x756',
  'В Нью-Йорк по приглашению главы крупного юридического концерна прибывает Кевин Ломакс, молодой адвокат. До этого он был известен тем, что защищал исключительно негодяев и притом не проиграл ни одного процесса. На новом месте работы он вполне счастлив, он живет в роскошной квартире с любящей женой, его окружают интересные люди.',
  array['триллер', 'драма', 'детектив']::text[],
  array['США', 'Германия']::text[],
  1813,
  3797,
  'tt0118971',
  8.2,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:02:37.000Z',
  false,
  false,
  false
),
(
  'media-игра-в-кальмара-158',
  'Игра в кальмара',
  'Ojingeo geim',
  'series',
  2021,
  './posters/игра-в-кальмара-b39113b6.jpg',
  'https://image.tmdb.org/t/p/w1280/2meX1nMdScFOoV4370rqHWKmXhY.jpg',
  'Сон Ги-хун уже немолод, разведён, по уши погряз в долгах и сидит на шее у старенькой матери. Даже выигранные на скачках деньги в его руках долго не задерживаются, и однажды он встречает в метро загадочного незнакомца, который сначала предлагает сыграть в детскую игру, а затем вручает Ги-хуну немалую сумму и визитку. Но радость мужчины сменяется отчаянием, когда он узнаёт, что бывшая жена с новым мужем собираются увезти его дочь в Америку. Он звонит по номеру с визитки и становится последним участником тайных игр на выживание с призом в 40 миллионов долларов. Среди товарищей по несчастью оказываются его друг детства — прогоревший финансист, бандит, смертельно больной старик, северокорейская перебежчица, иммигрант из Пакистана и многие другие отчаянно нуждающиеся в деньгах.',
  array['триллер', 'драма']::text[],
  array[]::text[],
  93405,
  1301710,
  'tt10919420',
  7.7,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:02:38.000Z',
  false,
  false,
  false
),
(
  'media-гоголь-начало-159',
  'Гоголь. Начало',
  'Гоголь. Начало',
  'movie',
  2017,
  './posters/гоголь-начало-848f4baa.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a000001616ced74350af0ca257820425c66/1344x756',
  'Казаки, ведьмы, русалки, утопленницы и другие обитатели Диканьки встречаются лицом к лицу со своим создателем Николаем Гоголем, молодым писарем из Петербурга, который страдает загадочными припадками и может общаться с персонажами из потустороннего мира. Вместе со своим начальником, блестящим столичным следователем Яковом Гуро, начинающий писатель приезжает на Полтавщину расследовать таинственные убийства девушек. Здесь же он встречает свою любовь, которой суждено сыграть роковую роль в его судьбе. Но хватит ли юному литератору духу противостоять невообразимому? Ведь по слухам, которые активно разносят напуганные местные жители, за преступлениями стоит не кто иной, как сам Дьявол в обличии черного всадника.',
  array['детектив', 'фэнтези', 'приключения']::text[],
  array[]::text[],
  468988,
  945391,
  'tt6628228',
  6.2,
  5.8,
  null,
  null,
  null,
  '2026-06-21T12:02:39.000Z',
  false,
  false,
  false
),
(
  'media-богемская-рапсодия-160',
  'Богемская рапсодия',
  'Bohemian Rhapsody',
  'movie',
  2018,
  './posters/богемская-рапсодия-feb22617.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a0000016a4f879ad6e4cb0fbd98e35f6ad6/1344x756',
  'Чествование группы Queen, их музыки и их выдающегося вокалиста Фредди Меркьюри, который бросил вызов стереотипам и победил условности, чтобы стать одним из самых любимых артистов на планете. Фильм прослеживает головокружительный путь группы к успеху благодаря их культовым песням и революционному звуку, практически распад коллектива, поскольку образ жизни Меркьюри выходит из-под контроля, и их триумфальное воссоединение накануне концерта Live Aid, ставшим одним из величайших выступлений в истории рок-музыки.',
  array['биография', 'музыка', 'драма']::text[],
  array[]::text[],
  424694,
  568289,
  'tt1727824',
  8,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:02:40.000Z',
  false,
  false,
  false
),
(
  'media-голодные-игры-161',
  'Голодные игры',
  'The Hunger Games',
  'movie',
  2012,
  './posters/голодные-игры-89354a5d.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000016128445b53fb59d350205f40df2f/1344x756',
  'Будущее. Деспотичное государство ежегодно устраивает показательные игры на выживание, за которыми в прямом эфире следит весь мир. В этот раз участвовать будут юная Китнисс и тайно влюбленный в нее Пит. Они знакомы с детства, но теперь должны стать врагами. Ведь по нерушимому закону Голодных игр победить может только один из 24 участников. Судьям не важно, кто выиграет, главное — зрелище. И на этот раз зрелище будет незабываемым.',
  array['фантастика', 'триллер', 'драма', 'приключения']::text[],
  array['США']::text[],
  70160,
  468581,
  'tt1392170',
  7.3,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:02:41.000Z',
  false,
  false,
  false
),
(
  'media-бегущий-по-лезвию-2049-162',
  'Бегущий по лезвию 2049',
  'Blade Runner 2049',
  'movie',
  2017,
  './posters/бегущий-по-лезвию-2049-abfeb548.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017bc568084e9b50db4b97d11440bb/1344x756',
  'В недалеком будущем мир населен людьми и репликантами, созданными выполнять самую тяжелую работу. Работа офицера полиции Кей — держать репликантов под контролем в условиях нарастающего напряжения. Он случайно становится обладателем секретной информации, которая ставит под угрозу существование всего человечества. Желая найти ключ к разгадке, Кей решает разыскать Рика Декарда — бывшего офицера специального подразделения полиции Лос-Анджелеса, который бесследно исчез много лет назад.',
  array['фантастика', 'боевик', 'триллер', 'драма']::text[],
  array[]::text[],
  335984,
  589290,
  'tt1856101',
  7.8,
  8,
  null,
  null,
  null,
  '2026-06-21T12:02:42.000Z',
  false,
  false,
  false
),
(
  'media-моана-163',
  'Моана',
  'Moana',
  'cartoon',
  2016,
  './posters/моана-60d4243e.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a00000169051164510dc216469e844afe57/1344x756',
  'Бесстрашная Моана, дочь вождя маленького племени на острове в Тихом океане, больше всего на свете мечтает о приключениях и решает отправиться в опасное морское путешествие. Вместе с некогда могущественным полубогом Мауи им предстоит пересечь океан, сразиться со страшными чудовищами и разрушить древнее заклятие.',
  array['мультфильм', 'мюзикл', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array[]::text[],
  277834,
  837530,
  'tt3521164',
  7.8,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:02:43.000Z',
  false,
  false,
  false
),
(
  'media-человек-паук-нет-пути-домой-164',
  'Человек-паук: Нет пути домой',
  'Человек-паук: Нет пути домой',
  'movie',
  2002,
  './posters/человек-паук-нет-пути-домой-eba2ec22.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000169e7571b0993232db73c3cd072f8/1344x756',
  'Питер Паркер – обыкновенный школьник. Однажды он отправился с классом на экскурсию, где его кусает странный паук-мутант. Через время парень почувствовал в себе нечеловеческую силу и ловкость в движении, а главное – умение лазать по стенам и метать стальную паутину. Свои способности он направляет на защиту слабых. Так Питер становится настоящим супергероем по имени Человек-паук, который помогает людям и борется с преступностью. Но там, где есть супергерой, рано или поздно всегда объявляется и суперзлодей...',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  557,
  838,
  'tt0145487',
  7.5,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:02:44.000Z',
  false,
  false,
  false
),
(
  'media-железный-человек-2-165',
  'Железный человек 2',
  'Iron Man 2',
  'movie',
  2010,
  './posters/железный-человек-2-f79643b4.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a00000168e5c1479973d3664eeda12a98e3/1344x756',
  'Прошло полгода с тех пор, как мир узнал, что миллиардер-изобретатель Тони Старк является обладателем уникальной кибер-брони  Железного человека. Общественность требует, чтобы Старк передал технологию брони правительству США, но Тони не хочет разглашать её секреты, потому что боится, что она попадёт не в те руки. 

Между тем Иван Ванко — сын русского учёного, когда-то работавшего на фирму Старка, но потом уволенного и лишенного всего, намерен отомстить Тони за беды своей семьи. Для чего сооружает своё высокотехнологичное оружие.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  10138,
  411924,
  'tt1228705',
  7.5,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:02:45.000Z',
  false,
  false,
  false
),
(
  'media-веном-2-166',
  'Веном 2',
  'Venom: Let There Be Carnage',
  'movie',
  2021,
  './posters/веном-2-2fd04e97.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017ce64759e953040d947b3fad9e77/1344x756',
  'Более чем через год после того как в него вселился инопланетный симбиот Веном, журналист Эдди Брок пытается приспособиться к новой жизни с суперспособностями и постоянным желанием Венома откусывать людям головы. Брок пытается возродить свою карьеру и берет интервью у серийного убийцы Клетуса Кэседи, а тот случайно становится хозяином симбиота Карнажа и сбегает из тюрьмы прямо во время собственной казни.',
  array['фантастика', 'боевик', 'ужасы']::text[],
  array[]::text[],
  580489,
  1227967,
  'tt7097896',
  6.2,
  5.9,
  null,
  null,
  null,
  '2026-06-21T12:02:46.000Z',
  false,
  false,
  false
),
(
  'media-рапунцель-запутанная-история-167',
  'Рапунцель: Запутанная история',
  'Tangled',
  'cartoon',
  2010,
  './posters/рапунцель-запутанная-история-9ccd4d2d.jpg',
  './posters/рапунцель-запутанная-история-9ccd4d2d.jpg',
  'Обаятельный разбойник Флинн путешествует по жизни с легкостью, лишь потому, что он красив, болтлив и удачлив. И, казалос',
  array['мультфильм', 'мюзикл', 'фэнтези', 'мелодрама', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Япония']::text[],
  38757,
  84049,
  'tt0398286',
  8,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:02:47.000Z',
  false,
  false,
  false
),
(
  'media-ведьмак-168',
  'Ведьмак',
  'The Witcher',
  'series',
  2019,
  './posters/ведьмак-41f80365.jpg',
  './posters/ведьмак-41f80365.jpg',
  'Ведьмак Геральт, мутант и убийца чудовищ, на своей верной лошади по кличке Плотва путешествует по Континенту. За тугой мешочек чеканных монет этот мужчина избавит вас от всякой настырной нечисти — хоть от чудищ болотных, оборотней и даже заколдованных принцесс. В сельской глуши местную девушку Йеннифэр, которой сильно не повезло с внешностью, зато посчастливилось иметь способности к магии, отец продаёт колдунье в ученицы. А малолетняя наследница королевства Цинтра по имени Цири вынуждена пуститься в бега, когда их страну захватывает империя Нильфгаард. Судьбы этих троих окажутся тесно связаны, но скоро сказка сказывается, да не скоро дело делается.',
  array['фэнтези', 'приключения', 'драма', 'ужасы']::text[],
  array['США', 'Польша']::text[],
  null,
  1044004,
  'tt5180504',
  7.1,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:02:48.000Z',
  false,
  false,
  false
),
(
  'media-9-рота-169',
  '9 рота',
  '9 рота',
  'movie',
  2005,
  './posters/9-рота-3ae18fac.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a000001622527717aeed17324f4feb3f724/1344x756',
  'Начало 1989 года. Семь призывников после нескольких месяцев адской подготовки в учебке под командованием беспощадного старшины попадают в горнило афганской кампании. Группа десантников, бойцами которой стали новобранцы, получает задание — занять высоту и держать её до прохождения колонны.',
  array['военный', 'боевик', 'драма', 'история']::text[],
  array[]::text[],
  14097,
  84674,
  'tt0417397',
  6.9,
  7,
  null,
  null,
  null,
  '2026-06-21T12:02:49.000Z',
  false,
  false,
  false
),
(
  'media-железный-человек-3-170',
  'Железный человек 3',
  'Iron Man Three',
  'movie',
  2013,
  './posters/железный-человек-3-c38f51a1.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000178cbf75c1631b1bf82a201610b49/1344x756',
  'Когда мир Старка рушится на его глазах по вине неизвестных противников, Тони жаждет найти виновных и свести с ними счеты. Оказавшись в безвыходной ситуации, Старк может рассчитывать только на себя и свою изобретательность, чтобы защитить тех, кто ему дорог. Это становится настоящим испытанием для героя, которому придется не только сражаться с коварным врагом, но и разобраться в себе, чтобы найти ответ на вопрос, который давно его тревожит: что важнее - человек или костюм?',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Китай']::text[],
  68721,
  462762,
  'tt1300854',
  7.5,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:02:50.000Z',
  false,
  false,
  false
),
(
  'media-риддик-171',
  'Риддик',
  'Riddick',
  'movie',
  2013,
  './posters/риддик-1a65e2bb.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a00000161284318973b270e18c441162a82/1344x756',
  'Преданный своими и брошенный умирать на пустынной планете, Риддик сражается с хищниками за жизнь и становится сильнее и опаснее себя прежнего. Открывшие на него охоту галактические наемники оказываются пешками в грандиозном плане отмщения. С врагами, возникающими на его пути тогда, когда это нужно самому Риддику, он начинает поход во имя мести.',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array[]::text[],
  87421,
  455767,
  'tt1411250',
  6.8,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:02:51.000Z',
  false,
  false,
  false
),
(
  'media-семь-жизней-172',
  'Семь жизней',
  'Seven Pounds',
  'movie',
  2008,
  './posters/семь-жизней-0345a4ad.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017399a1cbe5590181a43a38c6ea48/1344x756',
  'Инженер Бен отправляется в необычное путешествие. В ходе своей поездки он встречает семерых незнакомцев, включая смертельно больную Эмили, которая называет себя девушкой с подбитыми крыльями. Бен неожиданно влюбляется в нее, что сильно усложняет его первоначальный план. Сможет ли он разгадать послание судьбы?',
  array['драма', 'мелодрама']::text[],
  array['США']::text[],
  11321,
  395787,
  'tt0814314',
  8.2,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:02:52.000Z',
  false,
  false,
  false
),
(
  'media-миньоны-173',
  'Миньоны',
  'Minions',
  'cartoon',
  2015,
  './posters/миньоны-ba81fae7.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a000001612cbad5d3d8c27e7545124aa35e/1344x756',
  'Миньоны живут на планете гораздо дольше нас. У них одна навязчивая идея – служить самой Гадкой личности из имеющихся в наличии. Динозавры, фараоны, Дракула, Наполеон – все они оказались недолговечны. И тогда миньоны рванули в Нью-Йорк. Их ждут деньги, власть, роковая суперзлодейка Скарлет Оверкилл и, конечно же, вкуснейшая “банана”! Во всяком случае, так им кажется.',
  array['мультфильм', 'фантастика', 'комедия', 'криминал', 'приключения', 'семейный']::text[],
  array[]::text[],
  211672,
  694051,
  'tt2293640',
  6.2,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:02:53.000Z',
  false,
  false,
  false
),
(
  'media-я-робот-174',
  'Я, робот',
  'I, Robot',
  'movie',
  2004,
  './posters/я-робот-e6874065.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a00000172283861f48df10b384c1e040a4b/1344x756',
  '2035 год. Роботы стали привычной частью быта. Однако ряд консервативно настроенных людей, среди которых полицейский Дел Спунер, не доверяют новым помощникам и видят в них угрозу. Однажды Спунер выезжает на место гибели давнего знакомого, профессора Альфреда Лэннинга, который был ведущим конструктором и теоретиком корпорации U.S. Robotics. Директор компании Лоуренс Робертсон считает, что Лэннинг просто покончил с собой, выбросившись из окна. Однако у Спунера есть сомнения в правдоподобности данной версии. Вместе со специалистом по психологии позитронных роботов Сьюзан Кэлвин он вскоре обнаруживает в лаборатории Лэннинга особенного робота, который вопреки второму закону робототехники игнорирует приказы человека и принимает собственные осознанные решения.',
  array['фантастика', 'боевик', 'триллер', 'детектив']::text[],
  array['США', 'Германия']::text[],
  2048,
  4886,
  'tt0343818',
  7.9,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:02:54.000Z',
  false,
  false,
  false
),
(
  'media-тор-175',
  'Тор',
  'Thor',
  'movie',
  2011,
  './posters/тор-2dc65223.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000016904e109ece54d43ffc6c8ad3d8e/1344x756',
  'Первый сезон. Семейство Хармонов переезжает из Бостона в Лос-Анджелес, чтобы начать новую жизнь, и поселяются в старинном отреставрированном особняке, не представляя, что его предыдущие жильцы так и не нашли покоя после смерти.
Второй сезон. Журналистка приезжает в лечебницу для душевнобольных преступников в надежде снять репортаж о новоприбывшем маньяке по прозвищу Кровавый Лик.
Третий сезон. Шабаш ведьм, замаскированный под элитный пансион для одаренных девушек, пытается защитить последних представительниц этого вида от вымирания.
Четвёртый сезон. История одного из последних цирков уродов в 1950-х годах.
Пятый сезон. Отель в центральной части Лос-Анджелеса скрывает в своих стенах множество секретов.
Шестой сезон. Пара из Калифорнии переезжает в новый дом, где её начинают преследовать паранормальные явления. 
Седьмой сезон. История президентских выборов 2016 года в США.
Восьмой сезон. Несколько стран запустили ядерные ракеты, уничтожившие большую часть населения Земли. Избранным «счастливчикам» посчастливилось укрыться в бункере.
Девятый сезон. Пять друзей отправляются работать вожатыми в детский лагерь. Осваиваясь на новой работе, молодые люди начинают понимать, что прошлое, которое их преследует, куда опаснее лагерных страшилок, рассказанных у костра.
Десятый сезон. Испытывающий проблемы писатель, его беременная жена и их дочь переезжают на зиму в пляжный городок. Как только они обустраиваются, истинные жители города начинают заявлять о себе. А также группа студентов, отправившихся в поход, оказывается в эпицентре заговора, который готовился десятилетиями.',
  array['ужасы', 'триллер', 'драма']::text[],
  array['США']::text[],
  10195,
  258941,
  'tt0800369',
  8,
  7,
  null,
  null,
  null,
  '2026-06-21T12:02:55.000Z',
  false,
  false,
  false
),
(
  'media-монстры-на-каникулах-176',
  'Монстры на каникулах',
  'Hotel Transylvania',
  'cartoon',
  2012,
  './posters/монстры-на-каникулах-b964d951.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017fad081f2b78f2ab65ae104ed7aa/1344x756',
  'Граф Дракула для себя и своей любимой дочки построил хорошо укрытый от посторонних глаз отель, куда монстры со всего света приезжают отдохнуть от семейных забот, повседневной суеты и, конечно… людей. Но вездесущие американские туристы проникают и сюда.',
  array['мультфильм', 'фэнтези', 'комедия', 'семейный']::text[],
  array['США']::text[],
  76492,
  280174,
  'tt0837562',
  7.6,
  7,
  null,
  null,
  null,
  '2026-06-21T12:02:56.000Z',
  false,
  false,
  false
),
(
  'media-назад-в-будущее-2-177',
  'Назад в будущее 2',
  'Back to the Future Part II',
  'movie',
  1989,
  './posters/назад-в-будущее-2-14c6bfe3.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000016128b1f9b0eef660985ee93c5daf/1344x756',
  'Продолжение фантастической истории о приключениях американского подростка во времени. На этот раз с помощью модернизированной Доком машины времени Марти из 80-х попадает в будущее.

Дети Марти в беде, и их надо выручать. Приходится повозиться со злодеем...',
  array['фантастика', 'приключения', 'боевик', 'комедия', 'семейный']::text[],
  array[]::text[],
  165,
  5502,
  'tt0096874',
  8.3,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:02:57.000Z',
  false,
  false,
  false
),
(
  'media-мстители-эра-альтрона-178',
  'Мстители: Эра Альтрона',
  'Мстители: Эра Альтрона',
  'movie',
  2012,
  './posters/мстители-эра-альтрона-becf918c.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000168e7cbf1bf723879ea306ad8f363/1344x756',
  'Человечество на грани уничтожения. На этот раз людям угрожает Альтрон — искусственный интеллект, ранее созданный для того, чтобы защищать Землю от любых угроз. Однако, главной угрозой он посчитал человечество. Международная организация Щ.И.Т. распалась, и теперь мир не способен справиться с таким мощным врагом, потому люди вновь обращаются за помощью к Величайшим Героям Земли — Мстителям. Однако Альтрон слишком силен, и есть большая вероятность, что даже им не удастся остановить начало надвигающейся Эры Альтрона, где нет места для людей…',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  24428,
  263531,
  'tt0848228',
  7.4,
  8,
  null,
  null,
  null,
  '2026-06-21T12:02:58.000Z',
  false,
  false,
  false
),
(
  'media-балерина-179',
  'Балерина',
  'Ballerina',
  'movie',
  2025,
  './posters/балерина-840b1fcb.jpg',
  'https://avatars.mds.yandex.net/get-ott/13074011/2a000001971cab11992d25d2453e3b10452d/1344x756',
  'Когда Ева была маленькой, вооруженные люди во главе с мужчиной по прозвищу Канцлер оставили её сиротой. Так девочка оказалась в криминальной семье «Русска рома» на попечении Директрисы нью-йоркской балетной школы, одновременно служащей академией наемных убийц. Не добившись успехов в балете, но боевые курсы окончив с отличием, Ева отправляется в Европу мстить за папу.',
  array['боевик', 'триллер']::text[],
  array[]::text[],
  541671,
  1047492,
  'tt7181546',
  7.2,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:02:59.000Z',
  false,
  false,
  false
),
(
  'media-вверх-180',
  'Вверх',
  'Up',
  'movie',
  2009,
  './posters/вверх-1a5ae4ca.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a00000168f00e708cf5a66a53587d8f0f45/1344x756',
  '78-летний ворчун Карл Фредриксен считает, что жизнь обходит его стороной. Чтобы сдержать обещание, данное своей почившей жене, он решает осуществить свою мечту о великом приключении, привязав тысячи воздушных шариков к своему дому и улетев в дебри Южной Америки. Не пролетев и полмили, путешественник обнаруживает, что он ненароком прихватил с собой крайне разговорчивого и неисправимо жизнерадостного 8-летнего мальчика по имени Рассел...',
  array['мультфильм', 'драма', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  14160,
  325381,
  'tt1049413',
  8,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:03:00.000Z',
  false,
  false,
  false
),
(
  'media-никто-181',
  'Никто',
  'Nobody',
  'movie',
  2021,
  './posters/никто-56c6e93d.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017c074fadcf98ebfe243c20dbecae/orig',
  'Непримечательный и незаметный семьянин Хатч живёт скучной жизнью обычного аудитора, пока однажды в его дом не вламываются грабители. И это бы сошло им с рук, если бы они не забрали браслетик его маленькой дочки. Не в силах это терпеть, Хатч отправляется на поиски наглецов, а на обратном пути ввязывается в драку с пьяными хулиганами, пристававшими к девушке в общественном транспорте. От души помахав кулаками, наш аудитор отправляет дебоширов в больницу, но оказывается, что один из пострадавших — брат влиятельного русского бандита. И он теперь жаждет мести.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США', 'Япония']::text[],
  615457,
  1309596,
  'tt7888964',
  7.4,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:03:01.000Z',
  false,
  false,
  false
),
(
  'media-люси-182',
  'Люси',
  'Lucy',
  'movie',
  2014,
  './posters/люси-d09a1e47.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c07251326495f3fd3da3dd53e2b/orig',
  'Еще вчера она была просто сексапильной блондинкой, а сегодня — самое опасное и смертоносное создание на планете со сверхъестественными способностями и интеллектом. То, что совсем недавно лучшие умы мира считали фантастической теорией, для нее стало реальностью. И теперь из добычи она превратится в охотницу.',
  array['боевик', 'фантастика']::text[],
  array['Франция', 'Германия', 'Тайвань', 'Канада', 'Великобритания', 'США']::text[],
  240832,
  760326,
  'tt2872732',
  6.8,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:03:02.000Z',
  false,
  false,
  false
),
(
  'media-война-миров-z-183',
  'Война миров Z',
  'World War Z',
  'movie',
  2013,
  './posters/война-миров-z-6ef603cf.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a000001967d1a607c6e4325ea64792f2327/orig',
  'Бывший сотрудник ООН Джерри Лэйн оказывается в эпицентре эпидемии неизвестного вируса, который за считанные секунды превращает людей в зомби. Пытаясь найти противоядие против вируса, Лэйн путешествует вместе со своей группой почти по всему миру, поражённому эпидемией. Теперь судьба всего мира висит на волоске, и Джерри — его единственная надежда.',
  array['ужасы', 'фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  72190,
  261636,
  'tt0816711',
  7,
  7,
  null,
  null,
  null,
  '2026-06-21T12:03:03.000Z',
  false,
  false,
  false
),
(
  'media-васаби-184',
  'Васаби',
  'Wasabi',
  'movie',
  2001,
  './posters/васаби-582ab996.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017e7c2d839edf8bd7a8f3adae90c2/orig',
  'Исполняя последнюю волю бывшей любовницы, французский полицейский Юбер Фиорентини отправляется в Японию, где ему приходится встретиться со старым другом и дочерью, о существовании которой он даже не подозревал. А также получить наследство в 200 млн. долларов и познакомиться с якудза, которые охотятся за этими деньгами.',
  array['боевик', 'триллер', 'драма', 'комедия', 'криминал']::text[],
  array['Франция', 'Япония']::text[],
  2110,
  14288,
  'tt0281364',
  7.6,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:03:04.000Z',
  false,
  false,
  false
),
(
  'media-ледниковый-период-185',
  'Ледниковый период',
  'Ice Age',
  'cartoon',
  2002,
  './posters/ледниковый-период-e12f710c.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c280e362c5f91b266a7cd54e6e2/orig',
  'Чтобы избежать холода, приближающегося из-за наступления ледникового периода, животные мигрируют на юг. Однако одинокий, угрюмый мамонт Манфред и бесшабашный ленивец Сид решают остаться.
Случайно эта парочка наталкивается на человеческого детёныша. Они отправляются в путешествие, чтобы вернуть его людям. В пути к ним присоединяется хитрый саблезубый тигр.',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  425,
  707,
  'tt0268380',
  8,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:03:05.000Z',
  false,
  false,
  false
),
(
  'media-солнцестояние-186',
  'Солнцестояние',
  'Midsommar',
  'movie',
  2019,
  './posters/солнцестояние-05f1acd3.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c2892f98a42f7d92bc93906c08b/orig',
  'После внезапной смерти родителей и сестры девушка Дэни впадает в жёсткую депрессию. Её бойфренд Кристиан, студент-антрополог, которого давно тяготят эти непростые отношения, вынужденно приглашает её в поездку в Швецию на праздник летнего солнцестояния, куда он планировал отправиться вместе с тремя друзьями-одногруппниками для работы над диссертацией. В отрезанном от цивилизованного мира поселении в этот день проводятся уникальные обряды с многовековой традицией. Однако вскоре после прибытия компания выясняет, что местные обряды далеко не безобидны.',
  array['ужасы', 'драма', 'триллер']::text[],
  array['США', 'Швеция']::text[],
  530385,
  1178137,
  'tt8772262',
  6.4,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:03:06.000Z',
  false,
  false,
  false
),
(
  'media-старикам-тут-не-место-187',
  'Старикам тут не место',
  'No Country for Old Men',
  'movie',
  2007,
  './posters/старикам-тут-не-место-87a58db9.jpg',
  'https://avatars.mds.yandex.net/get-ott/12808873/2a0000019662462b3c9d6a145984a9319a1c/orig',
  'Обычный работяга обнаруживает в пустыне гору трупов, набитый героином грузовик и соблазнительную сумму в два миллиона долларов наличными. Он решает взять деньги себе, и результатом становится волна насилия, которую не может остановить вся полиция Западного Техаса.',
  array['триллер', 'драма', 'криминал']::text[],
  array['США']::text[],
  6977,
  195434,
  'tt0477348',
  7.7,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:03:07.000Z',
  false,
  false,
  false
),
(
  'media-малефисента-188',
  'Малефисента',
  'Maleficent',
  'movie',
  2014,
  './posters/малефисента-312745bc.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c284df6f62ee1c2c7e057f495a3/orig',
  'Юная волшебница Малефисента вела уединенную жизнь в зачарованном лесу, окруженная сказочными существами, но однажды все изменилось… В её мир вторглись люди, которые принесли с собой разрушение и хаос, и Малефисенте пришлось встать на защиту своих подданных, призвав на помощь могущественные тёмные силы.',
  array['фэнтези', 'семейный', 'мелодрама', 'приключения']::text[],
  array['США', 'Великобритания']::text[],
  102651,
  496849,
  'tt1587310',
  7.3,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:03:08.000Z',
  false,
  false,
  false
),
(
  'media-малефисента-владычица-тьмы-189',
  'Малефисента: Владычица тьмы',
  'Maleficent: Mistress of Evil',
  'movie',
  2019,
  './posters/малефисента-владычица-тьмы-4a6589df.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c2872a520bc79b2e21c08219ac2/orig',
  'Принцесса Аврора выросла, и теперь принц Филип делает ей предложение. Наивные молодые люди уверены, что их брак сможет объединить два королевства и помирить людей с волшебными существами, но не тут-то было. На совместном ужине с родителями Филипа — королём Джоном и королевой Ингрит — куда была приглашена и крёстная Авроры, происходит ссора, в результате чего король слегает с недомоганием явно магического свойства, а Малефисента вынуждена спасаться бегством.',
  array['фэнтези', 'драма', 'боевик']::text[],
  array['США']::text[],
  420809,
  916498,
  'tt4777008',
  7.1,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:03:09.000Z',
  false,
  false,
  false
),
(
  'media-малышка-на-миллион-190',
  'Малышка на миллион',
  'Million Dollar Baby',
  'movie',
  2004,
  './posters/малышка-на-миллион-642b1245.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017c0749ecadba778ee386ef6cfcd9/orig',
  'Тренеру по боксу Фрэнку Данну так и не удалось воспитать чемпиона. Он владеет спортивным залом в Лос-Анджелесе, где всё ещё проводит тренировки. Дочь не отвечает на его письма, а его лучший боец подписал контракт с другим менеджером. Неожиданно в жизни Фрэнка появляется Мэгги Фицжеральд, 31-летняя официантка, мечтающая стать боксером. Фрэнк не желает тренировать женщину, но упорство Мэгги заставляет его передумать. Впереди - их главное сражение, требующее собрать в кулак всю волю и мужество.',
  array['драма', 'спорт']::text[],
  array['США']::text[],
  70,
  81297,
  'tt0405159',
  8.1,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:03:10.000Z',
  false,
  false,
  false
),
(
  'media-сумерки-191',
  'Сумерки',
  'Twilight',
  'movie',
  2008,
  './posters/сумерки-ef327666.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c0779e15d49e1160a4a42905da1/orig',
  'Семнадцатилетняя девушка Белла переезжает к отцу в небольшой городок Форкс. Она влюбляется в загадочного одноклассника, который, как оказалось, происходит из семьи вампиров, отказавшихся от нападений на людей. Влюбиться в вампира. Это страшно? Это романтично, это прекрасно и мучительно, но это не может кончиться добром, особенно в вечном противостоянии вампирских кланов, где малейшее отличие от окружающих уже превращает вас во врага.',
  array['фэнтези', 'драма', 'мелодрама']::text[],
  array['США']::text[],
  8966,
  401177,
  'tt1099212',
  6.7,
  5.4,
  null,
  null,
  null,
  '2026-06-21T12:03:11.000Z',
  false,
  false,
  false
),
(
  'media-амели-192',
  'Амели',
  'Le Fabuleux destin d''Amélie Poulain',
  'movie',
  2001,
  './posters/амели-7d03eaa6.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000183f49cce9044a2dedd5a625cc023/orig',
  'Как полет крошечной мухи может вызвать где-то далеко мощный ураган, так и странные и, на первый взгляд, непонятные поступки тихой одинокой девушки, живущей в мире своих фантазий, могут навсегда изменить жизнь совершенно разных людей, подарив им счастье и раскрасив окружающий мир яркими красками. Эту девушку зовут Амели Пулен.',
  array['мелодрама', 'комедия']::text[],
  array['Франция', 'Германия']::text[],
  194,
  341,
  'tt0211915',
  8,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:03:12.000Z',
  false,
  false,
  false
),
(
  'media-безумный-макс-дорога-ярости-193',
  'Безумный Макс: Дорога ярости',
  'Mad Max: Fury Road',
  'movie',
  2015,
  './posters/безумный-макс-дорога-ярости-1a26eacb.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c074f85dc607e1a5d471d27587d/orig',
  'Преследуемый призраками прошлого Макс уверен, что лучший способ выжить — скитаться в одиночестве. Но он попадает в плен и вынужденно присоединяется к бунтарям, бегущим через пустыню от тирании Несмертного Джо. Они забрали у него кое-что очень ценное, и разъярённый диктатор бросает все силы в погоню за мятежниками.',
  array['боевик', 'фантастика', 'приключения']::text[],
  array['Австралия', 'США']::text[],
  76341,
  453406,
  'tt1392190',
  7.8,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:03:13.000Z',
  false,
  false,
  false
),
(
  'media-крестный-отец-194',
  'Крестный отец',
  'The Godfather',
  'movie',
  1972,
  './posters/крестный-отец-d80f3603.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a000001963f72757763becfb93aa835d006/orig',
  'Криминальная сага, повествующая о нью-йоркской сицилийской мафиозной семье Корлеоне. Фильм охватывает период 1945-1955 годов.

Глава семьи, Дон Вито Корлеоне, выдаёт замуж свою дочь. В это время со Второй мировой войны возвращается его любимый сын Майкл. Майкл, герой войны, гордость семьи, не выражает желания заняться жестоким семейным бизнесом. Дон Корлеоне ведёт дела по старым правилам, но наступают иные времена, и появляются люди, желающие изменить сложившиеся порядки. На Дона Корлеоне совершается покушение.',
  array['драма', 'криминал']::text[],
  array['США']::text[],
  238,
  325,
  'tt0068646',
  8.7,
  9.2,
  null,
  null,
  null,
  '2026-06-21T12:03:14.000Z',
  false,
  false,
  false
),
(
  'media-форсаж-195',
  'Форсаж',
  'The Fast and the Furious',
  'movie',
  2001,
  './posters/форсаж-222e46dc.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017e9e7a0215073fbbf4b4f78980c7/orig',
  'Его зовут Брайан, и он — фанат турбин и нитроускорителей. Он пытается попасть в автобанду легендарного Доминика Торетто, чемпиона опасных и незаконных уличных гонок. Брайан также полицейский, и его задание — втереться в доверие к Торетто, подозреваемому в причастности к дерзким грабежам грузовиков, совершаемым прямо на ходу.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США', 'Германия']::text[],
  9799,
  666,
  'tt0232500',
  7.8,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:03:15.000Z',
  false,
  false,
  false
),
(
  'media-двойной-форсаж-196',
  'Двойной форсаж',
  '2 Fast 2 Furious',
  'movie',
  2003,
  './posters/двойной-форсаж-6ea56d03.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017e7cde1bbe5f4e625e97823d1db6/orig',
  'Бывший полицейский Брайан О`Коннер с напарником Романом Пирсом собирается перевезти крупную сумму грязных денег известного мафиози Картера Верона. Но на самом деле эта работа является только прикрытием для Брайана, который вместе с тайным агентом Моникой Фуэнтес должен уличить изворотливого преступника.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США', 'Германия']::text[],
  null,
  323,
  'tt0322259',
  7.2,
  6,
  null,
  null,
  null,
  '2026-06-21T12:03:16.000Z',
  false,
  false,
  false
),
(
  'media-форсаж-токийский-дрифт-197',
  'Тройной форсаж: Токийский дрифт',
  'The Fast and the Furious: Tokyo Drift',
  'movie',
  2006,
  './posters/тройной-форсаж-токийский-дрифт-b5b7d607.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001616b864cfcc125c52d7188e8a08c/1344x756',
  'Старшеклассник Шон Босуэлл только и делает, что попадает в неприятности. После очередной выходки — импровизированных гонок и аварии — парню уже светит тюрьма, тогда мать решает отправить его к отцу в Японию. В первый же день в японской школе он знакомится с соотечественником, а тот притаскивает нового друга на подпольные соревнования по дрифт-рейсингу. Тут Шону открывается доселе невиданное искусство прохождения поворотов — и он сразу же ввязывается в спор, с позором проиграв который, оказывается должен местному авторитету по имени Хан.',
  array['боевик', 'криминал', 'мелодрама', 'триллер']::text[],
  array['США', 'Германия']::text[],
  null,
  106079,
  'tt0463985',
  6.9,
  6.1,
  null,
  null,
  null,
  '2026-06-21T12:03:17.000Z',
  false,
  false,
  false
),
(
  'media-форсаж-4-198',
  'Форсаж 4',
  'Fast & Furious',
  'movie',
  2009,
  './posters/форсаж-4-bd92afab.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c284040acc03f00b742da3cf57a/orig',
  'Когда кривая дорожка заставляет скрывающегося от правосудия Дома Торетто вернуться в Лос-Анджелес, вражда с агентом Брайаном О’Коннером вспыхивает с новой силой. Но у парочки обнаруживается общий противник, и Дому с Брайаном приходится заключить перемирие в надежде одержать над ним победу. Грабя конвои, роя подкопы и пересекая границы, они приходят к идеальной форме отмщения — педали газа, вдавленной до упора.',
  array['криминал', 'боевик', 'триллер', 'драма']::text[],
  array['Япония', 'США']::text[],
  null,
  395978,
  'tt1013752',
  6.9,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:03:18.000Z',
  false,
  false,
  false
),
(
  'media-форсаж-5-199',
  'Форсаж 5',
  'Fast Five',
  'movie',
  2011,
  './posters/форсаж-5-b8bb5705.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c072a66997657ab48b9436a3974/orig',
  'Бывший полицейский Брайан О`Коннер вместе с бывалым лихачом Домиником Торетто совсем немного увлеклись жизнью вне закона. После того, как Брайан и Миа Торетто освободили Доминика, они не рискуют долго задерживаться на одном месте, так как являются желанной целью порядком недовольных копов.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США', 'Япония']::text[],
  null,
  496943,
  'tt1596343',
  7.6,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:03:19.000Z',
  false,
  false,
  false
),
(
  'media-прибытие-200',
  'Прибытие',
  'Arrival',
  'movie',
  2016,
  './posters/прибытие-2a66020b.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c2861e5dd1672f8a6f9cf68c82e/orig',
  'Неожиданное появление неопознанных летающих объектов в разных точках планеты повергает мир в трепет. Намерения пришельцев не ясны — вооружённые силы всего мира приведены в полную боевую готовность, а лучшие умы человечества пытаются понять, как разговаривать с непрошеными гостями. Правительство США обращается за помощью к лингвисту Луизе Бэнкс и физику Иэну Доннели, чтобы те в кратчайшие сроки помогли найти общий язык с пришельцами.',
  array['фантастика', 'драма']::text[],
  array['США', 'Канада']::text[],
  329865,
  718811,
  'tt2543164',
  7.6,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:03:20.000Z',
  false,
  false,
  false
),
(
  'media-трансформеры-201',
  'Трансформеры',
  'Transformers',
  'movie',
  2007,
  './posters/трансформеры-b94a434e.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a000001966cae2f38dba70bfae9bed549b1/orig',
  'В течение многих столетий две расы роботов-инопланетян — Автоботы и Десептиконы — вели войну, ставкой в которой была судьба Вселенной. И вот война докатилась до Земли. В то время, когда силы зла ищут ключ к верховной власти, наш последний шанс на спасение находится в руках юного землянина. Единственное, что стоит между несущими зло Десептиконами и высшей властью - это ключ, находящийся в руках простого парнишки.
Обычный подросток, Сэм Уитвикки озабочен повседневными хлопотами — школа, друзья, машины, девочки. Не ведая о том, что он является последним шансом человечества на спасение, Сэм вместе со своей подругой Микаэлой оказывается вовлеченным в противостояние Автоботов и Десептиконов. Только тогда Сэм понимает истинное значение семейного девиза Уитвикки — «без жертв нет победы!»',
  array['фантастика', 'боевик']::text[],
  array['США']::text[],
  1858,
  81288,
  'tt0418279',
  7.6,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:03:21.000Z',
  false,
  false,
  false
),
(
  'media-знакомьтесь-джо-блэк-202',
  'Знакомьтесь, Джо Блэк',
  'Meet Joe Black',
  'movie',
  1998,
  './posters/знакомьтесь-джо-блэк-f6b23535.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a00000187bd50949da0a4a996ade1e5e0b2/orig',
  'В жизни газетного магната Уильяма Пэрриша появляется сама Смерть, принявшая обличье молодого человека по имени Джо Блэк. Смерть, уставшая от своих привычных обязанностей, предлагает Пэрришу необычное соглашение: магнат станет проводником Джо в мире живых, а по окончании каникул Смерть заберет Пэрриша с собой.',
  array['фэнтези', 'драма', 'мелодрама']::text[],
  array['США']::text[],
  297,
  5059,
  'tt0119643',
  8,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:03:22.000Z',
  false,
  false,
  false
),
(
  'media-алиса-в-стране-чудес-203',
  'Алиса в Стране чудес',
  'Alice in Wonderland',
  'movie',
  2010,
  './posters/алиса-в-стране-чудес-a48f9676.jpg',
  './posters/алиса-в-стране-чудес-a48f9676.jpg',
  'Жизнь 19-летней Алисы Кингсли принимает неожиданный оборот. На викторианской вечеринке, устроенной в её честь, Алисе дел',
  array['фэнтези', 'детектив', 'приключения', 'семейный']::text[],
  array['США', 'Италия', 'Канада', 'Япония', 'Великобритания']::text[],
  12155,
  405609,
  'tt1014759',
  7.1,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:03:23.000Z',
  false,
  false,
  false
),
(
  'media-ван-хельсинг-204',
  'Ван Хельсинг',
  'Van Helsing',
  'movie',
  2004,
  './posters/ван-хельсинг-1284c084.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017c076db13d1ac886ee4f4f435f83/orig',
  'Глубоко в Карпатах лежит таинственная страна Трансильвания — мир, в котором зло встречается на каждом шагу, мир, в котором опасность оживает с заходом солнца, и где монстры, живущие в глубине человеческих кошмаров, обретают форму. В этот мир попадает легендарный охотник на чудовищ Ван Хельсинг.

В своей непрерывной битве против сил зла и за освобождение мира от порождений ада Ван Хельсинг, по заданию секретного общества, отправляется в Трансильванию, чтобы скрестить оружие со смертельно опасным, наделённым неведомой силой графом Дракулой. На помощь ему внезапно приходит бесстрашная Анна Валери, поставившая целью избавить свою семью от векового проклятья, наложенного убитым вампиром.',
  array['фэнтези', 'боевик', 'триллер', 'детектив', 'приключения']::text[],
  array['США', 'Чехия']::text[],
  7131,
  3491,
  'tt0338526',
  7.6,
  6.1,
  null,
  null,
  null,
  '2026-06-21T12:03:24.000Z',
  false,
  false,
  false
),
(
  'media-гонка-205',
  'Гонка',
  'Rush',
  'movie',
  2013,
  './posters/гонка-e5211333.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c077d316883ce07e9c9d1000354/orig',
  '1970-е. Два непримиримых соперника в гонках «Формула-1» — обаятельный плейбой-англичанин Джеймс Хант и дисциплинированный перфекционист-австриец Ники Лауда — доводят себя до предела физической и психологической выносливости ради победы. Каждый из них стремится быть первым, а ошибка на трассе может стоить гонщикам жизни.',
  array['спорт', 'драма', 'биография']::text[],
  array['Великобритания', 'Германия', 'США']::text[],
  96721,
  596125,
  'tt1979320',
  8.1,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:03:25.000Z',
  false,
  false,
  false
),
(
  'media-человек-паук-возвращение-домой-206',
  'Человек-паук: Возвращение домой',
  'Spider-Man: Homecoming',
  'movie',
  2017,
  './posters/человек-паук-возвращение-домой-81aa3fd7.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c07517452ef198a79f973973cd5/orig',
  'После встречи с командой Мстителей Питер Паркер возвращается домой и пытается жить обычной жизнью под опекой тёти Мэй. Но теперь за Питером приглядывает кое-кто ещё. Тони Старк видел Человека-паука в деле и должен стать его наставником. Когда новый злодей Стервятник угрожает уничтожить всё, что дорого Питеру, приходит время показать всем, что такое настоящий супергерой.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  315635,
  690593,
  'tt2250912',
  7.2,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:03:26.000Z',
  false,
  false,
  false
),
(
  'media-первый-мститель-противостояние-207',
  'Первый мститель: Противостояние',
  'Captain America: Civil War',
  'movie',
  2016,
  './posters/первый-мститель-противостояние-b9cd12e1.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c286a5d007ce626d6aaad8db7d9/orig',
  'Мстители под руководством Капитана Америки оказываются участниками разрушительного инцидента, имеющего международный масштаб. Эти события заставляют правительство задуматься над тем, чтобы начать регулировать действия всех людей с особыми способностями, введя «Акт о регистрации супергероев», вынуждая их подчиняться ООН.

Видя в этом договоре нарушение гражданских прав и плевок на всё, во что он верит, Стив Роджерс открыто восстаёт против нового режима, ведя за собой группу героев, поддерживающих его мнение. В свою очередь, Тони Старк видит смысл в новом договоре и становится во главе героев, поддерживающих его. Всё это приводит к неизбежному расколу Мстителей и войне между сторонами. Сражаясь друг с другом, Старк и Роджерс не знают о том, что таинственный Барон Земо уже строит свои собственные планы, воспользовавшись расколом величайших героев Земли.',
  array['фантастика', 'боевик']::text[],
  array['США', 'Германия']::text[],
  271110,
  822708,
  'tt3498820',
  7.4,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:03:27.000Z',
  false,
  false,
  false
),
(
  'media-пацаны-208',
  'Пацаны',
  'The Boys',
  'series',
  2019,
  './posters/пацаны-b35d99b9.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c0791e438d10a65d3d63c4381b9/orig',
  'Супергерои являются настоящими звездами. Их все знают и обожают. Но за идеальным фасадом скрывается мрачный мир наркотиков и секса, а большинство героев в жизни не самые приятные люди. Противостоит им отряд, неофициально известный как «Пацаны».',
  array['фантастика', 'боевик', 'драма', 'комедия', 'криминал']::text[],
  array['США']::text[],
  76479,
  460586,
  'tt1190634',
  8.2,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:03:28.000Z',
  false,
  false,
  false
),
(
  'media-бременские-музыканты-209',
  'Бременские музыканты',
  'Бременские музыканты',
  'cartoon',
  1969,
  './posters/бременские-музыканты-3dc5816c.jpg',
  './posters/бременские-музыканты-3dc5816c.jpg',
  'Музыкальная фантазия, любимая не одним поколением зрителей, о необыкновенных, очень опасных и веселых приключениях бреме',
  array['мультфильм', 'детский', 'короткометражка', 'мюзикл', 'фэнтези']::text[],
  array['СССР']::text[],
  33108,
  46223,
  'tt0211281',
  8.3,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:03:29.000Z',
  false,
  false,
  false
),
(
  'media-хэнкок-210',
  'Хэнкок',
  'Hancock',
  'movie',
  2008,
  './posters/хэнкок-313ea29d.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c07560ae79fab81b0cc9af7d475/orig',
  'Есть герои, есть супергерои, и есть Хэнкок. Обладание сверхспособностями предполагает ответственность, все знают это — кроме него. За любую задачу он берётся с душой и лучшими намерениями, спасает жизни людей — ценой нечеловеческих разрушений и неисчислимого ущерба. В конце концов, терпение общественности подходит к концу: люди благодарны своему местному герою, но иногда не понимают, чем заслужили такое наказание.

Хэнкок не из тех парней, кого волнует какое-то там общественное мнение, но однажды, после очередного подвига, спасая высокопоставленного пиарщика Рэя, он вдруг понимает, что в некотором смысле он может быть уязвим. С этим трудно смириться, особенно по мнению Мэри, жены Рэя, считающей, что Хэнкок попросту безнадежен.',
  array['фантастика', 'боевик', 'драма', 'комедия']::text[],
  array['США']::text[],
  8960,
  102151,
  'tt0448157',
  7.5,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:03:30.000Z',
  false,
  false,
  false
),
(
  'media-призрак-211',
  'Призрак',
  'Призрак',
  'movie',
  2015,
  './posters/призрак-cd1c70dc.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017c286b86e7b2eca7ffc84a54630a/orig',
  'Еще вчера Юрий Гордеев — амбициозный авиаконструктор и любимец женщин — был в шаге от своего триумфа. Его самолет ЮГ-1 должен был стать настоящим прорывом в отечественной авиации. Но сегодня его никто не видит и не слышит, и конкурент по бизнесу беспрепятственно закрывает его компанию. Все потому, что Юра разбился в автокатастрофе и стал призраком.

Школьник Ваня Кузнецов был пустым местом всегда. Жертва гиперопеки матери, объект насмешек одноклассников, он боится даже заговорить с девочкой, в которую давно влюблен. У Юры есть неделя, чтобы закончить дело своей жизни и поднять самолет в воздух. Ваня — единственный, кто его видит и может ему помочь.',
  array['комедия', 'семейный', 'фантастика', 'драма']::text[],
  array['Россия']::text[],
  332512,
  839960,
  'tt4559932',
  7.1,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:03:31.000Z',
  false,
  false,
  false
),
(
  'media-2012-212',
  '2012',
  '2012',
  'movie',
  2009,
  './posters/2012-676474c9.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000017c0724a34dcf392082f8fee64d07/orig',
  'Согласно календарю индейцев Майя, в 2012 году планеты солнечной системы окажутся на одной линии друг с другом, что приведет к глобальным природным катаклизмам: сильнейшие землетрясения, цунами и извержения вулканов превратят страны и целые континенты в руины. Недавно ученые подтвердили, что этот миф может стать реальностью.',
  array['фантастика', 'приключения', 'боевик']::text[],
  array['США']::text[],
  null,
  413447,
  'tt1190080',
  6.9,
  5.9,
  null,
  null,
  null,
  '2026-06-21T12:03:32.000Z',
  false,
  false,
  false
),
(
  'media-человек-паук-3-враг-в-отражении-213',
  'Человек-паук 3: Враг в отражении',
  'Spider-Man 3',
  'movie',
  2007,
  './posters/человек-паук-3-враг-в-отражении-aff220e5.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017c074a59b51c1e068d57a06f4a2a/orig',
  'Прошло пять лет с тех пор, как Питер осознал, что «с Великой силой приходит Великая ответственность!», и три года с тех пор, как он отказался быть борцом за справедливость. Но сейчас кажется, Питер прекрасно научился жить несколькими жизнями. Он хорошо учится, Нью-Йорк признал его героем, отношения с Мэри Джейн прочные. Питер даже намеревается сделать ей предложение!

Но к сожалению, его лучший друг Гарри Озборн по-прежнему считает, что именно Человек-паук несет ответственность за безвременную кончину его отца. Гарри намеревается отомстить ему в образе Нового Гоблина. В то же самое время на криминальной сцене Нью-Йорка появляется другой могущественный суперзлодей, известный как Песочный Человек.

Когда Человек-паук надевает таинственный черный костюм, его темная, более мстительная сторона полностью поглощает его. Эти трансформации заставляют его встретиться с самым сильным противником ― с самим собой…',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  559,
  82441,
  'tt0413300',
  7.1,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:03:33.000Z',
  false,
  false,
  false
),
(
  'media-босс-молокосос-214',
  'Босс-молокосос',
  'The Boss Baby',
  'cartoon',
  2017,
  './posters/босс-молокосос-68911867.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000018388cbdf9c0a786701d7ff3ef771/orig',
  'Семилетний фантазёр Тима ревнует родителей к недавно появившемуся на свет брату. Но малыш – совсем не простой ребенок, он обладает лидерскими задатками, уже носит деловые костюмы и во всём видит бизнес-задачу. Однажды Тим узнает о заговоре, целью которого является уничтожение любви, и организовал его директор компании «Puppy Co». Но одному Тиму не по силам его остановить. Мальчику предстоит объединиться со своим нелюбимым братом, чтобы спасти родителей и вернуть порядок на Земле.',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  295693,
  842567,
  'tt3874544',
  6.6,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:03:34.000Z',
  false,
  false,
  false
),
(
  'media-мумия-215',
  'Мумия',
  'The Mummy',
  'movie',
  2026,
  './posters/мумия-2d73a910.jpg',
  './posters/мумия-2d73a910.jpg',
  'Семью работающего в Каире американского журналиста Чарли Кэннона постигает трагедия — бесследно пропадает его маленькая дочь Кэти. Поиски ни к чему не приводят, полиция бессильно разводит руками. Восемь лет спустя Кэнноны живут в Нью-Мексико и внезапно получают обнадёживающее известие: Кэти нашлась. Девочку обнаружили в древнем саркофаге, она не говорит и очень плохо выглядит.',
  array['ужасы']::text[],
  array['Ирландия', 'США']::text[],
  null,
  6785084,
  null,
  6.3,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:03:35.000Z',
  false,
  false,
  false
),
(
  'media-шестое-чувство-216',
  'Шестое чувство',
  'The Sixth Sense',
  'movie',
  1999,
  './posters/шестое-чувство-9cf5434b.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017c076cb48a1988ea34594f7c9b13/orig',
  'Детский психиатр Малкольм Кроу сталкивается со странным случаем: девятилетнего Коула посещают страшные видения — призраки умерших. Как врач Малкольм бессилен помочь, но как человек он пытается найти ключ к ужасному миру Коула.',
  array['триллер', 'драма', 'детектив']::text[],
  array['США']::text[],
  745,
  395,
  'tt0167404',
  8.2,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:03:36.000Z',
  false,
  false,
  false
),
(
  'media-аватар-легенда-об-аанге-217',
  'Аватар: Легенда об Аанге',
  'Avatar: The Last Airbender',
  'cartoon',
  2004,
  './posters/аватар-легенда-об-аанге-5c12d026.jpg',
  './posters/аватар-легенда-об-аанге-5c12d026.jpg',
  'Мир разделен на четыре народа: Водные племена, королевство Земли, Воздушные кочевники и Огненная нация. Представители ка',
  array['мультфильм', 'фэнтези', 'боевик', 'приключения', 'семейный']::text[],
  array['США', 'Корея Южная']::text[],
  246,
  401152,
  'tt0417299',
  8.7,
  9.3,
  null,
  null,
  null,
  '2026-06-21T12:03:37.000Z',
  false,
  false,
  false
),
(
  'media-отряд-самоубийц-миссия-навылет-218',
  'Отряд самоубийц: Миссия навылет',
  'The Suicide Squad',
  'movie',
  2021,
  './posters/отряд-самоубийц-миссия-навылет-ee28aeb3.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017dc8f6a2f08b30a92ad70a9f89d5/orig',
  'Отобрав наиболее перспективных заключенных из тюрьмы, в которой содержатся не только самые опасные преступники, но и люди со сверхспособностями, и даже не люди, правительственный агент отправляет их на самоубийственное задание в одну латиноамериканскую страну, где недавно произошел военный переворот. А чтобы те наверняка не сбежали, каждому в голову вживляется взрывное устройство.',
  array['боевик', 'фантастика', 'комедия', 'фэнтези']::text[],
  array['США']::text[],
  436969,
  1008477,
  'tt6334354',
  6.7,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:03:38.000Z',
  false,
  false,
  false
),
(
  'media-сплит-219',
  'Сплит',
  'Split',
  'movie',
  2017,
  './posters/сплит-bb644e4b.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c07903eec1cc7442283cd005ce2/orig',
  'Cредь бела дня с многолюдной парковки незнакомец похищает трёх школьниц. Они приходят в себя в закрытом помещении, а в душе владельца таятся 23 лика страха. Сменяя друг друга, личности ведут обычную для них жизнь — работают и ходят к психотерапевту, периодически напоминая пленницам, что они дожидаются 24-ю личность, которая скоро явит себя миру.',
  array['триллер']::text[],
  array['США', 'Япония']::text[],
  381288,
  930534,
  'tt4972582',
  7,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:03:39.000Z',
  false,
  false,
  false
),
(
  'media-троя-220',
  'Троя',
  'Troy',
  'movie',
  2004,
  './posters/троя-202cb2b0.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c2800708c8538ed3827cfbdfa3f/orig',
  '1193 год до нашей эры. Парис украл прекрасную Елену, жену царя Спарты Менелая. За честь Менелая вступается его брат – царь Агамемнон. Его армия под предводительством Ахиллеса подошла к Трое и взяла город в кровавую осаду, длившуюся долгих десять лет… Два мира будут воевать за честь и власть. Тысячи умрут за славу. И за любовь нация сгорит дотла.',
  array['боевик', 'драма', 'мелодрама', 'история']::text[],
  array['США', 'Мальта', 'Великобритания']::text[],
  652,
  3442,
  'tt0332452',
  8,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:03:40.000Z',
  false,
  false,
  false
),
(
  'media-бэтмен-начало-221',
  'Бэтмен: Начало',
  'Batman Begins',
  'movie',
  2005,
  './posters/бэтмен-начало-d75af00e.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c0728164ffaea8c1bd826125cd8/orig',
  'В детстве юный наследник огромного состояния Брюс Уэйн оказался свидетелем убийства своих родителей, и тогда он решил бороться с преступностью. Спустя годы он отправляется в путешествие по миру, чтобы найти способ восстановить справедливость. Обучение у мудрого наставника боевым искусствам дает ему силу и смелость. Вернувшись в родной город, Уэйн становится Бэтменом и ведет борьбу со злом.',
  array['боевик', 'приключения', 'фантастика', 'драма']::text[],
  array['США', 'Великобритания']::text[],
  272,
  47237,
  'tt0372784',
  7.9,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:03:41.000Z',
  false,
  false,
  false
),
(
  'media-папины-дочки-222',
  'Папины дочки',
  'Папины дочки',
  'series',
  2007,
  './posters/папины-дочки-c6293bfc.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017c283fe86c0216640a4e9cd61ced/orig',
  'Практикующего психотерапевта, эксперта в семейных отношениях, бросает жена. Одного… с пятью дочерьми на руках. Но это – только начало кошмара. С ним остается теща и куча проблем! Сможет ли отец-одиночка, он же – отчаявшийся психотерапевт, распугавший всех клиентов жалобами на жизнь, прокормить семью и снова найти личное счастье? На горизонте уже появилась молодая, привлекательная и очень богатая клиентка – жена миллионера с Рублевки. Смогут ли папины дочки разжалобить ее и уговорить взять папу сначала на работу, а потом, глядишь, и в мужья?',
  array['комедия', 'семейный']::text[],
  array['Россия']::text[],
  45909,
  394047,
  'tt1796154',
  5.9,
  4.9,
  null,
  null,
  null,
  '2026-06-21T12:03:42.000Z',
  false,
  false,
  false
),
(
  'media-ч-рная-молния-223',
  'Чёрная Молния',
  'Чёрная Молния',
  'movie',
  2009,
  './posters/черная-молния-2009ee70.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017c073dcd38c1d7355158d9d665f3/orig',
  'Двадцатилетний Дима Майков получает в подарок от отца старенький автомобиль ГАЗ-21 «Волга» и внезапно обнаруживает, что его машина летает, как настоящий истребитель. Дима  активно включается в бизнес по доставке цветов, проносясь над столичными пробками и наслаждаясь полётами, пока однажды беда не заставляет его по-другому взглянуть на мир. 

Теперь ему предстоит встать на защиту тех, кем он дорожит, и сразиться со Злом в небе над новогодней Москвой.',
  array['фантастика', 'боевик', 'мелодрама', 'комедия', 'приключения']::text[],
  array['Россия']::text[],
  null,
  439906,
  'tt1569364',
  5.4,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:03:43.000Z',
  false,
  false,
  false
),
(
  'media-декстер-224',
  'Декстер',
  'Dexter',
  'series',
  2006,
  './posters/декстер-b2c4b0e3.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c17cd6b8c754bcb8cbd5ff6e6b0/orig',
  'Я — Декстер. Декстер Морган. Я работаю судмедэкспертом в полиции Майами. Я не понимаю любви, мне безразличен секс, и у меня нет чувств. А еще я серийный убийца.

Мой отец был полицейским и прекрасно обучил меня прятать улики. Обычным гражданам не стоит меня бояться, я убиваю только преступников, подонков, до которых почему-либо не смогла или не захотела добраться полиция. Я убиваю их, аккуратно распиливаю на части и избавляюсь от трупов. Убийство для меня — творческий акт, искусство.

Но однажды… В Майами появляется некто, равный мне. Даже превзошедший меня. Сумевший меня заинтересовать. Предложивший мне своего рода товарищеское состязание. Кто этот загадочный и неуловимый «икс»?..',
  array['триллер', 'драма', 'криминал', 'детектив']::text[],
  array['США']::text[],
  1405,
  277537,
  'tt0773262',
  8.3,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:03:44.000Z',
  false,
  false,
  false
),
(
  'media-мистер-и-миссис-смит-225',
  'Мистер и миссис Смит',
  'Mr. & Mrs. Smith',
  'movie',
  2005,
  './posters/мистер-и-миссис-смит-7781e15d.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a00000184f2a76d8f5faeb904119341f35b/orig',
  'Джон и Джейн женаты не так давно, но уже устали от брака. Им кажется, что они знают друг о друге всё. Но есть кое-что, что каждый предпочитает держать при себе: оба они – наёмные убийцы, которые тайно путешествуют по миру, выполняя опасные миссии. Эти отдельные тайные приключения становятся их общей судьбой в тот момент, когда Джейн получает заказ на Джона, а Джон - на Джейн.',
  array['боевик', 'триллер', 'мелодрама', 'комедия', 'криминал']::text[],
  array['США']::text[],
  787,
  12090,
  'tt0356910',
  7.6,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:03:45.000Z',
  false,
  false,
  false
),
(
  'media-первый-мститель-226',
  'Первый мститель',
  'Captain America: The First Avenger',
  'movie',
  2011,
  './posters/первый-мститель-3f6a9b86.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017c075af4dcc92826e840391e9c59/orig',
  'Стив Роджерс добровольно соглашается принять участие в эксперименте, который превратит его в суперсолдата, известного как Первый мститель. Роджерс вступает в вооруженные силы США вместе с Баки Барнсом и Пегги Картер, чтобы бороться с враждебной организацией ГИДРА, которой управляет безжалостный Красный Череп.',
  array['фантастика', 'боевик', 'приключения', 'военный']::text[],
  array['США']::text[],
  1771,
  160946,
  'tt0458339',
  6.7,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:03:46.000Z',
  false,
  false,
  false
),
(
  'media-человек-паук-2-227',
  'Человек-паук 2',
  'Spider-Man 2',
  'movie',
  2004,
  './posters/человек-паук-2-7f7fa6de.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017c08123dd3ddd2d7a062a1e7f95a/orig',
  'Тихоня Питер Паркер балансирует на грани двух своих жизней: супергероя Человека-паука и обычного студента колледжа. Его отношения со всеми, кто ему дорог, в опасности или зашли в тупик. А жизнь приготовила ему новое испытание. Он должен использовать все свои суперспособности, чтобы одолеть жестокого безумца доктора Отто Октавиуса.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  558,
  2898,
  'tt0316654',
  7.3,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:03:47.000Z',
  false,
  false,
  false
),
(
  'media-помни-228',
  'Помни',
  'Memento',
  'movie',
  2000,
  './posters/помни-fc2b8099.jpg',
  './posters/помни-fc2b8099.jpg',
  'Леонард Шелби ищет убийцу жены, но из-за нарушения кратковременной памяти вынужден оставлять себе подсказки в виде записей, фотографий и татуировок.',
  array['триллер', 'детектив', 'драма']::text[],
  array['США']::text[],
  null,
  335,
  'tt0209144',
  7.9,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:03:48.000Z',
  false,
  false,
  false
),
(
  'media-пролетая-над-гнездом-кукушки-229',
  'Пролетая над гнездом кукушки',
  'One Flew Over the Cuckoo''s Nest',
  'movie',
  1975,
  './posters/пролетая-над-гнездом-кукушки-f9fef49f.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000017c07838b83cdc15fd2757f214742/orig',
  'Рэндла Патрика Макмёрфи, патологического преступника и бунтаря, переводят из колонии в психиатрическую клинику, чтобы установить, является он душевнобольным или нет. В клинике он обнаруживает, что отделение контролирует хладнокровная, строгая и одержимая распорядком старшая медсестра Милдред Рэтчед. Макмёрфи намерен не подчиняться абсурдным, на его взгляд, правилам и одновременно повеселиться от души. Его бунтарская натура заражает других пациентов, но сестра Рэтчед решительно настроена пресечь это.',
  array['драма']::text[],
  array['США']::text[],
  510,
  336,
  'tt0073486',
  8.5,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:03:49.000Z',
  false,
  false,
  false
),
(
  'media-кунг-фу-панда-230',
  'Кунг-фу Панда',
  'Kung Fu Panda',
  'cartoon',
  2008,
  './posters/кунг-фу-панда-fec18726.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c08178c56595a584239ddedc8ab/orig',
  'Спасение Долины Мира и всех ее обитателей от непобедимого и безжалостного мастера Тай Лунга должно лечь на плечи Воина Дракона, Избранного среди лучших из лучших, коим становится… неуклюжий, ленивый и вечно голодный панда По.

Ему предстоит долгий и трудный путь к вершинам мастерства кунг-фу бок о бок с легендарными воинами: Тигрой, Обезьяной, Богомолом, Гадюкой и Журавлем. По постигнет тайну древнего Свитка и станет Воином Дракона только в том случае, если сможет поверить в себя…',
  array['мультфильм', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  9502,
  103734,
  'tt0441773',
  7.8,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:03:50.000Z',
  false,
  false,
  false
),
(
  'media-зв-здные-войны-эпизод-i-скрытая-угроза-231',
  'Звёздные войны: Эпизод 1 – Скрытая угроза',
  'Star Wars: Episode I - The Phantom Menace',
  'movie',
  1999,
  './posters/звездные-войны-эпизод-1-скрытая-угроза-4e831425.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a0000016fb27b97beee97f93c92540dd577/1344x756',
  'Мирная и процветающая планета Набу. Торговая федерация, не желая платить налоги, вступает в прямой конфликт с королевой Амидалой, правящей на планете, что приводит к войне. На стороне королевы и республики в ней участвуют два рыцаря-джедая: учитель и ученик, Квай-Гон-Джин и Оби-Ван Кеноби...',
  array['фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  1893,
  6695,
  'tt0120915',
  7.9,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:03:51.000Z',
  false,
  false,
  false
),
(
  'media-зв-здные-войны-эпизод-ii-атака-клонов-232',
  'Звёздные войны: Эпизод 2 – Атака клонов',
  'Star Wars: Episode II - Attack of the Clones',
  'movie',
  2002,
  './posters/звездные-войны-эпизод-2-атака-клонов-48e99964.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000016fb275ff7078a739d72656495e08/1344x756',
  'Действие разворачивается через 10 лет после событий, описанных в первом эпизоде знаменитой саги. Республика все глубже погружается в пучину противоречий и хаоса. Движение сепаратистов, представленное сотнями планет и могущественным альянсом корпораций, грозит стать новой угрозой для Галактики, с которой не смогут справиться даже джедаи.

Назревающий конфликт, заранее спланированный могущественными, но пока еще не разоблаченными силами, ведет к началу Клонических войн и к закату республики. Чтобы противостоять угрозе вселенских масштабов, Верховный канцлер Палпатин добивается консолидации власти в республике в своих руках и отдает приказ о создании республиканской армии для поддержки малочисленной группы джедаев в их борьбе с превосходящим противником.',
  array['фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  1894,
  844,
  'tt0121765',
  7.8,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:03:52.000Z',
  false,
  false,
  false
),
(
  'media-зв-здные-войны-эпизод-iii-месть-ситхов-233',
  'Звёздные войны: Эпизод 3 – Месть ситхов',
  'Star Wars: Episode III - Revenge of the Sith',
  'movie',
  2005,
  './posters/звездные-войны-эпизод-3-месть-ситхов-7e12789c.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a0000016fb2769074a29aa7cfb346e9e28d/1344x756',
  'Идёт третий год Войн клонов. Галактическая Республика, некогда бывшая спокойным и гармоничным государством, превратилась в поле битвы между армиями клонов, возглавляемых канцлером Палпатином, и армадами дроидов, которых ведёт граф Дуку, тёмный лорд ситхов. Республика медленно погружается во тьму. Лишь рыцари-джедаи, защитники мира и справедливости, могут противостоять злу, которое вскоре поглотит галактику. Но настоящая битва идёт в душе у молодого рыцаря-джедая Энакина, который разрывается между долгом джедая и любовью к своей жене, сенатору Падме Амидале. И от того, какое чувство в нём победит, зависит будущее всего мира.',
  array['фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  1895,
  5619,
  'tt0121766',
  8.1,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:03:53.000Z',
  false,
  false,
  false
),
(
  'media-зв-здные-войны-эпизод-iv-новая-надежда-234',
  'Звёздные войны: Эпизод 4 – Новая надежда',
  'Star Wars',
  'movie',
  1977,
  './posters/звездные-войны-эпизод-4-новая-надежда-e792f67c.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000016fb264111a4fa5ab487b46990394/1344x756',
  'Татуин. Планета-пустыня. Уже постаревший рыцарь Джедай Оби Ван Кеноби спасает молодого Люка Скайуокера, когда тот пытается отыскать пропавшего дроида. С этого момента Люк осознает свое истинное назначение: он один из рыцарей Джедай. В то время как гражданская война охватила галактику, а войска повстанцев ведут бои против сил злого Императора, к Люку и Оби Вану присоединяется отчаянный пилот-наемник Хан Соло, и в сопровождении двух дроидов, R2D2 и C-3PO, этот необычный отряд отправляется на поиски предводителя повстанцев – принцессы Леи. Героям предстоит отчаянная схватка с устрашающим Дартом Вейдером – правой рукой Императора и его секретным оружием – «Звездой Смерти».',
  array['фантастика', 'боевик', 'фэнтези', 'приключения']::text[],
  array['США']::text[],
  11,
  333,
  'tt0076759',
  8.1,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:03:54.000Z',
  false,
  false,
  false
),
(
  'media-зв-здные-войны-эпизод-v-империя-наносит-ответный-удар-235',
  'Звёздные войны: Эпизод 5 – Империя наносит ответный удар',
  'Star Wars: Episode V - The Empire Strikes Back',
  'movie',
  1980,
  './posters/звездные-войны-эпизод-5-империя-наносит-ответный-удар-e68a9527.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000016fb26a17f68021587d8d0311cc09/1344x756',
  'Борьба за Галактику обостряется в пятом эпизоде космической саги. Войска Императора начинают массированную атаку на повстанцев и их союзников.

Хан Соло и принцесса Лейя укрываются в Заоблачном Городе, в котором их и захватывает Дарт Вейдер, в то время как Люк Скайуокер находится на таинственной планете джунглей Дагоба.

Там Мастер - джедай Йода обучает молодого рыцаря навыкам обретения Силы. Люк даже не предполагает, как скоро ему придется воспользоваться знаниями старого Мастера: впереди битва с превосходящими силами Императора и смертельный поединок с Дартом Вейдером.',
  array['фантастика', 'боевик', 'фэнтези', 'приключения']::text[],
  array['США']::text[],
  1891,
  338,
  'tt0080684',
  8.1,
  8.7,
  null,
  null,
  null,
  '2026-06-21T12:03:55.000Z',
  false,
  false,
  false
),
(
  'media-зв-здные-войны-эпизод-vi-возвращение-джедая-236',
  'Звёздные войны: Эпизод 6 – Возвращение Джедая',
  'Star Wars: Episode VI - Return of the Jedi',
  'movie',
  1983,
  './posters/звездные-войны-эпизод-6-возвращение-джедая-9f1d2da2.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000016fb270899613ca817d7c5fc08750/1344x756',
  'В шестом эпизоде «Звездных войн» Дарт Вейдер создает вторую «Звезду Смерти». Он объединяет все силы зла, чтобы с помощью этого смертоносного оружия нанести последний сокрушительный удар по повстанцам и их союзникам.

Люк Скайуокер вместе с принцессой Лейей и верными дроидами R2D2 и C-3PO отправляется спасать своего друга Хана Соло, который попал в плен к отвратительному Джаббе Хатту - могущественному повелителю преступников.

Повстанцы высаживаются на планету Эндор, чтобы оттуда вместе с Люком и его отрядом предпринять последнюю атаку на Имперский флот, атаку, от которой зависит судьба Галактики. В этом эпизоде Люк Скайуокер и Дарт Вейдер встречаются в последний раз... ибо из поединка между отцом и сыном живым выйдет только один...',
  array['фантастика', 'боевик', 'фэнтези', 'приключения']::text[],
  array['США']::text[],
  1892,
  447,
  'tt0086190',
  8.2,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:03:56.000Z',
  false,
  false,
  false
),
(
  'media-зв-здные-войны-эпизод-vii-пробуждение-силы-237',
  'Звёздные войны: Пробуждение силы',
  'Star Wars: Episode VII - The Force Awakens',
  'movie',
  2015,
  './posters/звездные-войны-пробуждение-силы-a52b4fa8.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000016fb27ce4b7143ae77e999943ebbc/1344x756',
  'Через тридцать лет после гибели Дарта Вейдера и Императора галактика по-прежнему в опасности. Государственное образование Первый Орден во главе с таинственным верховным лидером Сноуком и его правой рукой Кайло Реном идёт по стопам Империи, пытаясь захватить всю власть. В это нелёгкое время судьба сводит юную девушку Рей и бывшего штурмовика Первого Ордена Финна с героями войны с Империей - Ханом Соло, Чубаккой и генералом Леей. Вместе они должны дать бой Первому Ордену, однако настаёт тот момент, когда становится очевидно, что лишь джедаи могут остановить Сноука и Кайло Рена.',
  array['фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  140607,
  714888,
  'tt2488496',
  7.4,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:03:57.000Z',
  false,
  false,
  false
),
(
  'media-зв-здные-войны-эпизод-viii-последние-джедаи-238',
  'Звёздные войны: Последние джедаи',
  'Star Wars: Episode VIII - The Last Jedi',
  'movie',
  2017,
  './posters/звездные-войны-последние-джедаи-7f190724.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000016fb27d8a446a687218defab35631/1344x756',
  'Новая история о противостоянии света и тьмы, добра и зла начинается после гибели Хана Соло. В Галактике, где Первый Орден и Сопротивление яростно сражаются друг с другом в войне, героиня Рей пробудила в себе Силу. Но что произойдет, когда она встретится с единственным оставшимся в живых рыцарем-джедаем - Люком Скайуокером?

Кайло Рен перешел на Темную сторону Силы и убил собственного отца, заняв высокую позицию в Первом Ордене. Но куда приведут его неуемные амбиции, подаренные по наследству грозным дедом Дартом Вейдером? Ситуация обострится еще больше, когда мать Кайло, лидер Сопротивления Лея, вместе с По, Финном и BB-8 приступит к новой миссии.',
  array['фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  181808,
  718223,
  'tt2527336',
  6.6,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:03:58.000Z',
  false,
  false,
  false
),
(
  'media-социальная-сеть-239',
  'Социальная сеть',
  'The Social Network',
  'movie',
  2010,
  './posters/социальная-сеть-97c6cea2.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c07a45ead016b9f7c52bba4fef6/orig',
  'Октябрьский вечер 2003 года. Студента Гарварда Марка Цукерберга бросает его девушка. Раздосадованный Марк, подогретый алкоголем, решает отомстить и публикует злобный и оскорбительный пост в адрес бывшей подруги. Но этого парню показалось мало. Он решает создать сайт, позволяющий сформировать рейтинг студенток по степени привлекательности. Марк взламывает базы данных различных общежитий, скачивает фотографии и всю необходимую информацию, используя алгоритм, предложенный его другом Эдуардо. После сбора данных Марк создаёт страницу facemash.com, где студенты могут проголосовать за одну из двух девушек, которая по их мнению является более привлекательной. Так начинается история создания одной из крупнейшей социальных сетей мира, которая впоследствии получит название Facebook.',
  array['драма', 'биография']::text[],
  array['США']::text[],
  37799,
  427198,
  'tt1285016',
  7.7,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:03:59.000Z',
  false,
  false,
  false
),
(
  'media-капитан-марвел-240',
  'Капитан Марвел',
  'Captain Marvel',
  'movie',
  2019,
  './posters/капитан-марвел-dbfd34bf.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c286ece158ca7f8a7cb96704eda/orig',
  'После столкновения с враждующими инопланетными расами пилот военно-воздушных сил Кэрол Дэнверс обретает суперсилу и становится неуязвимой. Ей предстоит совладать со своими новыми способностями, чтобы противостоять могущественному врагу.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Австралия']::text[],
  299537,
  843859,
  'tt4154664',
  6.5,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:04:00.000Z',
  false,
  false,
  false
),
(
  'media-в-поисках-немо-241',
  'В поисках Немо',
  'Finding Nemo',
  'cartoon',
  2003,
  './posters/в-поисках-немо-c0a98dcd.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017c0780b861d9cdc029e5c21d5a9f/orig',
  'Рыба-клоун Марлин после трагической гибели жены в одиночку растит своего единственного сына Немо. Однажды любопытный Немо, чтобы доказать, что он уже взрослый, плывет в открытый океан и попадает в неприятности. Марлин отправляется на поиски сына.',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  12,
  7908,
  'tt0266543',
  7.9,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:04:01.000Z',
  false,
  false,
  false
),
(
  'media-уэйн-242',
  'Уэйн',
  'Wayne',
  'series',
  2019,
  './posters/уэйн-54bbd579.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c2d1a2941700bd030f7041b7ccb/orig',
  '16-летний Уэйн вместе с подругой отправляется на мотоцикле из Бостона во Флориду, чтобы вернуть старенький Понтиак своего отца, который был украден до его смерти.',
  array['боевик', 'триллер', 'драма', 'комедия']::text[],
  array['США']::text[],
  84231,
  1167154,
  'tt7765404',
  8.1,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:04:02.000Z',
  false,
  false,
  false
),
(
  'media-назад-в-будущее-3-243',
  'Назад в будущее 3',
  'Back to the Future Part III',
  'movie',
  1990,
  './posters/назад-в-будущее-3-6679ae96.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c075a35657b4c943f20eabf5f6d/orig',
  'Несмотря на просьбы Дока не навещать его в XIX веке, Марти вновь хочет вмешаться в ход событий и на машине времени уносится в 1885 год, навстречу Дикому Западу. 

Это путешествие, как и все прежние, не обходится без опасных приключений, но смелый Марти, рискуя жизнью, предотвращает трагедию в жизни Эммета Брауна. Тот, в свою очередь, без памяти влюбляется в очаровательную незнакомку, которую спасает от гибели, и поездка домой — в будущее — теряет для него всякий смысл.',
  array['фантастика', 'комедия', 'приключения', 'вестерн', 'семейный']::text[],
  array['США']::text[],
  196,
  7103,
  'tt0099088',
  8.2,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:04:03.000Z',
  false,
  false,
  false
),
(
  'media-всегда-говори-да-244',
  'Всегда говори «ДА»',
  'Yes Man',
  'movie',
  2008,
  './posters/всегда-говори-да-2fb39b29.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017c071eb5cb7f3874c1bc92877ef1/orig',
  'Депрессивный главный герой всегда и всем говорил «нет» — например, друзьям, если они зовут куда-то. Но в один прекрасный день он заключает соглашение, по которому всегда должен отвечать «Да» на любое предложение...',
  array['комедия']::text[],
  array['США', 'Австралия']::text[],
  10201,
  391772,
  'tt1068680',
  7.8,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:04:04.000Z',
  false,
  false,
  false
),
(
  'media-гринч-245',
  'Гринч',
  'The Grinch',
  'cartoon',
  2018,
  './posters/гринч-3a6d870b.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000018383c140728320e250627328ff30/orig',
  'Любой бы на месте Гринча позеленел и взбесился. Как порядочный интроверт он живёт в тёмной пещере на самой вершине горы подальше ото всех, но эти «все» готовят грандиознейшее празднование нового года. Они шумят, всё украшают и дико бесят. Кто бы отказал себе в удовольствии испортить праздник? Гринч решает украсть Новый год.',
  array['мультфильм', 'фэнтези', 'комедия', 'семейный']::text[],
  array['Франция', 'США']::text[],
  360920,
  738499,
  'tt2709692',
  7.6,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:04:05.000Z',
  false,
  false,
  false
),
(
  'media-господин-никто-246',
  'Господин Никто',
  'Mr. Nobody',
  'movie',
  2009,
  './posters/господин-никто-71cab41b.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017c072ed73af981714068155fd43b/orig',
  'Проснувшийся немощным стариком Немо Никто оказывается последним смертным в гротескном будущем. Все люди уже давно бессмертны и с удовольствием наблюдают за телешоу, где главная звезда — дряхлый и безумный старик Немо, доживающий последние дни. К нему приходит журналист, и Немо рассказывает ему свою историю.',
  array['фантастика', 'мелодрама', 'фэнтези', 'драма']::text[],
  array['Бельгия', 'Германия', 'Канада', 'Франция', 'Великобритания', 'Люксембург', 'США']::text[],
  null,
  232785,
  'tt0485947',
  7.9,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:04:06.000Z',
  false,
  false,
  false
),
(
  'media-привидение-247',
  'Привидение',
  'Ghost',
  'movie',
  1990,
  './posters/привидение-e7fd5cb1.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a000001963f6f5e9245a88c8fc8229d13ca/orig',
  'Влюблённая пара Сэм и Молли возвращается домой после приятного вечера, когда на них нападает грабитель. Защищаясь, Сэм погибает и становится призраком. Он узнаёт, что его смерть не была случайной, а над его возлюбленной нависла смертельная опасность. Чтобы предупредить Молли, Сэм начинает обход практикующих медиумов и, о чудо! - находит женщину, которая действительно может его слышать. Только вот сама она не горит желанием помогать назойливому привидению.',
  array['мелодрама', 'фэнтези', 'триллер', 'драма']::text[],
  array['США']::text[],
  251,
  1991,
  'tt0099653',
  8.2,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:04:07.000Z',
  false,
  false,
  false
),
(
  'media-логан-248',
  'Логан',
  'Logan',
  'movie',
  2017,
  './posters/логан-7aa08451.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c2866cf1475923a73cff2f88d15/orig',
  'В недалеком будущем уставший от жизни Логан заботится о больном профессоре Икс, который прячется неподалеку от мексиканской границы. Но Логан больше не сможет скрывать свое прошлое, когда встретится с юным мутантом, которого преследуют темные силы.',
  array['фантастика', 'боевик', 'триллер', 'драма']::text[],
  array['США']::text[],
  263115,
  807682,
  'tt3315342',
  7.7,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:04:08.000Z',
  false,
  false,
  false
),
(
  'media-планета-ка-пэкс-249',
  'Планета Ка-Пэкс',
  'K-PAX',
  'movie',
  2001,
  './posters/планета-ка-пэкс-75759a03.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000018dac30445052532b57b954f8e6f6/orig',
  'В Манхэттенский психиатрический институт привозят странного человека в черных очках. Он зовет себя Протом и утверждает, что его родина — далекая планета Ка-Пэкс, откуда он мгновенно перенесся на Землю в луче света. Несмотря на все усилия, опытному доктору Пауэллу не удается разгадать загадку таинственного пациента, который охотно и весьма убедительно доказывает всем свое внеземное происхождение и заранее назначает дату своего возвращения на Ка-Пэкс.',
  array['фантастика', 'драма', 'детектив']::text[],
  array['Германия', 'США']::text[],
  null,
  723,
  'tt0272152',
  8.1,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:04:09.000Z',
  false,
  false,
  false
),
(
  'media-прометей-250',
  'Прометей',
  'Prometheus',
  'movie',
  2012,
  './posters/прометей-4a5fcd6d.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017c077b0e8be325b07181998c3e02/orig',
  'В поисках сакрального знания группа ученых отправляется в самые темные и неизведанные чертоги вселенной. Именно там, в отдаленных мирах, находясь на пределе своих умственных и физических возможностей, им предстоит вступить в безжалостную схватку за будущее всего человечества.',
  array['фантастика', 'ужасы']::text[],
  array['Великобритания', 'США']::text[],
  70981,
  467099,
  'tt1446714',
  7,
  7,
  null,
  null,
  null,
  '2026-06-21T12:04:10.000Z',
  false,
  false,
  false
),
(
  'media-первый-мститель-другая-война-251',
  'Первый мститель: Другая война',
  'Captain America: The Winter Soldier',
  'movie',
  2014,
  './posters/первый-мститель-другая-война-c8ae4eec.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000017c077509f4942f32ee58c479800e/orig',
  'После беспрецедентных событий, впервые собравших вместе команду Мстителей, Стив Роджерс, известный также как Капитан Америка, оседает в Вашингтоне и пытается приспособиться к жизни в современном мире. Но покой этому герою только снится – пытаясь помочь коллеге из агентства Щ.И.Т., Стив оказывается в центре событий, грозящих катастрофой мирового масштаба. Для того, чтобы разоблачить злодейский заговор, Капитан Америка объединяется с Черной вдовой. К ним также присоединяется новый соратник, известный как Сокол, однако никто из них даже не подозревает, на что способен новый враг.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  100402,
  676266,
  'tt1843866',
  7.4,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:04:11.000Z',
  false,
  false,
  false
),
(
  'media-тор-2-царство-тьмы-252',
  'Тор 2: Царство тьмы',
  'Thor: The Dark World',
  'movie',
  2013,
  './posters/тор-2-царство-тьмы-45956d1e.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c073f49b0befc844179b2463825/orig',
  'Могучему Тору предстоит вновь сразиться за восстановление порядка в галактике, встретившись лицом к лицу с древней инопланетной расой, ведомой злодеем Малекитом, который жаждет установить в девяти мирах власть тьмы. Столкнувшись с врагом, более могущественным, чем сам Один и всё воинство Асгарда, Тор должен собрать силы для решающей битвы. Его ждёт опасное путешествие и долгожданная встреча со своей земной возлюбленной Джейн Фостер. Сможет ли Тор рискнуть самым дорогим, чтобы не дать тьме воцариться на просторах вселенной?',
  array['фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  76338,
  595938,
  'tt1981115',
  7.4,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:04:12.000Z',
  false,
  false,
  false
),
(
  'media-третий-лишний-253',
  'Третий лишний',
  'Ted',
  'movie',
  2012,
  './posters/третий-лишний-0836bf1b.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c284f06b1b829db67c33900bcb8/orig',
  'Джон влюблен в красавицу Лори. Он работает в прокате автомобилей и имеет большие планы на будущее. Но в их отношения вмешивается третий, давний друг Джона – Тед. Он отрывается сутки напролет, предпочитает случайные связи и не желает терять друга. Но никто на самом деле не знает на что он способен, ведь Тед – большой плюшевый медведь.',
  array['комедия']::text[],
  array['США']::text[],
  72105,
  507817,
  'tt1637725',
  6.7,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:04:13.000Z',
  false,
  false,
  false
),
(
  'media-эквилириум-254',
  'Эквилибриум',
  'Equilibrium',
  'movie',
  2002,
  './posters/эквилибриум-0c930286.jpg',
  'https://avatars.mds.yandex.net/get-ott/1648503/2a00000196498326714c0926b1315eed08d5/1344x756',
  'В будущем люди лишены возможности выражать эмоции. Это цена, которую человечество платит за устранение из своей жизни войны. Теперь книги, искусство и музыка находятся вне закона, а любое чувство — преступление, наказуемое смертью.

Для приведения в жизнь существующего правила используется принудительное применение лекарства прозиум. Правительственный агент Джон Престон борется с теми, кто нарушает правила. В один прекрасный момент он забывает принять очередную дозу лекарства, и с ним происходит духовное преображение, что приводит его к конфликту не только с режимом, но и с самим собой.',
  array['фантастика', 'боевик', 'триллер', 'драма']::text[],
  array['США']::text[],
  7299,
  309,
  'tt0238380',
  7.9,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:04:14.000Z',
  false,
  false,
  false
),
(
  'media-чужой-255',
  'Чужой',
  'Alien',
  'movie',
  1979,
  './posters/чужой-678d4fa1.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c0768bdbd4bb13d0b98927e2c6a/orig',
  'В далеком будущем возвращающийся на Землю грузовой космический корабль перехватывает исходящий с неизвестной планеты сигнал. Экипаж, в соответствии с основными инструкциями, обязан найти и исследовать источник сигнала. Оказавшись на планете, астронавты повсюду обнаруживают предметы, по виду напоминающие гигантские коконы.',
  array['ужасы', 'фантастика', 'триллер']::text[],
  array['Великобритания', 'США']::text[],
  348,
  386,
  'tt0078748',
  8.1,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:04:15.000Z',
  false,
  false,
  false
),
(
  'media-миллионер-из-трущоб-256',
  'Миллионер из трущоб',
  'Slumdog Millionaire',
  'movie',
  2008,
  './posters/миллионер-из-трущоб-f6f36e74.jpg',
  'https://avatars.mds.yandex.net/get-ott/13051577/2a00000197a372863cbd23907c90d064ee03/orig',
  'Джамал Малик, 18-летний сирота из трущоб в Мумбаи, всего в одном шаге от победы в телеигре «Кто хочет стать миллионером?» и выигрыша 20 миллионов рупий. Прервав игру, его арестовывает полиция по подозрению в мошенничестве. Откуда юнец, выросший на улице, может знать так много?
На допросе в полиции Джамал рассказывает печальную историю своей жизни: о пережитых приключениях вместе с братом, о стычках с местными бандами, о своей трагической любви. Каждая глава личной истории удивительным образом дала ему ответы на вопросы телевикторины.
Когда игру возобновят, инспектору полиции и шестидесяти миллионам зрителей захочется выяснить ответ только на один вопрос: зачем этот юноша, без явного стремления к богатству, решил принять участие в телепрограмме?',
  array['драма', 'мелодрама']::text[],
  array['Великобритания', 'США']::text[],
  12405,
  413362,
  'tt1010048',
  7.7,
  8,
  null,
  null,
  null,
  '2026-06-21T12:04:16.000Z',
  false,
  false,
  false
),
(
  'media-крепкий-орешек-257',
  'Крепкий орешек',
  'Die Hard',
  'movie',
  1988,
  './posters/крепкий-орешек-be6a1a3e.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c2806c1d571596b1f31a5789d0e/orig',
  'В суперсовременном небоскребе Лос-Анджелеса полицейский Джон Макклейн ведет смертельную схватку с бандой политических террористов, взявших в заложники два десятка человек, в число которых попадает и его жена. Началось все с того, что парень приехал в город к жене, оказался на рождественском приеме, а кончилось настоящей войной...',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США']::text[],
  562,
  471,
  'tt0095016',
  8,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:04:17.000Z',
  false,
  false,
  false
),
(
  'media-крепкий-орешек-2-258',
  'Крепкий орешек 2',
  'Die Hard 2',
  'movie',
  1990,
  './posters/крепкий-орешек-2-1bdaf747.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c074d443efc179e302d83062fe7/orig',
  'В канун Рождества группа террористов захватила международный аэропорт и теперь удерживает тысячи путешествующих в качестве заложников. Террористы - банда отступников из первоклассно вооруженных коммандос, возглавляемых одержимым жестокостью офицером. Их цель - спасти наркобарона.

Банда подготовлена ко всевозможным случайностям, за исключением одной - встречи с Джоном Макклейном. Брюс Уиллис возвращается в роли супергероя-полицейского, который сражается не только с террористами, но и с некомпетентным шефом полиции аэропорта, тупоголовым начальником антитеррористического подразделения и страшным снежным бураном.

Заложники мечутся, смертельно испуганные и растерянные. А Джон Макклейн ведет схватку и со временем. Его жена находится в одном из самолетов, кружащих над аэропортом. А топливо - на пределе...',
  array['боевик']::text[],
  array['США']::text[],
  null,
  7374,
  'tt0099423',
  7.8,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:04:18.000Z',
  false,
  false,
  false
),
(
  'media-крепкий-орешек-3-259',
  'Крепкий орешек 3: Возмездие',
  'Die Hard with a Vengeance',
  'movie',
  1995,
  './posters/крепкий-орешек-3-возмездие-9cbc6d30.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001612cc670f6bafd98169a060b48bb/1344x756',
  'Террорист Саймон устраивает взрыв в центре Нью-Йорка. Он не требует денег и не выдвигает политических требований. Он лишь хочет сыграть в игру, участником которой должен стать временно отстранённый полицейский Джон Макклейн, иначе прогремит следующий взрыв.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США']::text[],
  1572,
  8162,
  'tt0112864',
  7.7,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:04:19.000Z',
  false,
  false,
  false
),
(
  'media-аладдин-260',
  'Аладдин',
  'Aladdin',
  'cartoon',
  1992,
  './posters/аладдин-3319b35c.jpg',
  './posters/аладдин-3319b35c.jpg',
  'В славном граде Аграба живёт Аладдин, веселый воришка с золотым сердцем. Живёт и не знает, что вскоре его ждёт величайше',
  array['мультфильм', 'мюзикл', 'фэнтези', 'мелодрама', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  812,
  2361,
  'tt0103639',
  8.2,
  8,
  null,
  null,
  null,
  '2026-06-21T12:04:20.000Z',
  false,
  false,
  false
),
(
  'media-игра-на-понижение-261',
  'Игра на понижение',
  'The Big Short',
  'movie',
  2015,
  './posters/игра-на-понижение-6682ac41.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a00000199d321cb33d1ef53192ddb53a9e2/orig',
  '2005 год. Изучая данные ипотек по стране, чудаковатый финансовый гений и управляющий хедж-фонда Scion Capital Майкл Бьюрри обращает внимание на одну деталь и приходит к выводу, что американский рынок ипотечных кредитов может скоро лопнуть. В связи с этим он страхует около миллиарда долларов своих клиентов через кредитный дефолтный своп. Клиенты фонда Бьюрри волнуются из-за возможных потерь, ведь рынок ипотечных кредитов представляется весьма стабильным, но Майкл твёрдо стоит на своём. Вскоре эту странную активность замечают несколько финансистов на Уолл-стрит. Изучив данные, они осознают, что опасения Бьюрри имеют под собой веские основания. Более того, сыграв на понижение, можно заработать миллионы.',
  array['драма', 'комедия', 'биография']::text[],
  array['США']::text[],
  318846,
  501333,
  'tt1596363',
  7.3,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:04:21.000Z',
  false,
  false,
  false
),
(
  'media-афоня-262',
  'Афоня',
  'Афоня',
  'movie',
  1975,
  './posters/афоня-0a11e17f.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c078a1e492138a9e5e3983f3d4a/orig',
  'Слесарь-сантехник ЖЭКа Афанасий Борщев, которого жильцы запросто зовут Афоня, — человек открытой и доброй души. Живет он, правда, бездумно и легко, но, может быть, именно потому бывает ему порой тоскливо. Тогда он наведывается в местную пивную, где есть у него друг Федул. Однажды на танцах он знакомится с милой и трогательной девушкой Катей, полюбившей Афоню. Но он избегает ее, так как сердце его пленила хозяйка квартиры 58, красавица Елена. Однако вскоре выясняется, что она замужем. Почувствовав себя одиноким и никому не нужным, он едет в деревню к тетке.',
  array['драма', 'мелодрама', 'комедия']::text[],
  array['СССР']::text[],
  20870,
  43423,
  'tt0072613',
  8.2,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:04:22.000Z',
  false,
  false,
  false
),
(
  'media-олдбой-263',
  'Олдбой',
  'Oldeuboi',
  'movie',
  2003,
  './posters/олдбой-d75c7784.jpg',
  'https://avatars.mds.yandex.net/get-ott/13074011/2a000001993864e7c79673450d05339f6060/orig',
  '1988 год. Обыкновенный и ничем непримечательный бизнесмен О Дэ-cу в день трёхлетия дочери по пути домой напивается, начинает хулиганить и закономерно попадает в полицейский участок. Из участка его под своё поручительство забирает друг детства. Пока тот звонит жене незадачливого пьяницы, О Дэ-су пропадает. Неизвестные похищают его и помещают в комнату без окон на 15 лет.',
  array['триллер', 'драма', 'детектив']::text[],
  array['Корея Южная']::text[],
  670,
  75871,
  'tt0364569',
  7.9,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:04:23.000Z',
  false,
  false,
  false
),
(
  'media-храброе-сердце-264',
  'Храброе сердце',
  'Braveheart',
  'movie',
  1995,
  './posters/храброе-сердце-004cbd13.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017c07549df694bc30cc703c423502/orig',
  'Действие фильма начинается в 1280 году в Шотландии. Это история легендарного национального героя Уильяма Уоллеса, посвятившего себя борьбе с англичанами при короле Эдварде Длинноногом.
Он рано лишился отца, погибшего от рук англичан, и его забрал к себе дядя Оргайл, который дал ему хорошее образование в Европе. На родину Уильям возвращается уже взрослым человеком, мечтающим завести семью и жить мирной жизнью.
Но судьба распорядилась иначе. Его невесту убили англичане, и он начал свой крестовый поход за свободу.',
  array['драма', 'военный', 'биография', 'история']::text[],
  array['США']::text[],
  197,
  399,
  'tt0112573',
  8.3,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:04:24.000Z',
  false,
  false,
  false
),
(
  'media-город-героев-265',
  'Город героев',
  'Big Hero 6',
  'cartoon',
  2014,
  './posters/город-героев-cb5bed58.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c285fc9706b1b259d96fc613119/orig',
  'Юный Хиро Хамада — прирожденный изобретатель и гений конструирования роботов. Вместе со старшим братом Тадаши они воплощают в жизнь самые передовые идеи в Техническом университете города будущего Сан-Франсокио. После серии загадочных событий друзья оказываются в центре коварного заговора. Отчаявшись, Хиро решает использовать веселого и добродушного экспериментального робота Бэймакса, перепрограммировав его в неуязвимую боевую машину.',
  array['мультфильм', 'фантастика', 'комедия', 'семейный', 'боевик']::text[],
  array['США']::text[],
  177572,
  693126,
  'tt2245084',
  8,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:04:25.000Z',
  false,
  false,
  false
),
(
  'media-большой-лебовски-266',
  'Большой Лебовски',
  'The Big Lebowski',
  'movie',
  1998,
  './posters/большой-лебовски-5bb340f2.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c0741144fb84382ac2fa32ffa55/orig',
  'Лос-Анджелес, 1991 год, война в Персидском заливе. Главный герой по прозвищу Чувак считает себя совершенно счастливым человеком. Его жизнь составляют игра в боулинг и выпивка. Но внезапно его счастье нарушается, гангстеры по ошибке принимают его за миллионера-однофамильца, требуют деньги, о которых он ничего не подозревает, и, ко всему прочему, похищают жену миллионера, будучи уверенными, что «муж» выплатит за нее любую сумму.',
  array['комедия', 'криминал']::text[],
  array['США', 'Великобритания']::text[],
  115,
  555,
  'tt0118715',
  7.8,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:04:26.000Z',
  false,
  false,
  false
),
(
  'media-человек-муравей-267',
  'Человек-муравей',
  'Ant-Man',
  'movie',
  2015,
  './posters/человек-муравей-8166d4eb.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017c077e880439d8043370b7111f9b/orig',
  'Вооружившись удивительной способностью уменьшаться в размере, но обладать при этом большой силой, мошенник Скотт Лэнг должен стать героем и помочь своему наставнику доктору Хэнку Пиму сохранить в тайне существование костюма Человека-муравья от угроз нового поколения.',
  array['фантастика', 'боевик', 'комедия', 'приключения']::text[],
  array['США']::text[],
  102899,
  195496,
  'tt0478970',
  7.1,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:04:27.000Z',
  false,
  false,
  false
),
(
  'media-хранители-268',
  'Хранители',
  'Watchmen',
  'movie',
  2009,
  './posters/хранители-78eb0437.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a000001967d02040f21e20dbe69c912a631/orig',
  'Действие картины разворачивается в параллельной реальности, в Америке 1985 года. В этом мире супергерои стали частью повседневной жизни общества, а Часы Судного Дня, застыв в пяти минутах от полуночи, отсчитывают время до столкновения ведущих мировых держав. После убийства одного из своих прежних коллег Роршах – супергерой, никогда не снимающий маски, -  полон решимости свершить суд Линча. 

Он начинает расследование заговора, призванного уничтожить или дискредитировать всех супергероев прошлого и настоящего. Он собирает своих соратников – отставной легион борцов с преступностью, но лишь один из них обладает подлинной силой. Постепенно Роршах осознает пугающий масштаб заговора, связанного с общим прошлым героев и сулящего катастрофические последствия в будущем. Они хранят наш покой, но кто охранит нас от Хранителей?',
  array['фантастика', 'боевик', 'детектив']::text[],
  array['США']::text[],
  13183,
  252641,
  'tt0409459',
  7.5,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:04:28.000Z',
  false,
  false,
  false
),
(
  'media-оппенгеймер-269',
  'Оппенгеймер',
  'Oppenheimer',
  'movie',
  2023,
  './posters/оппенгеймер-d8be5022.jpg',
  './posters/оппенгеймер-d8be5022.jpg',
  'История жизни американского физика-теоретика Роберта Оппенгеймера, который во времена Второй мировой войны руководил Манхэттенским проектом — секретными разработками ядерного оружия.',
  array['биография', 'драма', 'история']::text[],
  array['США', 'Великобритания']::text[],
  null,
  4664634,
  'tt15398776',
  8.1,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:04:29.000Z',
  false,
  false,
  false
),
(
  'media-ярость-270',
  'Ярость',
  'Fury',
  'movie',
  2014,
  './posters/ярость-82630f69.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c074b38c976d4b041ca302c54c6/orig',
  'Стоит апрель 1945 года, и победа над немцами уже близка: союзники уверенно продвигаются вглубь вражеской территории, и оказываемое им сопротивление становится слабее день ото дня. Это, однако, совсем не значит, что на пути к победе союзные войска не потеряют еще множество людей, и многие солдаты сознают, что смерть ждет на расстоянии вытянутой руки. Среди закаленных в бою пессимистов, каждый день ведущих ожесточенные бои во имя победы, экипаж танка, на чьем дуле намалевано слово «Ярость». 
Командир экипажа, известный как Уордэдди, воевавший с нацистскими войсками сначала в Африке, а потом в Европе, преследует одну цель – выжить в войне и сохранить жизнь своим людям. Этот относительно амбициозный план оказывается под угрозой, когда к его экипажу присоединяется зеленый новобранец Норман Эллисон, никогда раньше не воевавший и еще не успевший привыкнуть к тому, что должен быстро и без колебаний обрывать человеческие жизни. Надежда выжить становится совсем призрачной, когда экипажу «Ярости» приходится принять участие в самоубийственной миссии.',
  array['боевик', 'драма', 'военный']::text[],
  array['США', 'Китай', 'Великобритания']::text[],
  228150,
  740644,
  'tt2713180',
  7.4,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:04:30.000Z',
  false,
  false,
  false
),
(
  'media-джуманджи-зов-джунглей-271',
  'Джуманджи: Зов джунглей',
  'Jumanji: Welcome to the Jungle',
  'movie',
  2017,
  './posters/джуманджи-зов-джунглей-5d894eb6.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c0785ad843b52bd0582a2c1d64f/orig',
  'Четверо подростков оказываются внутри игры Джуманджи. Их ждет схватка с носорогами, черными мамбами, а на каждом шагу будет подстерегать бесконечная череда ловушек и головоломок. В игре они перевоплощаются: робкий и застенчивый Спенсер превращается в отважного и сильного исследователя, здоровяк Фридж – в коротышку-зоолога, модница и красавица Беттани – в полного профессора, а неуклюжая Марта становится бесстрашной и ловкой амазонкой. Друзьям придется привыкнуть к новым ролям, постараться не погибнуть и найти дорогу домой.',
  array['фэнтези', 'боевик', 'комедия', 'приключения']::text[],
  array['США', 'Австралия', 'Канада', 'Мексика', 'Индия', 'Великобритания']::text[],
  353486,
  695609,
  'tt2283362',
  6.9,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:04:31.000Z',
  false,
  false,
  false
),
(
  'media-реальные-пацаны-272',
  'Реальные пацаны',
  'Реальные пацаны',
  'series',
  2010,
  './posters/реальные-пацаны-c95fa756.jpg',
  'https://avatars.mds.yandex.net/get-ott/12808873/2a0000019b11e8ac04780ba9f0d0d07ee79e/orig',
  'Три парня поступили в университет, который уделяет большое внимание американскому футболу. Троица должна быстро адаптироваться к новой жизни, совмещая футбол, девушек, занятия и безостановочные гулянки.',
  array['комедия', 'спорт']::text[],
  array['США', 'Канада']::text[],
  46005,
  568082,
  'tt1837341',
  8,
  5.8,
  null,
  null,
  null,
  '2026-06-21T12:04:32.000Z',
  false,
  false,
  false
),
(
  'media-как-приручить-дракона-2-273',
  'Как приручить дракона 2',
  'How to Train Your Dragon 2',
  'movie',
  2014,
  './posters/как-приручить-дракона-2-55432af0.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c07670bad61ef4ea522073d53df/orig',
  'С момента примирения викингов и драконов прошло пять лет. Пока Астрид, Сморкала и остальные ребята проводят время, соревнуясь друг с другом в популярных на острове драконьих гонках, Иккинг и Беззубик путешествуют по небу, составляя карту неизвестных мест. Когда одно из их приключений приводит к открытию тайной ледяной пещеры, которая является домом для сотен ранее невиданных диких драконов и таинственного драконьего всадника, два друга оказываются в центре битвы за защиту мира.',
  array['мультфильм', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  82702,
  512883,
  'tt1646971',
  8,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:04:33.000Z',
  false,
  false,
  false
),
(
  'media-как-приручить-дракона-3-274',
  'Как приручить дракона 3',
  'How to Train Your Dragon: The Hidden World',
  'movie',
  2019,
  './posters/как-приручить-дракона-3-40b41508.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c074af3354a83e531d95abd2d32/orig',
  'Когда-то викинги жили в гармонии с драконами. В те времена они делили радость, горе… и последние штаны. Казалось, что так будет всегда, но появление загадочной Дневной Фурии изменило жизнь острова. И теперь Иккинг и Беззубик столкнутся с безжалостным охотником на драконов, жаждущим уничтожить все, что им дорого.',
  array['мультфильм', 'фэнтези', 'боевик', 'драма', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Япония']::text[],
  166428,
  706655,
  'tt2386490',
  7.8,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:04:34.000Z',
  false,
  false,
  false
),
(
  'media-хранители-снов-275',
  'Хранители снов',
  'Rise of the Guardians',
  'cartoon',
  2012,
  './posters/хранители-снов-52055e00.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017c0790555c5456a925358f1ec59a/orig',
  'Когда злой дух Кромешник посягает на самое дорогое – детские мечты, Северянин, Ледяной Джек, Кролик, Зубная Фея и Песочный Человек впервые объединяются, чтобы создать команду Хранителей снов...',
  array['мультфильм', 'фэнтези', 'приключения', 'семейный']::text[],
  array['США']::text[],
  81188,
  466581,
  'tt1446192',
  7.9,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:04:35.000Z',
  false,
  false,
  false
),
(
  'media-ход-королевы-276',
  'Ход королевы',
  'The Queen''s Gambit',
  'series',
  2020,
  './posters/ход-королевы-6001b210.jpg',
  './posters/ход-королевы-6001b210.jpg',
  'Штат Кентукки, 1957 год. После смерти матери 9-летняя Элизабет Хармон остаётся сиротой и отправляется в католический приют. В этом заведении детям регулярно дают «витамины», и по совету другой подопечной Бет оставляет зелёные капсулы на ночь. Там же девочка знакомится с пожилым уборщиком Шайбелем, которого просит научить её играть в шахматы. Днём, сбежав со скучных занятий под предлогом помыть тряпку, Бет играет с мистером Шайбелем в его подвальной подсобке, а ночью, приняв зелёную капсулу, прокручивает шахматные партии на потолке общей спальни. Через несколько лет девочку удочерят, у неё начнётся совсем другая жизнь, но главную роль в ней будет играть любовь к шахматам.',
  array['драма', 'спорт']::text[],
  array['США']::text[],
  87739,
  1253633,
  'tt10048342',
  8.3,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:04:36.000Z',
  false,
  false,
  false
),
(
  'media-люди-икс-277',
  'Люди Икс',
  'X-Men',
  'movie',
  2000,
  './posters/люди-икс-de4a3439.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017c2815c833fe47dab83456ed8ca3/orig',
  'Они — дети атомного века, сверхлюди, новое звено в цепи эволюции. Каждый из них был рождён в результате уникальной генетической мутации, наделившей их с детства необыкновенными способностями. Под руководством профессора Чарльза Ксавьера, телепата с мировым именем, одарённые ученики научились контролировать и управлять своими удивительными способностями в интересах человечества.

Но не все мутанты разделяют взгляды профессора: могущественный мутант Магнето, которому подвластны все металлы, собрал команду единомышленников. Он не верит, что люди и мутанты когда-либо смогут мирно сосуществовать. Они разработали план, чтобы осуществить захват власти над миром, и тогда, только ученики профессора Ксавьера смогут защитить этот мир…',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  36657,
  886,
  'tt0120903',
  7.7,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:04:37.000Z',
  false,
  false,
  false
),
(
  'media-люди-икс-2-278',
  'Люди Икс 2',
  'X2',
  'movie',
  2003,
  './posters/люди-икс-2-d991cd60.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017c270518c4a93543492a4781bc1a/orig',
  'Мутанты продолжают борьбу против общества, которое страшит их, и которому они не доверяют. Их положение становится еще более отчаянным, когда они подвергаются мощной атаке неизвестного противника, обладающего экстраординарными способностями. 

Внезапное нападение снова становится причиной политического и общественного резонанса по поводу введения повсеместной регистрации мутантов и причиной образования движения против мутантов, возглавляемого Уильямом Страйкером, бывшим военным, который, по слухам, проводил опыты над мутантами.',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array['Канада', 'США']::text[],
  36658,
  298,
  'tt0290334',
  7.6,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:04:38.000Z',
  false,
  false,
  false
),
(
  'media-люди-икс-последняя-битва-279',
  'Люди Икс: Последняя битва',
  'X-Men: The Last Stand',
  'movie',
  2006,
  './posters/люди-икс-последняя-битва-5ff6dfbd.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c077c0bb91ffa5920986ef24490/orig',
  'Людям Икс во главе с профессором Чарльзом Ксавьером приходится вступить в битву с самой эволюцией в лице их воскресшего товарища по команде. Джин Грэй, возрождённая в облике Тёмного Феникса, представляет опасность не только для самой себя, но и для мутантов и для всей человеческой расы.',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array['Канада', 'США', 'Великобритания']::text[],
  36668,
  61361,
  'tt0376994',
  7.4,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:04:39.000Z',
  false,
  false,
  false
),
(
  'media-люди-икс-начало-росомаха-280',
  'Люди Икс: Начало. Росомаха',
  'X-Men Origins: Wolverine',
  'movie',
  2009,
  './posters/люди-икс-начало-росомаха-b1ce39ca.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c0753d5d0bae7a38558d7398219/orig',
  'История о неистовом и романтичном прошлом Росомахи, его сложных отношениях с Виктором Кридом и о зловещей программе Оружие Х. При этом Росомаха встречается со многими мутантами, и узнает несколько легенд о мире Людей Икс.',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array['США', 'Великобритания']::text[],
  2080,
  104904,
  'tt0458525',
  7.5,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:04:40.000Z',
  false,
  false,
  false
),
(
  'media-росомаха-бессмертный-281',
  'Росомаха: Бессмертный',
  'The Wolverine',
  'movie',
  2013,
  './posters/росомаха-бессмертный-0c0b982b.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017c072908dc32c15d19966e2d11a4/orig',
  'Новая глава приключений Росомахи развернётся в Японии, где Логану предстоит выяснить, что острее - когти Росомахи или меч Серебряного Самурая.',
  array['фантастика', 'боевик']::text[],
  array['США', 'Великобритания']::text[],
  76170,
  462754,
  'tt1430132',
  6.5,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:04:41.000Z',
  false,
  false,
  false
),
(
  'media-люди-икс-дни-минувшего-будущего-282',
  'Люди Икс: Дни минувшего будущего',
  'X-Men: Days of Future Past',
  'movie',
  2014,
  './posters/люди-икс-дни-минувшего-будущего-f8a35306.jpg',
  'https://avatars.mds.yandex.net/get-ott/1648503/2a0000017c0776cc371e7111d9f91dc3fd15/orig',
  'В недалёком будущем мутанты близки к истреблению роботами-охотниками на мутантов Стражами. Единственная надежда для мутантов выжить - предотвратить череду роковых событий, приведших к появлению Стражей. С помощью своих способностей, Китти Прайд перемещает сознание Росомахи в его молодое тело в 1973 год. В прошлом всё оказывается не так радужно: молодой Профессор Икс окончательно разочарован в своих идеях, Магнето содержится в тюрьме глубоко под землёй, а человечество не знает как реагировать на широкое появление мутантов...',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array['США', 'Великобритания', 'Канада']::text[],
  127585,
  597687,
  'tt1877832',
  7.8,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:04:42.000Z',
  false,
  false,
  false
),
(
  'media-джуманджи-283',
  'Джуманджи',
  'Jumanji',
  'movie',
  1995,
  './posters/джуманджи-d421e0ac.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017c073a41fe88a0e7f575bd47b1b9/orig',
  'Случайно обнаружив настольную игру со странным названием «Джуманджи», Алан Пэрриш на глазах изумленной подружки перенёсся в джунгли. Он находился в этом мире до тех пор, пока двое ни о чем не подозревающих подростков не нашли игру и не сняли волшебные чары, освободив Алана из многолетнего плена. Теперь им предстоит решить нелегкую задачу - пройти немыслимые препятствия и одержать победу над силами магии.',
  array['фэнтези', 'триллер', 'приключения', 'семейный']::text[],
  array['США']::text[],
  8844,
  8161,
  'tt0113497',
  7.8,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:04:43.000Z',
  false,
  false,
  false
),
(
  'media-сопрано-284',
  'Сопрано',
  'The Sopranos',
  'series',
  1999,
  './posters/сопрано-cde5d6e9.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c0794961cfb170d9ead352a11b3/orig',
  'Мафиозный босс Северного Джерси Тони Сопрано эффективно решает проблемы «семьи». Но с собственной роднёй ситуация сложнее: дети от рук отбились, брак под угрозой, в отношениях с пожилой матерью сплошное недопонимание. После серии панических атак он решает тайно посещать психотерапевта.',
  array['драма', 'криминал']::text[],
  array['США']::text[],
  1398,
  79848,
  'tt0141842',
  8.7,
  9.2,
  null,
  null,
  null,
  '2026-06-21T12:04:44.000Z',
  false,
  false,
  false
),
(
  'media-город-грехов-285',
  'Город грехов',
  'Sin City',
  'movie',
  2005,
  './posters/город-грехов-23bfa3fd.jpg',
  'https://avatars.mds.yandex.net/get-ott/12808873/2a0000019666f881841caeb8902071c8565a/orig',
  'Город грехов — это бездна преступлений. Здесь полиция коррумпирована, а улицы смертельно опасны. Тем не менее один из жителей пытается найти убийцу своей подруги. Другой, фотограф, случайно становится свидетелем убийства полицейского и старается скрыть преступление.',
  array['боевик', 'триллер', 'криминал', 'детектив']::text[],
  array['США']::text[],
  187,
  77443,
  'tt0401792',
  7.9,
  8,
  null,
  null,
  null,
  '2026-06-21T12:04:45.000Z',
  false,
  false,
  false
),
(
  'media-город-грехов-2-женщина-ради-которой-стоит-убивать-286',
  'Город грехов 2: Женщина, ради которой стоит убивать',
  'Sin City: A Dame to Kill For',
  'movie',
  2014,
  './posters/город-грехов-2-женщина-ради-которой-стоит-убивать-a8648e02.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000017c077ea9a2bc0460f0464b77f502/orig',
  'В темных переулках Города Грехов Дуайт планирует жестоко отомстить женщине по имени Ава Лорд, которая его предала, в то время как Нэнси пытается смириться со смертью детектива Хартигана.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США']::text[],
  null,
  103391,
  'tt0458481',
  6.9,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:04:46.000Z',
  false,
  false,
  false
),
(
  'media-300-спартанцев-287',
  '300 спартанцев',
  '300',
  'movie',
  2007,
  './posters/300-спартанцев-263f03a1.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c2812f76e1dc954ce389c851103/orig',
  'Рассказ о кровопролитной битве при Фермопилах в 480 году до н. э., в которой триста отважных спартанцев во главе с царем Леонидом преградили путь многотысячной армии персидского царя Ксеркса.',
  array['фэнтези', 'боевик', 'военный']::text[],
  array['США']::text[],
  1271,
  81924,
  'tt0416449',
  7.6,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:04:47.000Z',
  false,
  false,
  false
),
(
  'media-ральф-против-интернета-288',
  'Ральф против интернета',
  'Ralph Breaks the Internet',
  'cartoon',
  2018,
  './posters/ральф-против-интернета-92707cb6.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017c073eb91ceb323dd63fa3875022/orig',
  'На этот раз Ральф и Ванилопа фон Кекс выйдут за пределы зала игровых автоматов и отправятся покорять бескрайние просторы интернета, который может и не выдержать сокрушительного обаяния громилы.',
  array['мультфильм', 'комедия', 'приключения', 'семейный', 'фантастика']::text[],
  array['США', 'Сингапур']::text[],
  404368,
  988782,
  'tt5848272',
  7.4,
  7,
  null,
  null,
  null,
  '2026-06-21T12:04:48.000Z',
  false,
  false,
  false
),
(
  'media-зверопой-289',
  'Зверопой',
  'Sing',
  'cartoon',
  2016,
  './posters/зверопой-871c14fc.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017c074bde64c910f4ada10f0a8273/orig',
  'В мире, который населяют звери, коала по имени Бастер Мун изо всех сил пытается предотвратить закрытие своего театра. В итоге Бастер и его друг баран Эдди устраивают конкурс певцов, предлагая принять участие каждому жителю города.',
  array['мультфильм', 'мюзикл', 'комедия', 'семейный']::text[],
  array['Япония', 'США', 'Франция']::text[],
  335797,
  821008,
  'tt3470600',
  7.4,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:04:49.000Z',
  false,
  false,
  false
),
(
  'media-обитель-зла-290',
  'Обитель зла',
  'Resident Evil',
  'movie',
  2026,
  './posters/обитель-зла-a6ef9499.jpg',
  './posters/обитель-зла-a6ef9499.jpg',
  'В гигантской подземной лаборатории выходит из-под контроля опаснейший вирус и мгновенно превращает своих жертв в прожорливых зомби. Достаточно одного их укуса или царапины, чтобы человек стал обезумевшим пожирателем живой плоти.

Военные посылают в кишащий голодными монстрами секретный комплекс отряд спецназа, к которому присоединяются Элис и полицейский Мэтт. У них есть только три часа, чтобы уничтожить вирус.',
  array['ужасы', 'боевик', 'фантастика']::text[],
  array['Германия', 'США']::text[],
  1423191,
  7378605,
  'tt35538033',
  7.5,
  null,
  null,
  null,
  null,
  '2026-06-21T12:04:50.000Z',
  false,
  false,
  false
),
(
  'media-коралина-в-стране-кошмаров-291',
  'Коралина в Стране Кошмаров',
  'Coraline',
  'cartoon',
  2008,
  './posters/коралина-в-стране-кошмаров-8b515901.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c0759e96807ea8050668eda2390/orig',
  'Маленькая Коралина попадает в другой мир, скрытый за секретной дверцей. Этот мир — ее альтернативная жизнь, которая не перестает ее радовать, все здесь хорошо, но только до поры до времени. Однажды она понимает, что ее настоящим родителям за ее проделки угрожает смертельная опасность.',
  array['мультфильм', 'фэнтези']::text[],
  array['США']::text[],
  14836,
  49166,
  'tt0327597',
  7.7,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:04:51.000Z',
  false,
  false,
  false
),
(
  'media-пипец-292',
  'Пипец',
  'Kick-Ass',
  'movie',
  2010,
  './posters/пипец-9da4b7cc.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c284351097c4fda88b360e76c26/orig',
  'Главный герой — школьник Дэйв Лизевски, который, вырядившись в костюм супергероя, пытается бороться с преступниками, хотя не обладает для этого никакими способностями. Параллельно два персонажа — 11-летняя девочка, которая крошит гангстеров мечом, и сынок гангстера, ведущий собственное расследование, пытаются выяснить личность Дэйва…',
  array['боевик', 'комедия', 'триллер', 'криминал']::text[],
  array['Великобритания', 'США']::text[],
  null,
  419200,
  'tt1250777',
  6.9,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:04:52.000Z',
  false,
  false,
  false
),
(
  'media-человек-швейцарский-нож-293',
  'Человек-швейцарский нож',
  'Swiss Army Man',
  'movie',
  2016,
  './posters/человек-швейцарский-нож-b4832be7.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c074183e03505a1d964d086c7d1/orig',
  'Оказавшись на необитаемом острове, Хэнк уже был готов потерять всякую надежду, но вдруг на берегу он находит труп и называет его Мэнни. Многофункциональность Мэнни поможет Хэнку выжить в диких условиях и заново обрести радость жизни. Вместе они отправятся в эпическое приключение, которое вернет Хэнка к девушке его мечты.',
  array['фэнтези', 'драма', 'комедия', 'приключения']::text[],
  array['США']::text[],
  347031,
  918087,
  'tt4034354',
  6.8,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:04:53.000Z',
  false,
  false,
  false
),
(
  'media-история-игрушек-294',
  'История игрушек',
  'Toy Story',
  'cartoon',
  1995,
  './posters/история-игрушек-58db6fe6.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017c0725d960566ee00d90c2aefba4/orig',
  'В спальне Энди его игрушки начинают жить своей жизнью, как только он выходит из комнаты. В день рождения мальчика царит паника — все боятся появления новой игрушки, которая перетянет внимание на себя. И только любимчик Эдди, ковбой Вуди, ни о чем не переживает. Однако, когда мальчик получает в подарок фигурку астронавта Базза Лайтера, Вуди очень быстро оказывается забыт. И тогда ковбой решает вернуть себе первое место в сердце Энди.',
  array['мультфильм', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  862,
  482,
  'tt0114709',
  7.9,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:04:54.000Z',
  false,
  false,
  false
),
(
  'media-трансформеры-месть-падших-295',
  'Трансформеры: Месть падших',
  'Transformers: Revenge of the Fallen',
  'movie',
  2009,
  './posters/трансформеры-месть-падших-18de2a27.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a000001966d171203c7bc8967a807312fbd/orig',
  'Прошло два года с тех пор, как юный Сэм Уитвикки спас Вселенную, предотвратив глобальную битву между враждующими воинственными расами инопланетных роботов. Несмотря на свои героические подвиги, Сэм остается обычным подростком с обычными проблемами — поступлением в колледж и родителями. Разумеется, отъезд будет тяжело воспринят лучшим другом, роботом — телохранителем Бамблби.

Сэм изо всех сил пытается забыть о конфликте в Мишн Сити и вернуться к нормальной жизни. Однако битва между Автоботами и Десептиконами, несмотря на гриф полной секретности, многое изменила...',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Иордания', 'Великобритания']::text[],
  8373,
  373391,
  'tt1055369',
  7.1,
  6,
  null,
  null,
  null,
  '2026-06-21T12:04:55.000Z',
  false,
  false,
  false
),
(
  'media-новый-человек-паук-высокое-напряжение-296',
  'Новый Человек-паук: Высокое напряжение',
  'The Amazing Spider-Man 2',
  'movie',
  2014,
  './posters/новый-человек-паук-высокое-напряжение-96699deb.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a00000185ce3ecae617bf452be977da4297/orig',
  'Питер Паркер под маской Человека-паука по-прежнему спасает мир от злодеев и преступников, а свободное время проводит со своей возлюбленной Гвен, но школьная пора близится к концу, и впереди ждет взрослая жизнь. Питер помнит, что дал отцу Гвен слово навсегда уйти из жизни девушки, однако сдержать обещание не так просто.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  102382,
  602409,
  'tt1872181',
  6.7,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:04:56.000Z',
  false,
  false,
  false
),
(
  'media-люцифер-297',
  'Люцифер',
  'Lucifer',
  'series',
  2016,
  './posters/люцифер-e02ed06d.jpg',
  './posters/люцифер-e02ed06d.jpg',
  'Заскучавший и несчастный повелитель преисподней Люцифер Морнингстар оставил свой престол и отправился в современный Лос-Анджелес, где основал ночной клуб «Lux». Очаровательный, харизматичный и дьявольски привлекательный Люцифер наслаждается отдыхом — вином, женщинами и музыкой, пока красивая поп-звезда не оказывается убитой на пороге его клуба.',
  array['фэнтези', 'драма']::text[],
  array['США']::text[],
  63174,
  893621,
  'tt4052886',
  7.8,
  8,
  null,
  null,
  null,
  '2026-06-21T12:04:57.000Z',
  false,
  false,
  false
),
(
  'media-лицо-со-шрамом-298',
  'Лицо со шрамом',
  'Scarface',
  'movie',
  1983,
  './posters/лицо-со-шрамом-d4ddcf97.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c0789fa8ca1e905426d1644d130/orig',
  'Весной 1980 года был открыт порт Мэйриэл Харбор, и тысячи кубинских беженцев ринулись в Соединенные Штаты на поиски Американской Мечты. Один из них нашел ее на залитых солнцем улицах Майами. Богатство, власть и страсть превзошли даже самые невероятные его мечты. Его звали Тони Монтана. Мир запомнил его под другим именем — Лицо со шрамом.',
  array['драма', 'криминал', 'боевик']::text[],
  array['США']::text[],
  111,
  4695,
  'tt0086250',
  8.2,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:04:58.000Z',
  false,
  false,
  false
),
(
  'media-оно-2-299',
  'Оно 2',
  'It Chapter Two',
  'movie',
  2019,
  './posters/оно-2-322ea3ae.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017c071e2b9fd1afd68eb049bb15d9/orig',
  'Прошло 27 лет после первой встречи ребят с демоническим Пеннивайзом. Они выросли, покинули родной городок и практически забыли о тех страшных событиях, но неожиданно в их взрослые проблемы вторгается телефонный звонок. Майк никогда не покидал Дерри, всё это время собирал информацию о жутком клоуне и ждал. Когда в городе начали происходить новые убийства, Майк просит старых друзей вновь собраться вместе и разобраться со злом города Дерри раз и навсегда.',
  array['ужасы', 'фэнтези', 'драма']::text[],
  array['США', 'Канада']::text[],
  474350,
  686898,
  'tt7349950',
  6.6,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:04:59.000Z',
  false,
  false,
  false
),
(
  'media-суперсемейка-300',
  'Суперсемейка',
  'The Incredibles',
  'cartoon',
  2004,
  './posters/суперсемейка-7bfbc7c6.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c2801fc9771010ec68b91a8cc89/orig',
  'История семьи супергероев, члены которой уже отошли от больших дел и предпочитают жить непримечательной жизнью обычных землян. Они обладают сверхъестественными способностями и относительно легко справлялись с мировым злом.

Но в один прекрасный день папаше и его домочадцам приходится вновь облачиться в прорезиненные костюмы супергероев (а для этого нужно срочно сбросить лишние килограммы) и в очередной раз спасти человечество от таинственного злодея.',
  array['мультфильм', 'боевик', 'приключения', 'семейный']::text[],
  array['США']::text[],
  9806,
  38903,
  'tt0317705',
  7.5,
  8,
  null,
  null,
  null,
  '2026-06-21T12:05:00.000Z',
  false,
  false,
  false
),
(
  'media-суперсемейка-2-301',
  'Суперсемейка 2',
  'Incredibles 2',
  'cartoon',
  2018,
  './posters/суперсемейка-2-0b1f2716.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c0799051fdf91721080e9bb4907/orig',
  'После событий первой части суперсемейка пользуется повышенным вниманием со стороны журналистов. Неожиданно оказалось, что обаятельная Миссис Исключительная гораздо лучше смотрится на экране телевизора, чем её муж. Мистеру Исключительному все чаще приходится сидеть дома и приглядывать за детьми, у каждого из которых есть свои причины быть недовольным собственными суперспособностями. Однако семейным проблемам предстоит отступить на второй план, когда окажется, что спокойствию Суперсемейки угрожает новый могущественный враг.',
  array['мультфильм', 'фантастика', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Япония']::text[],
  260513,
  839650,
  'tt3606756',
  7.4,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:05:01.000Z',
  false,
  false,
  false
),
(
  'media-чужие-302',
  'Чужие',
  'Aliens',
  'movie',
  1986,
  './posters/чужие-36dcec9b.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c280333725899021452d3f7e0be/orig',
  'Чужой – совершенный организм. Идеальная машина для убийства, чье физическое превосходство сочетается с его феноменальной жаждой уничтожения. Офицер Элен Рипли и команда космического корабля Ностромо один раз уже встретилась с такой тварью. В живых осталась только Элен.

Капсула с Элен найдена спасателями после многих лет блуждания в космосе. Ей сообщают, что планета L.V. 426 колонизирована, и ей придется вернуться туда, где начался ее кошмар, ибо связь с колонистами прервалась. И вот в составе группы космического десанта Рипли отправляется на проклятую планету. Но теперь их там поджидает не один Чужой, а тысячи. Кто сможет выжить в этой войне: чудовища, способные только убивать, или люди, способные мыслить?',
  array['ужасы', 'фантастика', 'боевик', 'триллер']::text[],
  array['Великобритания', 'США']::text[],
  679,
  406,
  'tt0090605',
  8.1,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:05:02.000Z',
  false,
  false,
  false
),
(
  'media-притяжение-303',
  'Притяжение',
  'Притяжение',
  'movie',
  2017,
  './posters/притяжение-4c0847ba.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017e7c5c5937ab7ed568ee2d7b36ff/orig',
  '…Как только что стало известно, сбитый над Москвой неопознанный объект имеет, возможно, внеземное происхождение. Большая часть столичного Чертаново оцеплена, к месту крушения стягиваются представители силовых структур, решается вопрос об эвакуации местных жителей. По словам нашего источника в Минобороны сейчас специальная комиссия пытается вступить в контакт с так называемыми гостями. В эти минуты мы готовим экстренный выпуск новостей, и о развитии событий вы узнаете первыми…',
  array['фантастика', 'боевик', 'мелодрама']::text[],
  array['Россия']::text[],
  401513,
  840250,
  'tt4731148',
  5.8,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:05:03.000Z',
  false,
  false,
  false
),
(
  'media-интерны-304',
  'Интерны',
  'Интерны',
  'series',
  2010,
  './posters/интерны-d90620d7.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000019d8b90c3f4611beb853b02b73c24/orig',
  'Интерны все время попадают в идиотские ситуации, а их руководитель доктор Быков с удовольствием за этим наблюдает.',
  array['комедия']::text[],
  array['Россия']::text[],
  32651,
  501998,
  'tt1647423',
  7.5,
  7,
  null,
  null,
  null,
  '2026-06-21T12:05:04.000Z',
  false,
  false,
  false
),
(
  'media-фокус-305',
  'Фокус',
  'Focus',
  'movie',
  2014,
  './posters/фокус-68f52c4c.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c077602161f4e7d7482f6893fe1/orig',
  'История об опытном мошеннике, который влюбляется в девушку, делающую первые шаги на поприще нелегального отъема средств у граждан. Отношения становятся для них проблемой, когда обнаруживается, что романтика мешает их нечестному бизнесу.',
  array['драма', 'мелодрама', 'комедия', 'криминал']::text[],
  array['США', 'Аргентина']::text[],
  256591,
  680824,
  'tt2381941',
  7.1,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:05:05.000Z',
  false,
  false,
  false
),
(
  'media-человек-муравей-и-оса-306',
  'Человек-муравей и Оса',
  'Ant-Man and the Wasp',
  'movie',
  2018,
  './posters/человек-муравей-и-оса-da016e2a.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c287507dc5661d1e3fa86e6cff1/orig',
  'Скотт Лэнг, известный также как Человек-муравей, уже заслужил право оказаться в команде Мстителей. Но желание быть ближе к дочери удерживает его в родном Сан-Франциско до тех пор, пока доктор Хэнк Пим, создавший когда-то изменяющий размеры своего владельца чудо-костюм, не призывает Скотта присоединиться к новой опасной миссии. А помогать в противостоянии с коварным врагом Человеку-муравью будет новая напарница — Оса.',
  array['фантастика', 'боевик', 'комедия', 'приключения']::text[],
  array['США', 'Канада']::text[],
  363088,
  935940,
  'tt5095030',
  7,
  7,
  null,
  null,
  null,
  '2026-06-21T12:05:06.000Z',
  false,
  false,
  false
),
(
  'media-платформа-307',
  'Платформа',
  'El hoyo',
  'movie',
  2019,
  './posters/платформа-55aae770.jpg',
  './posters/платформа-55aae770.jpg',
  'Горен соглашается на участие в некоем эксперименте и вскоре приходит в себя в почти пустой комнате уровня 48, где имеются большие прямоугольные отверстия в полу и потолке. На каждом уровне находятся два человека, а сколько всего уровней — неизвестно. Этажи связывает общий колодец, по которому раз в день опускается платформа с едой, и чем ниже находятся люди, тем меньше у них шансов поесть. Каждый месяц происходит рокировка, и обитатели верхних уровней могут оказаться в самом низу, и наоборот. Поскольку разрешалось взять с собой один предмет, Горен выбрал томик «Дон Кихота», а его сосед прихватил большой самозатачивающийся нож.',
  array['фантастика', 'триллер', 'драма']::text[],
  array['Испания']::text[],
  619264,
  1134778,
  'tt8228288',
  7.1,
  7,
  null,
  null,
  null,
  '2026-06-21T12:05:07.000Z',
  false,
  false,
  false
),
(
  'media-курьер-308',
  'Курьер',
  'Курьер',
  'movie',
  1986,
  './posters/курьер-0b3c0d00.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017c0727dd75bb8a0a395405051db8/orig',
  'Выпускник школы Иван, дожидаясь призыва в армию, работает курьером в редакции. Окружающие его солидные взрослые люди с трудом приспосабливаются к удивительной способности этого парня любое событие превратить в невероятное происшествие - порой уморительно веселое, а, подчас, и непоправимо грустное…',
  array['драма', 'мелодрама', 'комедия']::text[],
  array['СССР']::text[],
  70966,
  45028,
  'tt0091364',
  8,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:05:08.000Z',
  false,
  false,
  false
),
(
  'media-джек-ричер-309',
  'Джек Ричер',
  'Jack Reacher',
  'movie',
  2012,
  './posters/джек-ричер-5333bdec.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a000001966846cdc5455affa62a3c610dfc/orig',
  'Снайпер убивает нескольких случайных прохожих. Его находят и арестовывают. Все улики указывают на него. На допросе вместо признания он пишет имя - Джек Ричер. Больше обвиняемый не может ничего сказать, поскольку после избиения заключёнными впадает в кому. Загадочный Джек Ричер появляется незамедлительно. Что теперь будет с убийцей?',
  array['боевик', 'триллер', 'детектив', 'криминал']::text[],
  array['США']::text[],
  75780,
  417583,
  'tt0790724',
  7.2,
  7,
  null,
  null,
  null,
  '2026-06-21T12:05:09.000Z',
  false,
  false,
  false
),
(
  'media-чернобыль-зона-отчуждения-310',
  'Чернобыль: Зона отчуждения',
  'Чернобыль: Зона отчуждения',
  'series',
  2014,
  './posters/чернобыль-зона-отчуждения-00868e6c.jpg',
  './posters/чернобыль-зона-отчуждения-00868e6c.jpg',
  'Пятеро молодых людей прыгают в старую «Волгу» и отправляются на поиски вора, часом ранее укравшего из квартиры родителей главного героя восемь миллионов рублей. Грабитель — обычный московский айтишник, — вместо того чтобы залечь на дно, тут же записывает видеоотчет, в котором сообщает, что финальная точка его путешествия — Чернобыльская АЭС и город Припять.',
  array['триллер', 'фантастика']::text[],
  array['Россия']::text[],
  64542,
  727157,
  'tt4032214',
  7.6,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:05:10.000Z',
  false,
  false,
  false
),
(
  'media-человек-бензопила-311',
  'Человек-бензопила',
  'Chainsaw Man',
  'anime',
  2022,
  './posters/человек-бензопила-2388f03a.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000019836a23dd0127dd746b3e1668029/orig',
  'Подросток Дэндзи всего лишь хотел жить обычной жизнью и есть досыта, но оставшиеся после смерти отца долги вынуждают парня влачить полуголодное существование и жить в халупе. Чтобы расплатиться с якудза, он уже продал некоторые свои органы и даже промышляет нелегальной охотой на демонов, в чём ему помогает необычный питомец Потита, демоническое существо с бензопилой. Однажды коварные бандиты заманивают Дэндзи в ловушку и приносят в жертву, но Потита не даёт парню умереть. Он сращивает его тело и становится сердцем своего хозяина — теперь Дэндзи обладает повышенной регенерацией, а также может превращать конечности в бензопилы. Такого ценного кадра нанимает Бюро общественной безопасности, чтобы тот снова охотился на демонов. На новой работе парень знакомится с коллегами — меланхоличным охотником Аки, вспыльчивой полудемонессой Пауэр — и влюбляется в свою загадочную начальницу Макиму.',
  array['аниме', 'мультфильм', 'боевик', 'фэнтези', 'ужасы']::text[],
  array['Япония']::text[],
  114410,
  2040161,
  'tt13616990',
  8.5,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:05:11.000Z',
  false,
  false,
  false
),
(
  'media-сонни-бой-312',
  'Sonny Boy',
  'Sonny Boy',
  'anime',
  2021,
  './posters/sonny-boy-02f44c66.jpg',
  './posters/sonny-boy-02f44c66.jpg',
  'Это должны были быть обычные летние каникулы. Однако внезапно школа и 36 учеников перемещаются в другое измерение, и теперь школьникам, обнаружившим у себя сверхъестественные способности, надо будет научиться выживать в фантастическом мире.',
  array['аниме', 'мультфильм', 'фантастика', 'приключения']::text[],
  array['Япония']::text[],
  47246,
  4433040,
  null,
  8,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:05:12.000Z',
  false,
  false,
  false
),
(
  'media-монстры-на-каникулах-2-313',
  'Монстры на каникулах 2',
  'Hotel Transylvania 2',
  'movie',
  2015,
  './posters/монстры-на-каникулах-2-7c01632d.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c07592d5b4c83695303ec5d5dad/orig',
  'Граф Дракула выдает свою очаровательную дочь замуж за парня из мира людей, и у молодой «смешанной» пары рождается сын. Станет ли этот ребенок человеком или вампиром?',
  array['мультфильм', 'фэнтези', 'комедия', 'семейный']::text[],
  array['США', 'Канада']::text[],
  null,
  716891,
  'tt2510894',
  7.2,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:05:13.000Z',
  false,
  false,
  false
),
(
  'media-эдвард-руки-ножницы-314',
  'Эдвард руки-ножницы',
  'Edward Scissorhands',
  'movie',
  1990,
  './posters/эдвард-руки-ножницы-996c81ad.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c281406630514105f393024f0eb/orig',
  'Старый и одинокий учёный создал человеческое существо. Но не успел доделать его до конца и умер. У сотворённого им молодого человека не хватает рук — вместо них у него нечто вроде ножниц, и он вынужден жить в заброшенном доме в полном одиночестве. Однажды его обнаруживает женщина-коммивояжер и берет к себе жить. В новом доме Эдвард пытается социализироваться — подружиться с младшим сыном своей новой «мамы» и с её прекрасной дочерью.',
  array['фэнтези', 'драма', 'мелодрама']::text[],
  array['США']::text[],
  162,
  8421,
  'tt0099487',
  8,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:05:14.000Z',
  false,
  false,
  false
),
(
  'media-война-миров-315',
  'Война миров',
  'War of the Worlds',
  'movie',
  2005,
  './posters/война-миров-8e3c54c7.jpg',
  './posters/война-миров-8e3c54c7.jpg',
  'На Землю нападают высокоразвитые пришельцы. Пока армия ведёт ожесточённые бои с захватчиками, обычный семьянин Рэй стрем',
  array['фантастика', 'боевик', 'триллер']::text[],
  array['США', 'Канада', 'Япония']::text[],
  74,
  81289,
  'tt0407304',
  7.1,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:05:15.000Z',
  false,
  false,
  false
),
(
  'media-человек-из-стали-316',
  'Человек из стали',
  'Man of Steel',
  'movie',
  2013,
  './posters/человек-из-стали-0c5c7cb8.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a00000186bb5ab3d02d76c2dab0e7d38da8/orig',
  'Кларк Кент (Кал-Эл) - молодой человек, который чувствует себя чужаком из-за своей невероятной силы. Много лет назад он был отправлен на Землю с развитой планеты Криптон, и теперь задается вопросом: зачем? 
Воспитанный приемными родителями Мартой и Джонатаном Кентами, Кларк знает: обладать сверхспособностями - значит принимать сложные решения. Но когда человечество более всего нуждается в стабильности, оно подвергается нападению. Сможет ли герой восстановить мир или воспользуется своей силой для того, чтобы окончательно его разрушить?',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Великобритания']::text[],
  49521,
  252667,
  'tt0770828',
  6.9,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:05:16.000Z',
  false,
  false,
  false
),
(
  'media-бэтмен-против-супермена-на-заре-справедливости-317',
  'Бэтмен против Супермена: На заре справедливости',
  'Batman v Superman: Dawn of Justice',
  'movie',
  2016,
  './posters/бэтмен-против-супермена-на-заре-справедливости-cb16365b.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c07258ccfddd149f83e45c335f0/orig',
  'Опасаясь, что действия богоподобного супергероя так и останутся бесконтрольными, грозный и могущественный страж Готэм Сити бросает вызов самому почитаемому в наши дни спасителю Метрополиса, в то время как весь остальной мир решает, какой герой ему по-настоящему нужен. И пока Бэтмен и Супермен пребывают в состоянии войны друг с другом, возникает новая угроза, которая ставит человечество под самую большую опасность, с которой оно когда-либо сталкивалось.',
  array['фантастика', 'боевик', 'фэнтези', 'приключения']::text[],
  array['США', 'Марокко']::text[],
  209112,
  770631,
  'tt2975590',
  6.8,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:05:17.000Z',
  false,
  false,
  false
),
(
  'media-мгла-318',
  'Мгла',
  'The Mist',
  'movie',
  2007,
  './posters/мгла-8aa1f62c.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017c077186dea4401fb77ed6643ffb/orig',
  'Маленький городок накрывает сверхъестественный туман, отрезая людей от внешнего мира. Группе героев, оказавшихся в этот момент в супермаркете, приходится вступить в неравный бой с обитающими в тумане монстрами.',
  array['ужасы', 'фантастика', 'триллер']::text[],
  array['США']::text[],
  5876,
  273302,
  'tt0884328',
  7.6,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:05:18.000Z',
  false,
  false,
  false
),
(
  'media-американский-психопат-319',
  'Американский психопат',
  'American Psycho',
  'movie',
  2000,
  './posters/американский-психопат-3b795883.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000017c078f06bf98b117df5310532aa3/orig',
  'Днем он ничем не отличается от окружающих, и в толпе вы не обратите на него внимания. Но ночью этот благовидный гражданин превращается в изощренного убийцу.',
  array['драма', 'криминал', 'триллер']::text[],
  array['США']::text[],
  1359,
  588,
  'tt0144084',
  7.2,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:05:19.000Z',
  false,
  false,
  false
),
(
  'media-выжить-после-320',
  'Выжить после',
  'Выжить после',
  'series',
  2013,
  './posters/выжить-после-3ce090b1.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c07a8e790409bcd945cabbba63c/orig',
  'Одиннадцать незнакомых подростков просыпаются в подземном бункере. Они не знают друг друга и не помнят, как сюда попали. Но одно ясно совершенно точно – им необходимо выбираться. Воздух катастрофически заканчивается, а воды нет и подавно. С невероятными усилиями, проявляя недюжинную смекалку и сообразительность, работая в команде, герои выбираются на поверхность, ожидая, что неприятности позади. Но все оказывается еще хуже: Москва превратилась из процветающей столицы в нелюдимую пустыню, пораженную зомби-вирусом. Большинство населения эвакуировали, а тем, кто остался пришлось очень несладко.',
  array['триллер']::text[],
  array['Россия']::text[],
  65673,
  732760,
  'tt3857398',
  8.2,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:05:20.000Z',
  false,
  false,
  false
),
(
  'media-хищник-321',
  'Хищник',
  'Predator',
  'movie',
  1987,
  './posters/хищник-fbc260fb.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c07502eda6b22221d0880d35645/orig',
  'Американский вертолет был сбит партизанами в Южной Америке, оставшийся в живых экипаж попал в плен. ЦРУ бросает лучшие силы для освобождения американских граждан — элитная группа спецназа во главе с Датчем отправляется в южноамериканские джунгли. Но там немало повидавшим бойцам предстоит встретиться с невероятным ужасом.',
  array['ужасы', 'боевик', 'фантастика']::text[],
  array['США', 'Мексика']::text[],
  106,
  6303,
  'tt0093773',
  7.9,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:05:21.000Z',
  false,
  false,
  false
),
(
  'media-солярис-322',
  'Солярис',
  'Солярис',
  'movie',
  1972,
  './posters/солярис-dd8ab89b.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c073709ab241157de40345c2553/orig',
  'На космическую станцию, сотрудники которой давно и тщетно пытаются сладить с загадкой планеты Солярис, полностью покрытой Океаном, прибывает новый учёный, психолог Крис Кельвин. Его задача - разобраться в странных сообщениях, поступающих со станции и закрыть её вместе со всей бесплодной «соляристикой». Поначалу ему кажется, что немногие уцелевшие на станции ученые сошли с ума, а потом он и сам становится жертвой жуткого наваждения: ему является его жена Хари, некоторое время назад покончившая с собой.',
  array['фантастика', 'драма', 'детектив']::text[],
  array['СССР']::text[],
  593,
  43911,
  'tt0069293',
  8,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:05:22.000Z',
  false,
  false,
  false
),
(
  'media-ральф-323',
  'Ральф',
  'Wreck-It Ralph',
  'movie',
  2012,
  './posters/ральф-09fbd389.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c285479dbc5c1c4318bc54660a8/orig',
  'Ральф – второстепенный персонаж восьмибитной компьютерной игры, и ему надоело находиться в тени главного героя, мастера на все руки Феликса, который всегда появляется, чтобы «исправить» ситуацию! После тридцати лет добросовестной работы в роли злодея, в течение которых все похвалы выпадали на долю Феликса, Ральф больше не хочет быть плохим.

Он решает отправиться в путешествие по аркадным играм разных жанров, чтобы доказать всем, что и он тоже может быть героем. На своем пути Ральф встречает героев разных игр - хладнокровную сержанта Калхун из игры Hero’s Duty, оберегающую планету от нашествия инопланетных захватчиков, и сладкую на вид, но острую на язычок Сластену фон Дю, чье существование в гоночном симуляторе Sugar Rush внезапно оказывается под угрозой. Наконец-то Ральфу выпадает шанс показать, что он умеет не только разрушать!',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  82690,
  586260,
  'tt1772341',
  7.8,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:05:23.000Z',
  false,
  false,
  false
),
(
  'media-левиафан-324',
  'Левиафан',
  'Левиафан',
  'movie',
  2014,
  './posters/левиафан-95061871.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017c07698223f18d7c62296d55f0cd/orig',
  'Автослесарь Николай Сергеев живет со второй женой Лилей и сыном от первого брака в маленьком северном городке на берегу моря. Нечистый на руку мэр города Шелевят пытается изъять его дом и землю для собственных нужд. После двух проигранных судов Николай обращается за помощью к бывшему сослуживцу Дмитрию, успешному столичному адвокату. Но приезд московского гостя лишь усложняет и без того непростую жизнь Николая.',
  array['драма', 'криминал']::text[],
  array['Россия']::text[],
  265180,
  705356,
  'tt2802154',
  7,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:05:24.000Z',
  false,
  false,
  false
),
(
  'media-борат-325',
  'Борат',
  'Borat: Cultural Learnings of America for Make Benefit Glorious Nation of Kazakhstan',
  'movie',
  2006,
  './posters/борат-7a64dfa1.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017c08121eadebc9f059c15ab0ddca/orig',
  'Телеведущий из Казахстана Борат отправляется в США, чтобы сделать репортаж об этой «величайшей в мире стране». Однако по прибытии оказалось, что главная цель его визита — поиски Памелы Андерсон с целью жениться на ней, а вовсе не съемки документального фильма.',
  array['комедия']::text[],
  array['США', 'Великобритания']::text[],
  496,
  102474,
  'tt0443453',
  6.7,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:05:25.000Z',
  false,
  false,
  false
),
(
  'media-бамблби-326',
  'Бамблби',
  'Bumblebee',
  'movie',
  2018,
  './posters/бамблби-5d4b43ff.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000019be553d17db4bb54d24d3341fe58/orig',
  '1987 год. Скрываясь от преследования, Бамблби прибывает на Землю, где тут же сталкивается с военными. В результате стычки с землянами и десептиконом, Бамблби лишается голосового модуля, повреждает память и перед отключкой сканирует стоящий неподалёку Фольксваген Жук. Через некоторое время девушка по имени Чарли находит его на автомобильной свалке в калифорнийском городке и получает в качестве подарка на 18-летие. Закатив машину в свой гараж, Чарли с удивлением обнаруживает, что ей достался не простой жёлтый Фольксваген Жук.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Китай']::text[],
  424783,
  952241,
  'tt4701182',
  6.7,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:05:26.000Z',
  false,
  false,
  false
),
(
  'media-мост-в-терабитию-327',
  'Мост в Терабитию',
  'Bridge to Terabithia',
  'movie',
  2006,
  './posters/мост-в-терабитию-58679f1c.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c281186a3373be6fca674cf0829/orig',
  'Надежды ученика пятого класса Джесса Аарона стать самым быстрым бегуном в классе разбились после того, как новичок Лесли Берк одержала победу в соревнованиях.

Оснований для враждебности по отношению друг к друг у Джесса и Лесли более чем достаточно, и тем не менее между ними завязывается дружба. Как тут не подружиться, если приходится быть королем и королевой в обнаруженном в лесу волшебном царстве?',
  array['фэнтези', 'драма', 'приключения', 'семейный']::text[],
  array['США', 'Новая Зеландия']::text[],
  1265,
  78753,
  'tt0398808',
  7.7,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:05:27.000Z',
  false,
  false,
  false
),
(
  'media-милые-кости-328',
  'Милые кости',
  'The Lovely Bones',
  'movie',
  2009,
  './posters/милые-кости-dc92cd3a.jpg',
  'https://avatars.mds.yandex.net/get-ott/13051577/2a000001966795e9ad32a55edd8e164a90f9/orig',
  '14-летнюю Сюзи Сэлмон изнасиловал и убил сосед-маньяк. В качестве бесплотного призрака она наблюдает за жизнью своей семьи и жаждет поимки душегуба.',
  array['драма', 'фэнтези', 'триллер', 'криминал']::text[],
  array['США', 'Великобритания', 'Новая Зеландия']::text[],
  7980,
  51319,
  'tt0380510',
  7.1,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:05:28.000Z',
  false,
  false,
  false
),
(
  'media-том-и-джерри-329',
  'Том и Джерри',
  'Tom and Jerry',
  'movie',
  2021,
  './posters/том-и-джерри-172d57da.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000186b195f68d6c7147e38e3246bbc8/orig',
  'Неугомонная парочка отважных друзей в полнометражном мультфильме «Том и Джерри». Здесь они не гоняются друг за другом, а помогают маленькой девочке найти пропавшего отца.

Но не думайте, что вам будет скучно. Ведь это Том и Джерри! Погони, море опасностей, злые козни старой тетки. Неповторимый юмор и радость от общения с любимыми героями!',
  array['мультфильм', 'мюзикл', 'семейный']::text[],
  array['США']::text[],
  587807,
  468562,
  'tt1361336',
  7.5,
  5.3,
  null,
  null,
  null,
  '2026-06-21T12:05:29.000Z',
  false,
  false,
  false
),
(
  'media-кот-в-сапогах-330',
  'Кот в сапогах',
  'Puss in Boots',
  'cartoon',
  2011,
  './posters/кот-в-сапогах-911c65ae.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000018c62deb9bf50cf14c81ab87db824/orig',
  'Кот в сапогах объединяется с умным Шалтай-Болтаем и сообразительной Кисой, чтобы украсть знаменитую Гусыню, несущую золотые яйца.',
  array['мультфильм', 'фэнтези', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Индия']::text[],
  417859,
  102146,
  'tt0448694',
  7.1,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:05:30.000Z',
  false,
  false,
  false
),
(
  'media-кот-в-сапогах-2-последнее-желание-331',
  'Кот в сапогах 2: Последнее желание',
  'Puss in Boots: The Last Wish',
  'movie',
  2022,
  './posters/кот-в-сапогах-2-последнее-желание-1429ade9.jpg',
  './posters/кот-в-сапогах-2-последнее-желание-1429ade9.jpg',
  'Промотав восемь жизней из девяти и до смерти испугавшись пришедшего за ним волка-охотника за головами, Кот в сапогах внимает совету врача и заселяется в дом местной кошатницы. Там он смиряется с установленными порядками, отращивает бороду и теряет тягу к подвигам и приключениям, но только до тех пор, пока не узнаёт о волшебной карте, которая указывает путь к где-то упавшей Звезде желаний. Кот в сапогах твёрдо решает отыскать Звезду, чтобы вернуть свои жизни, и в компании притворявшегося кошачьим пёсика отправляется красть карту у известного коллекционера волшебных предметов Джека Хорнера.',
  array['мультфильм', 'комедия', 'приключения', 'фэнтези']::text[],
  array['США', 'Япония']::text[],
  315162,
  840821,
  'tt3915174',
  8.1,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:05:31.000Z',
  false,
  false,
  false
),
(
  'media-армагеддон-332',
  'Армагеддон',
  'Armageddon',
  'movie',
  1998,
  './posters/армагеддон-4a3a5a4f.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c079d9a42b1058df868849747a5/orig',
  'Тень гигантского астероида легла на Землю. До рокового столкновения остаются считанные дни. Наступает Армагеддон, трагический финал мировой истории. Чтобы предотвратить катастрофу, необходимо чудо - или совместные усилия лучших из лучших вместе с технологической мощью всего созданного человеком. Сможет ли горстка людей противостоять вселенской стихии?..',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array['США']::text[],
  95,
  2941,
  'tt0120591',
  7.8,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:05:32.000Z',
  false,
  false,
  false
),
(
  'media-аритмия-333',
  'Аритмия',
  'Аритмия',
  'movie',
  2017,
  './posters/аритмия-f96020e6.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a00000193f9639976eda99c3a076bb1c739/orig',
  'Олег — талантливый врач скорой помощи, который отдаёт все силы работе. Его жена Катя — тоже врач, она работает в приёмном отделении больницы. Постепенно супруги отдаляются, Олег начинает злоупотреблять алкоголем из-за стресса, и Катя решает развестись. Вскоре у мужчины начинаются проблемы на работе из-за нового начальника, который печётся только о статистике и правилах.',
  array['мелодрама', 'драма']::text[],
  array['Россия']::text[],
  457842,
  992605,
  'tt6952604',
  7.8,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:05:33.000Z',
  false,
  false,
  false
),
(
  'media-очень-страшное-кино-334',
  'Очень страшное кино',
  'Scary Movie',
  'movie',
  2000,
  './posters/очень-страшное-кино-8e82deec.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c280b36fee34d24af0a5ba0af45/orig',
  'Дрю разговаривает по телефону с незнакомцем, будто бы попавшим не туда, но вскоре она понимает, что её собеседник — маньяк. Дрю выбегает из дома, за ней гонится убийца, но её случайно сбивает на машине собственный отец.',
  array['комедия', 'ужасы']::text[],
  array['США']::text[],
  4247,
  5932,
  'tt0175142',
  7,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:05:34.000Z',
  false,
  false,
  false
),
(
  'media-чудо-женщина-335',
  'Чудо-женщина',
  'Wonder Woman',
  'movie',
  2017,
  './posters/чудо-женщина-0d3e8200.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c072896360dd0e60cf13c6e7ec7/orig',
  'Перед тем как стать Чудо-женщиной, она была Дианой — принцессой амазонок, обученной быть непобедимой воительницей. И когда на берегах ограждённого от внешнего мира её родного острова терпит крушение американский пилот и рассказывает о масштабном конфликте, бушующем во внешнем мире, Диана покидает дом, чтобы противостоять злу. Сражаясь бок о бок с человеком в войне за мир, Диана обнаружит всю полноту своей власти и своё истинное предназначение.',
  array['фантастика', 'боевик', 'военный', 'фэнтези']::text[],
  array['США', 'Китай']::text[],
  297762,
  102383,
  'tt0451279',
  6.8,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:05:35.000Z',
  false,
  false,
  false
),
(
  'media-тетрадь-смерти-336',
  'Тетрадь смерти',
  'Death Note',
  'anime',
  2017,
  './posters/тетрадь-смерти-4bcf4dca.jpg',
  './posters/тетрадь-смерти-4bcf4dca.jpg',
  'Старшекласснику Лайту Ягами в руки попадает тетрадь синигами Рюка. Каждый человек, чьё имя записать в эту тетрадку, умрёт, поэтому Лайт решает бороться со злом на земле.',
  array['аниме', 'мультфильм', 'фэнтези', 'триллер', 'драма', 'криминал', 'детектив']::text[],
  array['Япония']::text[],
  351460,
  463596,
  'tt1241317',
  8.6,
  4.5,
  null,
  null,
  null,
  '2026-06-21T12:05:36.000Z',
  false,
  false,
  false
),
(
  'media-тьма-337',
  'Тьма',
  'Dark',
  'series',
  2017,
  './posters/тьма-5dee406e.jpg',
  './posters/тьма-5dee406e.jpg',
  'История четырёх семей, живущих спокойной и размеренной жизнью в маленьком немецком городке. Видимая идиллия рушится, когда бесследно исчезают двое детей и воскресают тёмные тайны прошлого.',
  array['триллер', 'фантастика', 'драма', 'криминал', 'детектив']::text[],
  array['Германия', 'США']::text[],
  70523,
  1032606,
  'tt5753856',
  8.1,
  8.7,
  null,
  null,
  null,
  '2026-06-21T12:05:37.000Z',
  false,
  false,
  false
),
(
  'media-последний-самурай-338',
  'Последний самурай',
  'The Last Samurai',
  'movie',
  2003,
  './posters/последний-самурай-2f803b9a.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017c0736751e88387c03908bd7ad1c/orig',
  'Действие разворачивается в Японии 70-ых годов девятнадцатого века. Капитан Нейтон Альгрен, уважаемый американский военный офицер, нанят Императором Японии для обучения первой армии Страны Восходящего Солнца современному искусству ведения боевых действий.

Император пытается искоренить древних воинов-самураев, готовясь к более прозападной политике правительства…',
  array['боевик', 'драма', 'военный', 'история']::text[],
  array['США', 'Новая Зеландия', 'Япония']::text[],
  616,
  6764,
  'tt0325710',
  8.1,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:05:38.000Z',
  false,
  false,
  false
),
(
  'media-гаттака-339',
  'Гаттака',
  'Gattaca',
  'movie',
  1997,
  './posters/гаттака-e076c98e.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c072575c8d1c2c125a140caad4f/orig',
  'Добро пожаловать в Гаттаку - совершенный мир будущего. Здесь каждый генетически запрограммирован, и печальная судьба ожидает тех, кто был рожден в любви, а не в лаборатории. Такова судьба Винсента Фримана, молодого человека, получившего при рождении ярлык «не пригоден».

Винсент обладает весомыми недостатками: он подвержен страстям, он поддается эмоциям, и он верит в то, что его мечты сбудутся. Вот почему он покупает личность другого человека, пытаясь обмануть власти и стать уважаемым членом Корпорации Будущего Гаттака.

И когда он уже на пути к свободе, убийство грозит раскрытием его реальной личности. Несмотря на свою невиновность, Винсенту есть что скрывать и есть что терять. Но так трудно остаться в живых, когда в тебе живут два человека…',
  array['фантастика', 'драма', 'мелодрама', 'детектив']::text[],
  array['США']::text[],
  782,
  5012,
  'tt0119177',
  8,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:05:39.000Z',
  false,
  false,
  false
),
(
  'media-аркейн-340',
  'Аркейн',
  'Arcane: League of Legends',
  'series',
  2021,
  './posters/аркейн-306d56b4.jpg',
  './posters/аркейн-306d56b4.jpg',
  'История разворачивается в утопическом краю Пилтовер и жестоком подземном мире Заун и рассказывает о становлении двух легендарных чемпионов Лиги и о той силе, что разведёт их по разные стороны баррикад.',
  array['мультфильм', 'фантастика', 'фэнтези', 'боевик', 'драма']::text[],
  array['США', 'Франция']::text[],
  null,
  4445150,
  'tt11126994',
  8.7,
  9,
  null,
  null,
  null,
  '2026-06-21T12:05:40.000Z',
  false,
  false,
  false
),
(
  'media-невероятный-халк-341',
  'Невероятный Халк',
  'The Incredible Hulk',
  'movie',
  2008,
  './posters/невероятный-халк-2d682189.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c0757e7f891c1c96f0d159706ee/orig',
  'История доктора Брюса Баннера, который ищет лекарство от своего необычного «заболевания», превращающего его во время эмоционального стресса в гигантского зеленого монстра Халка. Находясь в бегах от армии, стремящейся его захватить, Брюс почти находит лекарство, но все старания идут прахом, когда у Халка вдруг появляется новый, невероятно сильный противник.',
  array['фантастика', 'боевик', 'триллер']::text[],
  array['США', 'Канада']::text[],
  1724,
  255380,
  'tt0800080',
  6.9,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:05:41.000Z',
  false,
  false,
  false
),
(
  'media-шрэк-третий-342',
  'Шрэк Третий',
  'Shrek the Third',
  'cartoon',
  2007,
  './posters/шрэк-третий-28625418.jpg',
  'https://avatars.mds.yandex.net/get-ott/1648503/2a0000017c0779b13b8b1818078aa2f4ba3e/orig',
  'Король Гарольд внезапно умер, и теперь великан Шрек вынужден стать королем Далекой-Далекой страны. Шрек уважает семейные традиции своей жены Фионы, но править страной очень не хочет, и поэтому и отправляется с Ослом и Котом в сапогах на поиски нового короля. Но пока Шрек ищет короля Артура - другого наследника, принц Чарминг замышляет новые пакости.',
  array['мультфильм', 'фэнтези', 'мелодрама', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Австралия']::text[],
  810,
  84020,
  'tt0413267',
  6.7,
  6.1,
  null,
  null,
  null,
  '2026-06-21T12:05:42.000Z',
  false,
  false,
  false
),
(
  'media-каратэ-пацан-343',
  'Каратэ-пацан',
  'The Karate Kid',
  'movie',
  2010,
  './posters/каратэ-пацан-18668d1c.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000017c07891dccbb72454b9722e53698/orig',
  'Двенадцатилетний Дре Паркер вместе с матерью-одиночкой переезжает из Детройта в незнакомый Пекин, не зная языка и местных традиций. Здесь он встречает китайскую девочку Мей Ин. Однако кроме её отца, не одобряющего их дружбу из-за культурных предрассудков, на его пути встаёт банда хулиганов во главе с Ченом, владеющим искусством кунг-фу. Единственный способ победить его — встретиться с врагом на близящемся грандиозном турнире боевых искусств. В этом мальчику решает помочь знаток боевых искусств мистер Хан.',
  array['драма', 'спорт', 'семейный']::text[],
  array['Китай', 'Гонконг', 'США']::text[],
  38575,
  412162,
  'tt1155076',
  7.1,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:05:43.000Z',
  false,
  false,
  false
),
(
  'media-флэш-344',
  'Флэш',
  'The Flash',
  'series',
  2014,
  './posters/флэш-fdefc57f.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a00000186346b6983b7c0a61c39f3152ff3/orig',
  'Когда Барри Аллен был маленьким, больше всего на свете ему хотелось быть супергероем — тем, кто превосходит лимиты человеческого организма и использует во благо данную ему силу. В одиннадцать лет он на собственном опыте узнал, что люди с необычными способностями действительно существуют: его мать была убита одним из таких людей. Повзрослев и став судмедэкспертом, Барри не отбросил мыслей о сверхлюдях и продолжал искать доказательства их существования, что не лучшим образом сказывалось на его репутации и общении с коллегами. Впрочем, однажды его усилия были вознаграждены.',
  array['фантастика', 'фэнтези', 'боевик', 'драма', 'приключения']::text[],
  array['США']::text[],
  60735,
  817506,
  'tt3107288',
  7.7,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:05:44.000Z',
  false,
  false,
  false
),
(
  'media-лучше-звоните-солу-345',
  'Лучше звоните Солу',
  'Better Call Saul',
  'series',
  2015,
  './posters/лучше-звоните-солу-95c837a9.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c079fa1c914dd8ece10dfe1a1b5/orig',
  'История об испытаниях и невзгодах, которые приходится преодолеть Солу Гудману, адвокату по уголовным делам, в тот период, когда он пытается открыть свою собственную адвокатскую контору в Альбукерке, штат Нью-Мексико.',
  array['драма', 'криминал']::text[],
  array['США']::text[],
  60059,
  796660,
  'tt3032476',
  8.1,
  9,
  null,
  null,
  null,
  '2026-06-21T12:05:45.000Z',
  false,
  false,
  false
),
(
  'media-хеллбой-346',
  'Хеллбой',
  'Hellboy',
  'movie',
  2019,
  './posters/хеллбой-37e80d62.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c0777e992615d635e7b0496140c/orig',
  'Близится час битвы Хеллбоя с Кровавой королевой, буквально собирающей себя по частям и жаждущей отомстить всему человечеству за века в заточении. Оказавшись в эпицентре столкновения миров людей и монстров, Хеллбой будет вынужден пройти через ад, чтобы предотвратить надвигающийся апокалипсис.',
  array['фэнтези', 'боевик', 'приключения']::text[],
  array['США', 'Великобритания', 'Болгария', 'Канада', 'Португалия', 'Франция']::text[],
  456740,
  693331,
  'tt2274648',
  6.2,
  5.3,
  null,
  null,
  null,
  '2026-06-21T12:05:46.000Z',
  false,
  false,
  false
),
(
  'media-хеллбой-2-золотая-армия-347',
  'Хеллбой II: Золотая армия',
  'Hellboy II: The Golden Army',
  'movie',
  2008,
  './posters/хеллбой-ii-золотая-армия-5b43956f.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000161adce66514d6b27afe06a9f537f/1344x756',
  'Древнее перемирие между человечеством и тайным миром было нарушено, на Земле вот-вот разверзнется ад. Безжалостный лидер, вхожий в оба мира, бросает вызов своей крови и пробуждает на свет неодолимую армию чудовищ. 
Настало время самому Хэллбою сразиться с беспощадным диктатором и его воинами. Вместе со своей пополняющей ряды командой «Бюро Паранормальных Исследований и Развития» он будет бороздить границы двух миров — нашего и скрытого, где самые невероятные существа обретают плоть. Хэллбою, порождённому обоими мирами, но не принадлежащему ни одному из них, предстоит выбирать между жизнью, которую он знает, и неизвестностью новой судьбы.',
  array['ужасы', 'боевик', 'фантастика', 'фэнтези', 'приключения']::text[],
  array['США', 'Германия', 'Венгрия']::text[],
  null,
  81472,
  'tt0411477',
  7.1,
  7,
  null,
  null,
  null,
  '2026-06-21T12:05:47.000Z',
  false,
  false,
  false
),
(
  'media-принцесса-мононоке-348',
  'Принцесса Мононоке',
  'Mononoke-hime',
  'anime',
  1997,
  './posters/принцесса-мононоке-8766e7cc.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c073aacd7d6ed3d44e60181a097/orig',
  'Убив вепря, юный принц Аситака навлек на себя смертельное проклятие. Старая знахарка предсказала, что только он сам способен изменить свою судьбу, и отважный воин отправился в опасное путешествие. Так он оказался в загадочной стране, где люди под предводительством злой госпожи Эбоси воевали с обитателями леса: духами, демонами и гигантскими существами, каких Аситака раньше никогда не видел. 
И была с ними принцесса Мононоке - повелительница зверей и дочь волчицы. Теперь судьба всех зависит только от одного воина - принца Аситаки.',
  array['аниме', 'мультфильм', 'фэнтези', 'драма', 'приключения']::text[],
  array['Япония']::text[],
  128,
  441,
  'tt0119698',
  8.1,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:05:48.000Z',
  false,
  false,
  false
),
(
  'media-дэдпул-и-росомаха-349',
  'Дэдпул и Росомаха',
  'Deadpool & Wolverine',
  'movie',
  2024,
  './posters/дэдпул-и-росомаха-e0f633dc.jpg',
  './posters/дэдпул-и-росомаха-e0f633dc.jpg',
  'Уэйд Уилсон попадает в организацию «Управление временными изменениями», что вынуждает его вернуться к своему альтер-эго Дэдпулу и изменить историю с помощью Росомахи.',
  array['фантастика', 'боевик', 'комедия', 'приключения']::text[],
  array['США', 'Великобритания', 'Новая Зеландия', 'Канада']::text[],
  null,
  1008444,
  'tt6263850',
  7.3,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:05:49.000Z',
  false,
  false,
  false
),
(
  'media-доктор-стрэндж-в-мультивселенной-безумия-350',
  'Доктор Стрэндж: В мультивселенной безумия',
  'Doctor Strange in the Multiverse of Madness',
  'movie',
  2022,
  './posters/доктор-стрэндж-в-мультивселенной-безумия-ca6abcd0.jpg',
  './posters/доктор-стрэндж-в-мультивселенной-безумия-ca6abcd0.jpg',
  'Доктор Стрэндж при помощи Вонга спасает от гигантского осьминога девушку-подростка по имени Америка Чавес, которая при сильном испуге может открывать порталы в параллельные вселенные. Пытаясь уберечь новую знакомую от злой силы, жаждущей заполучить её способность, Доктор вместе с Америкой пускается в путешествие по мультивселенной.',
  array['фэнтези', 'боевик', 'приключения', 'фантастика', 'ужасы']::text[],
  array['США']::text[],
  null,
  1219909,
  'tt9419884',
  6.5,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:05:50.000Z',
  false,
  false,
  false
),
(
  'media-телепорт-351',
  'Телепорт',
  'Jumper',
  'movie',
  2008,
  './posters/телепорт-2beaaec5.jpg',
  'https://avatars.mds.yandex.net/get-ott/17687051/2a0000019c09f162cb12a2e2519509a6782a/orig',
  'Подросток из неблагополучного района Дэвид Райс всегда считал себя обычным парнем, пока однажды он не узнал, что он может телепортироваться с места на место. Новые способности открыли перед ним весь мир. Он может побывать в Нью-Йорке и Токио, посетить античные развалины в Риме, увидеть «крышу мира» с горы Эверест, увидеть 20 рассветов и 20 закатов. И все - в один день. 

Теперь ему не нужны деньги - он может взять сколько хочет. Однако в один момент он обнаруживает, что стал мишенью. На него, как и на других подобных ему, объявлена охота. На протяжении тысячи лет тайное общество стремится уничтожить их. Вскоре Дэвид узнает истинную ценность его новых способностей.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['Канада', 'США']::text[],
  8247,
  220589,
  'tt0489099',
  7,
  6.1,
  null,
  null,
  null,
  '2026-06-21T12:05:51.000Z',
  false,
  false,
  false
),
(
  'media-таксист-352',
  'Таксист',
  'Taxi Driver',
  'movie',
  1976,
  './posters/таксист-b847d9cd.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c078c1f216547f8beae2e0b2946/orig',
  'Тусклый свет слепых фонарей, скелеты фабричных труб, задыхающихся в собственном дыму. Вавилонские башни небоскребов, все это — ад Нового времени, Нью-Йорк.

Ветеран вьетнамской войны Трэвис Бикл ведет свое одинокое такси по ночным улицам бесконечного города, и перед ним разворачивается мрачная панорама человеческих грехов. Как ветхозаветный пророк, он надеется, что однажды небеса пошлют на землю спасительный дождь, который очистит Нью-Йорк от вековой грязи.

А когда умирает надежда, остается только ненависть. Огненный ливень обрушится на головы грешников. Таксист позаботится об этом.',
  array['триллер', 'драма', 'криминал']::text[],
  array['США']::text[],
  103,
  358,
  'tt0075314',
  7.7,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:05:52.000Z',
  false,
  false,
  false
),
(
  'media-виноваты-звезды-353',
  'Виноваты звезды',
  'The Fault in Our Stars',
  'movie',
  2014,
  './posters/виноваты-звезды-ad2380fe.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000017c071d981874e852a3052652b6c6/orig',
  'Хэйзел больна раком. Несмотря на то, что болезнь временно отступила, девушка не чувствует ни капли радости. Она ходит в группу поддержки, где однажды знакомится с Огастусом Уотерсом и моментально влюбляется в него. Огастус и Хэйзел отправляются в полное страсти и жизни путешествие, которое лишний раз покажет им, что весь смысл жизни можно найти в любом ее отрезке.',
  array['драма', 'мелодрама']::text[],
  array['США']::text[],
  222935,
  724703,
  'tt2582846',
  7.9,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:05:53.000Z',
  false,
  false,
  false
),
(
  'media-приключения-буратино-354',
  'Приключения Буратино',
  'Приключения Буратино',
  'movie',
  1975,
  './posters/приключения-буратино-f0125dfe.jpg',
  'https://avatars.mds.yandex.net/get-ott/17687051/2a0000019b70006c818190b2a4bc0b8060f0/orig',
  'На дурака не нужен нож: ему немного подпоешь - и делай с ним, что хошь! Деревянный человечек, которого Папа Карло выстругал из полена, получился веселым, дерзким, шумным и доверчивым. Лиса Алиса и Кот Базилио завели его в Страну Дураков, но мудрая Черепаха Тортила помогла Буратино найти Золотой Ключик, потому что он был ужасно обаятельным.',
  array['детский', 'мюзикл', 'фэнтези', 'приключения', 'семейный']::text[],
  array['СССР']::text[],
  null,
  77301,
  'tt0073570',
  7.9,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:05:54.000Z',
  false,
  false,
  false
),
(
  'media-пила-355',
  'Пила',
  'Saw',
  'movie',
  2003,
  './posters/пила-3222366e.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000167371cb6e2fba690e7c4d7fc5896/orig',
  'Дэвид, работавший санитаром в больнице, рассказывает детективу ужасающую историю о том, как был похищен и вынужден играть в мерзкую игру на выживание.',
  array['короткометражка', 'ужасы', 'триллер']::text[],
  array['США', 'Австралия']::text[],
  246355,
  251613,
  'tt0495241',
  7.7,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:05:55.000Z',
  false,
  false,
  false
),
(
  'media-пила-2-356',
  'Пила 2',
  'Saw II',
  'movie',
  2005,
  './posters/пила-2-c94d0071.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c0727bd5a6aa5830a67ac29d3e6/orig',
  'Во время расследования подробностей кровавых деяний неизвестного убийцы детектив Эрик Мэтьюс ловит себя на мысли, что они очень похожи на работу Пилы - известного маньяка, который смог исчезнуть, оставив за собой множество жертв. Метьюс оказывается прав. Пила снова взялся за своё ужасное ремесло. Только на этот раз вместо двух людей он запирает в комнате смерти с единственным способом выбраться живым сразу восемь человек. Восемь незнакомцев оказываются вынуждены вступить в игру сумасшедшего убийцы, ставками в которой будут их жизни.',
  array['ужасы', 'триллер', 'детектив', 'криминал']::text[],
  array['Канада', 'США']::text[],
  null,
  86206,
  'tt0432348',
  7.4,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:05:56.000Z',
  false,
  false,
  false
),
(
  'media-пила-3-357',
  'Пила 3',
  'Saw III',
  'movie',
  2006,
  './posters/пила-3-dbb96a5d.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c071d6eaa061c94aa200a0de143/orig',
  'Изобретатель зловещих игр остается безнаказанным и в итоге бесследно исчезает вместе со своей сообщницей Амандой. В то время, как детективы тщетно пытаются выйти на его след, доктор Линн Денлон становится очередной жертвой этого маньяка.

Однажды ночью Линн похищают и привозят в заброшенный склад, где она встречается с самим злодеем, находящимся на смертном одре. Ей приказывают сохранять жизнь этому безумцу, пока Джефф, еще одна из его жертв, не доведет до конца начатую с ним «игру»…',
  array['ужасы', 'триллер', 'детектив', 'криминал']::text[],
  array['Канада', 'США']::text[],
  null,
  184417,
  'tt0489270',
  7,
  6.2,
  null,
  null,
  null,
  '2026-06-21T12:05:57.000Z',
  false,
  false,
  false
),
(
  'media-хищные-птицы-потрясающая-история-харли-квинн-358',
  'Хищные птицы: Потрясающая история Харли Квинн',
  'Birds of Prey and the Fantabulous Emancipation of One Harley Quinn',
  'movie',
  2020,
  './posters/хищные-птицы-потрясающая-история-харли-квинн-36530ead.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017c2d0c6a628f544ed5099d2179ee/orig',
  'Харли Квинн рассталась с Джокером и через некоторое время сообщила об этом всему Готэму, взорвав тот самый химзавод. Девушка больше не тоскует по несостоявшимся отношениям — ей некогда, так как она также потеряла неприкосновенность, и теперь за ней охотятся разной степени криминализованности граждане, желающие отомстить. А в это время ставший крёстным отцом Готема Роман Сайонис очень жаждет вернуть пропавший бриллиант, который стащила у его подручного малолетняя карманница.',
  array['боевик', 'криминал', 'комедия']::text[],
  array['США', 'Великобритания']::text[],
  495764,
  1111004,
  'tt7713068',
  5.9,
  6.1,
  null,
  null,
  null,
  '2026-06-21T12:05:58.000Z',
  false,
  false,
  false
),
(
  'media-перевозчик-359',
  'Перевозчик',
  'The Transporter',
  'movie',
  2002,
  './posters/перевозчик-92ace224.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c079c6cb02d33869f7be031d75d/orig',
  'Бывший десантник Фрэнк Мартин имеет неплохой бизнес — перевозит любые грузы по французскому Средиземноморью и делает свою работу быстро и качественно. Недостатка в клиентах нет, ведь он всегда неукоснительно соблюдает три правила: не меняет условий сделки, не спрашивает никаких имен и никогда не заглядывает в багаж. Но однажды, перевозя груз клиента по имени Уолл Стрит, Фрэнк обнаруживает, что мешок шевелится. Впервые за все время Мартин нарушает правило, заглядывает внутрь пакета и обнаруживает там красивую женщину, которая оказывается дочерью видного китайского мафиози…',
  array['боевик', 'триллер', 'криминал']::text[],
  array['Франция', 'США']::text[],
  4108,
  6206,
  'tt0293662',
  7.4,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:05:59.000Z',
  false,
  false,
  false
),
(
  'media-черепашки-ниндзя-360',
  'Черепашки-ниндзя',
  'Teenage Mutant Ninja Turtles',
  'movie',
  2014,
  './posters/черепашки-ниндзя-e0e17df7.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a000001967cec9d047f559ea90af4186d54/orig',
  'Город нуждается в героях. Тьма окутала Нью-Йорк в виде Шреддера и его зловещего Клана Футов, у которого имеется железный контроль над всеми, от полицейских до политиков. Будущее выглядит мрачным, пока из канализации не поднимается и не раскрывает своё предназначение четвёрка отверженных братьев — Черепашки-ниндзя. Черепашки должны работать вместе с бесстрашным репортёром Эйприл и её саркастическим оператором Верном Фенвиком, чтобы спасти город и разгадать дьявольский план Шреддера.',
  array['фантастика', 'боевик', 'комедия', 'приключения']::text[],
  array['США']::text[],
  98566,
  461206,
  'tt1291150',
  6.2,
  5.8,
  null,
  null,
  null,
  '2026-06-21T12:06:00.000Z',
  false,
  false,
  false
),
(
  'media-13-й-район-361',
  '13-й район',
  'Banlieue 13',
  'movie',
  2004,
  './posters/13-й-район-541427dd.jpg',
  'https://avatars.mds.yandex.net/get-ott/1648503/2a0000018bd69dd40367c06f21485eae1464/orig',
  '2010 год, Париж, из-за разгула преступности некоторые особо опасные районы французской столицы превращены в закрытые зоны и отгорожены от благополучных кварталов бетонной стеной. За ней бурлит совсем другая жизнь: здесь процветает торговля наркотиками и оружием, здесь даже дети не выходят из дома без оружия, здесь давно не работают школы и поликлиники, здесь полиция не стоит на страже закона, а лишь защищает себя или же сама поддерживает беззаконие. Главарю одной из банд удается завладеть нейтронной бомбой. Чтобы обезвредить ее, в 13-й район отправляется офицер спецназа Дамьен. Проникать в опасные зоны - его профессия, однако на этот раз ему все-таки нужен проводник. Союзником Дамьена становится Лейто, отчаянный парень, сестра которого похищена тем же самым главарем...',
  array['фантастика', 'боевик', 'триллер', 'криминал']::text[],
  array['Франция']::text[],
  null,
  81522,
  'tt0414852',
  7.6,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:06:01.000Z',
  false,
  false,
  false
),
(
  'media-13-й-район-ультиматум-362',
  '13-й район: Ультиматум',
  'Banlieue 13 Ultimatum',
  'movie',
  2009,
  './posters/13-й-район-ультиматум-366748fa.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c07829a042365cdd98ac7498096/orig',
  '13-й район, три года спустя. Стена, отделяющая неблагополучный пригород от столицы, стала больше, выше и отгородила от цивилизации еще больше кварталов. По эту сторону стены правят пять банд, контролирующих этнические районы. Правительство как никогда заинтересовано в «урегулировании проблемы», и разведывательные службы охотно инициируют беспорядки. Лейто и Дамьену снова приходится объединить усилия, чтобы спасти Париж от хаоса.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['Франция']::text[],
  15357,
  418794,
  'tt1247640',
  6.6,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:06:02.000Z',
  false,
  false,
  false
),
(
  'media-трансформеры-последний-рыцарь-363',
  'Трансформеры: Последний рыцарь',
  'Transformers: The Last Knight',
  'movie',
  2017,
  './posters/трансформеры-последний-рыцарь-221b0c43.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a000001966d309c3d8662fcfb8cdaaa7cf0/orig',
  'Оптимус Прайм исчез. Люди ведут войну с Трансформерами. Ключ к нашему будущему погребен в тайнах прошлого, в скрытой истории Трансформеров на Земле… Миссия по спасению мира ложится на плечи разношерстной компании, состоящей из Кейда Йегера, Бамблби, английского лорда и профессора из Оксфорда. Пришла пора действовать! Жертвы станут героями. Герои станут злодеями. Выстоит только один мир: их или наш.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Китай', 'Канада']::text[],
  335988,
  811609,
  'tt3371366',
  5.8,
  5.2,
  null,
  null,
  null,
  '2026-06-21T12:06:03.000Z',
  false,
  false,
  false
),
(
  'media-12-обезьян-364',
  '12 обезьян',
  'Twelve Monkeys',
  'movie',
  1995,
  './posters/12-обезьян-df4a86ff.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c072a7089bc8c38f336af9a28c3/orig',
  '2035 год... Чудовищный, неизлечимый вирус уничтожил пять миллиардов человек, то есть 99% населения Земли. Оставшиеся в живых люди вынуждены обитать под землей.

Уголовник Джеймс Коул добровольно отправляется в опасное путешествие на машине времени. Он должен попасть в прошлое, чтобы помочь ученым найти источник возникновения этого ужасного вируса и раскрыть загадку таинственных «Двенадцати обезьян»...',
  array['фантастика', 'триллер', 'детектив']::text[],
  array['США']::text[],
  63,
  502,
  'tt0114746',
  7.8,
  8,
  null,
  null,
  null,
  '2026-06-21T12:06:04.000Z',
  false,
  false,
  false
),
(
  'media-собачья-жизнь-365',
  'Собачья жизнь',
  'A Dog''s Purpose',
  'movie',
  2017,
  './posters/собачья-жизнь-7982da0d.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c077fafb6149e2780d89fa72872/orig',
  'Собаки бывают разных пород, мастей и размеров. Они - наши самые верные и преданные друзья, а мы учим их хорошим манерам. Собаки всегда думают о нас, правда, иногда и о беконе. Все они каждый день провожают и встречают нас у порога в уверенности, что пока им есть, кого облизывать и любить, в их жизни есть смысл.',
  array['фэнтези', 'драма', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Индия']::text[],
  381289,
  571896,
  'tt1753383',
  8.1,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:06:05.000Z',
  false,
  false,
  false
),
(
  'media-адреналин-366',
  'Адреналин',
  'Crank',
  'movie',
  2006,
  './posters/адреналин-c11acc86.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000180cc2d904a8d750027e2cfb158fb/orig',
  'Наемный убийца Чев узнает, что недоброжелатели отравили его редким китайским ядом, который начнет действовать немедленно, как только пульс перестанет биться ниже определенной отметки.
Теперь Чеву нужно успеть сделать все свои дела, попробовать раздобыть противоядие и отомстить своим отравителям, стараясь создавать вокруг себя как можно более напряженную обстановку и не расслабляться ни на секунду.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США']::text[],
  1948,
  180609,
  'tt0479884',
  7.3,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:06:06.000Z',
  false,
  false,
  false
),
(
  'media-fallout-367',
  'Fallout',
  'Fallout',
  'series',
  2024,
  './posters/fallout-b0f765e8.jpg',
  './posters/fallout-b0f765e8.jpg',
  'В 2296 году, более 200 лет спустя после ядерной войны, потомки привилегированных и богатых живут в автономных благоустроенных бункерах, а остальное человечество выживает в жёстких условиях. Умница и красавица Люси Маклин из убежища № 33 собиралась выйти замуж за парня из № 32, но вместо этого их дом подвергается нападению рейдеров, которые похищают её отца. Несмотря на всеобщие возражения, наивная девушка отправляется на поверхность полная решимости отыскать предводительницу рейдеров, и вскоре её путь пересекается с членом военно-религиозной организации «Братство стали» Максимусом и мутантом-охотником за головами, в прошлом — звездой вестернов Купером Говардом.',
  array['фантастика', 'боевик', 'драма', 'приключения']::text[],
  array['США']::text[],
  106379,
  1394131,
  'tt12637874',
  8,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:06:07.000Z',
  false,
  false,
  false
),
(
  'media-невезучие-368',
  'Невезучие',
  'Tais-toi!',
  'movie',
  2003,
  './posters/невезучие-434ab1e7.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000018cf84ba7a18675f868510b36f540/orig',
  'Перед заключением Руби успевает спрятать награбленные деньги, украденные у мафии. В тюрьме он знакомится с верзилой Квентином, и теперь они вместе думают, как выбраться на волю и вытащить деньги из тайника. К тому же Руби мечтает отомстить своим бывшим компаньонам за смерть своей возлюбленной. Но ему всё время не везёт.',
  array['комедия', 'криминал']::text[],
  array['Франция', 'Италия']::text[],
  null,
  14287,
  'tt0310203',
  7.8,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:06:08.000Z',
  false,
  false,
  false
),
(
  'media-пункт-назначения-369',
  'Пункт назначения',
  'Final Destination',
  'movie',
  2000,
  './posters/пункт-назначения-4ce77bbc.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c07280d59cda7be8f9eef7296b3/orig',
  'Алекс летит с классом в Париж на 10 дней. В аэропорту он узнает, что время вылета совпадает с его днем рождения. Потом, повинуясь плохому предчувствию, Алекс в панике покидает самолет, и некоторое время спустя самолет взрывается. Но вскоре пассажиры, сошедшие вместе с Алексом, начинают гибнуть один за другим.',
  array['ужасы', 'фантастика', 'триллер', 'детектив']::text[],
  array['США']::text[],
  null,
  668,
  'tt0195714',
  7.5,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:06:09.000Z',
  false,
  false,
  false
),
(
  'media-пункт-назначения-2-370',
  'Пункт назначения 2',
  'Final Destination 2',
  'movie',
  2003,
  './posters/пункт-назначения-2-1ae39d4b.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c072504fcd241fecb460e23b39c/orig',
  'Решив провести уикэнд с друзьями подальше от дома, Кимберли становится свидетелем жуткой автокатастрофы: на скоростной магистрали из грузовика начинают выпадать брёвна, что приводит к серии столкновений автомобилей и множеству жертв, среди которых она сама. Секунду спустя Кимберли снова чувствует биение своего сердца: она жива и по-прежнему за рулём машины. Видение? А может быть, предупреждение? Однако теперь и Кимберли, и водители, чьи машины из-за неё не смогли выехать к смерти, обречены — старуха с косой отказывается вычёркивать их из своего списка.',
  array['ужасы', 'триллер']::text[],
  array['США', 'Канада']::text[],
  null,
  669,
  'tt0309593',
  7.1,
  6.2,
  null,
  null,
  null,
  '2026-06-21T12:06:10.000Z',
  false,
  false,
  false
),
(
  'media-пункт-назначения-3-371',
  'Пункт назначения 3',
  'Final Destination 3',
  'movie',
  2006,
  './posters/пункт-назначения-3-f15bcb9a.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c074b1c3b586a4487669868ad3e/orig',
  'У старшеклассницы Венди однажды возникает предчувствие, что она и её друзья станут жертвами несчастного случая, который произойдёт на «американских горках». Когда же самые страшные опасения Венди подтверждаются, те, кто сумел всё-таки обмануть смерть, теперь вынуждены за это расплачиваться.',
  array['ужасы', 'триллер']::text[],
  array['США']::text[],
  null,
  89674,
  'tt0414982',
  6.4,
  5.9,
  null,
  null,
  null,
  '2026-06-21T12:06:11.000Z',
  false,
  false,
  false
),
(
  'media-ривердэйл-372',
  'Ривердэйл',
  'Riverdale',
  'series',
  2017,
  './posters/ривердэйл-51950e5c.jpg',
  './posters/ривердэйл-51950e5c.jpg',
  'Сериал расскажет о жизни и приключениях жизнерадостного и никогда не унывающего подростка Арчи Эндрюса, а также Бэтти Купер, Вероники Лодж и их друзей из Ривердэйла. Действие разворачивается в современном мире, а главные герои решают исследовать сокрытую от посторонних сторону жизни маленького мирного городка со всеми его странностями и тёмными тайнами, прячущимися за праздничным фасадом.',
  array['драма', 'мелодрама', 'криминал', 'детектив']::text[],
  array['США']::text[],
  69050,
  962012,
  'tt5420376',
  6.5,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:06:12.000Z',
  false,
  false,
  false
),
(
  'media-терминатор-3-восстание-машин-373',
  'Терминатор 3: Восстание машин',
  'Terminator 3: Rise of the Machines',
  'movie',
  2003,
  './posters/терминатор-3-восстание-машин-24541ddf.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000018dac255646f88ef25d562e037236/orig',
  'Прошло десять лет с тех пор, как Джон Коннор помог предотвратить Судный День и спасти человечество от массового уничтожения. Теперь ему 25, Коннор не живет «как все» - у него нет дома, нет кредитных карт, нет сотового телефона и никакой работы.

Его существование нигде не зарегистрировано. Он не может быть прослежен системой Skynet - высокоразвитой сетью машин, которые когда-то попробовали убить его и развязать войну против человечества. Пока из теней будущего не появляется T-X - Терминатрикс, самый сложный киборг-убийца Skynet.

Посланная назад сквозь время, чтобы завершить работу, начатую её предшественником, T-1000, эта машина так же упорна, как прекрасен её человеческий облик. Теперь единственная надежда Коннору выжить - Терминатор, его таинственный прежний убийца. Вместе они должны одержать победу над новыми технологиями T-X и снова предотвратить Судный День...',
  array['фантастика', 'боевик']::text[],
  array['США', 'Германия', 'Великобритания']::text[],
  null,
  319,
  'tt0181852',
  6.8,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:06:13.000Z',
  false,
  false,
  false
),
(
  'media-элвин-и-бурундуки-374',
  'Элвин и бурундуки',
  'Alvin and the Chipmunks',
  'movie',
  2007,
  './posters/элвин-и-бурундуки-cea6d1e6.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a00000199cdfeca6bd1f176bf3aa5305b55/orig',
  'Жизнь композитора была одной черной полосой до тех пор, пока он не встретил трех живых поющих бурундуков: лидера Элвина, гения Саймона и феерического Теодора. Поначалу они плохо уживаются вместе, но в конце концов Дэйв принимает бурундуков и даже становится менеджером их рок-группы, которая собирает стадионы поклонников.',
  array['семейный', 'комедия', 'музыка']::text[],
  array['США']::text[],
  null,
  278361,
  'tt0952640',
  6.3,
  5.3,
  null,
  null,
  null,
  '2026-06-21T12:06:14.000Z',
  false,
  false,
  false
),
(
  'media-лабиринт-фавна-375',
  'Лабиринт Фавна',
  'El laberinto del fauno',
  'movie',
  2006,
  './posters/лабиринт-фавна-7e379ae8.jpg',
  './posters/лабиринт-фавна-7e379ae8.jpg',
  'Испания, 1944 год. Группа повстанцев сражается с фашистами в горных лесах северной Наварры. Офелия — 10-летняя одинокая и мечтательная девочка — переезжает со своей беременной матерью Кармен в военный лагерь отчима, капитана Видаля. Видаль — высокомерный и жестокий офицер армии Франко, который должен очистить район от повстанцев любой ценой. Офелия, увлеченная волшебными сказками, обнаруживает старинный заброшенный лабиринт неподалеку от дома.

Фея проводит ее в центр лабиринта, где она встречает Фавна, хозяина подземелья. Фавн утверждает, что знает её истинную судьбу и её тайное предназначение. Оказывается Офелия — пропавшая принцесса волшебного королевства, которую многие века разыскивает ее отец. Фавн предлагает ей возможность вернуться в волшебное королевство. Но для начала она должна пройти три испытания до наступления полнолуния. Никто не должен узнать об этом.',
  array['фэнтези', 'драма', 'военный']::text[],
  array['Мексика', 'Испания']::text[],
  null,
  103733,
  'tt0457430',
  7.6,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:06:15.000Z',
  false,
  false,
  false
),
(
  'media-космическая-одиссея-2001-года-376',
  '2001 год: Космическая одиссея',
  '2001: A Space Odyssey',
  'movie',
  1968,
  './posters/2001-год-космическая-одиссея-637edd1b.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a00000169b0571d92f4b7d4b60fa4218b59/1344x756',
  'Экипаж космического корабля «Дискавери» — капитаны Дэйв Боумэн, Фрэнк Пул и их бортовой компьютер HAL 9000 — должны исследовать район галактики и понять, почему инопланетяне следят за Землей. На этом пути их ждет множество неожиданных открытий.',
  array['фантастика', 'приключения']::text[],
  array['Великобритания', 'США']::text[],
  62,
  380,
  'tt0062622',
  7.9,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:06:16.000Z',
  false,
  false,
  false
),
(
  'media-мегамозг-377',
  'Мегамозг',
  'Megamind',
  'cartoon',
  2010,
  './posters/мегамозг-15dc3fd1.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a00000185067f3487f452642067fddda938/orig',
  'Мегамозг – самый гениальный и самый неудачливый злодей в мире. Вот уже много лет он пытается покорить Метро-Сити самыми разнообразными способами. Но каждая такая попытка кончается провалом по вине супергероя по имени Метро-Мэн. Но злодей убивает супергероя, и внезапно Мегамозг лишается цели в жизни. Суперзлодей без супергероя.

Единственный выход – создать нового супергероя, которого он называет Титаном. Но Титан решает, что быть злодеем куда интереснее. Вот только ему не хочется править миром, он желает его уничтожить. Мегамозгу предстоит непростой выбор. Сможет ли злой гений стать героем – спасителем человечества.',
  array['мультфильм', 'фантастика', 'фэнтези', 'боевик', 'комедия', 'криминал', 'приключения', 'семейный']::text[],
  array['США', 'Австралия']::text[],
  null,
  405608,
  'tt1001526',
  7.6,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:06:17.000Z',
  false,
  false,
  false
),
(
  'media-восстание-планеты-обезьян-378',
  'Восстание планеты обезьян',
  'Rise of the Planet of the Apes',
  'movie',
  2011,
  './posters/восстание-планеты-обезьян-77b4b3d5.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017c0733f40dc216476ba954844564/orig',
  'Молодой ученый испытывает на обезьянах новое лекарство от болезни Альцгеймера. У препарата обнаруживается удивительный побочный эффект: он повышает в несколько раз интеллект приматов.',
  array['фантастика', 'боевик', 'триллер', 'драма']::text[],
  array['США']::text[],
  61791,
  463738,
  'tt1318514',
  7.4,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:06:18.000Z',
  false,
  false,
  false
),
(
  'media-алиса-в-зазеркалье-379',
  'Алиса в Зазеркалье',
  'Alice Through the Looking Glass',
  'movie',
  2016,
  './posters/алиса-в-зазеркалье-9f223aa0.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c0788fa5ee4e06893fd8bd6a8d3/orig',
  'На этот раз главной героине предстоит отправиться в путешествие во времени, полное неожиданностей и ярких открытий, чтобы спасти своего друга, Безумного Шляпника.',
  array['фэнтези', 'детектив', 'приключения', 'семейный']::text[],
  array['США', 'Таиланд', 'Великобритания', 'Индия', 'Канада']::text[],
  null,
  723988,
  'tt2567026',
  6.7,
  6.2,
  null,
  null,
  null,
  '2026-06-21T12:06:19.000Z',
  false,
  false,
  false
),
(
  'media-веном-последний-танец-380',
  'Веном: Последний танец',
  'Venom: The Last Dance',
  'movie',
  2024,
  './posters/веном-последний-танец-d0bbe827.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a00000193f9ad504008ecfed70ea115ca2d/orig',
  'Приспособившись к совместному существованию, Эдди и Веном стали друзьями и вместе сражаются со злодеями. Но теперь за Эдди охотятся военные, а за Веномом — его инопланетные сородичи, угрожающие всему живому.',
  array['фантастика', 'боевик', 'ужасы']::text[],
  array['США', 'Великобритания', 'Канада']::text[],
  null,
  4766559,
  null,
  6.6,
  6,
  null,
  null,
  null,
  '2026-06-21T12:06:20.000Z',
  false,
  false,
  false
),
(
  'media-приключения-кота-леопольда-381',
  'Приключения кота Леопольда',
  'Приключения кота Леопольда',
  'cartoon',
  1975,
  './posters/приключения-кота-леопольда-17e1919b.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a0000017c076ef5de998451d6d0f89af46d/orig',
  'Доброму коту Леопольду, постоянно досаждают две мышки, требующие взаимности...',
  array['мультфильм', 'детский']::text[],
  array['СССР']::text[],
  null,
  325788,
  'tt1074757',
  7.8,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:06:21.000Z',
  false,
  false,
  false
),
(
  'media-тор-любовь-и-гром-382',
  'Тор: Любовь и гром',
  'Thor: Love and Thunder',
  'movie',
  2022,
  './posters/тор-любовь-и-гром-9a9a4864.jpg',
  './posters/тор-любовь-и-гром-9a9a4864.jpg',
  'Джейн Фостер берет на себя обязанности Бога-громовержца и становится обладательницей молота Мьёльнира.',
  array['фантастика', 'фэнтези', 'боевик', 'мелодрама', 'комедия', 'приключения']::text[],
  array['Австралия', 'США']::text[],
  616037,
  1282688,
  'tt10648342',
  6,
  6.1,
  null,
  null,
  null,
  '2026-06-21T12:06:22.000Z',
  false,
  false,
  false
),
(
  'media-хроники-риддика-383',
  'Хроники Риддика',
  'The Chronicles of Riddick',
  'movie',
  2004,
  './posters/хроники-риддика-76b31338.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017c074cc1d8c1b42fbbefdd875f45/orig',
  'Последние пять лет Риддик провел в бегах среди забытых миров на задворках галактики, прячась от наемников, назначивших цену за его голову. Теперь беглец оказался на планете Гелион, где живет прогрессивное многонациональное общество, завоеванное лордом Маршалом, фанатиком, решившим поработить человечество армадой своих воинов, некромонгеров. 

Совершив побег из подземной тюрьмы, где температура колебалась от арктических ночей до вулканических дней, Риддик встречает Кайру, последнюю оставшуюся в живых женщину из раннего периода его жизни. Его попытки освободить себя и Кайру приводят их на борт главного корабля некромонгеров, где ему предстоит столкнуться с лордом Маршалом в апокалиптической битве не на жизнь, а на смерть.',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array['США']::text[],
  null,
  5900,
  'tt0296572',
  7.4,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:06:23.000Z',
  false,
  false,
  false
),
(
  'media-трон-384',
  'Трон',
  'Tron',
  'movie',
  1982,
  './posters/трон-ee7fb0a1.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c078dc5d20c53615e4a15dfd5f0/orig',
  'Вам никогда не приходила в голову мысль попасть внутрь компьютерной игры и стать ее участником? До определенного момента Кевин Флинн тоже думал, что это возможно лишь в его воображении.

Но, оказавшись в секретной лаборатории, разрабатывающей компьютерные программы, ему предоставляется возможность увидеть электронный мир изнутри и сразиться в жестокой схватке с самим хозяином виртуальной вселенной.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Тайвань', 'Япония', 'Великобритания']::text[],
  97,
  17463,
  'tt0084827',
  6.9,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:06:24.000Z',
  false,
  false,
  false
),
(
  'media-трон-наследие-385',
  'Трон: Наследие',
  'Tron: Legacy',
  'movie',
  2010,
  './posters/трон-наследие-aa1a029e.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017c077accad23c6041027b72b95bb/orig',
  '1989 год. Кевин Флинн, глава корпорации ENCOM и создатель знаменитой игры Tron для аркадных автоматов, внезапно пропадает без вести, а его сын Сэм остаётся на попечении бабушки с дедушкой. Спустя десятилетия повзрослевший Сэм, который ведёт бунтарский образ жизни, находит тайный рабочий кабинет отца и случайно попадает в созданную им цифровую реальность.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Индия', 'Великобритания', 'Мексика', 'Япония', 'Канада']::text[],
  null,
  399959,
  'tt1104001',
  6.9,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:06:25.000Z',
  false,
  false,
  false
),
(
  'media-история-игрушек-2-386',
  'История игрушек 2',
  'Toy Story 2',
  'movie',
  1999,
  './posters/история-игрушек-2-9fb729f6.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c072df7934cf9e12e3558fc86b5/orig',
  'Ковбой Вуди и другие игрушки мальчика по имени Энди продолжают жить, радуясь каждому дню. Но неожиданно все меняется, когда Вуди похищает коварный коллекционер. Оказывается, что Вуди - очень ценная игрушка из коллекции, за которую японский музей игрушек готов заплатить огромную сумму. В доме коллекционера он находит своих родственников. В это время игрушки во главе с Баззом Лайтером отправляются спасать Вуди.
Вуди узнает о своем знаменитом прошлом, Базз и другие игрушки проходят опасный и тернистый путь. Перед Вуди встает непростой вопрос - каково же его предназначение? Блистать в витрине музея или же вернуться к любимому хозяину?',
  array['мультфильм', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  null,
  405,
  'tt0120363',
  7.8,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:06:26.000Z',
  false,
  false,
  false
),
(
  'media-мадагаскар-2-387',
  'Мадагаскар 2',
  'Madagascar: Escape 2 Africa',
  'cartoon',
  2008,
  './posters/мадагаскар-2-ee605728.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017c0777f318a795c399b38f0f3666/orig',
  'Полная опасностей жизнь на Мадагаскаре не оправдала ожиданий привыкших к комфорту Нью-Йоркского зоопарка животных - льва Алекса, зебры Марти, жирафа Мелмана и бегемотихи Глории - и друзья решают сбежать с острова. По случайности, всю компанию заносит в Африку, где Алекс встречает свою семью, с которой он был разлучен в детстве. Неожиданно для себя Алекс понимает, что после жизни в зоопарке у него гораздо меньше общего с родственниками, чем ему хотелось бы...',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Великобритания']::text[],
  10527,
  161066,
  'tt0479952',
  7.6,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:06:27.000Z',
  false,
  false,
  false
),
(
  'media-кунг-фу-панда-3-388',
  'Кунг-фу Панда 3',
  'Kung Fu Panda 3',
  'movie',
  2016,
  './posters/кунг-фу-панда-3-b0708f27.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c07348eedaccc599bd485c46522/orig',
  'Воссоединившись со своим давно потерянным отцом, По отправляется в тайный рай для панд, где его ожидает встреча с множеством веселых сородичей. Однако вскоре злодей Кай начинает зачистку - он уничтожает кунг-фу мастеров по всему Китаю.
По предстоит сделать невозможное - обучить боевому искусству деревню, полную его веселых, неуклюжих собратьев, и тем самым превратить их в отряд кунг-фу панд.',
  array['мультфильм', 'фэнтези', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Китай']::text[],
  null,
  692865,
  'tt2267968',
  7.3,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:06:28.000Z',
  false,
  false,
  false
),
(
  'media-тупой-и-еще-тупее-389',
  'Тупой и еще тупее',
  'Dumb and Dumber',
  'movie',
  1994,
  './posters/тупой-и-еще-тупее-405542bb.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017c074091d060271e2250f3a8ae5e/orig',
  'Два полных придурка Ллойд Крисмас и Харри Данн пытаются нагнать красивую девушку Мэри Суонсон, чтобы вручить ей оставленный ею в аэропорту чемоданчик. Им невдомек, что Мэри оставила его специально, в качестве выкупа по договоренности с похитителями ее мужа. На переделанной в собаку машине двое недотёп пересекают Америку от Род-Айленда до Колорадо, где и находят обворожительную Мэри, в которую Ллойд уже успел втюриться без памяти.',
  array['комедия']::text[],
  array['США']::text[],
  null,
  5185,
  'tt0109686',
  7.2,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:06:29.000Z',
  false,
  false,
  false
),
(
  'media-диктатор-390',
  'Диктатор',
  'The Dictator',
  'movie',
  2012,
  './posters/диктатор-535c0d3f.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c078b229bf31016718163520575/orig',
  'Героическая история диктатора, который рискует своей жизнью, чтобы ни в коем случае не допустить демократию в свою столь любовно угнетаемую страну.',
  array['комедия']::text[],
  array['США']::text[],
  null,
  518040,
  'tt1645170',
  6.5,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:06:30.000Z',
  false,
  false,
  false
),
(
  'media-рио-391',
  'Рио',
  'Rio',
  'cartoon',
  2011,
  './posters/рио-6661acef.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c07680b8156000714e9cb5ba5b0/orig',
  'Попугай-зануда оставляет свою уютную сытую жизнь в клетке и из Миннесоты отправляется в Рио-де-Жанейро, город грез всех и каждого. Неминуемые встречи, препятствия, опасности и любовь к последней представительнице его рода заставляют нашего героя по-другому посмотреть на мир и из скучного домоседа превратиться в милого симпатягу, узнавшего цену дружбе и смелости.',
  array['мультфильм', 'мюзикл', 'мелодрама', 'комедия', 'приключения', 'семейный', 'музыка']::text[],
  array['США', 'Великобритания']::text[],
  46195,
  465674,
  'tt1436562',
  7.5,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:06:31.000Z',
  false,
  false,
  false
),
(
  'media-ранго-392',
  'Ранго',
  'Rango',
  'cartoon',
  2011,
  './posters/ранго-2693d6b9.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a00000196683add991f952f61fbcfbd329e/orig',
  'Хамелеон Ранго живёт в террариуме и считает себя героем, которому никак не удаётся проявить себя. Но когда он внезапно оказывается в городке Грязь, у него появляется такая возможность. Ранго провозглашает себя борцом за справедливость и начинает вести себя как шериф на Диком Западе. Он ещё не знает, что быть хорошим парнем в этих краях — не самая завидная участь.',
  array['мультфильм', 'боевик', 'комедия', 'приключения', 'семейный', 'вестерн']::text[],
  array['США', 'Великобритания']::text[],
  44896,
  426053,
  'tt1192628',
  7.2,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:06:32.000Z',
  false,
  false,
  false
),
(
  'media-шаг-вперед-393',
  'Шаг вперед',
  'Step Up',
  'movie',
  2006,
  './posters/шаг-вперед-4f760be2.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c07437c643fa7a373d9c0ce7784/orig',
  'Тайлер Гейдж - уличный танцор. Безумный и рискованный стиль жизни заставляет его конфликтовать с обществом. После очередного инцидента, Тайлер попадает в школу, в которой будет вынужден отрабатывать наказание в виде исправительных работ. Там он знакомится с девушкой, которая открывает ему глаза на его талант и окружающий его мир…',
  array['драма', 'мелодрама', 'криминал', 'музыка']::text[],
  array['США']::text[],
  null,
  167491,
  'tt0462590',
  7.6,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:06:33.000Z',
  false,
  false,
  false
),
(
  'media-призрачный-гонщик-394',
  'Призрачный гонщик',
  'Ghost Rider',
  'movie',
  2007,
  './posters/призрачный-гонщик-0010a202.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c28026810099519fd2bf10d098f/orig',
  'Чтобы спасти своего отца от верной гибели, байкер-экстремал Джонни Блэйз заключает сделку с дьяволом, объектом которой является его бессмертная душа. Проходят годы, и дьявол предъявляет свои права по контракту. Он превращает Джонни в Призрачного Гонщика, агента потусторонних сил, обладающего сверхчеловеческими способностями.',
  array['фэнтези', 'боевик', 'триллер']::text[],
  array['США', 'Австралия']::text[],
  1250,
  3948,
  'tt0259324',
  6.3,
  5.3,
  null,
  null,
  null,
  '2026-06-21T12:06:34.000Z',
  false,
  false,
  false
),
(
  'media-лило-и-стич-395',
  'Лило и Стич',
  'Lilo & Stitch',
  'cartoon',
  2025,
  './posters/лило-и-стич-2bbcbc50.jpg',
  './posters/лило-и-стич-2bbcbc50.jpg',
  'Забота о шестилетней Лило Пелекаи легла на плечи её старшей сестры Нани. Нани приходится много работать, и поэтому девочка часто остаётся одна. Однажды соседка советует взять в приюте собаку, чтобы помочь девочке справиться с одиночеством. Однако из приюта Лило забирает милого инопланетянина вместо собаки. Девочка даёт новому питомцу кличку Стич, но на родной планете его прозвали Эксперимент 626. Он — беглец от галактической полиции и продукт генетических экспериментов, который отличается склонностью к разрушениям, способностью к быстрому обучению и почти полной неуязвимостью.',
  array['фантастика', 'комедия', 'семейный']::text[],
  array['США', 'Австралия', 'Канада']::text[],
  552524,
  1348367,
  'tt11655566',
  7.4,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:06:35.000Z',
  false,
  false,
  false
),
(
  'media-нечто-396',
  'Нечто',
  'The Thing',
  'movie',
  1982,
  './posters/нечто-0ca2a34d.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a00000181aaef279c6f938928bd12084aa1/orig',
  'Команде ученых американской исследовательской станции в Антарктике предстоит столкнуться с необъяснимым кошмаром. Отрезанные от остального мира полярники вступают в схватку с инопланетной тварью, способной принимать обличье земных существ.',
  array['ужасы', 'фантастика']::text[],
  array['США', 'Канада']::text[],
  null,
  8366,
  'tt0084787',
  7.9,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:06:36.000Z',
  false,
  false,
  false
),
(
  'media-призрак-в-доспехах-397',
  'Призрак в доспехах',
  'Koukaku Kidoutai',
  'anime',
  1995,
  './posters/призрак-в-доспехах-f011917e.jpg',
  'https://avatars.mds.yandex.net/get-ott/13074011/2a00000198c2b6a27c89e84fe7d7f2d92736/orig',
  '2029 год. Благодаря значительным достижениям в кибернетике люди могут заменять свои конечности роботизированными частями. Майор Мотоко Кусанаги из 9-го отдела службы безопасности ведёт расследование, связанное с таинственным хакером, который называет себя Кукловод.',
  array['аниме', 'мультфильм', 'фантастика', 'боевик', 'детектив', 'криминал']::text[],
  array['Япония']::text[],
  9323,
  8228,
  'tt0113568',
  8.1,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:06:37.000Z',
  false,
  false,
  false
),
(
  'media-гадкий-я-3-398',
  'Гадкий я 3',
  'Despicable Me 3',
  'cartoon',
  2017,
  './posters/гадкий-я-3-d1cd688d.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c2869e130a2ca929cb711ca5d0f/orig',
  'Грядет третье пришествие. Многодетный суперагент, роковая красотка и бессмертные миньоны на подходе. Готовьтесь. Они будут править миром.',
  array['мультфильм', 'фантастика', 'комедия', 'криминал', 'приключения', 'семейный']::text[],
  array['США', 'Франция', 'Япония']::text[],
  324852,
  820599,
  'tt3469046',
  6.3,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:06:38.000Z',
  false,
  false,
  false
),
(
  'media-rrr-рядом-рев-т-революция-399',
  'RRR: Рядом ревёт революция',
  'RRR',
  'movie',
  2022,
  './posters/rrr-рядом-ревет-революция-8549578b.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000018227b24b507e2f540920bcf48771/orig',
  'Британская Индия. Рама — преданный полицейский на службе у колониального правительства, способный расправиться с толпой в одиночку, и местные считают его предателем. Бхим — парень из племени гондов, который вынужден отправиться в Дели на поиски похищенной губернатором маленькой соплеменницы. Узнав об этом, губернатор объявляет награду за его поимку, и, чтобы продвинуться по службе, Рама берётся за это задание. Спасая упавшего в реку мальчика, Рама и Бхим становятся друзьями. Они не знают ни настоящих личностей, ни скрытых мотивов друг друга, но это знакомство положит начало большим переменам.',
  array['боевик', 'драма', 'история']::text[],
  array['Индия']::text[],
  null,
  1115117,
  'tt8178634',
  8,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:06:39.000Z',
  false,
  false,
  false
),
(
  'media-треугольник-400',
  'Треугольник',
  'Triangle',
  'movie',
  2009,
  './posters/треугольник-c9287404.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c0738b0ad471698f73036b1174f/orig',
  'Грег приглашает подругу Джесс, молодую мать-одиночку, провести день на яхте в кругу своих друзей. Начавшийся сильный шторм переворачивает судно, и оставшиеся в живых вынуждены дожидаться помощи, цепляясь за перевернутый корпус яхты. Спасение приходит в виде огромного океанского лайнера, но судно оказывается пустым, а часы на борту остановились. Однако вскоре становится ясно: молодые люди не одни на этом огромном корабле, за ними кто-то неустанно следит.',
  array['фантастика', 'триллер']::text[],
  array['Великобритания', 'Австралия', 'США']::text[],
  null,
  406403,
  'tt1187064',
  6.8,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:06:40.000Z',
  false,
  false,
  false
),
(
  'media-доктор-кто-401',
  'Доктор Кто',
  'Doctor Who',
  'series',
  2005,
  './posters/доктор-кто-9d0abab8.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017cd7c4b48779a5ab388c07aa6f4a/orig',
  'Инопланетянин-гуманоид, называющий себя Доктором, путешествует через время и пространство в космическом корабле, который снаружи выглядит как полицейская будка 50-х годов. Доктор чрезвычайно эксцентричен и имеет невероятные знания в областях технологии, истории и науки. Он путешествует вместе со своими компаньонами, попутно борясь против космических злодеев и спасая Землю от бесчисленных врагов.',
  array['фантастика', 'драма', 'комедия', 'приключения', 'семейный']::text[],
  array['Великобритания']::text[],
  null,
  252089,
  'tt0436992',
  8.6,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:06:41.000Z',
  false,
  false,
  false
),
(
  'media-ведьмина-служба-доставки-402',
  'Ведьмина служба доставки',
  'Majo no takkyûbin',
  'anime',
  1989,
  './posters/ведьмина-служба-доставки-eea449d4.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c073a13dd757344e205ce694577/orig',
  'Молодая ведьма Кики по достижении 13 лет должна прожить среди людей определённое время. Вместе с котом Дзидзи она отправляется в город, где знакомится с добрым пекарем, который помогает ей начать собственное дело - экстренную службу доставки. Новая работа знакомит Кики со множеством различных людей и предоставляет возможность обрести новых друзей и совершить массу всевозможных проделок.',
  array['аниме', 'мультфильм', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['Япония']::text[],
  null,
  2428,
  'tt0097814',
  8.1,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:06:42.000Z',
  false,
  false,
  false
),
(
  'media-артур-и-минипуты-403',
  'Артур и минипуты',
  'Arthur et les Minimoys',
  'movie',
  2006,
  './posters/артур-и-минипуты-04a2e20b.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017c07495af1f8af349ddfffa746f3/orig',
  'На долю десятилетнего Артура выпало непростое испытание: дом его бабушки хотят отнять за долги, и никто не поможет ему справиться с этой напастью. Быть может, ключ к решению проблемы кроется в сокровищах его дедушки, которые спрятаны где-то в стране минипутов, сказочных крохотных существ.

В стране минипутов Артур знакомится с принцессой Селенией и ее братом Барахлюшем. Их ждет множество захватывающих приключений, опасностей и интриг на пути, который ведет их в запретный город Некрополис. Там детской компании предстоит пережить недетский ужас в лице Ужасного Урдалака…',
  array['мультфильм', 'фэнтези', 'приключения', 'семейный']::text[],
  array['Франция', 'США', 'Великобритания']::text[],
  9992,
  14328,
  'tt0344854',
  6.6,
  6,
  null,
  null,
  null,
  '2026-06-21T12:06:43.000Z',
  false,
  false,
  false
),
(
  'media-великолепная-семерка-404',
  'Великолепная семерка',
  'The Magnificent Seven',
  'movie',
  2016,
  './posters/великолепная-семерка-899319db.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017c075694c0506caf6ac7bafda19a/orig',
  'Семеро отважных искателей приключений встают на защиту жителей маленького городка, угнетаемого бандой грабителей.',
  array['вестерн', 'боевик', 'приключения']::text[],
  array['США', 'Австралия']::text[],
  null,
  682648,
  'tt2404435',
  6.8,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:06:44.000Z',
  false,
  false,
  false
),
(
  'media-машинист-405',
  'Машинист',
  'The Machinist',
  'movie',
  2003,
  './posters/машинист-82fe373d.jpg',
  './posters/машинист-82fe373d.jpg',
  'Тревор Резник не спит уже целый год. Превратившись в живой скелет, он балансирует на грани сна и реальности. Он устал отличать пугающие видения от странных событий своей каждодневной жизни, но в последнее время они стали пересекаться самым страшным и непредсказуемым образом.',
  array['триллер', 'драма']::text[],
  array['Испания', 'США']::text[],
  null,
  5206,
  null,
  7.6,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:06:45.000Z',
  false,
  false,
  false
),
(
  'media-дети-шпионов-406',
  'Дети шпионов',
  'Spy Kids',
  'movie',
  2001,
  './posters/дети-шпионов-fc0c731e.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a000001966c72cad4c868f53cdb005ea7bc/orig',
  'Два самых крутых шпиона в мире - Грегорио Кортес и Ингрид - имели одно и то же секретное задание: уничтожить друг друга. Но личная встреча не принесла им ничего хорошего: они поженились и завязали со своим ремеслом... Через несколько лет они вынуждены снова вернуться в игру. Причиной послужило то, что их бывшие коллеги, самые крутые профессионалы, начали исчезать один за другим. Однако вскоре Кортесов похищает коварный злодей, и теперь судьба пленников зависит от того, смогут ли их дети правильно распорядиться новейшими сверхсекретными шпионскими штучками из арсенала легендарных родителей.',
  array['фантастика', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  null,
  841,
  'tt0227538',
  6.6,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:06:46.000Z',
  false,
  false,
  false
),
(
  'media-магическая-битва-407',
  'Магическая битва',
  'Jujutsu Kaisen',
  'anime',
  2020,
  './posters/магическая-битва-443eda97.jpg',
  './posters/магическая-битва-443eda97.jpg',
  'Мир, в котором демоны питаются людьми, а те об этом даже не догадываются. Когда-то давно самый могущественный демон был повержен, а части его тела разбросаны по свету. Тот, кто сможет их собрать и поглотить, получит безграничную власть и даже сможет уничтожить человечество. 
Физически развитого старшеклассника Юдзи Итадори волнуют насущные проблемы — почти всё время парень проводит в больнице с дедушкой, поэтому, чтобы отвязаться от настырных предложений вступить в спортивные клубы, записывается в оккультный. И внезапно оказывается в эпицентре борьбы за людские судьбы, когда его приятели снимают заклятье с некоего магического артефакта.',
  array['аниме', 'мультфильм', 'боевик', 'ужасы', 'фэнтези']::text[],
  array['Япония']::text[],
  95479,
  1381125,
  'tt12343534',
  8.2,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:06:47.000Z',
  false,
  false,
  false
),
(
  'media-куб-408',
  'Куб',
  'Cube',
  'movie',
  1997,
  './posters/куб-60a46bd2.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c0754b3eb08829a3fb1d37b2725/orig',
  'Группа людей, которые никак друг с другом не связаны, неизвестным образом внезапно оказываются в странной кубической комнате, каждая грань которой снабжена люком, ведущим в другую такую же комнату. Единственная разница между комнатами – это их цвет и наличие разнообразных смертоносных ловушек. Совместными усилиями эти пятеро пытаются найти ключ к выходу из этого гигантского сооружения.',
  array['фантастика', 'триллер', 'ужасы']::text[],
  array['Канада']::text[],
  null,
  20590,
  'tt0123755',
  7.2,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:06:48.000Z',
  false,
  false,
  false
),
(
  'media-аннигиляция-409',
  'Аннигиляция',
  'Annihilation',
  'movie',
  2017,
  './posters/аннигиляция-665461d7.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017c07935ad83dd076a681f2e1a7e4/orig',
  'На Землю прилетает маленький метеорит и становится причиной больших перемен. Он врезается в одиноко стоящий маяк, что загадочным образом меняет территорию вокруг. 

Профессор биологии Лина уже год страдает от потери мужа-военного. Кейн отправился на очередное секретное задание, но так и не вернулся — жив ли, погиб — женщина не знает, пока однажды дезориентированный и слегка бледный супруг внезапно не появляется на пороге их спальни. Радость длится недолго — у мужчины начинаются судороги и множественные кровотечения, а по дороге в больницу машину скорой помощи тормозят военные. Лина приходит в себя на секретной базе рядом с той самой загадочной зоной, куда уже отправлялись многочисленные экспедиции, но вернулся, оказывается, только один человек — её Кейн.',
  array['ужасы', 'фантастика', 'триллер', 'драма']::text[],
  array['США', 'Великобритания']::text[],
  300668,
  749370,
  'tt2798920',
  6.7,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:06:49.000Z',
  false,
  false,
  false
),
(
  'media-зеленый-фонарь-410',
  'Зеленый Фонарь',
  'Green Lantern',
  'movie',
  2011,
  './posters/зеленый-фонарь-be4b08a3.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000018616e6da862e0695b480d603dfca/orig',
  'Смелые защитники межгалактического закона, Корпус Зеленых фонарей, сумели обуздать злобного Параллакса, наводящего ужас и питающегося чужими страхами, но не надолго. Он снова набирает силу и направляется на Землю. Между тем впервые за всю историю в отряде Зеленых Фонарей появляется человек - летчик-испытатель Хэл Джордан. Чтобы победить Параллакса и спасти возлюбленную Кэрол Феррис, ему придется научиться управлять мощью специального кольца, дарующего сверхспособности, и преодолеть свои страхи.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США']::text[],
  null,
  406294,
  'tt1133985',
  5.9,
  5.5,
  null,
  null,
  null,
  '2026-06-21T12:06:50.000Z',
  false,
  false,
  false
),
(
  'media-робокоп-411',
  'Робокоп',
  'RoboCop',
  'movie',
  1987,
  './posters/робокоп-15ddf900.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017e7c8549d113c95da95d17d9b8b5/orig',
  'После гибели одного из лучших полицейских врачи-экспериментаторы создают из него неуязвимого киборга Робокопа, который в одиночку борется с бандой преступников. Однако прочная броня не спасает Робокопа от мучительных, обрывочных воспоминаний о прошлом: он постоянно видит кошмарные сны, в которых погибает от рук жестоких преступников. Теперь он не только ждёт правосудия,… но и жаждет мести!',
  array['фантастика', 'боевик', 'триллер', 'криминал']::text[],
  array['США']::text[],
  null,
  7410,
  'tt0093870',
  7.7,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:06:51.000Z',
  false,
  false,
  false
),
(
  'media-тролли-412',
  'Тролли',
  'Trolls',
  'cartoon',
  2016,
  './posters/тролли-fe6a44e3.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017c285ed59f9836d5a946d188433b/orig',
  'Отправляясь в увлекательное приключение, которое заведет ее далеко от привычного мира, Розочка объединяет усилия с самым мрачным троллем на свете по имени Цветан, чтобы спасти друзей из лап зловещих бергенов и их лидера, короля Хряща Старшего. Невероятные приключения Розочки проверят её силу, покажут истинную суть и бросят вызов способности ударяться в пение в любой момент.',
  array['мультфильм', 'мюзикл', 'фэнтези', 'мелодрама', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Индия', 'Великобритания', 'Германия']::text[],
  136799,
  689774,
  'tt1679335',
  6.8,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:06:52.000Z',
  false,
  false,
  false
),
(
  'media-гостья-из-будущего-413',
  'Гостья из будущего',
  'Гостья из будущего',
  'cartoon',
  1984,
  './posters/гостья-из-будущего-823271a0.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000018d7864ae73efdda8e0d5c8a9e0ae/orig',
  'Борьба с космическими пиратами, захватывающее путешествие во времени, знакомство с девочкой из будущего Алисой Селезневой — вот к чему привел обычный поход за кефиром мальчика Колю из 6-го класса.',
  array['фантастика', 'семейный', 'приключения']::text[],
  array['СССР']::text[],
  5904,
  77185,
  'tt0086723',
  8.3,
  8,
  null,
  null,
  null,
  '2026-06-21T12:06:53.000Z',
  false,
  false,
  false
),
(
  'media-angry-birds-в-кино-414',
  'Angry Birds в кино',
  'The Angry Birds Movie',
  'cartoon',
  2016,
  './posters/angry-birds-в-кино-b163bfc8.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c2857e670ef9cf254ebdc4bfe66/orig',
  'На тропическом острове, где живут нелетающие птицы, царит атмосфера радости и счастья. Красная птица Ред с самого детства чувствовал себя изгоем, поэтому ничего удивительного, что теперь он не вписывается в окружающую действительность и не разделяет всеобщего оптимизма. Однажды на райский остров прибывают зелёные свиньи, и дружелюбные птицы устраивают им радушный приём, но кроме Реда никто не замечает, что свиньи задумали подлость.',
  array['мультфильм', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Финляндия', 'Канада']::text[],
  null,
  609618,
  'tt1985949',
  6.3,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:06:54.000Z',
  false,
  false,
  false
),
(
  'media-микки-17-415',
  'Микки 17',
  'Mickey 17',
  'movie',
  2025,
  './posters/микки-17-d3e6aab2.jpg',
  './posters/микки-17-d3e6aab2.jpg',
  '2050-е. Спасаясь от кровожадного кредитора, который расчленяет своих должников, Микки Барнс вместе с приятелем записывается в колонисты планеты Нифльхейм. Вот только друг становится пилотом, а сам Микки вынужден стать расходным сотрудником — выполнять смертельные задания, умирать и снова оживать в свеженапечатанном теле, в которое загружается его сознание. Упавшего в пещеру Микки 17 спасают обитающие на Нифльхейме ползуны, а вернувшись на базу, парень встречает свою 18-ю версию.',
  array['фантастика', 'комедия', 'приключения']::text[],
  array['США']::text[],
  696506,
  1320476,
  'tt12299608',
  6.7,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:06:55.000Z',
  false,
  false,
  false
),
(
  'media-пингвины-мадагаскара-416',
  'Пингвины Мадагаскара',
  'Penguins of Madagascar',
  'cartoon',
  2014,
  './posters/пингвины-мадагаскара-f41b47f5.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000188336fa38f721bc1695c67ab4bfa/orig',
  'Четвёрка пингвинов-шпионов - Шкипер, Ковальски, Рико и Прапор - объединяется с командой «Северный ветер», помогающей беззащитным животным. Им предстоит остановить злобного осьминога, доктора Октавиуса Брайна, желающего уничтожить этот мир...',
  array['мультфильм', 'фантастика', 'боевик', 'комедия', 'криминал', 'приключения', 'семейный']::text[],
  array['США', 'Великобритания']::text[],
  270946,
  607608,
  'tt1911658',
  6.7,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:06:56.000Z',
  false,
  false,
  false
),
(
  'media-13-причин-почему-417',
  '13 причин почему',
  '13 Reasons Why',
  'series',
  2017,
  './posters/13-причин-почему-b5996585.jpg',
  './posters/13-причин-почему-b5996585.jpg',
  'Однажды Клэй Дженсен находит на пороге своего дома коробку с аудиокассетами, записанными Ханной Бейкер. Он был влюблен в эту девушку в школе, пока она однажды не покончила жизнь самоубийством. В своих записях Ханна указала 13 причин, которые толкнули её на это. И Клэй — одна из них.',
  array['триллер', 'драма', 'детектив']::text[],
  array['США']::text[],
  66788,
  582358,
  'tt1837492',
  7.3,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:06:57.000Z',
  false,
  false,
  false
),
(
  'media-игла-418',
  'Игла',
  'Игла',
  'movie',
  1988,
  './posters/игла-ad81450d.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a00000189f32873e3f22045eb4408fa4f66/orig',
  'Вернувшись в Алма-Ату, Моро не хочет, чтобы родители знали о его приезде, и в поисках временного пристанища вспоминает о девушке, которую он когда-то любил. Дина рада встрече, но Моро что-то настораживает в ней. Наблюдая за её странными знакомыми, Моро понимает, что Дина стала наркоманкой. Как спасти девушку? Моро увозит её к морю, где Дина начинает выздоравливать. Однако после возвращения в город всё начинается сначала. Моро решает расправиться с поставщиками наркотиков.',
  array['триллер', 'драма']::text[],
  array['СССР']::text[],
  20847,
  42576,
  'tt0097561',
  7.3,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:06:58.000Z',
  false,
  false,
  false
),
(
  'media-хранитель-времени-419',
  'Хранитель времени',
  'Hugo',
  'movie',
  2011,
  './posters/хранитель-времени-941153f7.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a000001966c4ca455e6da53b2c7763d78b4/orig',
  'После того, как ловкому и изобретательному Хьюго удается раскрыть секрет таинственного механизма, оставленного его погибшим отцом, он отправляется в путешествие, которое изменит тех, кто его окружает, и приведет в безопасное и прекрасное место, которое он назовет своим домом.',
  array['драма', 'детектив', 'приключения', 'семейный']::text[],
  array['США', 'Великобритания']::text[],
  44826,
  462247,
  'tt0970179',
  6.9,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:06:59.000Z',
  false,
  false,
  false
),
(
  'media-халк-420',
  'Халк',
  'Hulk',
  'movie',
  2003,
  './posters/халк-fe9e764b.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a0000017f0c9d1d2fa4d32ee42ec809937a/orig',
  'Доктор Брюс Бэннер (ученый, работающий над изобретением новой бомбы) подвергся воздействию гамма-лучей и превратился в Халка - существо невероятной физической силы, которое в состоянии ярости становится огромным зеленым монстром.

Его преследуют военные под предводительством генерала Росса, ему приходится бежать через всю страну. В конце концов, у него возникает любовная история с генеральской дочкой Бетти...',
  array['фантастика', 'боевик']::text[],
  array['США']::text[],
  null,
  7962,
  'tt0286716',
  6.2,
  5.7,
  null,
  null,
  null,
  '2026-06-21T12:07:00.000Z',
  false,
  false,
  false
),
(
  'media-el-camino-во-все-тяжкие-421',
  'El Camino: Во все тяжкие',
  'El Camino',
  'movie',
  2019,
  './posters/el-camino-во-все-тяжкие-7e7168f6.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a00000175654a51c6406b8e874359856b7d/orig',
  'Джесси Пинкман сбежал от неонацистов. Не зная, куда ему податься, он скрывается от полиции, похитителей и прошлого. Теперь он должен понять, как ему жить дальше.',
  array['драма', 'криминал']::text[],
  array['США']::text[],
  559969,
  1209193,
  'tt9243946',
  7.2,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:07:01.000Z',
  false,
  false,
  false
),
(
  'media-каспер-422',
  'Каспер',
  'Casper',
  'movie',
  1995,
  './posters/каспер-e19cf842.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000017c078254aeb57329309d7d808d57/orig',
  'Каспер, маленькое дружелюбное привидение, не желает походить на своих хулиганствующих собратьев. Его дружба с девочкой-подростком Кэт помогает ей и ее отцу, доктору Харви, справиться с опасностями, поджидающими их в населенном призраками доме.',
  array['фэнтези', 'комедия', 'семейный']::text[],
  array['США']::text[],
  null,
  7098,
  'tt0112642',
  7.2,
  6.2,
  null,
  null,
  null,
  '2026-06-21T12:07:02.000Z',
  false,
  false,
  false
),
(
  'media-чебурашка-423',
  'Чебурашка. Секрет праздника',
  'Чебурашка. Секрет праздника',
  'cartoon',
  2020,
  './posters/чебурашка-секрет-праздника-7f861dab.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000018dcfe129b06bd30c44e39894351a/orig',
  'Празднование Нового года для Чебурашки и Гены оказывается под угрозой, когда бесследно исчезает главный атрибут праздника — новогодняя ёлка.',
  array['короткометражка', 'мультфильм', 'детский']::text[],
  array['Россия']::text[],
  783416,
  4297471,
  'tt13890884',
  8.1,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:07:03.000Z',
  false,
  false,
  false
),
(
  'media-пипец-2-424',
  'Пипец 2',
  'Kick-Ass 2',
  'movie',
  2013,
  './posters/пипец-2-cddaac21.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c07358577723986275bd57fc402/orig',
  'В реальной жизни не встретишь супергероя из комиксов, зато в ней есть настоящие герои. Харизматичный Пипец и гламурная Убивашка обрели новую команду поддержки во главе с Полковником Америка. Но кроме обычных мафиози, над городом нависает новая угроза – молодой мажор с очень дурным характером и целой армией отмороженных наемников.',
  array['боевик', 'триллер', 'драма', 'комедия', 'криминал']::text[],
  array['Великобритания', 'США', 'Япония']::text[],
  59859,
  520648,
  'tt1650554',
  6.2,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:07:04.000Z',
  false,
  false,
  false
),
(
  'media-поездка-в-америку-425',
  'Поездка в Америку',
  'Coming to America',
  'movie',
  1988,
  './posters/поездка-в-америку-57707660.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a000001965d14774d2da5ac6db43e5dc2e9/orig',
  'Наследный принц африканского государства Замунда хочет сам найти себе жену и едет для этого в Америку. Там он представляется обычным человеком и устраивается на работу в закусочную, так как его выбор пал на дочь хозяина.',
  array['мелодрама', 'комедия']::text[],
  array['США']::text[],
  9602,
  5281,
  'tt0094898',
  7.7,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:07:05.000Z',
  false,
  false,
  false
),
(
  'media-лего-фильм-426',
  'ЛЕГО Фильм',
  'The Lego Movie',
  'cartoon',
  2014,
  './posters/лего-фильм-1ef09f49.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000018655382a5751886f2f7bfc969ebb/orig',
  'В пластмассовом мире все заняты своими пластмассовыми делами, всё идёт по плану, и для обычного лего-строителя Эммета нет места идеальней — для него в этом мире всё прекрасно. Но однажды ему придётся присоединиться к походу против злого лего-диктатора, планирующего склеить вместе всю лего-вселенную.',
  array['мультфильм', 'фантастика', 'фэнтези', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Дания', 'Австралия', 'Норвегия']::text[],
  null,
  471644,
  'tt1490017',
  7.2,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:07:06.000Z',
  false,
  false,
  false
),
(
  'media-эван-всемогущий-427',
  'Эван Всемогущий',
  'Evan Almighty',
  'movie',
  2007,
  './posters/эван-всемогущий-4910f0d6.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000017c0724f2d95f5a53e465a0d6f181/orig',
  'Эван Бакстер, которого всемогущий босс Брюс заставил лепетать всякую чушь во время прямого эфира новостей, уходит с работы на телевидении. Но жизнь его сразу же пошла в гору: он стал конгрессменом. И тут вновь появился Господь Бог, который открыл перед Эваном ужасы грядущего и попросил срочно приступить к строительству ковчега, беря пример с Ноя.',
  array['фэнтези', 'комедия', 'семейный']::text[],
  array['США']::text[],
  null,
  103791,
  'tt0413099',
  6.1,
  5.4,
  null,
  null,
  null,
  '2026-06-21T12:07:07.000Z',
  false,
  false,
  false
),
(
  'media-вонка-428',
  'Вонка',
  'Wonka',
  'movie',
  2023,
  './posters/вонка-38c82a87.jpg',
  './posters/вонка-38c82a87.jpg',
  'Мечтая открыть магазин в городе, известном своим шоколадом, молодой бедный начинающий фокусник и изобретатель Вилли Вонка обнаруживает, что индустрией управляет картель жадных шоколатье. Пытаясь осуществить мечту, он использует все свои фантазии и дерзкие идеи и борется с хитрыми конкурентами.',
  array['мюзикл', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Великобритания', 'Канада']::text[],
  787699,
  1013343,
  'tt6166392',
  7.2,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:07:08.000Z',
  false,
  false,
  false
),
(
  'media-евангелион-429',
  'Евангелион',
  'Shin Seiki Evangelion',
  'anime',
  1995,
  './posters/евангелион-0b2723e8.jpg',
  './posters/евангелион-0b2723e8.jpg',
  '2015 год, Токио-3. 15 лет назад произошёл Второй удар, в результате чего Антарктида растаяла, Землю накрыла глобальная катастрофа, и большая часть человечества погибла. Теперь планету атакуют Ангелы — загадочные существа, противостоять которым могут только созданные с помощью биотехнологий гигантские роботы серии «Евангелион». 

14-летний Синдзи Икари направлялся на встречу с отцом, руководителем влиятельной организации NERV, когда Токио-3 подвергся нападению Ангела. С помощью капитана Мисато Кацураги парень добирается до подземной штаб-квартиры NERV и единственное, что он хочет сказать отцу — как сильно его ненавидит. Но на месте выясняется, что Синдзи должен прямо сейчас залезть в робота и сражаться с Ангелом.',
  array['аниме', 'мультфильм', 'фантастика', 'боевик', 'драма']::text[],
  array['Япония']::text[],
  890,
  95323,
  'tt0112159',
  8.1,
  8.5,
  null,
  null,
  null,
  '2026-06-21T12:07:09.000Z',
  false,
  false,
  false
),
(
  'media-академия-амбрелла-430',
  'Академия «Амбрелла»',
  'The Umbrella Academy',
  'series',
  2019,
  './posters/академия-амбрелла-67dd7fe6.jpg',
  './posters/академия-амбрелла-67dd7fe6.jpg',
  'Расформированная группа супергероев воссоединяется после смерти своего приемного отца, который готовил их к спасению мира от неизвестной угрозы. Вместе они должны продолжить его дело.',
  array['фантастика', 'фэнтези', 'боевик', 'драма', 'комедия', 'приключения']::text[],
  array['США']::text[],
  null,
  463401,
  'tt1312171',
  7.5,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:07:10.000Z',
  false,
  false,
  false
),
(
  'media-финес-и-ферб-431',
  'Финес и Ферб',
  'Phineas and Ferb',
  'series',
  2007,
  './posters/финес-и-ферб-80b96084.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017f463ab83928d14d242b71ee61fb/orig',
  'Двое братьев, Финес и Ферб, каждый день разрабатывают безумные планы и изобретают невероятные вещи. Тем временем домашний любимец мальчиков, утконос Перри, пытается помешать злобному ученому по имени Хайнц Фуфелшмертц.',
  array['мультфильм', 'мюзикл', 'фантастика', 'боевик', 'комедия', 'семейный']::text[],
  array['США', 'Китай', 'Тайвань', 'Корея Южная']::text[],
  null,
  449993,
  'tt0852863',
  7.7,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:07:11.000Z',
  false,
  false,
  false
),
(
  'media-врата-штейна-432',
  'Врата Штейна',
  'Steins;Gate',
  'anime',
  2011,
  './posters/врата-штейна-4a79d36f.jpg',
  'https://avatars.mds.yandex.net/get-ott/17687051/2a0000019b69127391acbe26ee87f8e579b5/orig',
  'Акихабара – интереснейшее место, где обитают самые разные люди – от слегка сдвинутых по фазе до больных на всю голову. Именно такая компания собралась в Лаборатории проблем времени, что над лавкой старых телевизоров. Возглавляет её 18-летний Ринтаро Окабэ, сумасшедший ученый и борец с мировым заговором. В серьезном деле нельзя без хакера – вот и он, Итару Хасида, конечно же, толстяк и истинный отаку. Добрая фея лаборатории – Маюри Сиина, подруга детства Ринтаро, официантка мейд-кафе и фанатка косплея, а научную мощь бригады резко повысила юный гений Курису Макисэ, истосковавшаяся по интеллектуальным приключениям и простому человеческому теплу.

Компании удалось построить машину времени из микроволновки и барахла с ближайшей распродажи. Великие открытия делают любители, а профессионалы строят «Титаники». Вот только потом началось такое, что парадокс дедушки нервно удалился курить в сторонку, а главный герой трижды проклял тот день, когда сдуру открыл врата Штейна. Он был готов рисковать своей жизнью, но не чужими. Впрочем, сделанного, как известно, не воротишь. Или… все же можно?',
  array['аниме', 'мультфильм', 'фантастика', 'триллер']::text[],
  array['Япония']::text[],
  42509,
  586251,
  'tt1910272',
  8.4,
  8.8,
  null,
  null,
  null,
  '2026-06-21T12:07:12.000Z',
  false,
  false,
  false
),
(
  'media-хантер-х-хантер-433',
  'Хантер х Хантер',
  'Hunter x Hunter',
  'anime',
  2011,
  './posters/хантер-х-хантер-d94470c0.jpg',
  './posters/хантер-х-хантер-d94470c0.jpg',
  'Лицензия члена организации Охотников дает множество привилегий — возможность бесплатно путешествовать первым классом, до',
  array['аниме', 'мультфильм', 'боевик', 'комедия']::text[],
  array['Япония']::text[],
  46298,
  647602,
  'tt2098220',
  8.6,
  9,
  null,
  null,
  null,
  '2026-06-21T12:07:13.000Z',
  false,
  false,
  false
),
(
  'media-бруклин-9-9-434',
  'Бруклин 9-9',
  'Brooklyn Nine-Nine',
  'series',
  2013,
  './posters/бруклин-9-9-a1c47764.jpg',
  'https://avatars.mds.yandex.net/get-ott/1648503/2a0000017c077f1942731826d1f927bbf0e5/orig',
  'История о том, что происходит, когда у детектива, не воспринимающего ничего серьезно, появляется новый руководитель, который хочет, чтобы он рос и с уважением относился к своей должности.',
  array['комедия', 'криминал']::text[],
  array['США']::text[],
  48891,
  714102,
  'tt2467372',
  8.3,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:07:14.000Z',
  false,
  false,
  false
),
(
  'media-облачно-возможны-осадки-в-виде-фрикаделек-435',
  'Облачно, возможны осадки в виде фрикаделек',
  'Cloudy with a Chance of Meatballs',
  'movie',
  2009,
  './posters/облачно-возможны-осадки-в-виде-фрикаделек-350747f8.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c072fcefdd5e6add1ac8a32f9e8/orig',
  'Главный герой мультфильма - странный ученый Флинт Локвуд, который всегда хотел изобрести что-то такое, что сделает его родной городок счастливым. И вот, у него получилось! Еды теперь хватает всем, и она просто падает с неба, создавая новые пейзажи вокруг: можно любоваться закатом с живописных холмов из фруктового желе. Но иногда вкусные осадки в виде фрикаделек могут превратиться в настоящий торнадо из аппетитных, но уже опасных спагетти! Гениальному ученому и его друзьям предстоит исправить положение дел в сытом городке.',
  array['мультфильм', 'фантастика', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  null,
  276363,
  'tt0844471',
  6.9,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:07:15.000Z',
  false,
  false,
  false
),
(
  'media-планета-51-436',
  'Планета 51',
  'Planet 51',
  'cartoon',
  2009,
  './posters/планета-51-220cb10a.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c0744fcd9334e31b0c281a1303f/orig',
  'Главный герой - американский астронавт капитан Чарльз «Чак» Бейкер - высаживается на планете 51, которая по началу кажется ему необитаемой. К своему удивлению он обнаруживает, что планету населяют маленькие зеленые человечки. Их сообщество очень напоминает счастливую одноэтажную Америку времен 1950-х годов. И боятся эти человечки только нападения со стороны инопланетных захватчиков... таких, как Чак!

Полагаясь на помощь своего робота-напарника «Ровера» и нового друга Лема, Чаку придется обогнуть весь этот изумительный и непонятный мир, чтобы не оказаться навечно экспонатом в Музее Инопланетных Захватчиков Планеты 51.',
  array['мультфильм', 'фантастика', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['Испания', 'Великобритания', 'США', 'Канада']::text[],
  null,
  279221,
  'tt0762125',
  6.6,
  6,
  null,
  null,
  null,
  '2026-06-21T12:07:16.000Z',
  false,
  false,
  false
),
(
  'media-берсерк-437',
  'Берсерк',
  'Kenpu denki baruseruku',
  'anime',
  1997,
  './posters/берсерк-467fbee8.jpg',
  './posters/берсерк-467fbee8.jpg',
  'Через предательство и кровь к власти пришел новый король. Его демоны-подручные чинят несправедливости до тех пор, пока в',
  array['аниме', 'мультфильм', 'фэнтези', 'боевик', 'драма']::text[],
  array['Япония', 'Польша']::text[],
  35935,
  257376,
  'tt0318871',
  8.6,
  8.7,
  null,
  null,
  null,
  '2026-06-21T12:07:17.000Z',
  false,
  false,
  false
),
(
  'media-лего-фильм-бэтмен-438',
  'Лего Фильм: Бэтмен',
  'The Lego Batman Movie',
  'cartoon',
  2017,
  './posters/лего-фильм-бэтмен-e23f4afc.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c286e7ce6c287e9690eed04a552/orig',
  'Готэму вновь грозит опасность. И на его стражу становится единственный герой, которого заслуживает этот город — Бэтмен, альтер-эго миллиардера Брюса Уэйна, живущего в своем огромном родовом поместье вместе с дворецким Альфредом. Но в этот раз супергерой вынужден взять в напарники парня по имени Дик Грейсон, которого он, сам того не помня, усыновил на одном из светских приемов. Новоиспеченный Робин составит Бэтмену компанию в борьбе с его заклятым врагом Джокером, сбежавшим из психиатрической больницы Аркхем, а также Харли Куинн, готовой на самые сумасшедшие злодеяния, чтобы оставить в дураках всю Лигу Справедливости, включая Бэтгерл.',
  array['мультфильм', 'фантастика', 'фэнтези', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Дания', 'Австралия']::text[],
  null,
  843480,
  'tt4116284',
  7.1,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:07:18.000Z',
  false,
  false,
  false
),
(
  'media-смурфики-439',
  'Смурфики',
  'The Smurfs',
  'cartoon',
  2011,
  './posters/смурфики-454756bd.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017c078ae8da181fbe4782759a92be/orig',
  'По сюжету, спасаясь от злого волшебника Гаргамеля, крошечные смурфики оказываются за пределами родной деревни.  Из своего волшебного мира они попадают прямиком в  наш мир, а именно в Центральный парк города Нью-Йорк. Теперь маленьким отважным смурфикам предстоит отыскать дорогу домой и не попасть в лапы Гаргамеля.',
  array['мультфильм', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Бельгия', 'Канада']::text[],
  41513,
  178591,
  'tt0472181',
  6.1,
  5.4,
  null,
  null,
  null,
  '2026-06-21T12:07:19.000Z',
  false,
  false,
  false
),
(
  'media-тарзан-440',
  'Тарзан',
  'Tarzan',
  'cartoon',
  1999,
  './posters/тарзан-ac9242cf.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017c0739d42fe17502324a280f7b8e/orig',
  'Тарзан вырос среди горилл, в таинственных джунглях, где  не ступала нога человека. Враждебное царство диких животных долго не  хотело признавать хрупкого мальчика.

Вскоре на глазах у всех слабый  малыш превратился в храброго юношу и стал быстрее пантеры, сильнее  тигра, зорче орла. Но когда Тарзан становится настоящим королем  джунглей, происходит неожиданная встреча, которая полностью меняет его  жизнь...',
  array['мультфильм', 'драма', 'комедия', 'приключения', 'семейный', 'музыка']::text[],
  array['США', 'Япония']::text[],
  null,
  2260,
  'tt0120855',
  7.7,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:07:20.000Z',
  false,
  false,
  false
),
(
  'media-голый-пистолет-441',
  'Голый пистолет',
  'The Naked Gun',
  'movie',
  2025,
  './posters/голый-пистолет-572d1521.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000019bebbde1237b34694d64a582aaeb/orig',
  'Обезвредив грабителей банка, Фрэнк Дребин — младший разнёс половину здания, за что получил запрет заниматься этим делом и был назначен на новое — расследование автоаварии, в которой погиб один из сотрудников технологической компании. Бывалый коп подозревает, что эти происшествия связаны, когда судьба сводит его с красоткой Бэт — сестрой погибшего и авторшей популярных детективных романов.',
  array['комедия', 'детектив', 'боевик', 'криминал']::text[],
  array['США', 'Канада']::text[],
  1035259,
  817971,
  'tt3402138',
  6.2,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:07:21.000Z',
  false,
  false,
  false
),
(
  'media-аэроплан-442',
  'Аэроплан',
  'Airplane!',
  'movie',
  1980,
  './posters/аэроплан-7192feca.jpg',
  './posters/аэроплан-7192feca.jpg',
  'Пытаясь вернуть бросившую его стюардессу Илейн, бывший военный лётчик Тед Страйкер, страдающий посттравматическим стрессовым расстройством, поднимается на борт пассажирского лайнера, на котором летит Илейн. Во время полёта выясняется, что из-за пищевого отравления пилоты не в состоянии управлять самолётом. Единственной надеждой пассажиров на спасение становится Тед Страйкер.',
  array['комедия']::text[],
  array['США']::text[],
  813,
  3231,
  'tt0080339',
  7.4,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:07:22.000Z',
  false,
  false,
  false
),
(
  'media-мистер-бин-443',
  'Мистер Бин',
  'Mr. Bean',
  'series',
  1990,
  './posters/мистер-бин-a71f3cf8.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000017c07a8acbdd568765b71f16982f4/orig',
  'Мистер Бин - личность выдающаяся по количеству глупейших ситуаций, в которых он оказывается, и по оригинальности методов, с помощью которых он из этих ситуаций выкручивается.

Идет ли он в библиотеку, сдает ли экзамены, собирается ли пообедать или прыгнуть с вышки в бассейне, заезжает ли он на автостоянку или раздевается на пляже, - редко кто сделает это так смешно и остроумно, как мистер Бин.',
  array['комедия', 'семейный']::text[],
  array['Великобритания']::text[],
  4327,
  161087,
  'tt0096657',
  7.9,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:07:23.000Z',
  false,
  false,
  false
),
(
  'media-губка-боб-444',
  'Губка Боб квадратные штаны',
  'SpongeBob SquarePants',
  'series',
  1999,
  './posters/губка-боб-квадратные-штаны-7eca77b6.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a00000199a3d455c1da1717b952b951d8f6/1344x756',
  'Население подводного городка Бикини Боттом составляют разные морские обитатели. Среди них — лучший друг Спанч Боба, морская звезда по имени Патрик, раздражительный надменный кальмар Сквидвард и белка по имени Сэнди, которая тоже живёт под водой, используя гидрокостюм. Спанч Боб работает поваром в закусочной «Красти Краб», которой управляет старый скряга мистер Крабс.',
  array['мультфильм', 'фэнтези', 'комедия', 'семейный']::text[],
  array['США']::text[],
  11836,
  161268,
  'tt0206512',
  7.1,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:07:24.000Z',
  false,
  false,
  false
),
(
  'media-семь-самураев-445',
  'Семь самураев',
  'Shichinin no samurai',
  'movie',
  1954,
  './posters/семь-самураев-bccda646.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a00000190fed1bab167825d3c21622f5962/orig',
  'Япония, XVI век. В стране полыхает гражданская война, повсюду орудуют банды разбойников и мародеров. Бедные крестьяне нанимают для защиты деревни семерых самураев, которые немного сплачивают раздробленных и малодушных селян в процессе подготовки и укрепления деревни.',
  array['драма', 'боевик']::text[],
  array['Япония']::text[],
  null,
  332,
  'tt0047478',
  8.2,
  8.6,
  null,
  null,
  null,
  '2026-06-21T12:07:25.000Z',
  false,
  false,
  false
),
(
  'media-седьмая-печать-446',
  'Седьмая печать',
  'Det sjunde inseglet',
  'movie',
  1957,
  './posters/седьмая-печать-0fb12ffe.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a00000197cf93fe80bb770fe54c05886483/orig',
  'В середине XIV века рыцарь Антониус Блок и его оруженосец возвращаются после десяти лет крестовых походов в родную Швецию. Блок устал от жизни и не видит ничего, ради чего стоило бы продолжать влачить существование. Но прежде он хочет убедиться, что бог существует.',
  array['фэнтези', 'драма']::text[],
  array['Швеция']::text[],
  490,
  425,
  'tt0050976',
  8,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:07:26.000Z',
  false,
  false,
  false
),
(
  'media-древо-жизни-448',
  'Древо жизни',
  'The Tree of Life',
  'movie',
  2010,
  './posters/древо-жизни-b18d45f6.jpg',
  './posters/древо-жизни-b18d45f6.jpg',
  'Мы наблюдаем за развитием 11-летнего Джека, одного из трех братьев. Поначалу ребенку все кажется чудесным. Глазами своей души он наблюдает за поступками мамы. Она представляет собой любовь и милосердие, в то время как отец пытается научить сына, что в реальном мире на первое место необходимо всегда ставить себя. Каждый родитель старается переманить Джека на свою сторону, и он должен примириться с их притязаниями. Действительность становится мрачнее, когда главному герою в первый раз приходится столкнуться с болью, страданиями и смертью. Некогда абсолютно ясный мир превращается в лабиринт.',
  array['фэнтези', 'драма']::text[],
  array['США']::text[],
  null,
  256408,
  'tt0478304',
  6.6,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:07:28.000Z',
  false,
  false,
  false
),
(
  'media-исходный-код-449',
  'Исходный код',
  'Source Code',
  'movie',
  2011,
  './posters/исходный-код-01fffae0.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017c0749a9d428753c7d2de945f6b2/orig',
  'Солдат по имени Коултер мистическим образом оказывается в теле неизвестного мужчины, погибшего в железнодорожной катастрофе. Коултер вынужден переживать чужую смерть снова и снова до тех пор, пока не поймет, кто – зачинщик катастрофы.',
  array['фантастика', 'боевик', 'триллер', 'драма']::text[],
  array['США', 'Канада', 'Франция', 'Германия']::text[],
  45612,
  409295,
  'tt0945513',
  7.8,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:07:29.000Z',
  false,
  false,
  false
),
(
  'media-приключения-электроника-450',
  'Приключения Электроника',
  'Приключения Электроника',
  'movie',
  1979,
  './posters/приключения-электроника-6efb8e99.jpg',
  './posters/приключения-электроника-6efb8e99.jpg',
  'Советский ученый создает гениального робота. Это мальчик, который способен решать любые математические задачи, он пишет самые лучшие сочинения и отлично поет! А еще он - точная копия школьника Сережи Сыроежкина. Сообразительный мальчуган быстренько перекладывает все свои обязанности на электронного двойника. Тот делает за него домашние задания, ходит в школу и выполняет хозяйственные дела, которые Сыроежкину поручают родители...',
  array['фантастика', 'детский', 'приключения', 'комедия', 'криминал', 'семейный']::text[],
  array['СССР']::text[],
  65518,
  77284,
  'tt0078673',
  8,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:07:30.000Z',
  false,
  false,
  false
),
(
  'media-час-пик-451',
  'Час пик',
  'Rush Hour',
  'movie',
  1998,
  './posters/час-пик-eaacb0e3.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a00000186b8930587e256670556a5585614/orig',
  'В Лос-Анджелесе злодеи похищают малолетнюю дочь китайского консула, которую в Гонконге учил кунг-фу инспектор Ли. Консул вызывает Ли в Америку, чтобы тот принял участие в освобождении девочки.

Агенты ФБР обратились к полиции с просьбой выделить им самого никчемного сотрудника, чтобы тот взял китайца на себя и показал ему достопримечательности, а главное, не позволил вмешиваться в их дела.

Им оказывается самый болтливый полицейский Картер. Естественно, после ряда разногласий и недоразумений они объединяют усилия и всерьез берутся за вызволение заложницы.',
  array['боевик', 'комедия', 'криминал']::text[],
  array['США']::text[],
  2109,
  6345,
  'tt0120812',
  7.8,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:07:31.000Z',
  false,
  false,
  false
),
(
  'media-час-пик-2-452',
  'Час пик 2',
  'Rush Hour 2',
  'movie',
  2001,
  './posters/час-пик-2-7655f536.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a0000017c074a8f44681018c572c6e26da0/orig',
  'Полицейские Ли и Картер прибывают в Гонконг по следам преступников, замышляющих похищение китайских сокровищ и их продажу в США. По мере того как детективы всё больше и больше запутываются в преступном заговоре, поездка от Гонконга до Лос-Анджелеса и Лас-Вегаса оборачивается для них опасным и интересным приключением.',
  array['боевик', 'триллер', 'комедия', 'криминал']::text[],
  array['США', 'Гонконг']::text[],
  null,
  812,
  'tt0266915',
  7.4,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:07:32.000Z',
  false,
  false,
  false
),
(
  'media-час-пик-3-453',
  'Час пик 3',
  'Rush Hour 3',
  'movie',
  2007,
  './posters/час-пик-3-5c4a8ef9.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000018317fc837fbb5e9cd9332e9b4171/orig',
  'Лос-анджелесского полицейского Картера понижают в должности. Гонконгский полицейский Ли должен охранять китайского посла Хана, который прибыл в Лос-Анджелес, чтобы разоблачить китайский криминальный синдикат. Наёмному убийце удаётся ранить посла. Ли бросается вдогонку за преступником. Картер мчится на место преступления, но только мешает Ли.',
  array['боевик', 'триллер', 'комедия', 'криминал']::text[],
  array['США', 'Германия']::text[],
  5174,
  22978,
  'tt0293564',
  7,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:07:33.000Z',
  false,
  false,
  false
),
(
  'media-убить-билла-454',
  'Убить Билла',
  'Kill Bill: Vol. 1',
  'movie',
  2003,
  './posters/убить-билла-7f32cfaa.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c07263bfee0256f77776d195476/orig',
  'В беременную наёмную убийцу по кличке Чёрная Мамба во время бракосочетания стреляет человек по имени Билл. Но голова у женщины оказалась крепкой — пролежав четыре года в коме, бывшая невеста приходит в себя. Она горит желанием найти предателей. Теперь только безжалостная месть успокоит сердце Чёрной Мамбы, и она начинает по очереди убивать членов банды Билла, решив оставить главаря напоследок.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США', 'Япония']::text[],
  24,
  2717,
  'tt0266697',
  7.7,
  8.2,
  null,
  null,
  null,
  '2026-06-21T12:07:34.000Z',
  false,
  false,
  false
),
(
  'media-убить-билла-2-455',
  'Убить Билла 2',
  'Kill Bill: Vol. 2',
  'movie',
  2004,
  './posters/убить-билла-2-900c3535.jpg',
  './posters/убить-билла-2-900c3535.jpg',
  'Убив двух человек из банды, Невеста лишь наполовину приблизилась к цели. Теперь на очереди Бад и Элли Драйвер. Еще два опасных шага перед последней схваткой, в которой она должна убить Билла.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США']::text[],
  393,
  47015,
  'tt0378194',
  7.6,
  8,
  null,
  null,
  null,
  '2026-06-21T12:07:35.000Z',
  false,
  false,
  false
),
(
  'media-кин-дза-дза-456',
  'Кин-дза-дза!',
  'Кин-дза-дза!',
  'movie',
  1986,
  './posters/кин-дза-дза-e748700c.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000167282f45e6ca465ab9e23f989e31/1344x756',
  'Прораб Владимир Николаевич Машков и не подозревал, что обычный путь до универсама за хлебом и макаронами обернется межгалактическим путешествием. А все эта встреча со странным босоногим человеком с каким-то маленьким устройством — «машинкой перемещения», как тот ее назвал. Владимир Николаевич нажал кнопку и оказался вместе со случайным попутчиком, студентом в кроличьей шапке, в какой-то пустыне — на планете Плюк в тентуре, галактика Кин-дза-дза в спирали.',
  array['фантастика', 'драма', 'комедия']::text[],
  array['СССР']::text[],
  20874,
  44811,
  'tt0091341',
  8.1,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:07:36.000Z',
  false,
  false,
  false
),
(
  'media-бегущий-в-лабиринте-457',
  'Бегущий в лабиринте',
  'The Maze Runner',
  'movie',
  2014,
  './posters/бегущий-в-лабиринте-d375ea74.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001612caf5e7431016ac250e1e98bba/1344x756',
  'Главный герой — подросток Томас, который просыпается в лифте, но ничего не помнит, кроме своего имени. Он оказывается среди других подростков, научившихся выживать в замкнутом пространстве. Раз в 30 дней прибывает новый мальчик. Группа ребят проживает в «Приюте» уже три года. Они кормятся тем, что удается вырастить на земле, и пытаются найти выход из лабиринта, окружающего их место жительства. Но однажды появляется девочка в состоянии комы...',
  array['фантастика', 'триллер', 'приключения']::text[],
  array['США', 'Великобритания']::text[],
  198663,
  575613,
  'tt1790864',
  6.8,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:07:37.000Z',
  false,
  false,
  false
),
(
  'media-бегущий-в-лабиринте-испытание-огн-м-458',
  'Бегущий в лабиринте: Испытание огнём',
  'Maze Runner: The Scorch Trials',
  'movie',
  2015,
  './posters/бегущий-в-лабиринте-испытание-огнем-8a5c8183.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001612cca01cfe9f2c02a723017d759/1344x756',
  'Лабиринт пройден, но Томасу, Терезе, Минхо и прочим глэйдерам не приходится расслабляться. Таинственное руководство ПОРОКА - секретной организации, устроившей гонки на выживание - назначает ребятам новые, смертельно опасные испытания. На сей раз их ждет переход по раскаленной пустыне и встреча с жертвами неизлечимой болезни, которые отличаются буйным нравом и непредсказуемым поведением.',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array['США']::text[],
  null,
  842673,
  'tt4046784',
  6.2,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:07:38.000Z',
  false,
  false,
  false
),
(
  'media-бегущий-в-лабиринте-лекарство-от-смерти-459',
  'Бегущий в лабиринте: Лекарство от смерти',
  'Maze Runner: The Death Cure',
  'movie',
  2018,
  './posters/бегущий-в-лабиринте-лекарство-от-смерти-9134b1bc.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000016301b6c528b6073e8909a684599e/1344x756',
  'Томас возглавляет отряд выживших глейдеров, чтобы выполнить последнюю и самую опасную миссию. Ради спасения своих друзей они должны проникнуть в легендарный Последний город, смертоносный лабиринт, контролируемый ПОРОКом. Каждый, кто выживет там, наконец-то получит ответы на все вопросы, которые мучили глейдеров с момента попадания в лабиринт.',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array['США']::text[],
  null,
  896631,
  'tt4500922',
  6.3,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:07:39.000Z',
  false,
  false,
  false
),
(
  'media-пираты-карибского-моря-проклятие-черной-жемчужины-460',
  'Пираты Карибского моря: Проклятие Черной жемчужины',
  'Pirates of the Caribbean: The Curse of the Black Pearl',
  'movie',
  2003,
  './posters/пираты-карибского-моря-проклятие-черной-жемчужины-14b913d5.jpg',
  './posters/пираты-карибского-моря-проклятие-черной-жемчужины-14b913d5.jpg',
  'Жизнь харизматичного авантюриста, капитана Джека Воробья, полная увлекательных приключений, резко меняется, когда его заклятый враг капитан Барбосса похищает корабль Джека Черную Жемчужину, а затем нападает на Порт Ройал и крадет прекрасную дочь губернатора Элизабет Свонн.

Друг детства Элизабет Уилл Тернер вместе с Джеком возглавляет спасательную экспедицию на самом быстром корабле Британии, чтобы вызволить девушку и заодно отобрать у злодея Черную Жемчужину. Вслед за этой парочкой отправляется амбициозный коммодор Норрингтон, который к тому же числится женихом Элизабет.

Однако Уилл не знает, что над Барбоссой висит вечное проклятие, при лунном свете превращающее его с командой в живых скелетов. Проклятье будет снято лишь тогда, когда украденное золото Ацтеков будет возвращено пиратами на старое место.',
  array['фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  22,
  4374,
  'tt0325980',
  8.4,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:07:40.000Z',
  false,
  false,
  false
),
(
  'media-пираты-карибского-моря-сундук-мертвеца-461',
  'Пираты Карибского моря: Сундук мертвеца',
  'Pirates of the Caribbean: Dead Man''s Chest',
  'movie',
  2006,
  './posters/пираты-карибского-моря-сундук-мертвеца-f10e5855.jpg',
  './posters/пираты-карибского-моря-сундук-мертвеца-f10e5855.jpg',
  'Вновь оказавшись в ирреальном мире, лихой капитан Джек Воробей неожиданно узнает, что является должником легендарного капитана «Летучего Голландца» Дэйви Джонса. Джек должен в кратчайшие сроки решить эту проблему, иначе ему грозит вечное проклятие и рабское существование после смерти. Вдобавок ко всему, срывается свадьба Уилла Тернера и Элизабет Суонн, которые вынуждены составить Джеку компанию в его злоключениях…',
  array['фэнтези', 'боевик', 'приключения']::text[],
  array['США']::text[],
  58,
  63991,
  'tt0383574',
  8.2,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:07:41.000Z',
  false,
  false,
  false
),
(
  'media-пираты-карибского-моря-на-краю-света-462',
  'Пираты Карибского моря: На краю света',
  'Pirates of the Caribbean: At World''s End',
  'movie',
  2007,
  './posters/пираты-карибского-моря-на-краю-света-f2872f37.jpg',
  './posters/пираты-карибского-моря-на-краю-света-f2872f37.jpg',
  'Новые приключения Джека Воробья и его друзей Уилла Тернера и Элизабет Суонн. На этот раз Уиллу и Элизабет придется объединиться с самим Капитаном Барбоссой для того, чтобы отправиться на край света и спасти своего друга — Джека. Ситуация осложняется тем, что Элизабет попадает к сингапурским пиратам…',
  array['приключения', 'боевик', 'фэнтези']::text[],
  array['США']::text[],
  285,
  102124,
  'tt0449088',
  8.1,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:07:42.000Z',
  false,
  false,
  false
),
(
  'media-пираты-карибского-моря-на-странных-берегах-463',
  'Пираты Карибского моря: На странных берегах',
  'Pirates of the Caribbean: On Stranger Tides',
  'movie',
  2011,
  './posters/пираты-карибского-моря-на-странных-берегах-68956505.jpg',
  './posters/пираты-карибского-моря-на-странных-берегах-68956505.jpg',
  'Капитану Джеку Воробью предстоит столкнуться с женщиной из своего прошлого Анжеликой. Она вынуждает его взойти на корабль «Месть Королевы Анны», которым управляет коварный злодей Черная Борода. И теперь Джек не знает, кого бояться больше — ужасного злодея или же своей давней подруги.',
  array['приключения', 'боевик', 'фэнтези', 'комедия']::text[],
  array['США', 'Великобритания']::text[],
  1865,
  427076,
  'tt1298650',
  7.3,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:07:43.000Z',
  false,
  false,
  false
),
(
  'media-пираты-карибского-моря-мертвецы-не-рассказывают-сказки-464',
  'Пираты Карибского моря: Мертвецы не рассказывают сказки',
  'Pirates of the Caribbean: Dead Men Tell No Tales',
  'movie',
  2017,
  './posters/пираты-карибского-моря-мертвецы-не-рассказывают-сказки-597dd457.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000169c35fee6475fcad83a5af5fa34f/1344x756',
  'Исчерпавший свою удачу капитан Джек Воробей обнаруживает, что за ним охотится его старый неприятель, ужасный капитан Салазар и его мертвецы. Они только что сбежали из Дьявольского треугольника и намерены уничтожить всех пиратов, включая Джека. Поможет спастись лишь могущественный артефакт — трезубец Посейдона, который дарует своему обладателю полный контроль над морями.',
  array['фэнтези', 'боевик', 'комедия', 'приключения']::text[],
  array['США', 'Австралия', 'Великобритания', 'Канада']::text[],
  166426,
  575180,
  'tt1790809',
  6.7,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:07:44.000Z',
  false,
  false,
  false
),
(
  'media-гарри-поттер-и-философский-камень-465',
  'Гарри Поттер и философский камень',
  'Harry Potter and the Sorcerer''s Stone',
  'movie',
  2001,
  './posters/гарри-поттер-и-философский-камень-fc2ca527.jpg',
  './posters/гарри-поттер-и-философский-камень-fc2ca527.jpg',
  'Жизнь десятилетнего Гарри Поттера нельзя назвать сладкой: родители умерли, едва ему исполнился год, а от дяди и тёти, взявших сироту на воспитание, достаются лишь тычки да подзатыльники. Но в одиннадцатый день рождения Гарри всё меняется. Странный гость, неожиданно появившийся на пороге, приносит письмо, из которого мальчик узнаёт, что на самом деле он - волшебник и зачислен в школу магии под названием Хогвартс. А уже через пару недель Гарри будет мчаться в поезде Хогвартс-экспресс навстречу новой жизни, где его ждут невероятные приключения, верные друзья и самое главное — ключ к разгадке тайны смерти его родителей.',
  array['фэнтези', 'приключения', 'семейный']::text[],
  array['Великобритания', 'США']::text[],
  671,
  689,
  'tt0241527',
  8.3,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:07:45.000Z',
  false,
  false,
  false
),
(
  'media-гарри-поттер-и-тайная-комната-466',
  'Гарри Поттер и Тайная комната',
  'Harry Potter and the Chamber of Secrets',
  'movie',
  2002,
  './posters/гарри-поттер-и-тайная-комната-40c97343.jpg',
  './posters/гарри-поттер-и-тайная-комната-40c97343.jpg',
  'Гарри Поттер переходит на второй курс Школы чародейства и волшебства Хогвартс. Эльф Добби предупреждает Гарри об опасности, которая поджидает его там, и просит больше не возвращаться в школу.
Юный волшебник не следует совету эльфа и становится свидетелем таинственных событий, разворачивающихся в Хогвартсе. Вскоре Гарри и его друзья узнают о существовании Тайной Комнаты и сталкиваются с новыми приключениями, пытаясь победить темные силы.',
  array['фэнтези', 'приключения', 'семейный']::text[],
  array['Великобритания', 'США']::text[],
  672,
  688,
  'tt0295297',
  8.2,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:07:46.000Z',
  false,
  false,
  false
),
(
  'media-гарри-поттер-и-узник-азкабана-467',
  'Гарри Поттер и узник Азкабана',
  'Harry Potter and the Prisoner of Azkaban',
  'movie',
  2004,
  './posters/гарри-поттер-и-узник-азкабана-2edd4332.jpg',
  './posters/гарри-поттер-и-узник-азкабана-2edd4332.jpg',
  'Гарри, Рон и Гермиона возвращаются на третий курс школы чародейства и волшебства Хогвартс. На этот раз они должны раскрыть тайну узника, сбежавшего из тюрьмы Азкабан, чье пребывание на воле создает для Гарри смертельную опасность.',
  array['фэнтези', 'приключения', 'семейный']::text[],
  array['Великобритания', 'США']::text[],
  673,
  322,
  'tt0304141',
  8.3,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:07:47.000Z',
  false,
  false,
  false
),
(
  'media-гарри-поттер-и-кубок-огня-468',
  'Гарри Поттер и Кубок огня',
  'Harry Potter and the Goblet of Fire',
  'movie',
  2005,
  './posters/гарри-поттер-и-кубок-огня-fe893ebe.jpg',
  './posters/гарри-поттер-и-кубок-огня-fe893ebe.jpg',
  'Гарри Поттер, Рон и Гермиона возвращаются на четвёртый курс школы чародейства и волшебства Хогвартс. При таинственных обстоятельствах Гарри был отобран в число участников опасного соревнования — Турнира Трёх Волшебников, однако проблема в том, что все его соперники — намного старше и сильнее.
К тому же, знаки указывают на возвращение Лорда Волдеморта. Вскоре Гарри предстоит побороться не только за победу в соревновании, но и за свою жизнь.',
  array['фэнтези', 'приключения', 'семейный']::text[],
  array['Великобритания', 'США']::text[],
  674,
  8408,
  'tt0330373',
  8,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:07:48.000Z',
  false,
  false,
  false
),
(
  'media-гарри-поттер-и-орден-феникса-469',
  'Гарри Поттер и Орден Феникса',
  'Harry Potter and the Order of the Phoenix',
  'movie',
  2007,
  './posters/гарри-поттер-и-орден-феникса-2cc78e61.jpg',
  './posters/гарри-поттер-и-орден-феникса-2cc78e61.jpg',
  'Гарри проводит свой пятый год в школе Хогвартс и обнаруживает, что многие из членов волшебного сообщества отрицают факт недавнего состязания юного волшебника с воплощением вселенского зла Волдемортом. Все делают вид, что не имеют ни малейшего представления, что злодей вернулся. Однако впереди волшебников ждет необычная схватка.',
  array['фэнтези', 'приключения', 'семейный']::text[],
  array['Великобритания', 'США']::text[],
  675,
  48356,
  'tt0373889',
  7.9,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:07:49.000Z',
  false,
  false,
  false
),
(
  'media-гарри-поттер-и-принц-полукровка-470',
  'Гарри Поттер и Принц-полукровка',
  'Harry Potter and the Half-Blood Prince',
  'movie',
  2009,
  './posters/гарри-поттер-и-принц-полукровка-f92c341e.jpg',
  './posters/гарри-поттер-и-принц-полукровка-f92c341e.jpg',
  'Теперь не только мир волшебников, но и мир маглов ощущает на себе всевозрастающую силу Волан-де-Морта, а Хогвартс уже никак не назовешь надежным убежищем. Гарри подозревает, что в самом замке затаилась некая опасность, но Дамблдор больше сосредоточен на том, чтобы подготовить его к финальной схватке, которая, как он знает, уже не за горами. Вместе они пытаются разгадать секрет бессмертия Волан-де-Морта, а для этого Дамблдор приглашает на должность преподавателя по зельеварению своего старинного друга и коллегу — профессора Горация Слизнорта.',
  array['фэнтези', 'приключения', 'семейный']::text[],
  array['Великобритания', 'США']::text[],
  767,
  89515,
  'tt0417741',
  7.9,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:07:50.000Z',
  false,
  false,
  false
),
(
  'media-гарри-поттер-и-дары-смерти-часть-1-471',
  'Гарри Поттер и Дары Смерти: Часть I',
  'Harry Potter and the Deathly Hallows - Part 1',
  'movie',
  2010,
  './posters/гарри-поттер-и-дары-смерти-часть-i-35975408.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000019ac9c17f1fb84b00a9c0c66694ec/1344x756',
  'Гарри Поттера ждёт самое страшное испытание в жизни — смертельная схватка с Волан-де-Мортом. Друзья и враги Гарри предстают в совершенно неожиданном свете. Граница между добром и злом становится всё призрачнее.',
  array['фэнтези', 'приключения', 'семейный']::text[],
  array['Великобритания', 'США']::text[],
  12444,
  276762,
  'tt0926084',
  7.9,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:07:51.000Z',
  false,
  false,
  false
),
(
  'media-гарри-поттер-и-дары-смерти-часть-2-472',
  'Гарри Поттер и Дары Смерти: Часть II',
  'Harry Potter and the Deathly Hallows - Part 2',
  'movie',
  2011,
  './posters/гарри-поттер-и-дары-смерти-часть-ii-37dc5b8d.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000019ac9c31ff5b7e815a9acb99d12e3/1344x756',
  'Битва между добрыми и злыми силами мира волшебников перерастает во всеобщую войну. Ставки ещё никогда не были так высоки, а поиск убежища — столь сложен. И, быть может, именно Гарри Поттеру придется пожертвовать всем в финальном сражении с Волан-де-Мортом.',
  array['фэнтези', 'детектив', 'приключения', 'семейный']::text[],
  array['Великобритания', 'США']::text[],
  12445,
  407636,
  'tt1201607',
  8.2,
  8.1,
  null,
  null,
  null,
  '2026-06-21T12:07:52.000Z',
  false,
  false,
  false
),
(
  'media-матрица-473',
  'Матрица',
  'The Matrix',
  'movie',
  1999,
  './posters/матрица-7d4f7049.jpg',
  './posters/матрица-7d4f7049.jpg',
  'Жизнь Томаса Андерсона разделена на две части: днём он — самый обычный офисный работник, получающий нагоняи от начальства, а ночью превращается в хакера по имени Нео, и нет места в сети, куда он бы не смог проникнуть. Но однажды всё меняется. Томас узнаёт ужасающую правду о реальности.',
  array['фантастика', 'боевик']::text[],
  array['США', 'Австралия']::text[],
  603,
  301,
  'tt0133093',
  8.5,
  8.7,
  null,
  null,
  null,
  '2026-06-21T12:07:53.000Z',
  false,
  false,
  false
),
(
  'media-матрица-перезагрузка-474',
  'Матрица: Перезагрузка',
  'The Matrix Reloaded',
  'movie',
  2003,
  './posters/матрица-перезагрузка-3f941da7.jpg',
  './posters/матрица-перезагрузка-3f941da7.jpg',
  'Борцы за свободу Нео, Тринити и Морфеус продолжают руководить восстанием людей против армии машин. Они вынуждены использовать не только арсенал превосходного оружия, но и свои выдающиеся навыки. Участие в миссии по спасению человеческой расы от истребления приносит им более глубокое понимание Матрицы и осознание центральной роли Нео в судьбе человечества.',
  array['фантастика', 'боевик']::text[],
  array['США']::text[],
  604,
  299,
  'tt0234215',
  7.7,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:07:54.000Z',
  false,
  false,
  false
),
(
  'media-матрица-революция-475',
  'Матрица: Революция',
  'The Matrix Revolutions',
  'movie',
  2003,
  './posters/матрица-революция-df99cffe.jpg',
  './posters/матрица-революция-df99cffe.jpg',
  'Пока армия машин пытается уничтожить Зион, его жители из последних сил держат оборону.',
  array['фантастика', 'боевик']::text[],
  array['США', 'Австралия']::text[],
  605,
  316,
  'tt0242653',
  7.6,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:07:55.000Z',
  false,
  false,
  false
),
(
  'media-матрица-воскрешение-476',
  'Матрица: Воскрешение',
  'The Matrix Resurrections',
  'movie',
  2021,
  './posters/матрица-воскрешение-9b877186.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000017e4ecd63d99c1d2e7b5d90fd27ed/1344x756',
  'Геймдизайнер Томас Андерсон сделал себе имя работой над трилогией игр «Матрица». Хотя окружающий мир периодически даёт сбои и обнажает свою истинную сущность, бывший Нео исправно посещает психотерапевта, принимает пилюли и практически убедил себя, что всё это — игра его воображения. Но однажды на него выходит хакерша Багз и предлагает снова следовать за белым кроликом.',
  array['фантастика', 'боевик']::text[],
  array['США', 'Австралия']::text[],
  624860,
  1294123,
  'tt10838180',
  5.7,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:07:56.000Z',
  false,
  false,
  false
),
(
  'media-властелин-колец-братство-кольца-477',
  'Властелин колец: Братство кольца',
  'The Lord of the Rings: The Fellowship of the Ring',
  'movie',
  2001,
  './posters/властелин-колец-братство-кольца-9d0ee1f1.jpg',
  './posters/властелин-колец-братство-кольца-9d0ee1f1.jpg',
  'Сказания о Средиземье — это хроника Великой войны за Кольцо, длившейся не одну тысячу лет. Тот, кто владел Кольцом, получал неограниченную власть, но был обязан служить злу. 
Тихая деревня, где живут хоббиты. Придя на 111-й день рождения к своему старому другу Бильбо Бэггинсу, волшебник Гэндальф начинает вести разговор о кольце, которое Бильбо нашел много лет назад. Это кольцо принадлежало когда-то темному властителю Средиземья Саурону, и оно дает большую власть своему обладателю. Теперь Саурон хочет вернуть себе власть над Средиземьем. Бильбо отдает Кольцо племяннику Фродо, чтобы тот отнёс его к Роковой Горе и уничтожил.',
  array['фэнтези', 'приключения', 'драма', 'боевик']::text[],
  array['Новая Зеландия', 'США']::text[],
  120,
  328,
  'tt0120737',
  8.6,
  8.9,
  null,
  null,
  null,
  '2026-06-21T12:07:57.000Z',
  false,
  false,
  false
),
(
  'media-властелин-колец-две-крепости-478',
  'Властелин колец: Две крепости',
  'The Lord of the Rings: The Two Towers',
  'movie',
  2002,
  './posters/властелин-колец-две-крепости-dae098a8.jpg',
  './posters/властелин-колец-две-крепости-dae098a8.jpg',
  'Братство распалось, но Кольцо Всевластья должно быть уничтожено. Фродо и Сэм вынуждены довериться Голлуму, который взялся провести их к вратам Мордора. Громадная армия Сарумана приближается: члены братства и их союзники готовы принять бой. Битва за Средиземье продолжается.',
  array['фэнтези', 'приключения', 'драма', 'боевик']::text[],
  array['Новая Зеландия', 'США']::text[],
  121,
  312,
  'tt0167261',
  8.6,
  8.8,
  null,
  null,
  null,
  '2026-06-21T12:07:58.000Z',
  false,
  false,
  false
),
(
  'media-властелин-колец-возвращение-короля-479',
  'Властелин колец: Возвращение короля',
  'The Lord of the Rings: The Return of the King',
  'movie',
  2003,
  './posters/властелин-колец-возвращение-короля-d1c374ff.jpg',
  './posters/властелин-колец-возвращение-короля-d1c374ff.jpg',
  'Повелитель сил тьмы Саурон направляет свою бесчисленную армию под стены Минас-Тирита, крепости Последней Надежды. Он предвкушает близкую победу, но именно это мешает ему заметить две крохотные фигурки — хоббитов, приближающихся к Роковой Горе, где им предстоит уничтожить Кольцо Всевластья.',
  array['фэнтези', 'приключения', 'драма', 'боевик']::text[],
  array['Новая Зеландия', 'США']::text[],
  122,
  3498,
  'tt0167260',
  8.7,
  9,
  null,
  null,
  null,
  '2026-06-21T12:07:59.000Z',
  false,
  false,
  false
),
(
  'media-хоббит-нежданное-путешествие-480',
  'Хоббит: Нежданное путешествие',
  'The Hobbit: An Unexpected Journey',
  'movie',
  2012,
  './posters/хоббит-нежданное-путешествие-6908426e.jpg',
  './posters/хоббит-нежданное-путешествие-6908426e.jpg',
  'Хоббит Бильбо Бэггинс пускается в грандиозный поход, целью которого является отвоевание утраченного королевства гномов Эребор у дракона Смауга. Совершенно неожиданно с хоббитом налаживает контакт волшебник Гэндальф Серый. Так Бильбо находит себя и присоединяется к компании тринадцати гномов, возглавляемых легендарным воином Торином Дубощитом. Их путешествие пройдёт через Дикий Край, предательские земли, населенные гоблинами и орками, смертоносными варгами, гигантскими пауками, меняющим шкуры народом и волшебниками.

И хотя их цель находится на Востоке, среди пустошей Одинокой Горы, сначала им придется выбраться из туннелей гоблинов, где Бильбо встретит существо, которое изменит его жизнь навсегда - Голлума.',
  array['фэнтези', 'приключения', 'боевик']::text[],
  array['Новая Зеландия', 'США']::text[],
  49051,
  278522,
  'tt0903624',
  8.1,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:08:00.000Z',
  false,
  false,
  false
),
(
  'media-хоббит-пустошь-смауга-481',
  'Хоббит: Пустошь Смауга',
  'The Hobbit: The Desolation of Smaug',
  'movie',
  2013,
  './posters/хоббит-пустошь-смауга-cc3077f8.jpg',
  './posters/хоббит-пустошь-смауга-cc3077f8.jpg',
  'Продолжение путешествия хоббита Бильбо Бэггинса, волшебника Гэндальфа и 13 отважных гномов. Их компания должна добраться до Одинокой горы. А там они встретятся с величайшей опасностью из всех — созданием куда более ужасающим, чем все их прошлые противники, чудовищем, которое проверит на прочность не только их мужество, но крепость их дружбы и правильность выбранного пути, — драконом Смаугом.',
  array['фэнтези', 'приключения', 'боевик']::text[],
  array['Новая Зеландия', 'США']::text[],
  57158,
  408876,
  'tt1170358',
  8,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:08:01.000Z',
  false,
  false,
  false
),
(
  'media-хоббит-битва-пяти-воинств-482',
  'Хоббит: Битва пяти воинств',
  'The Hobbit: The Battle of the Five Armies',
  'movie',
  2014,
  './posters/хоббит-битва-пяти-воинств-cfdf01a2.jpg',
  './posters/хоббит-битва-пяти-воинств-cfdf01a2.jpg',
  'Когда отряд из тринадцати гномов нанимал хоббита Бильбо Бэгинса в качестве взломщика и четырнадцатого, «счастливого», участника похода к Одинокой горе, Бильбо полагал, что его приключения закончатся, когда он выполнит свою задачу - найдет сокровище, которое так необходимо предводителю гномов Торину. Путешествие в Эребор, захваченное драконом Смаугом королевство гномов, оказалось еще более опасным, чем предполагали гномы и даже Гэндальф - мудрый волшебник, протянувший Торину и его отряду руку помощи.
В погоню за гномами устремилась армия орков, ведомых пробудившимся в руинах древним злом, а эльфы и люди, с которыми Бильбо и его товарищам пришлось иметь дело во время путешествия и которые пострадали от последствий желания гномов вернуть свой дом, предъявили права на щедрое вознаграждение - часть сокровищ Одинокой горы. Скоро неподалеку от Одинокой горы встретятся пять армий, и лишь кровопролитная битва определит результаты смелого гномьего похода.',
  array['фэнтези', 'приключения', 'боевик']::text[],
  array['Новая Зеландия', 'США']::text[],
  122917,
  694633,
  'tt2310332',
  7.9,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:08:02.000Z',
  false,
  false,
  false
),
(
  'media-три-богатыря-алеша-попович-и-тугарин-змей-483',
  'Три богатыря: Алеша Попович и Тугарин Змей',
  'Три богатыря: Алеша Попович и Тугарин Змей',
  'cartoon',
  2004,
  './posters/три-богатыря-алеша-попович-и-тугарин-змей-ab3c52f5.jpg',
  './posters/три-богатыря-алеша-попович-и-тугарин-змей-ab3c52f5.jpg',
  'Алеша Попович вместе с верными друзьями отправляется исправлять ошибку и противостоять Тугарину Змею.',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['Россия']::text[],
  null,
  818570,
  null,
  3.4,
  null,
  null,
  null,
  null,
  '2026-06-21T12:08:03.000Z',
  false,
  false,
  false
),
(
  'media-три-богатыря-добрыня-никитич-и-змей-горыныч-484',
  'Три богатыря: Добрыня Никитич и Змей Горыныч',
  'Три богатыря: Добрыня Никитич и Змей Горыныч',
  'cartoon',
  2006,
  './posters/три-богатыря-добрыня-никитич-и-змей-горыныч-98ee342e.jpg',
  './posters/три-богатыря-добрыня-никитич-и-змей-горыныч-98ee342e.jpg',
  'Добрыня Никитич и Елисей отправляются спасать княжну Забаву и сталкиваются с интригами, погонями и Змеем Горынычем.',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['Россия']::text[],
  null,
  818570,
  null,
  3.4,
  null,
  null,
  null,
  null,
  '2026-06-21T12:08:04.000Z',
  false,
  false,
  false
),
(
  'media-три-богатыря-илья-муромец-и-соловей-разбойник-485',
  'Три богатыря: Илья Муромец и Соловей Разбойник',
  'Три богатыря: Илья Муромец и Соловей Разбойник',
  'cartoon',
  2007,
  './posters/три-богатыря-илья-муромец-и-соловей-разбойник-b184e205.jpg',
  './posters/три-богатыря-илья-муромец-и-соловей-разбойник-b184e205.jpg',
  'Илья Муромец возвращает украденного коня и вместе с князем отправляется за Соловьем Разбойником.',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['Россия']::text[],
  null,
  818570,
  null,
  3.4,
  null,
  null,
  null,
  null,
  '2026-06-21T12:08:05.000Z',
  false,
  false,
  false
),
(
  'media-ми-ми-мишки-486',
  'Ми-ми-мишки',
  'Ми-ми-мишки',
  'series',
  2015,
  './posters/ми-ми-мишки-3aa915ee.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000016e40c5890a0331279d01a5729110/1344x756',
  'Веселые истории двух медвежат: Кеши и Белой Тучки. Вместе с героями этой веселой истории, дети будут познавать окружающий мир, знакомиться с самыми разнообразными предметами и тем, как они сделаны, ходить на рыбалку, лечить деревья, вести расследования, добывать мед, читать книжку и даже слетают на Луну!',
  array['мультфильм', 'комедия', 'детский']::text[],
  array['Россия']::text[],
  null,
  923524,
  'tt7037778',
  8.1,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:08:06.000Z',
  false,
  false,
  false
),
(
  'media-ералаш-487',
  'Ералаш',
  'Ералаш',
  'series',
  1974,
  './posters/ералаш-2e788242.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000016acb97bb7568667b801c60332079/1344x756',
  'Детский юмористический киножурнал радовал и продолжает радовать уже не одно поколение взрослых и детей. Добрый юмор и понятные житейские истории придутся по душе абсолютно всем.',
  array['детский', 'комедия', 'семейный']::text[],
  array['СССР', 'Россия']::text[],
  62967,
  258382,
  'tt5495796',
  7.3,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:08:07.000Z',
  false,
  false,
  false
),
(
  'media-непосредственно-каха-489',
  'Непосредственно Каха!',
  'Непосредственно Каха!',
  'movie',
  2020,
  './posters/непосредственно-каха-04ce60d9.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a0000018355ba3355fe196efccb8faac06c/1344x756',
  'Каха запал на прекрасную Софу, но, чтобы подобраться к её сердцу, ему нужно на своей «копейке» обогнать новенькую BMW Руслана и забить на дружбу с Серго. Что же выберет Каха – любовь или дружбу?',
  array['комедия']::text[],
  array['Россия']::text[],
  679694,
  1345717,
  'tt13486236',
  4.6,
  1.7,
  null,
  null,
  null,
  '2026-06-21T12:08:09.000Z',
  false,
  false,
  false
),
(
  'media-никто-2-490',
  'Никто 2',
  'Nobody 2',
  'movie',
  2025,
  './posters/никто-2-e29c8f8b.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a00000198ad4a9dbcde3418f9fb529b8547/1344x756',
  'Разобравшись с русской мафией и уничтожив денежный склад, Хатч расстроил влиятельных людей, деньги которых сгорели в огне. Теперь, чтобы компенсировать потери, он снова работает киллером. После очередного сложного заказа Хатч просит босса о передышке. Он хочет взять отпуск и побыть с семьёй, от которой снова отдалился. Приехав с роднёй в небольшой курортный городок, где он однажды побывал в детстве, мужчина наслаждается безмятежным отдыхом, не подозревая, что скоро неприятности найдут его и здесь.',
  array['боевик', 'комедия', 'криминал']::text[],
  array['США', 'Канада']::text[],
  1007734,
  5905715,
  'tt28996126',
  6.2,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:08:10.000Z',
  false,
  false,
  false
),
(
  'media-чип-и-дейл-спешат-на-помощь-491',
  'Чип и Дейл спешат на помощь',
  'Chip ''n Dale: Rescue Rangers',
  'cartoon',
  1989,
  './posters/чип-и-дейл-спешат-на-помощь-82fa0560.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000171f4a86c90d4053f5a82113cf0ac/1344x756',
  'Непоседы Чип и Дейл всегда знают, как оказаться в центре событий. Поиски пропавших драгоценностей, детективные расследования, погони и сражения с хитроумными злодеями — герои не боятся самых рискованных приключений! Взяв в свою «Команду Спасателей» изобретательницу Гаечку, здоровяка Рокфора и его юркого помощника Вжика, друзья смело бросают вызов преступникам.',
  array['мультфильм', 'комедия', 'детский', 'детектив', 'приключения', 'семейный']::text[],
  array['США']::text[],
  1615,
  95231,
  'tt0096557',
  8.2,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:08:11.000Z',
  false,
  false,
  false
),
(
  'media-убойные-каникулы-492',
  'Убойные каникулы',
  'Tucker and Dale vs Evil',
  'movie',
  2010,
  './posters/убойные-каникулы-fa692fc5.jpg',
  './posters/убойные-каникулы-fa692fc5.jpg',
  'Группа студентов отправляется за город, чтобы вовсю оторваться на выходных: попить пивка, поплавать в озере и подышать чистым воздухом. Но в первый же день они сталкиваются с неотёсанными фермерами Такером и Дейлом, которые тоже решили как следует оттянуться на природе. Приняв этих на вид диковатых, но безобидных ребят за злобных маньяков, горожане решают дать деревенщине жёсткий отпор.',
  array['комедия', 'триллер']::text[],
  array['Канада', 'Великобритания', 'США', 'Индия']::text[],
  46838,
  485261,
  'tt1465522',
  7.7,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:08:12.000Z',
  false,
  false,
  false
),
(
  'media-голодные-игры-сойка-пересмешница-часть-i-493',
  'Голодные игры: Сойка-пересмешница. Часть I',
  'The Hunger Games: Mockingjay - Part 1',
  'movie',
  2014,
  './posters/голодные-игры-сойка-пересмешница-часть-i-36c88733.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a00000171791c967b8ec26f4ba47c97a8bb/1344x756',
  '75-ые Голодные игры изменили все. Китнисс нарушила правила, и непоколебимое до той поры деспотичное правление Капитолия пошатнулось. У людей появилась надежда, и ее символ – Сойка-пересмешница. Теперь, чтобы освободить захваченного в плен Пита и защитить своих близких, Китнисс придется  сражаться в настоящих битвах и стать еще сильнее, чем на арене игр.',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array['США', 'Канада', 'Франция']::text[],
  null,
  661911,
  'tt1951265',
  6.3,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:08:13.000Z',
  false,
  false,
  false
),
(
  'media-малыш-на-драйве-494',
  'Малыш на драйве',
  'Baby Driver',
  'movie',
  2017,
  './posters/малыш-на-драйве-f7170715.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017acd7c32e6bf4c2ddb8d1f6d14eb/1344x756',
  'Молодой парень по прозвищу Малыш обожает стильную музыку, быструю езду и адреналин в крови. Вся его жизнь — это сплошные погони и перестрелки, ведь он работает водителем, который помогает бандитам скрыться с места преступления. Однажды он по-настоящему влюбляется и решает выйти из игры, но сначала ему предстоит выполнить ещё одно задание, которое вполне может оказаться последним в его жизни.',
  array['боевик', 'драма', 'криминал', 'музыка']::text[],
  array['Великобритания', 'США']::text[],
  339403,
  902939,
  'tt3890160',
  7.2,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:08:14.000Z',
  false,
  false,
  false
),
(
  'media-сестры-495',
  'Сёстры',
  'Сёстры',
  'movie',
  2001,
  './posters/сестры-bdbeca35.jpg',
  './posters/сестры-bdbeca35.jpg',
  'Дина и Света ненавидят друг друга, хотя в них течет одна кровь. После выхода отца из тюрьмы девочки оказываются в опасности и вынуждены держаться вместе.',
  array['драма', 'криминал']::text[],
  array['Россия']::text[],
  null,
  41109,
  'tt0284492',
  7.8,
  7,
  null,
  null,
  null,
  '2026-06-21T12:08:15.000Z',
  false,
  false,
  false
),
(
  'media-гачиакута-496',
  'Гачиакута',
  'Gachiakuta',
  'anime',
  2025,
  './posters/гачиакута-5fa00b21.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000019b27703cb1d92c9b032498872479/1344x756',
  'Мальчик Рудо и его приёмный отец живут в трущобах процветающего города вместе с бедняками, которые с трудом сводят концы с концами. Богачи, напротив, купаются в роскоши и сбрасывают весь мусор в так называемую Бездну. Однажды Рудо ложно обвиняют в убийстве отца и приговаривают к изгнанию в Бездну, где мусор породил жестоких монстров. Чтобы выжить, узнать правду и отомстить, парню придётся овладеть новой силой и присоединиться к группе, известной как Мусорщики, которая сражается с огромными мусорными существами.',
  array['аниме', 'мультфильм', 'фэнтези', 'боевик']::text[],
  array['Япония']::text[],
  256721,
  7524629,
  'tt32612521',
  8.5,
  null,
  null,
  null,
  null,
  '2026-06-21T12:08:16.000Z',
  false,
  false,
  false
),
(
  'media-майор-пэйн-498',
  'Майор Пэйн',
  'Major Payne',
  'movie',
  1995,
  './posters/майор-пэйн-bab79695.jpg',
  './posters/майор-пэйн-bab79695.jpg',
  'Майору никак не присвоят звание полковника по одной причине — он умеет только убивать, ни на что другое он просто не способен. Его отправляют в отставку. Работа в полиции ему не подходит, и наш герой умирает от скуки в одном из номеров отеля.
Неожиданно он получает предложение стать инструктором по военной подготовке в военном учебном заведении. О том, как сложатся отношения «жестокосердного солдафона» с юными подопечными и одной весьма привлекательной преподавательницей, рассказывает эта веселая комедия, полная забавных шуток и смешных ситуаций.',
  array['комедия', 'приключения']::text[],
  array['США']::text[],
  11008,
  21503,
  'tt0110443',
  7.6,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:08:18.000Z',
  false,
  false,
  false
),
(
  'media-котенок-по-имени-гав-499',
  'Котенок по имени Гав',
  'Котенок по имени Гав',
  'cartoon',
  1976,
  './posters/котенок-по-имени-гав-17fd8d5f.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a00000177909ecec14cddc4971ca43e81d8/1344x756',
  'Мультфильм рассказывает о приключениях котёнка с необычным именем Гав и его друга - щенка.',
  array['мультфильм', 'детский']::text[],
  array['СССР']::text[],
  null,
  392206,
  null,
  8.3,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:08:19.000Z',
  false,
  false,
  false
),
(
  'media-астерикс-и-обеликс-против-цезаря-500',
  'Астерикс и Обеликс против Цезаря',
  'Astérix & Obélix contre César',
  'cartoon',
  1999,
  './posters/астерикс-и-обеликс-против-цезаря-c1481dd3.jpg',
  './posters/астерикс-и-обеликс-против-цезаря-c1481dd3.jpg',
  'В 50-м году до нашей эры римский император Юлий Цезарь покорил Галлию. Только жители одной маленькой деревеньки отказались платить налоги римлянам, ведь у них есть волшебный напиток, с помощью которого они с лёгкостью могут дать отпор римским легионам. А однажды неразлучные друзья Астерикс и Обеликс и вовсе бросают вызов самому Цезарю.',
  array['фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['Франция', 'Италия', 'Германия']::text[],
  null,
  13845,
  'tt0133385',
  7.3,
  6,
  null,
  null,
  null,
  '2026-06-21T12:08:20.000Z',
  false,
  false,
  false
),
(
  'media-астерикс-и-обеликс-миссия-клеопатра-501',
  'Астерикс и Обеликс: Миссия Клеопатра',
  'Astérix & Obélix: Mission Cléopâtre',
  'cartoon',
  2002,
  './posters/астерикс-и-обеликс-миссия-клеопатра-b83b5f74.jpg',
  './posters/астерикс-и-обеликс-миссия-клеопатра-b83b5f74.jpg',
  'Царица Египта Клеопатра заключает пари с римским императором Юлием Цезарем о том, что её подданные смогут всего за три месяца построить для него грандиозный дворец. Великой чести руководить строительством удостоен архитектор Нумеробис, которого в случае неудачи бросят на растерзание крокодилам.

Понимая, что в такие краткие сроки ему не уложиться, Нумеробис вызывает на помощь своего старого друга друида Панорамикса с зельем, дарующим сверхъестественную силу. А весёлые и находчивые друзья Астерикс и Обеликс сопровождают старца в Египет, чтобы помешать проискам Цезаря, не желающего проигрывать Клеопатре.',
  array['фэнтези', 'комедия', 'приключения']::text[],
  array['Франция', 'Германия']::text[],
  2899,
  595,
  'tt0250223',
  7.3,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:08:21.000Z',
  false,
  false,
  false
),
(
  'media-астерикс-на-олимпийских-играх-502',
  'Астерикс на Олимпийских играх',
  'Astérix aux jeux olympiques',
  'cartoon',
  2008,
  './posters/астерикс-на-олимпийских-играх-db6c1650.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000016128547553e900752fad435848de/1344x756',
  '50 лет до Рождества Христова. Вся Галлия под властью Римской Империи. Но не стоит огорчаться: одна деревня продолжает героическое сопротивление. Ведь ее жители уверены в собственной непобедимости.
Уже несколько месяцев сердце Полюбвикса бьется только ради греческой принцессы Ирины. Но, увы, он не единственный претендент на ее руку. Большая политика предназначила Ирину в жены Бруту, сыну Юлия Цезаря. Чтобы оттянуть нежеланный брак и дать шанс Полюбвиксу Ирина объявляет, что выйдет замуж за победителя Олимпийских Игр. Итак, Полюбвикс приступает к тренировкам под руководством тех, кто быстрее, выше, сильнее, и, разумеется, умнее всех — Астерикса и Обеликса. А хитрый Брут — к разработке коварного плана.',
  array['фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['Франция', 'Германия', 'Испания', 'Италия', 'Бельгия']::text[],
  null,
  195319,
  'tt0463872',
  6.3,
  5.2,
  null,
  null,
  null,
  '2026-06-21T12:08:22.000Z',
  false,
  false,
  false
),
(
  'media-не-грози-южному-централу-попивая-сок-у-себя-в-квартале-503',
  'Не грози Южному Централу, попивая сок у себя в квартале',
  'Don''t Be a Menace to South Central While Drinking Your Juice in the Hood',
  'movie',
  1995,
  './posters/не-грози-южному-централу-попивая-сок-у-себя-в-квартале-b8967394.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a00000171ab240d090b2f338cfc13637fac/1344x756',
  'Молодой афроамериканец по прозвищу Пепельница переезжает в Южный Централ к своему отцу, где редкий молодой человек доживает до 18. Парень гордится, что они с отцом почти ровесники — он старше папы всего на два года. Гордится он и бабулей, которая в свои-то преклонные годы покуривает и не стесняется крепких словечек. Вскоре Пепельница встречается со своим кузеном по прозвищу Лок Дог. Этот парень — гангстер, в его личном арсенале не только пистолеты и автомат, но и ядерная боеголовка советского производства — для самозащиты, разумеется. Пепельница должен выбрать дальнейший путь в жизни: он может стать «просто хорошим чернокожим» или «опасным чернокожим», примкнув к банде своего кузена.',
  array['комедия', 'криминал']::text[],
  array['США']::text[],
  10607,
  4807,
  'tt0116126',
  7.2,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:08:23.000Z',
  false,
  false,
  false
),
(
  'media-как-витька-чеснок-в-з-л-ху-штыря-в-дом-инвалидов-504',
  'Как Витька Чеснок вёз Лёху Штыря в дом инвалидов',
  'Как Витька Чеснок вёз Лёху Штыря в дом инвалидов',
  'movie',
  2017,
  './posters/как-витька-чеснок-вез-леху-штыря-в-дом-инвалидов-f65658eb.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000016128d4efad2422af1f8aca9e300a/1344x756',
  '27-летний Витька Чеснок — парень с детдомовским прошлым, который мечтает сбежать от мешающих ему жить жены и сына. Недолюбленный в детстве и очерствевший сердцем парень встречает отца-уголовника, а теперь ещё и калеку. Витька решает отвезти отца в дом инвалидов, не подозревая, какие приключения и опасности ждут их по дороге.',
  array['драма', 'комедия', 'криминал']::text[],
  array['Россия']::text[],
  null,
  909700,
  'tt5837242',
  7.2,
  7.1,
  null,
  null,
  null,
  '2026-06-21T12:08:24.000Z',
  false,
  false,
  false
),
(
  'media-выход-8-505',
  'Выход 8',
  '8ban Deguchi',
  'movie',
  2025,
  './posters/выход-8-9c8d8d0c.jpg',
  'https://avatars.mds.yandex.net/get-ott/12808873/2a00000198f114633cd3691ca99d626bddaa/1344x756',
  'Молодой человек едет на работу в метро в час пик. Ему звонит бывшая девушка, и, пока он с ней разговаривает, понимает, что заблудился в переходах и никак не может выбраться. Парень обречён проходить один и тот же отрезок до тех пор, пока 8 раз подряд не выполнит простые условия: заметив аномалию, нужно повернуть назад, а если в переходе ничего не изменилось, просто идти вперёд.',
  array['ужасы', 'драма']::text[],
  array['Япония']::text[],
  1408208,
  7519616,
  'tt35222590',
  6.6,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:08:25.000Z',
  false,
  false,
  false
),
(
  'media-эйс-вентура-розыск-домашних-животных-506',
  'Эйс Вентура: Розыск домашних животных',
  'Ace Ventura: Pet Detective',
  'movie',
  1993,
  './posters/эйс-вентура-розыск-домашних-животных-442ff46b.jpg',
  './posters/эйс-вентура-розыск-домашних-животных-442ff46b.jpg',
  'Он - лучший в своем деле, единственный и неповторимый! Он - Эйс Вентура, детектив по розыску домашних любимцев. Когда таинственные злоумышленники похищают дельфина по кличке Снежинка, талисман местной футбольной команды «Дельфины», Эйс тут же приступает к работе, проявляя чудеса изобретательности.

История принимает странный оборот, когда похищают еще и ведущего игрока «Дельфинов». Теперь Эйсу приходится разыскивать сразу двух млекопитающих. Он сталкивается нос к носу с акулой-людоедом, спасает любимую команду и очаровывает женщин. Находится ли он под прикрытием, под огнем или под водой, он обязательно найдет то, что ищет!',
  array['комедия', 'детектив', 'приключения']::text[],
  array['США']::text[],
  3049,
  2868,
  'tt0109040',
  7.8,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:08:26.000Z',
  false,
  false,
  false
),
(
  'media-закрытая-школа-507',
  'Закрытая школа',
  'Закрытая школа',
  'series',
  2011,
  './posters/закрытая-школа-27606aec.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000194f5661d24d07aeb2871b3bce3fc/1344x756',
  'Сериал рассказывает о жизни учеников и преподавателей элитной школы-интерната, расположенной в старинной усадьбе посреди мрачного, но живописного леса. В их замкнутом мире все переживания намного острее, чем в обычной школьной жизни. Несмотря на внешнее благополучие, ребята, оторванные от родного дома, очень ранимы и зачастую одиноки. По вечерам они рассказывают друг другу страшные легенды о лесе, глухой стеной окружающем их школу, говорят, будто в нем водятся странные существа и бесследно пропадают люди. Очень скоро ребята узнают, что правда гораздо страшнее легенд.',
  array['триллер', 'драма', 'детектив']::text[],
  array['Россия']::text[],
  46104,
  566575,
  'tt2394140',
  6.3,
  5.4,
  null,
  null,
  null,
  '2026-06-21T12:08:27.000Z',
  false,
  false,
  false
),
(
  'media-особо-опасен-508',
  'Особо опасен',
  'Wanted',
  'movie',
  2008,
  './posters/особо-опасен-2f952ad6.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001612cda97de601769f9cf09ac2286/1344x756',
  'Уэсли Гибсон - зануда и нытик, целыми днями торчащий в офисе. Босс его унижает, подружка ему изменяет. Его жизнь катится в никуда! Обнаружив, что отец, бросивший его в раннем детстве, зверски убит, Уэс оказывается втянут в тайное общество киллеров под названием «Братство». За его обучение берется опытная наставница Фокс, и этот когда-то прикованный к офисному креслу хлюпик развивает молниеносную реакцию и сверхчеловеческую скорость. Однако уже на первом задании Уэсли понимает: есть вещи посложнее, чем вершить судьбы других людей. Это - набраться храбрости и стать хозяином собственной жизни.',
  array['фантастика', 'боевик', 'триллер', 'криминал']::text[],
  array['США', 'Германия', 'Россия']::text[],
  8909,
  259876,
  'tt0493464',
  6.8,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:08:28.000Z',
  false,
  false,
  false
),
(
  'media-лки-509',
  'Ёлки',
  'Ёлки',
  'movie',
  2010,
  './posters/елки-123a3823.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000016f3d47a12da86a0fe563c7ef4bbf/1344x756',
  'Новогодние события происходят в 11 городах: Калининграде, Казани, Перми, Уфе, Бавлах, Екатеринбурге, Красноярске, Якутске, Новосибирске, Санкт-Петербурге и Москве. Герои фильма — таксист и поп-дива, бизнесмен и актер, сноубордист и лыжник, студент и пенсионерка, пожарный и директриса, вор и милиционер, гастарбайтер и президент России. Все они оказываются в самый канун Нового года в очень непростой ситуации, выйти из которой им поможет только чудо… или Теория шести рукопожатий, согласно которой каждый человек на земле знает другого через шесть знакомых.',
  array['комедия']::text[],
  array['Россия']::text[],
  57809,
  493768,
  'tt1782568',
  6.9,
  6.2,
  null,
  null,
  null,
  '2026-06-21T12:08:29.000Z',
  false,
  false,
  false
),
(
  'media-трансформеры-эпоха-истребления-510',
  'Трансформеры: Эпоха истребления',
  'Transformers: Age of Extinction',
  'movie',
  2014,
  './posters/трансформеры-эпоха-истребления-9faa4fa5.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a000001966cc2d453398d9741b70c54a039/1344x756',
  'Автоботы и десептиконы оставили человечество собирать планету по кусочкам. Между тем группа могущественных бизнесменов и гениальных ученых пытается извлечь уроки из ошибок прошлого и улучшить технологии до такой степени, чтобы выйти за пределы контролируемого ранее. В это время другая группа — древних мощных трансформеров — берет Землю под прицел.',
  array['фантастика', 'боевик', 'приключения']::text[],
  array['США', 'Китай', 'Гонконг']::text[],
  91314,
  651857,
  'tt2109248',
  6.1,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:08:30.000Z',
  false,
  false,
  false
),
(
  'media-закон-каменных-джунглей-511',
  'Закон каменных джунглей',
  'Закон каменных джунглей',
  'series',
  2015,
  './posters/закон-каменных-джунглей-8cfef201.jpg',
  'https://avatars.mds.yandex.net/get-ott/13074011/2a0000019b0329cd02175a28617c67c94289/1344x756',
  'Четыре парня с окраины Москвы решают заработать легких денег — перегнать машину с краденым грузом внутри. Не особо задумываясь о последствиях, они совершают одну ошибку за другой, и очень скоро у них появляются серьезные проблемы с законом. Теперь пути назад нет и, чтобы выжить, четырем друзьям нужно во что бы то ни стало держаться вместе.',
  array['драма', 'криминал']::text[],
  array['Россия']::text[],
  null,
  694406,
  null,
  7.8,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:08:31.000Z',
  false,
  false,
  false
),
(
  'media-f1-512',
  'F1',
  'F1',
  'movie',
  2025,
  './posters/f1-84673350.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000197b164eecfd7e495d4b6f5b8c9ae/1344x756',
  'В 1990-х Сонни Хейс был восходящей звездой «Формулы-1», но после серьёзной аварии ушёл из большого спорта. 30 лет спустя Сонни живёт в трейлере и зарабатывает участием в различных гонках и чемпионатах. Однажды к нему обращается старый друг Рубен Сервантес, тоже в прошлом гонщик, а ныне владелец гоночной команды-аутсайдера, с просьбой присоединиться к ним в качестве второго пилота и наставника для молодого многообещающего новичка.',
  array['драма', 'спорт']::text[],
  array['США']::text[],
  911430,
  5003510,
  'tt16311594',
  8.2,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:08:32.000Z',
  false,
  false,
  false
),
(
  'media-мир-дружба-жвачка-513',
  'Мир! Дружба! Жвачка!',
  'Мир! Дружба! Жвачка!',
  'series',
  2020,
  './posters/мир-дружба-жвачка-6d79a28b.jpg',
  './posters/мир-дружба-жвачка-6d79a28b.jpg',
  '1993 год. 14-летний Санька Рябинин живет обычной подростковой жизнью — ищет приключений, пытается защититься от районных хулиганов, не хочет ходить в музыкалку, не знает, как общаться с девчонками и с кого из взрослых брать пример. С одной стороны — честная, но бедная семья с мамой-продавщицей и папой-доцентом, с другой — опасный мир афганского ветерана дяди Алика. А тут еще и девочка Женя, знакомство с которой запускает череду необратимых событий, которые навсегда изменят жизнь Саньки, его семьи и друзей Вовки и Илюши. Сбегая от хулиганов, Санька угоняет машину местного криминального авторитета, и в один миг все меняется. А дальше - разборки со взрослыми кавказцами, ровесниками, между родителями на кухне, со своими пацанами и так далее. Сможет ли он справиться со всеми проблемами, оказавшись в самом сердце событий? Удастся ли ему спасти семью и понять, наконец, – кто он? Понятно только одно – жизнь уже не будет такой как прежде. Ни для самого Саньки, ни для всей России.',
  array['драма', 'комедия', 'криминал']::text[],
  array['Россия']::text[],
  102359,
  1306638,
  'tt12273194',
  7.9,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:08:33.000Z',
  false,
  false,
  false
),
(
  'media-проект-конец-света-514',
  'Проект «Конец света»',
  'Project Hail Mary',
  'movie',
  2026,
  './posters/проект-конец-света-7f10ed25.jpg',
  './posters/проект-конец-света-7f10ed25.jpg',
  'Микробиолог Райленд Грейс просыпается на космическом корабле, не помня ни себя, ни свою миссию. Мужчина выясняет, что он — единственный выживший из экипажа, отправленного к звезде Тау Кита в поисках спасения от катастрофы на Земле. Чтобы найти решение, Райленду предстоит положиться на свои обширные научные знания и изобретательность, но, возможно, ему не придётся искать в одиночку.',
  array['фантастика', 'триллер', 'драма', 'приключения', 'комедия']::text[],
  array['США']::text[],
  687163,
  1382256,
  'tt12042730',
  8.4,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:08:34.000Z',
  false,
  false,
  false
),
(
  'media-я-начало-515',
  'Я – начало',
  'I Origins',
  'movie',
  2014,
  './posters/я-начало-69f967a6.jpg',
  './posters/я-начало-69f967a6.jpg',
  'Молекулярный биолог и его ассистентка делают потрясающее открытие, способное коренным образом изменить современное общество.',
  array['фантастика', 'драма', 'мелодрама', 'детектив']::text[],
  array['США']::text[],
  244267,
  761933,
  'tt2884206',
  7.6,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:08:35.000Z',
  false,
  false,
  false
),
(
  'media-голодные-игры-сойка-пересмешница-часть-ii-516',
  'Голодные игры: Сойка-пересмешница. Часть II',
  'The Hunger Games: Mockingjay - Part 2',
  'movie',
  2015,
  './posters/голодные-игры-сойка-пересмешница-часть-ii-3458e209.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a00000171791db8fa8bcb253f36d7b87b43/1344x756',
  'Китнисс Эвердин и своенравный Пит Мелларк пытаются выиграть гражданскую войну между самодержавным Капитолием и истерзанными войной районами Панема...',
  array['фантастика', 'боевик', 'триллер', 'приключения']::text[],
  array['США', 'Германия', 'Канада', 'Франция']::text[],
  131634,
  663715,
  'tt1951266',
  6.6,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:08:36.000Z',
  false,
  false,
  false
),
(
  'media-блеф-517',
  'Блеф',
  'Bluff storia di truffe e di imbroglioni',
  'movie',
  1976,
  './posters/блеф-81209c06.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017b069778bba3566cd9d6daf65732/1344x756',
  'Белль Дюк имеет старые счеты с Филиппом Бэнгом, который отбывает свой срок за решёткой. Для того чтобы поквитаться с Филиппом, Белль Дюк вступает в сговор с другим аферистом по имени Феликс, чтобы тот организовал побег Филиппа Бэнга из тюрьмы. Побег удаётся, но парочка заодно обманывает и Белль Дюк, исчезнув прямо из-под её носа. Выясняется, что и Филипп Бэнг, в свою очередь, не прочь отомстить Белль Дюк. Для этого он задумывает грандиозную мистификацию, сродни покерному блефу.',
  array['комедия', 'криминал']::text[],
  array['Италия']::text[],
  null,
  10179,
  'tt0076101',
  8.2,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:08:37.000Z',
  false,
  false,
  false
),
(
  'media-адреналин-2-высокое-напряжение-518',
  'Адреналин: Высокое напряжение',
  'Crank: High Voltage',
  'movie',
  2009,
  './posters/адреналин-высокое-напряжение-5670d6d3.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000161288d7582b3f721b66700f127b6/1344x756',
  'Он умер, но обещал вернуться! Не чудом, а благодаря современным медицинским технологиям Чев Челиос получает новое сердце и новый шанс отомстить своим врагам. Теперь для постоянной подзарядки ему требуется электрический разряд мощностью не меньше, чем у автомобильного аккумулятора.',
  array['криминал', 'боевик', 'комедия', 'фэнтези']::text[],
  array['США']::text[],
  null,
  397541,
  'tt1121931',
  6.3,
  6.1,
  null,
  null,
  null,
  '2026-06-21T12:08:38.000Z',
  false,
  false,
  false
),
(
  'media-девять-519',
  'Девять',
  '9',
  'movie',
  2009,
  './posters/девять-0be21b6e.jpg',
  './posters/девять-0be21b6e.jpg',
  'В страшной войне человечество было уничтожено механическими монстрами. Перед тем как погибнуть, профессор, придумавший умные машины, сшил девять тряпичных кукол, вложив в них человеческие души. Девять героев, девять характеров, которым суждено изменить мир.',
  array['мультфильм', 'фантастика', 'боевик', 'драма', 'приключения']::text[],
  array['США', 'Германия']::text[],
  null,
  180577,
  'tt0472033',
  7.4,
  7,
  null,
  null,
  null,
  '2026-06-21T12:08:39.000Z',
  false,
  false,
  false
),
(
  'media-элвин-и-бурундуки-2-520',
  'Элвин и бурундуки 2',
  'Alvin and the Chipmunks: The Squeakquel',
  'movie',
  2009,
  './posters/элвин-и-бурундуки-2-d011f705.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a00000199d30ab48dbd1c9f9e12def5b448/1344x756',
  'Три веселых бурундука Элвин, Саймон и Теодор поступают в школу, где их ждут новые приключения. Им придется столкнуться с множеством преград со стороны одноклассников. Но самое главное — их ожидает соперничество с новой музыкальной командой, состоящей из девочек-бурундуков, которое впоследствии перерастет в большую дружбу.',
  array['семейный', 'комедия', 'музыка']::text[],
  array['США']::text[],
  null,
  414987,
  'tt1231580',
  5.7,
  4.6,
  null,
  null,
  null,
  '2026-06-21T12:08:40.000Z',
  false,
  false,
  false
),
(
  'media-элвин-и-бурундуки-3-521',
  'Элвин и бурундуки 3',
  'Alvin and the Chipmunks: Chipwrecked',
  'movie',
  2011,
  './posters/элвин-и-бурундуки-3-9ebdd0d6.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a00000199d2ed266f3ece921c16d94922c6/1344x756',
  'Приключения троицы непоседливых братьев-бурундуков продолжаются! Элвин, Саймон и Теодор, а также их подруги бурундушки, просто не могут оставаться в стороне от различного рода неприятностей, сулящих им новые увлекательные приключения. Что на этот раз уготовила судьба нашим героям, и как они выпутаются из очередной передряги — вот в чём вопрос.',
  array['семейный', 'комедия', 'музыка']::text[],
  array['США']::text[],
  null,
  502036,
  'tt1615918',
  5.3,
  4.4,
  null,
  null,
  null,
  '2026-06-21T12:08:41.000Z',
  false,
  false,
  false
),
(
  'media-луна-2112-522',
  'Луна 2112',
  'Moon',
  'movie',
  2009,
  './posters/луна-2112-ad20de28.jpg',
  './posters/луна-2112-ad20de28.jpg',
  'Контракт Сэма подходит к концу: он провел три года на Луне, следя за автоматизированной станцией по добыче редкого газа. Три долгих года в полном одиночестве, если не считать говорящего робота ГЕРТИ, могут изменить любого. За две недели до возвращения на Землю Сэм встречает сменщика.',
  array['фантастика', 'драма']::text[],
  array['Великобритания', 'США']::text[],
  null,
  406671,
  'tt1182345',
  7.6,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:08:42.000Z',
  false,
  false,
  false
),
(
  'media-спасатели-малибу-523',
  'Спасатели Малибу',
  'Baywatch',
  'movie',
  2017,
  './posters/спасатели-малибу-6121c346.jpg',
  './posters/спасатели-малибу-6121c346.jpg',
  'Группа пляжных спасателей во главе с лейтенантом Митчем Бьюкэнноном работает на побережье Флориды. Они всегда на посту и',
  array['комедия', 'боевик', 'детектив', 'криминал']::text[],
  array['США', 'Великобритания', 'Китай']::text[],
  null,
  468940,
  'tt1469304',
  5.9,
  5.5,
  null,
  null,
  null,
  '2026-06-21T12:08:43.000Z',
  false,
  false,
  false
),
(
  'media-ч-рная-дыра-524',
  'Чёрная дыра',
  'Pitch Black',
  'movie',
  1999,
  './posters/черная-дыра-9e068396.jpg',
  './posters/черная-дыра-9e068396.jpg',
  'В недалеком будущем космический корабль терпит бедствие, в результате чего совершает вынужденную посадку на отдаленной планете. Погибает капитан, часть команды и большинство пассажиров. Выжившие оказываются на жуткой и безжизненной планете, которая имеет три солнца, из-за чего все живое гибнет в этой пустыне. Но как только планета погружается в темноту, появляются какие-то странные существа.',
  array['ужасы', 'фантастика', 'боевик']::text[],
  array['США']::text[],
  null,
  785,
  'tt0134847',
  7.6,
  7,
  null,
  null,
  null,
  '2026-06-21T12:08:44.000Z',
  false,
  false,
  false
),
(
  'media-мы-из-будущего-525',
  'Мы из будущего',
  'Мы из будущего',
  'movie',
  2008,
  './posters/мы-из-будущего-9ad96f8e.jpg',
  './posters/мы-из-будущего-9ad96f8e.jpg',
  'Действие картины разворачивается в двух временных пластах: в наши дни и в годы войны, во время тяжелых оборонительных боев августа 1942-го. Главные герои ленты – четверо «черных следопытов» (людей этой сомнительной профессии называют также «черными копателями») – Борман, Череп, Чуха и Спирт. Они ведут раскопки в тех местах, где когда-то шли бои, чтобы потом продать найденные медали, ордена, документы и немецкое оружие.

Дело это выгодное, но опасное. Однажды на месте раскопок начинает происходить нечто странное: в найденных солдатских книжках, принадлежавших погибшим солдатам Красной армии, вдруг обнаруживаются фотографии самих «следопытов». Пытаясь прийти в себя, коллеги по «черному» бизнесу отправляются купаться в озере и... попадают в 1942 год. В самый разгар тяжелых боев.',
  array['фантастика', 'боевик', 'приключения', 'военный', 'история']::text[],
  array['Россия']::text[],
  null,
  260315,
  'tt1192431',
  7.2,
  6.6,
  null,
  null,
  null,
  '2026-06-21T12:08:45.000Z',
  false,
  false,
  false
),
(
  'media-воронины-526',
  'Воронины',
  'Воронины',
  'series',
  2009,
  './posters/воронины-999bf2ea.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000017f4e361b0f097eb4e18d531e7d75/1344x756',
  'Действие сериала разворачивается в одной московской «сталинке», где в трехкомнатной квартире живет молодая семья. Главные герои - молодая женщина Вера, ее муж Костя, два полугодовалых сына-близнеца и пятилетняя дочурка. Вроде бы, полная идиллия, жизнь удалась. Но не тут-то было! Ведь на одной с ними лестничной клетке живут родители Кости, которые и создают массу проблем нашим героям.',
  array['комедия', 'приключения']::text[],
  array['Россия']::text[],
  46078,
  479090,
  'tt2301092',
  5.4,
  4.7,
  null,
  null,
  null,
  '2026-06-21T12:08:46.000Z',
  false,
  false,
  false
),
(
  'media-после-нашей-эры-528',
  'После нашей эры',
  'After Earth',
  'movie',
  2013,
  './posters/после-нашей-эры-38d2b75f.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001773bc5f7d7c12377d23e681ddc8c/1344x756',
  'События разворачиваются через тысячу лет после катастрофы, вынудившей человечество покинуть Землю. Новым домом становится планета под названием Нова Прайм. Легендарный генерал Сайфер Рейдж возвращается с очередного боевого задания в семью, которая раньше обходилась без его родительского внимания, чтобы стать отцом своему 13-летнему сыну Китаю. Во время астероидной бури летательный аппарат с папой и сыном на борту терпит крушение, падая на незнакомую и опасную Землю. И пока Рейдж-старший на последнем дыхании лежит среди обломков своего корабля, сын должен пересечь враждебный ландшафт, чтобы запустить их спасательный маячок. Ни о чем большем юноша в своей жизни не мечтал: быть солдатом, как отец, — его заветное желание. Сегодня у него есть шанс проявить себя.',
  array['фантастика', 'приключения', 'боевик']::text[],
  array['США']::text[],
  null,
  577285,
  'tt1815862',
  5.8,
  4.8,
  null,
  null,
  null,
  '2026-06-21T12:08:48.000Z',
  false,
  false,
  false
),
(
  'media-хроники-нарнии-лев-колдунья-и-волшебный-шкаф-529',
  'Хроники Нарнии: Лев, колдунья и волшебный шкаф',
  'The Chronicles of Narnia: The Lion, the Witch and the Wardrobe',
  'movie',
  2005,
  './posters/хроники-нарнии-лев-колдунья-и-волшебный-шкаф-baaefc52.jpg',
  './posters/хроники-нарнии-лев-колдунья-и-волшебный-шкаф-baaefc52.jpg',
  'Четверых детей родители отправляют из Лондона в деревню, к старому профессору - другу семьи. В его доме дети обнаруживают таинственный платяной шкаф, посредством которого они попадают в сказочную страну Нарнию, где обитают фантастические люди, животные и существа.

Оказывается, что эта мирная страна находится под властью злой Колдуньи, из-за которой там - вечная зима. Дети должны помочь царю Аслану (Великому Льву, который много столетий назад основал Нарнию) победить Колдунью, разрушить заклинания и освободить жителей Нарнии… ',
  array['фэнтези', 'приключения', 'семейный']::text[],
  array['Великобритания', 'США']::text[],
  411,
  48162,
  'tt0363771',
  7.3,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:08:49.000Z',
  false,
  false,
  false
),
(
  'media-хроники-нарнии-принц-каспиан-530',
  'Хроники Нарнии: Принц Каспиан',
  'The Chronicles of Narnia: Prince Caspian',
  'movie',
  2008,
  './posters/хроники-нарнии-принц-каспиан-b847b248.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a00000169056873851602b2e142feaa5868/1344x756',
  'Люси, Сьюзен, Эдмунд и Питер Пэванси возвращаются в Нарнию. В Англии прошло совсем немного времени, а в Нарнии – века. Страна находится под гнетом зловещего короля Мираза. Но у волшебного королевства есть надежда – юный принц Каспиан. Чтобы помочь Каспиану отвоевать трон, четверо героев собирают армию мифических существ во главе с Асланом – основателем и покровителем Нарнии...',
  array['фэнтези', 'боевик', 'приключения', 'семейный']::text[],
  array['США', 'Польша', 'Словения', 'Чехия', 'Великобритания', 'Австралия', 'Германия', 'Гватемала', 'Новая Зеландия']::text[],
  null,
  243596,
  'tt0499448',
  6.9,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:08:50.000Z',
  false,
  false,
  false
),
(
  'media-шаг-вперед-2-улицы-531',
  'Шаг вперед 2: Улицы',
  'Step Up 2: The Streets',
  'movie',
  2008,
  './posters/шаг-вперед-2-улицы-101d40d8.jpg',
  './posters/шаг-вперед-2-улицы-101d40d8.jpg',
  'Уличная танцовщица Энди поступает в Мэрилендскую школу искусств. Занимаясь там, она стремится доказать, что ни в чем не уступает другим студентам. Познакомившись с лучшим учеником школы Чейзом, Энди создает коллектив, который должен выступать в подпольном танцевальном состязании «Улицы». В итоге Энди удается реализовать свои мечты и справиться с одолевающими ее проблемами...',
  array['драма', 'мелодрама', 'музыка']::text[],
  array['США']::text[],
  8328,
  310960,
  'tt1023481',
  7.3,
  6.2,
  null,
  null,
  null,
  '2026-06-21T12:08:51.000Z',
  false,
  false,
  false
),
(
  'media-шаг-вперед-3d-532',
  'Шаг вперед 3D',
  'Step Up 3D',
  'movie',
  2010,
  './posters/шаг-вперед-3d-17851de6.jpg',
  './posters/шаг-вперед-3d-17851de6.jpg',
  'Некоторые учатся танцевать, другие рождены для этого. Люк ищет танцоров для своей команды «Пираты». В конце концов, ему встречаются Лось, поступивший в Нью-Йоркский университет, и Натали, загадочная танцовщица, которая никогда не проигрывает. Смогут ли они превзойти лучших мировых танцоров?',
  array['драма', 'мелодрама', 'музыка']::text[],
  array['США']::text[],
  null,
  409185,
  'tt1193631',
  7.2,
  6.2,
  null,
  null,
  null,
  '2026-06-21T12:08:52.000Z',
  false,
  false,
  false
),
(
  'media-шаг-вперед-4-533',
  'Шаг вперед 4',
  'Step Up Revolution',
  'movie',
  2012,
  './posters/шаг-вперед-4-a870c790.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000016249a82a457e5754ce4b5c4399de/1344x756',
  'Эмили, дочь влиятельного бизнесмена, приезжает в Майами с намерением стать профессиональной танцовщицей, но влюбляется в Шона — юношу, чья танцевальная труппа устраивает музыкальные флэшмобы. Команда, называющаяся MOB, участвует в конкурсе с внушительным денежным призом, а тем временем отец Эмили намеревается перестроить родной район ребят и переселить тысячи людей. Эмили присоединяется к танцорам и организует протестные мобы, рискуя не осуществить мечту, но сражаясь за нечто более важное.',
  array['драма', 'мелодрама', 'музыка']::text[],
  array['США']::text[],
  null,
  576451,
  'tt1800741',
  7.1,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:08:53.000Z',
  false,
  false,
  false
),
(
  'media-призрачный-гонщик-2-534',
  'Призрачный гонщик 2',
  'Ghost Rider: Spirit of Vengeance',
  'movie',
  2012,
  './posters/призрачный-гонщик-2-d03f408b.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a00000161288c8d540c3871c3faf720ec46/1344x756',
  'Действие разворачивается на задворках Восточной Европы, где главный герой Джонни Блэйз безуспешно пытается справиться со своим проклятием. Его принимает на работу некая секта, которая пытается заполучить дьявола в тот момент, когда он пытается вселиться в тело его смертного сына на дне рождения мальчика.',
  array['фэнтези', 'боевик', 'триллер']::text[],
  array['ОАЭ', 'США']::text[],
  null,
  391886,
  'tt1071875',
  5,
  4.4,
  null,
  null,
  null,
  '2026-06-21T12:08:54.000Z',
  false,
  false,
  false
),
(
  'media-кунг-фу-панда-2-535',
  'Кунг-фу Панда 2',
  'Kung Fu Panda 2',
  'cartoon',
  2011,
  './posters/кунг-фу-панда-2-d7387d6d.jpg',
  './posters/кунг-фу-панда-2-d7387d6d.jpg',
  'Панда По наконец-то исполнит свою мечту, станет Воином Дракона и со своими друзьями и мастерами кунг-фу — Неистовой Пятеркой — защитит Великую Долину от страшного злодея и его легиона...',
  array['мультфильм', 'фэнтези', 'боевик', 'драма', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Канада']::text[],
  null,
  427878,
  'tt1302011',
  7.4,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:08:55.000Z',
  false,
  false,
  false
),
(
  'media-марти-великолепный-536',
  'Марти Великолепный',
  'Marty Supreme',
  'movie',
  2025,
  './posters/марти-великолепный-73e67765.jpg',
  'https://avatars.mds.yandex.net/get-ott/12808873/2a0000019b22419e51abf82f68b73bc447ee/1344x756',
  '1952 год, Нью-Йорк. 23-летний амбициозный и талантливый игрок в настольный теннис Марти Маузер временно работает продавцом обуви, чтобы накопить денег на билет в Великобританию. Его цель — стать первым американцем, который выиграет турнир British Open. Начальник предлагает ему должность менеджера, но парень грезит лишь чемпионским титулом и славой. Узнав, что босс уехал на выходные за город и не оставил ему месячное жалование, Марти, угрожая пистолетом, заставляет коллегу выдать из сейфа положенные ему деньги, бросает работу и отправляется в Лондон для участия в турнире.',
  array['драма', 'спорт']::text[],
  array['США']::text[],
  1317288,
  6290547,
  'tt32916440',
  7.2,
  7.8,
  null,
  null,
  null,
  '2026-06-21T12:08:56.000Z',
  false,
  false,
  false
),
(
  'media-винкс-537',
  'Клуб Винкс – Школа волшебниц',
  'Winx Club',
  'movie',
  2004,
  './posters/клуб-винкс-школа-волшебниц-d1910964.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a00000190e0385feedfe1db591acb6f489d/1344x756',
  'В волшебном мире Магикс есть школа Алфея, где юных фей обучают магическим навыкам. Пять учениц этой школы не только ходят в кафе после уроков и встречаются с мальчиками, но и защищают свою планету от зла. Каждая из фей обладает особенным характером. Техна может разобраться с любой электроникой, тихая и застенчивая Флора любит растения, Муза – настоящая меломанка, блондинка Стела – наследная принцесса планеты Солярия, а Блум является признанным лидером в этой компании. Клуб Винкс был создан феями в знак вечной дружбы и взаимной помощи. У каждой юной волшебницы есть миниатюрные помощницы – пикси.

Жизнь участниц Клуба Винкс была бы простой и беззаботной, если бы не злые Айси, Дарси и Сторми, три ведьмы из Клуба Трикс, которые пытаются подчинить себе мир. Их козни доставят немало хлопот феям, но только искренняя дружба и взаимовыручка поможет Клубу Винкс победить зло.',
  array['мультфильм', 'фэнтези', 'детский', 'комедия', 'приключения']::text[],
  array['Италия', 'США', 'Тайвань', 'Австралия', 'Китай', 'Канада', 'Германия', 'Филиппины', 'Корея Северная', 'Испания', 'Индия', 'Франция', 'Великобритания']::text[],
  1848,
  320024,
  'tt0421482',
  4.7,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:08:57.000Z',
  false,
  false,
  false
),
(
  'media-вольт-538',
  'Вольт',
  'Bolt',
  'cartoon',
  2008,
  './posters/вольт-10ee8ad2.jpg',
  './posters/вольт-10ee8ad2.jpg',
  'Пес по кличке «Вольт» всю жизнь снимается в телевизионном сериале, где его герой – суперпес – спасает человечество, используя свои необычайные способности. Сам Вольт свято верит в собственную неуязвимость, невероятную силу, и умение летать. Когда же судьба забрасывает его в полный опасностей Большой Город, он все еще думает, что это – лишь очередной эпизод телесериала...',
  array['мультфильм', 'фантастика', 'драма', 'комедия', 'криминал', 'приключения', 'семейный']::text[],
  array['США', 'Япония']::text[],
  null,
  70955,
  'tt0397892',
  7.3,
  6.8,
  null,
  null,
  null,
  '2026-06-21T12:08:58.000Z',
  false,
  false,
  false
),
(
  'media-глухарь-539',
  'Глухарь',
  'Глухарь',
  'series',
  2008,
  './posters/глухарь-ed23646d.jpg',
  './posters/глухарь-ed23646d.jpg',
  'Жизнь не очень ласкова к молодому следователю, капитану Сергею Глухареву, и его приятелю, сотруднику районного ДПС Денису Антошину, которые дружат с детства. Тяжелая и порой опасная работа, маленькая зарплата, неустроенная личная жизнь явно изменили их не в лучшую сторону. Но они сохраняют способность оставаться людьми и не терять присутствия духа в любой ситуации. 

И главное — они, как умеют, продолжают выполнять свою тяжелую работу — охраняют правопорядок и законность, защищают общество, которое, порой, не проявляет к скромным сотрудникам милиции совершенно никакой благодарности.',
  array['детектив', 'драма', 'криминал', 'комедия']::text[],
  array['Россия']::text[],
  null,
  431219,
  'tt1476589',
  8.1,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:08:59.000Z',
  false,
  false,
  false
),
(
  'media-superперцы-540',
  'SuperПерцы',
  'Superbad',
  'movie',
  2007,
  './posters/superперцы-240fa148.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a000001712bfbf65c7c046f9e62f605ed8d/1344x756',
  'Сквозь огонь и водку трое приятелей спешат на вечеринку года в честь прощания с девственностью. Как взять на абордаж красотку-одноклассницу, подорвать полицейскую машину и остаться в живых?..',
  array['комедия']::text[],
  array['США']::text[],
  null,
  261363,
  'tt0829482',
  7,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:09:00.000Z',
  false,
  false,
  false
),
(
  'media-дом-541',
  'Дом',
  'Home',
  'cartoon',
  2015,
  './posters/дом-4a63bbab.jpg',
  './posters/дом-4a63bbab.jpg',
  'Захватив Землю, раса пришельцев Був тут же начинает заниматься её благоустройством и реорганизацией. «Не стоит благодарности!» - заявляют они землянам, собранным на время в отдельных резервациях. Находчивая и рисковая девочка по имени Дар, чудом сбежавшая от пришельцев, встречает на своем пути самого раздражающе-оптимистичного Бува по имени О, отвергнутого своими инопланетными собратьями. Этой, казалось бы, несовместимой парочке предстоит спасти нашу планету. Хорошо, что у них есть летающий автомобиль!',
  array['мультфильм', 'фантастика', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Индия']::text[],
  null,
  688384,
  null,
  7.3,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:09:01.000Z',
  false,
  false,
  false
),
(
  'media-артур-и-месть-урдалака-542',
  'Артур и месть Урдалака',
  'Arthur et la vengeance de Maltazard',
  'movie',
  2009,
  './posters/артур-и-месть-урдалака-3e4d2b5a.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a00000162f2d63f1e40b948b3598f017bb7/1344x756',
  'Артур в восторге, потому что этой ночью заканчивается десятый лунный цикл, и он, наконец, сможет вернуться в страну минипутов, чтобы воссоединиться с Селенией. В деревне минипуты приготовили в его честь роскошный банкет, а маленькая принцесса облачилась в платье из лепестков роз. Но отец Артура именно в этот долгожданный день объявляет, что их каникулы у бабушки заканчиваются. Они уже собираются уезжать, как вдруг паук кладёт в ладонь Артуру рисунок, на котором написано слово S.O.S. Сомнений нет — Селении угрожает опасность!

Артур, не задумываясь, летит к ней на выручку. Даже если столкнуться с войсками Креба, нового тирана Семи Королевств, спасти Бархлюша, сражаться с крысами, лягушками и волосатыми пауками… Лишь для того, чтобы, попав в деревушку минипутов, узнать, что они вовсе не просили его о помощи! Но кто же мог приготовить такую изобретательную ловушку нашему юному герою?',
  array['мультфильм', 'фэнтези', 'приключения', 'семейный']::text[],
  array['Франция']::text[],
  26505,
  278222,
  'tt0940657',
  5.9,
  5.3,
  null,
  null,
  null,
  '2026-06-21T12:09:02.000Z',
  false,
  false,
  false
),
(
  'media-артур-и-война-двух-миров-543',
  'Артур и война двух миров',
  'Arthur 3: la guerre des deux mondes',
  'movie',
  2010,
  './posters/артур-и-война-двух-миров-f3aef694.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a000001612c9967fd148d6e5c9e6f8b2502/1344x756',
  'Какие дьявольские планы собирается осуществить Урдалак, чтобы завоевать новый мир, в который он переместился обманным путем? Как Артур, оставшийся в мире минипутов, сможет победить жестокого Ужасного У? Чтобы сохранить любовь прекрасной принцессы Селении, Артур готов вступить в бой против Урдалака как в мире минипутов, так и в большом реальном мире...',
  array['мультфильм', 'фэнтези', 'приключения', 'семейный']::text[],
  array['Франция']::text[],
  null,
  278221,
  'tt0940656',
  6,
  5.5,
  null,
  null,
  null,
  '2026-06-21T12:09:03.000Z',
  false,
  false,
  false
),
(
  'media-need-for-speed-жажда-скорости-544',
  'Need for Speed: Жажда скорости',
  'Need for Speed',
  'movie',
  2014,
  './posters/need-for-speed-жажда-скорости-cbb19243.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017881fbca7f11b0da0cf93a39ec06/1344x756',
  'История Тоби Маршалла, гениального автомеханика, чьей единственной отдушиной является участие в подпольных гонках. Чтобы сохранить семейную мастерскую, Тоби вынужден взять в партнеры богатого и заносчивого бывшего гонщика IndyCar Дино Брюстера. Когда дела Тоби наконец-то начинают идти в гору, Дино подставляет партнера, и Тоби обвиняют в преступлении, которого он не совершал. Спустя два года Тоби выходит из тюрьмы с мыслью о мести. Чтобы достичь своей цели, ему придется совершить невозможное и доказать, что даже в мире броских суперкаров самый невзрачный гонщик может финишировать первым.',
  array['боевик', 'триллер', 'драма', 'криминал']::text[],
  array['США', 'Индия']::text[],
  null,
  678975,
  'tt2369135',
  6.8,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:09:04.000Z',
  false,
  false,
  false
),
(
  'media-патруль-времени-545',
  'Патруль времени',
  'Predestination',
  'movie',
  2013,
  './posters/патруль-времени-092a3ac3.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a00000160560e0a9988c4b2c026f2bb7628/1344x756',
  'Будущее. Учёные создали машину времени, и с этим изобретением мир вышел на новый виток цивилизации. Благодаря изобретению появилась возможность исправлять ошибки прошлого, чтобы сделать настоящее лучше. Главный герой — полицейский времени, который уже успел предотвратить огромное количество преступлений благодаря путешествиям во времени. И теперь ему предстоит важное задание, которое изменит его жизнь.',
  array['фантастика', 'боевик', 'драма', 'детектив']::text[],
  array['Австралия', 'США']::text[],
  null,
  679924,
  null,
  6.9,
  7.4,
  null,
  null,
  null,
  '2026-06-21T12:09:05.000Z',
  false,
  false,
  false
),
(
  'media-крым-546',
  'Крым',
  'Крым',
  'movie',
  2017,
  './posters/крым-56e15d96.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000018dc6c8f91ddb228a9975cdd2ac6c/1344x756',
  'История о любви, вере и чести, о силе духа и настоящей дружбе на фоне реальных событий крымской весны 2014 года.

Судьба свела их в Крыму близ древнего города Мангуп-Кале. Это была любовь с первого взгляда… В непростые дни исторических перемен они должны сохранить свою жизнь и любовь.',
  array['драма', 'боевик']::text[],
  array['Россия']::text[],
  null,
  984364,
  'tt6990206',
  2.8,
  3.4,
  null,
  null,
  null,
  '2026-06-21T12:09:06.000Z',
  false,
  false,
  false
),
(
  'media-6-кадров-547',
  '6 кадров',
  '6 кадров',
  'show',
  2006,
  './posters/6-кадров-754e6046.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a000001807ab6c7c169c4a0b243f9228327/1344x756',
  'Шесть актеров мастерски разыгрывают миниатюрные кинокомедии с неожиданным финалом. Похоже, они сами считают, что вся жизнь - это скетч-шоу. И пусть злые языки твердят, что придумать смешную шутку невозможно. Еще как возможно! «Шесть кадров» в каждом выпуске показывают тебе, как весело и непринужденно можно обыграть любую жизненную ситуацию.',
  array['комедия']::text[],
  array['Россия']::text[],
  null,
  422950,
  null,
  6.2,
  5.3,
  null,
  null,
  null,
  '2026-06-21T12:09:07.000Z',
  false,
  false,
  false
),
(
  'media-уральские-пельмени-548',
  'Уральские пельмени',
  'Уральские пельмени',
  'show',
  2009,
  './posters/уральские-пельмени-f0f1210f.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017674ebce2937df789e71c43a5f2c/1344x756',
  'Сборник юмористических зарисовок команды «Уральские пельмени».',
  array['концерт', 'комедия']::text[],
  array['Россия']::text[],
  null,
  573840,
  null,
  7,
  5.4,
  null,
  null,
  null,
  '2026-06-21T12:09:08.000Z',
  false,
  false,
  false
),
(
  'media-пекло-549',
  'Пекло',
  'Sunshine',
  'movie',
  2007,
  './posters/пекло-094d218f.jpg',
  './posters/пекло-094d218f.jpg',
  '2057 год. Солнце погибает, и человечество сталкивается с перспективой вымирания. Последняя надежда планеты Земля связана с «Икаром 2» - космическим кораблем, экипаж которого состоит из восьми мужчин и женщин во главе с капитаном Канэдой. Их миссия – доставить ядерный заряд, с помощью которого предполагается повторно воспламенить гаснущее Солнце.

Во время путешествия, находясь далеко в космическом безмолвии, в отсутствии какого-либо радиоконтакта с Землей экипаж слышит слабые позывные сигналы с космического корабля «Икар 1», который пропал, выполняя семь лет назад ту же самую миссию.',
  array['фантастика', 'триллер']::text[],
  array['Великобритания', 'США']::text[],
  null,
  102245,
  'tt0448134',
  7.3,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:09:09.000Z',
  false,
  false,
  false
),
(
  'media-дети-шпионов-2-остров-несбывшихся-надежд-550',
  'Дети шпионов 2: Остров несбывшихся надежд',
  'Spy Kids 2: Island of Lost Dreams',
  'movie',
  2002,
  './posters/дети-шпионов-2-остров-несбывшихся-надежд-ad2747c1.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a000001966c7a406a04467fc7eb75e048ed/1344x756',
  'Кармен и Джуни стали профессиональными агентами Организации Супершпионов, но теперь у них появились конкуренты — Гэри и Герти Гигглзы, дети нового директора ОС Донагана Гигглза. Они выполняют задания исключительно ради собственно работы, а на семейные отношения им наплевать. Примечательно то, что Гэри нравится Кармен. На новом задании им поручили найти прибор под названием «трансмукер», способный отключать любые устройства. Джуни и Кармен перенимают эту миссию, а Гэри и Герти отправляют в пустыню.',
  array['фантастика', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  null,
  842,
  'tt0287717',
  6.2,
  5.3,
  null,
  null,
  null,
  '2026-06-21T12:09:10.000Z',
  false,
  false,
  false
),
(
  'media-дети-шпионов-3-игра-окончена-551',
  'Дети шпионов 3: Игра окончена',
  'Spy Kids 3-D: Game Over',
  'movie',
  2003,
  './posters/дети-шпионов-3-игра-окончена-8082e97f.jpg',
  'https://avatars.mds.yandex.net/get-ott/13074011/2a000001966c82180d6b4f3bb379d9327c86/1344x756',
  'Несовершеннолетние агенты Джуни и Кармен Кортез отправляются на выполнение нового задания. На этот раз их ждёт путешествие по виртуальному миру 3-D видеоигры, разработанной для того, чтобы существа, населяющие её, явились в реальный мир.

Дочь шпионов Кармен попалась в сети компьютерной игры, созданной зловещим Мастером Игрушек. Теперь её брат должен спасти сестру, а заодно и весь мир. И помогать ему в этом будут родители!',
  array['фантастика', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США']::text[],
  null,
  2207,
  'tt0338459',
  5.3,
  4.4,
  null,
  null,
  null,
  '2026-06-21T12:09:11.000Z',
  false,
  false,
  false
),
(
  'media-безумно-влюбл-нный-552',
  'Безумно влюблённый',
  'Innamorato pazzo',
  'movie',
  1981,
  './posters/безумно-влюбленный-3baa8bba.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000016b2d2b8420616d34dded7334d194/1344x756',
  'Челентано - невозмутимый и грубоватый малый Барнаба Чиккини - избалованный женским вниманием водитель автобуса в Риме. Однажды он случайно встречает принцессу небольшого княжества Сан-Тулип - красавицу Кристину. Но к чему приведет эта встреча - не знают ни он, ни она...',
  array['мелодрама', 'комедия']::text[],
  array['Италия']::text[],
  11441,
  147969,
  'tt0082562',
  8.2,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:09:12.000Z',
  false,
  false,
  false
),
(
  'media-конец-го-мира-553',
  'Конец ***го мира',
  'The End of the F***ing World',
  'series',
  2017,
  './posters/конец-го-мира-33379f99.jpg',
  './posters/конец-го-мира-33379f99.jpg',
  'Джеймсу семнадцать и он уверен, что он - психопат. Алисе тоже семнадцать, она - капризная новенькая девчонка в школе. Они с пол-оборота заводят подростковый роман, и Алиса уговаривает Джеймса начать поиски её отца. Джеймсу от Алисы тоже кое-что нужно.',
  array['триллер', 'драма', 'мелодрама', 'комедия', 'криминал', 'приключения']::text[],
  array['Великобритания']::text[],
  null,
  1071384,
  null,
  7.6,
  8,
  null,
  null,
  null,
  '2026-06-21T12:09:13.000Z',
  false,
  false,
  false
),
(
  'media-дом-монстр-554',
  'Дом-монстр',
  'Monster House',
  'cartoon',
  2006,
  './posters/дом-монстр-210e157e.jpg',
  'https://avatars.mds.yandex.net/get-ott/236744/2a0000017794ed221b74faa0c01c669839c9/1344x756',
  'Три приятеля ДиДжей, Чёдер и Дженни обнаруживают, что соседний дом — самое настоящее чудовище. Никто из взрослых им не верит: мало ли, что выдумают дети. Поэтому когда начинаются неприятности, друзьям приходится самим противостоять этому монстру.',
  array['мультфильм', 'фэнтези', 'комедия', 'детектив', 'семейный']::text[],
  array['США', 'Канада']::text[],
  null,
  77601,
  'tt0385880',
  6.9,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:09:14.000Z',
  false,
  false,
  false
),
(
  'media-подводная-братва-555',
  'Подводная братва',
  'Shark Tale',
  'cartoon',
  2004,
  './posters/подводная-братва-2bfdc9e9.jpg',
  'https://avatars.mds.yandex.net/get-ott/374297/2a0000017c9e070746efad36172018a604f5/1344x756',
  'Весь подводный мир встревожен убийством сына главы главной преступной группировки акул и, надо же, на месте преступления находят ничего не подозревающего малька Оскара! Всю жизнь Оскар мечтал быть сильным и знаменитым, и вот ему представился прекрасный случай воспользоваться чужой славой. Но очень скоро Оскар понимает, что он натворил, взяв титул «Гроза Акул»…',
  array['мультфильм', 'триллер', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Франция']::text[],
  10555,
  7464,
  'tt0307453',
  6.8,
  6,
  null,
  null,
  null,
  '2026-06-21T12:09:15.000Z',
  false,
  false,
  false
),
(
  'media-вход-в-пустоту-556',
  'Вход в пустоту',
  'Enter the Void',
  'movie',
  2009,
  './posters/вход-в-пустоту-b9ea6bf9.jpg',
  './posters/вход-в-пустоту-b9ea6bf9.jpg',
  '20-летний Оскар и его 18-летняя сестра Линда живут в Токио. Пока Оскар торгует наркотиками, Линда танцует стриптиз в ночном клубе. Однажды во время полицейской облавы Оскар получает пулю и, пока он умирает, его душа, верная обещанию никогда не оставлять сестру, отказывается покидать этот мир. Беспокойный дух парня скитается по городу, преследуемый невыносимыми кошмарами и галлюцинациями.',
  array['фэнтези', 'драма']::text[],
  array['Франция', 'Германия', 'Италия', 'Канада', 'Япония']::text[],
  null,
  406477,
  'tt1191111',
  7.2,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:09:16.000Z',
  false,
  false,
  false
),
(
  'media-три-икса-557',
  'Три икса',
  'xXx',
  'movie',
  2002,
  './posters/три-икса-b1e6d603.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a0000016bdb2fb3eeae49a3db44eda7e5ad/1344x756',
  'Ксандер Кэйдж - спортсмен-экстремал. Этот жестокий парень может сделать то, что не в силах выполнить другие люди. Именно поэтому он привлекает к себе внимание Управления национальной безопасности. Оснащенный самой современной шпионской техникой и оружием, Кэйдж должен просочиться в русские криминальные круги, находящиеся в Праге. Ничто не может помешать этому шпиону нового поколения ХХХ выполнить свою миссию, и никто не способен отвлечь его от задания. Никто, кроме таинственной девушки Елены...',
  array['боевик', 'триллер', 'приключения']::text[],
  array['США']::text[],
  null,
  6032,
  'tt0295701',
  7.1,
  5.9,
  null,
  null,
  null,
  '2026-06-21T12:09:17.000Z',
  false,
  false,
  false
),
(
  'media-тролли-мировой-тур-558',
  'Тролли. Мировой тур',
  'Trolls World Tour',
  'cartoon',
  2020,
  './posters/тролли-мировой-тур-b9fb79a3.jpg',
  'https://avatars.mds.yandex.net/get-ott/212840/2a000001712fbf4089f2de6f3b7ca63468eb/1344x756',
  'Поп-тролли в шоке – оказывается, мир музыки гораздо больше, чем они думали. Рейвы, оупен-эйры, классические концерты и, конечно, хип-хоп баттлы – впереди их ждет головокружительное веселье. Но неудержимая королева Рокс планирует уничтожить всё, чтобы миром безоговорочно правил хард-рок! Розочка, Цветан и их новые друзья отправляются в невероятное путешествие: им предстоит объединить всех троллей и помешать Рокс.',
  array['мультфильм', 'мюзикл', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Япония']::text[],
  446893,
  1040592,
  'tt6587640',
  6.9,
  6.1,
  null,
  null,
  null,
  '2026-06-21T12:09:18.000Z',
  false,
  false,
  false
),
(
  'media-лара-крофт-расхитительница-гробниц-559',
  'Лара Крофт: Расхитительница гробниц',
  'Lara Croft: Tomb Raider',
  'movie',
  2001,
  './posters/лара-крофт-расхитительница-гробниц-4d26fbfc.jpg',
  'https://avatars.mds.yandex.net/get-ott/12808873/2a0000019667800c0efa37e425f8c8f41331/1344x756',
  'Она владеет всеми видами рукопашного боя, стреляет из любых видов оружия и способна выжить в самых экстремальных условиях. Эта девушка - дочь лорда, леди Лара Крофт. Однажды под лестницей она обнаруживает вход в секретную комнату, где находятся старинные часы с удивительным устройством внутри.',
  array['фэнтези', 'боевик', 'триллер', 'приключения']::text[],
  array['Великобритания', 'Германия', 'Япония', 'США']::text[],
  1995,
  3950,
  'tt0146316',
  6.8,
  5.8,
  null,
  null,
  null,
  '2026-06-21T12:09:19.000Z',
  false,
  false,
  false
),
(
  'media-лара-крофт-колыбель-жизни-560',
  'Лара Крофт: Расхитительница гробниц 2 – Колыбель жизни',
  'Lara Croft Tomb Raider: The Cradle of Life',
  'movie',
  2003,
  './posters/лара-крофт-расхитительница-гробниц-2-колыбель-жизни-2af64efb.jpg',
  'https://avatars.mds.yandex.net/get-ott/2385704/2a00000196677e3e13591a203891c89bb80e/1344x756',
  'Лара противостоит злодею-интеллектуалу Чену Ло, возглавляющему триаду. Этот искусный бизнесмен и хладнокровный убийца задумал использовать Ящик Пандоры — оружие Судного дня.',
  array['фэнтези', 'боевик', 'приключения']::text[],
  array['США', 'Германия', 'Япония', 'Великобритания', 'Гонконг']::text[],
  null,
  7988,
  'tt0325703',
  6.6,
  5.5,
  null,
  null,
  null,
  '2026-06-21T12:09:20.000Z',
  false,
  false,
  false
),
(
  'media-союз-спасения-561',
  'Союз Спасения',
  'Союз Спасения',
  'movie',
  2019,
  './posters/союз-спасения-22fd7fa1.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a000001703e7623e7505f28ed9d502291da/1344x756',
  'Императорские войска России заняли Париж. Россия стала первой державой мира. Теперь всё кажется возможным. Молодые победители, гвардейские офицеры, уверены, что равенство и свобода наступят - здесь и сейчас. Ради этого они готовы принести в жертву всё — положение, богатство, любовь, жизнь… и саму страну.',
  array['история', 'биография', 'драма']::text[],
  array['Россия']::text[],
  null,
  972740,
  'tt8769146',
  5.9,
  5.2,
  null,
  null,
  null,
  '2026-06-21T12:09:21.000Z',
  false,
  false,
  false
),
(
  'media-наруто-562',
  'Наруто',
  'Naruto',
  'anime',
  2002,
  './posters/наруто-9cf04ceb.jpg',
  './posters/наруто-9cf04ceb.jpg',
  'Однажды демон напал на деревню, в которой родился мальчик Наруто. Чтобы остановить демона, глава поселения запечатал его внутри Наруто, но сам при этом погиб. Прошли годы. Наруто вырос, став неусидчивым и пока неопытным ниндзя. Он всё ещё живёт в родной деревне, где его не принимают и стараются избегать.',
  array['аниме', 'мультфильм', 'фэнтези', 'боевик', 'комедия']::text[],
  array['Япония']::text[],
  46260,
  283290,
  'tt0409591',
  7.8,
  8.4,
  null,
  null,
  null,
  '2026-06-21T12:09:22.000Z',
  false,
  false,
  false
),
(
  'media-корабль-призрак-563',
  'Корабль-призрак',
  'Ghost Ship',
  'movie',
  2002,
  './posters/корабль-призрак-b8f555e0.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a00000168a8cf6a23b585ca83bc3b326d12/1344x756',
  'В отдаленной части Берингова моря команда спасателей обнаруживает огромный пассажирский лайнер, который считается пропавшим уже 40 лет. Оказавшись на борту судна, спасатели сталкиваются с жутким прошлым корабля и теперь им придётся вступить в смертельную схватку с неизвестным.',
  array['ужасы', 'триллер', 'детектив']::text[],
  array['США', 'Австралия']::text[],
  null,
  5901,
  'tt0288477',
  7,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:09:23.000Z',
  false,
  false,
  false
),
(
  'media-босс-молокосос-снова-в-деле-564',
  'Босс-молокосос: Снова в деле',
  'The Boss Baby: Back in Business',
  'series',
  2018,
  './posters/босс-молокосос-снова-в-деле-dbde5c0f.jpg',
  'https://avatars.mds.yandex.net/get-ott/200035/2a0000018388cbd379c9c7a01b1737545695/1344x756',
  'Босс-молокосос возвращается в новых миссиях, где детский мир снова оказывается связан с корпоративными интригами и борьбой за любовь малышей.',
  array['мультфильм', 'комедия', 'семейный']::text[],
  array['США']::text[],
  null,
  null,
  null,
  6.6,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:09:24.000Z',
  false,
  false,
  false
),
(
  'media-человек-бензопила-фильм-история-резе-565',
  'Человек-бензопила. Фильм: История Резе',
  'Chainsaw Man Movie: Reze-hen',
  'movie',
  2025,
  './posters/человек-бензопила-фильм-история-резе-b1f6350e.jpg',
  'https://avatars.mds.yandex.net/get-ott/17669062/2a0000019b69c0cd5597756e8453956c19da/1344x756',
  'Сходив на свидание с Макимой, Дэндзи решает, что отныне его сердце будет принадлежать только ей. Но парень попадает под дождь и прячется в телефонной будке, где укрывается и милая девушка Резе. Она улыбается и приглашает Дэндзи в кафе, где сама работает, а тот, не в силах сопротивляться, начинает заходить туда каждый день и всё больше влюбляется в новую знакомую.',
  array['аниме', 'мультфильм', 'фэнтези', 'боевик']::text[],
  array['Япония']::text[],
  1218925,
  5430477,
  'tt30472557',
  8.3,
  8.3,
  null,
  null,
  null,
  '2026-06-21T12:09:25.000Z',
  false,
  false,
  false
),
(
  'media-сезон-охоты-566',
  'Сезон охоты',
  'Open Season',
  'movie',
  2006,
  './posters/сезон-охоты-3329ed9c.jpg',
  'https://avatars.mds.yandex.net/get-ott/2419418/2a000001776d1083943eceaeb34ff26b492f/1344x756',
  'Выросший среди людей гризли оказывается в лесу в самый разгар охотничьего сезона. И пока ухаживавший за медведем егерь ищет своего воспитанника, гризли водит дружбу с лесным оленем.',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Канада', 'Франция']::text[],
  null,
  77465,
  'tt0400717',
  7.1,
  6.1,
  null,
  null,
  null,
  '2026-06-21T12:09:26.000Z',
  false,
  false,
  false
),
(
  'media-токийский-гуль-567',
  'Токийский гуль',
  'Tokyo Ghoul',
  'anime',
  2014,
  './posters/токийский-гуль-0dd374fe.jpg',
  'https://avatars.mds.yandex.net/get-ott/1531675/2a0000017573e8a693aec9b41c469a842154/1344x756',
  'С обычным студентом Кэном Канэки случается беда, парень попадает в больницу. Но на этом неприятности не заканчиваются: ему пересаживают органы гулей – существ, поедающих плоть людей. После злосчастной операции Канэки становится одним из чудовищ, пытается стать своим, но для людей он теперь изгой, обреченный на уничтожение.',
  array['аниме', 'мультфильм', 'ужасы', 'фэнтези', 'боевик', 'триллер', 'драма']::text[],
  array['Япония']::text[],
  null,
  841681,
  null,
  7.1,
  7.7,
  null,
  null,
  null,
  '2026-06-21T12:09:27.000Z',
  false,
  false,
  false
),
(
  'media-14-568',
  '14+',
  '14+',
  'movie',
  2015,
  './posters/14-a4238632.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001612cc1d7548d7798d701efe389cc/1344x756',
  'Это история первой любви в 14 лет, разворачивающаяся в спальном районе большого города. История современных Ромео и Джульетты, живущих и в социальных сетях, и на улице с их жесткими правилами и законами, по которым влюбленные не хотят и не будут играть. 

Чтобы быть вместе, главным героям придется преодолеть многое: их школы враждуют между собой, поэтому их любовь – предательство своего района и своей компании. Родители не готовы принять тот факт, что дети стремительно взрослеют и что каждый подросток – личность, имеющий право выбора. Им мешают собственные комплексы и неуверенность в себе, боязнь сделать первый шаг и страх, что тебя не любят и оттолкнут. Но, несмотря на все эти препятствия, настоящая любовь победит всё.',
  array['мелодрама']::text[],
  array['Россия']::text[],
  null,
  781432,
  'tt4427076',
  6.7,
  6.9,
  null,
  null,
  null,
  '2026-06-21T12:09:28.000Z',
  false,
  false,
  false
),
(
  'media-защитники-569',
  'Защитники',
  'Защитники',
  'movie',
  2017,
  './posters/защитники-e7cc3d73.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000160561197127d3820dde51e5bf76d/1344x756',
  'Во времена Холодной войны секретная организация «Патриот» создала отряд супергероев, в который вошли участники из различных советских республик. Долгие годы героям приходилось скрывать свою сущность, однако в тяжелые для страны времена они вновь берутся за дело.',
  array['фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array['Россия']::text[],
  354556,
  841813,
  'tt4600952',
  2.8,
  4.1,
  null,
  null,
  null,
  '2026-06-21T12:09:29.000Z',
  false,
  false,
  false
),
(
  'media-третий-лишний-2-570',
  'Третий лишний 2',
  'Ted 2',
  'movie',
  2015,
  './posters/третий-лишний-2-c4a6d743.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001616b987413bd9dccc233a48f058d/1344x756',
  'Плюшевый беспредельщик продолжает сводить с ума чопорную интеллигенцию. На этот раз говорящий медвежонок с большим эго и его длинноногая блондинка решили завести ребёнка. Там, где не постаралась природа, на помощь приходят звёздные доноры, городские сумасшедшие и верный друг Джонни. Он только что расстался с Лори и готов к новым авантюрам. Третий здесь совсем не лишний.',
  array['комедия']::text[],
  array['США']::text[],
  null,
  731691,
  'tt2637276',
  6,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:09:30.000Z',
  false,
  false,
  false
),
(
  'media-три-метра-над-уровнем-неба-571',
  'Три метра над уровнем неба',
  '3 metros sobre el cielo',
  'movie',
  2010,
  './posters/три-метра-над-уровнем-неба-cc5b15c3.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a000001711ba8c8c1e8100d554ab1678960/1344x756',
  'История двух молодых людей, которые принадлежат к разным мирам. Баби ― богатая девушка, которая отображает доброту и невинность. Аче — мятежный мальчик, импульсивный, бессознательный, склонный к риску и опасности. Это маловероятно, практически невозможно, но их встреча неизбежна, и в этом неистовом путешествии между ними возникает первая большая любовь.',
  array['драма', 'мелодрама']::text[],
  array['Испания']::text[],
  61979,
  523362,
  'tt1648216',
  7.1,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:09:31.000Z',
  false,
  false,
  false
),
(
  'media-убойный-футбол-572',
  'Убойный футбол',
  'Siu Lam juk kau',
  'movie',
  2001,
  './posters/убойный-футбол-917225ec.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a0000016249b91d3a589dfbe0108a985d99/1344x756',
  'Шесть братьев под руководством опытного футбольного тренера создают непобедимую команду. Объединив неоценимый опыт древних мастеров кунг-фу и современную тактику футбольного боя, они подают заявку на участие в престижном футбольном первенстве с призом в миллион долларов.',
  array['комедия', 'спорт', 'боевик']::text[],
  array['Гонконг', 'Китай']::text[],
  null,
  833,
  'tt0286112',
  7,
  7.3,
  null,
  null,
  null,
  '2026-06-21T12:09:32.000Z',
  false,
  false,
  false
),
(
  'media-бой-с-тенью-573',
  'Бой с тенью',
  'Бой с тенью',
  'movie',
  2005,
  './posters/бой-с-тенью-c53736f7.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000194525a6b3f974af84b83d9114c45/1344x756',
  'Артем Колчин - всемирно известный боксер по прозвищу Большая Белая Надежда, претендент на чемпионский титул. Однако серьезная проблема со зрением свела на нет его шансы на победу в решающем бою. Артем ослеп и стал изгоем без былой поддержки и финансирования. Чтобы вернуться в спорт, ему придется за считанные дни найти деньги на дорогостоящую операцию, спасти любимую девушку от наемных убийц и поквитаться с предателями.',
  array['боевик', 'спорт', 'драма']::text[],
  array['Россия']::text[],
  null,
  81485,
  'tt0415678',
  6.6,
  5.8,
  null,
  null,
  null,
  '2026-06-21T12:09:33.000Z',
  false,
  false,
  false
),
(
  'media-бой-с-тенью-2-реванш-574',
  'Бой с тенью 2: Реванш',
  'Бой с тенью 2: Реванш',
  'movie',
  2007,
  './posters/бой-с-тенью-2-реванш-548e706a.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017800f3adf968380853206a2bc1a4/1344x756',
  'Артем Колчин снова на ринге. Финальный бой за чемпионский титул он проведет в Америке. Но перед финалом ему предстоит бой с мексиканским боксером, который также рвется к вершине. После тяжелого поединка мексиканец  умирает в госпитале. Но неприятности только начинаются.

Артему сообщают, что он забил на ринге приемного сына мексиканского наркобарона, который вынес суровый приговор: кровь Вики за кровь погибшего сына. На свете есть только один человек, который может помочь Колчину выпутаться из опасной переделки, и он - заклятый враг Артема…',
  array['боевик', 'драма', 'криминал', 'спорт']::text[],
  array['Россия']::text[],
  null,
  105013,
  'tt0459759',
  6.3,
  5.7,
  null,
  null,
  null,
  '2026-06-21T12:09:34.000Z',
  false,
  false,
  false
),
(
  'media-бой-с-тенью-3d-последний-раунд-575',
  'Бой с тенью 3D: Последний раунд',
  'Бой с тенью 3D: Последний раунд',
  'movie',
  2011,
  './posters/бой-с-тенью-3d-последний-раунд-ccd04ec3.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a000001612cabb50533831483dd59c54962/1344x756',
  'Боксер-чемпион Артем Колчин достиг всех мыслимых вершин и стал национальным героем. На ринг он больше не выходит, предпочитая тренерскую работу и бурную светскую жизнь с постоянным мельканием на ТВ, тусовками и интрижками. А дома его по утрам встречают дочка и жена Вика, которая больше не узнает в нем того Артема, которого она когда-то полюбила...

Внешне благополучному существованию Колчина приходит конец, когда его подопечного чуть не убивает в бою знаменитый Куэрте. Артем подозревает, что тот использует некую запрещенную технику. За доказательствами ему приходится отправиться в Гонконг, где его ждет старый знакомый Вагит Валиев: он знает о Куэрте то, чего не знает больше ни один человек. Чтобы победить, Артему придется снова выйти на ринг.',
  array['боевик', 'драма', 'криминал', 'приключения']::text[],
  array['Россия']::text[],
  null,
  541055,
  null,
  6.4,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:09:35.000Z',
  false,
  false,
  false
),
(
  'media-доказательство-смерти-576',
  'Доказательство смерти',
  'Death Proof',
  'movie',
  2007,
  './posters/доказательство-смерти-f9f04ec7.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000017f9312f2abda2d2a88a73cf258be/1344x756',
  'Три чирлидерши решают развлечься и отправляются в местную забегаловку. Они даже не подозревают, что на этот раз их ждет особый интерес со стороны мужчин. Каскадер Майк украдкой наблюдает за ними из своего автомобиля. Он любит быструю езду и не прочь подбросить до дома симпатичную девушку. А ещё он считает, что выйдет сухим из воды даже в случае самой ужасной аварии.',
  array['боевик', 'триллер', 'криминал']::text[],
  array['США']::text[],
  null,
  273064,
  null,
  7.1,
  7,
  null,
  null,
  null,
  '2026-06-21T12:09:36.000Z',
  false,
  false,
  false
),
(
  'media-орел-и-решка-577',
  'Орёл и решка',
  'Орёл и решка',
  'show',
  2011,
  './posters/орел-и-решка-5cd58d30.jpg',
  'https://image.tmdb.org/t/p/w1280/9kiO9qStPaLUVjecS7W1ZNYoD8M.jpg',
  'Каждые выходные двое ведущих отправляются в различные города мира. По правилам программы один ведущий должен прожить субботу и воскресенье на 100 долларов, а второй может тратить неограниченные средства, которые хранятся на золотой карте. Чтобы решить, кто из них будет жить как миллионер, а кто будет учиться выживанию, ведущие перед каждым путешествиям бросают монету, и каждый раз всё решает Орёл или решка.',
  array['реальное ТВ']::text[],
  array['Украина']::text[],
  123454,
  762373,
  'tt4553720',
  8.3,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:09:37.000Z',
  false,
  false,
  false
),
(
  'media-мир-наизнанку-578',
  'Мир наизнанку',
  'Мир наизнанку',
  'show',
  2010,
  './posters/мир-наизнанку-0d9b93cd.jpg',
  './posters/мир-наизнанку-0d9b93cd.jpg',
  'Каждый сезон программы «Мира наизнанку» представляет собой серию программ, посвященных одной отдельно взятой стране либо одному географическому региону. Программа посвящена жизни, работе, отдыху, привычкам, обрядам и национальным особенностям народов различных стран. Путешествие по одной стране или региону продолжается по несколько месяцев, в ходе которых съемочная группа пытается показать зрителю не туристические стороны данного региона,а самобытную культуру местного населения.',
  array['реальное ТВ', 'документальный']::text[],
  array['Украина']::text[],
  null,
  893442,
  'tt4606670',
  8.5,
  8.7,
  null,
  null,
  null,
  '2026-06-21T12:09:38.000Z',
  false,
  false,
  false
),
(
  'media-связь-579',
  'Связь',
  'Coherence',
  'movie',
  2012,
  './posters/связь-31d91ff7.jpg',
  './posters/связь-31d91ff7.jpg',
  'В ночь падения кометы на званом ужине собираются восемь друзей. Наслаждаясь общением, они даже не подозревают о грядущих',
  array['фантастика', 'триллер', 'драма']::text[],
  array['США', 'Великобритания']::text[],
  null,
  760829,
  'tt2866360',
  7,
  7.2,
  null,
  null,
  null,
  '2026-06-21T12:09:39.000Z',
  false,
  false,
  false
),
(
  'media-масяня-580',
  'Масяня',
  'Масяня',
  'series',
  2001,
  './posters/масяня-dbbb5343.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a0000017159e60f17ced338123214650aa1/1344x756',
  'Масяня — это обычная, особо не блещущая талантами и красотой девица, живущая той же обычной жизнью, что и мы все, собравшиеся в этой стране, да и не только в ней… Пьет пиво, болтается по злачным местам, встречается с приятелями и всячески пытается получить как можно больше удовольствия от жизни… Единственное, что ее отличает, — это непобедимый пофигизм и жизнелюбие…',
  array['мультфильм', 'комедия']::text[],
  array['Россия']::text[],
  null,
  447446,
  'tt16607152',
  6.2,
  null,
  null,
  null,
  null,
  '2026-06-21T12:09:40.000Z',
  false,
  false,
  false
),
(
  'media-утиные-истории-581',
  'Утиные истории',
  'DuckTales',
  'series',
  1987,
  './posters/утиные-истории-64510d59.jpg',
  'https://avatars.mds.yandex.net/get-ott/2439731/2a00000171a1761937e19c8ba385704d4b8d/1344x756',
  'Скрудж Макдак и его племянники Билли, Вилли и Дилли попадают в захватывающие приключения, из которых всегда с честью выбираются.',
  array['мультфильм', 'комедия', 'детский', 'приключения', 'семейный']::text[],
  array['США', 'Тайвань', 'Япония']::text[],
  720,
  81426,
  'tt0092345',
  7.9,
  8,
  null,
  null,
  null,
  '2026-06-21T12:09:41.000Z',
  false,
  false,
  false
),
(
  'media-ангелы-чарли-582',
  'Ангелы Чарли',
  'Charlie''s Angels',
  'movie',
  2000,
  './posters/ангелы-чарли-c4df52cf.jpg',
  'https://avatars.mds.yandex.net/get-ott/223007/2a0000016e69cd47fd3fd2b97f58cda7e823/1344x756',
  'Ангелы Чарли - суперсекретное элитное спецподразделение, которому доверяют только невыполнимые задания. С помощью новейших технологий, современного оружия, боевых искусств и убийственного женского очарования Ангелы Чарли творят чудеса. Но настоящим испытанием станет лишь это последнее задание...',
  array['боевик', 'триллер', 'комедия', 'криминал', 'приключения']::text[],
  array['Германия', 'США']::text[],
  null,
  633,
  'tt0160127',
  6.6,
  5.7,
  null,
  null,
  null,
  '2026-06-21T12:09:42.000Z',
  false,
  false,
  false
),
(
  'media-кик-бутовски-583',
  'Сорвиголова Кик Бутовски',
  'Kick Buttowski: Suburban Daredevil',
  'cartoon',
  2010,
  './posters/сорвиголова-кик-бутовски-3991f5c4.jpg',
  './posters/сорвиголова-кик-бутовски-3991f5c4.jpg',
  'Кик Бутовски — очень необычный ребенок, живущий в самом обычном городе. Он — великий борец с обыденностью, который стрем',
  array['мультфильм', 'боевик', 'комедия']::text[],
  array['США', 'Канада']::text[],
  17572,
  532800,
  'tt1600757',
  6.9,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:09:43.000Z',
  false,
  false,
  false
),
(
  'media-черный-плащ-584',
  'Черный Плащ',
  'Darkwing Duck',
  'series',
  1991,
  './posters/черный-плащ-5ba34128.jpg',
  './posters/черный-плащ-5ba34128.jpg',
  'Мультсериал, повествующий о приключениях загадочного супергероя-селезня по прозвищу Чёрный Плащ, который ведет борьбу со злодеями, представляющими смертельную угрозу для города Сен-Канар. В этом нелёгком деле ему помогают горе-пилот и механик Зигзаг МакКряк и непоседливая Гусёна.',
  array['мультфильм', 'приключения', 'комедия', 'детектив', 'семейный', 'детский']::text[],
  array['США', 'Корея Южная', 'Австралия', 'Франция', 'Япония', 'Тайвань']::text[],
  3319,
  395106,
  'tt0101076',
  7.7,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:09:44.000Z',
  false,
  false,
  false
),
(
  'media-геймеры-585',
  'Геймеры',
  'Геймеры',
  'series',
  2012,
  './posters/геймеры-8ce5ae50.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a00000180cd9a85a5bc0261c5fe55c4aa9d/1344x756',
  'Восьмисерийный сериал продолжает и развивает историю кинодилогии Павла Санаева «На игре». Кто-то из прежних героев погиб, кто-то попал в тюрьму - а значит таинственным высокопоставленным силам нужны новые идеальные солдаты для выполнения их античеловеческих планов. 

За дело готов взяться только Док. Он жив, он жаждет мести и готов на всё. Ничего не изменилось. Люди для Дока – грязь, а человеческая жизнь – ничто. Док набирает новичков, способных заменить прежний отряд, и единственный способ остановить его – собрать остатки старой команды. Эта задача выглядит неразрешимой: Вампир и Рита в тюрьме, а Ян и Комар в бегах.   У каждого персонажа свои задачи, у каждого поступка – своя предыстория. Здесь будет всё: фантастические перипетии и неожиданно светлые намерения, жестокие столкновения и безудержные страхи. Сменяя друг друга, закрутятся в штопор реальные и фантазийные ситуации. Конфликты обостряются до предела, а герои не остановятся ни перед чем. Одни – во имя смерти и победы любой ценой, другие – во имя жизни и всё еще возможного счастья.',
  array['боевик', 'приключения']::text[],
  array['Россия']::text[],
  62422,
  571334,
  'tt4216630',
  6,
  4.2,
  null,
  null,
  null,
  '2026-06-21T12:09:45.000Z',
  false,
  false,
  false
),
(
  'media-на-игре-586',
  'На игре',
  'На игре',
  'movie',
  2009,
  './posters/на-игре-908dde8b.jpg',
  './posters/на-игре-908dde8b.jpg',
  'После победы на турнире по киберспорту команда геймеров получает диски с новой игрой. Запустив ее, игроки переносят свои навыки в реальность и оказываются втянуты в опасную игру спецслужб.',
  array['боевик', 'фантастика', 'приключения']::text[],
  array['Россия']::text[],
  37656,
  424516,
  'tt1532382',
  6.1,
  5.6,
  null,
  null,
  null,
  '2026-06-21T12:09:46.000Z',
  false,
  false,
  false
),
(
  'media-на-игре-2-новый-уровень-587',
  'На игре 2. Новый уровень',
  'На игре 2. Новый уровень',
  'movie',
  2010,
  './posters/на-игре-2-новый-уровень-bbc8ff23.jpg',
  './posters/на-игре-2-новый-уровень-bbc8ff23.jpg',
  'После спецоперации команда геймеров понимает, что работает на преступный синдикат. За дисками охотятся все, а бывшие союзники начинают играть каждый за себя.',
  array['боевик', 'фантастика', 'приключения']::text[],
  array['Россия']::text[],
  null,
  451258,
  'tt1620549',
  5.8,
  5.1,
  null,
  null,
  null,
  '2026-06-21T12:09:47.000Z',
  false,
  false,
  false
),
(
  'media-асса-588',
  'Асса',
  'Асса',
  'movie',
  1987,
  './posters/асса-2582e406.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a000001794893c4e2729848cd8b19f2e332/1344x756',
  'Надеясь на яркую и безбедную жизнь, девушка связывает свою судьбу с криминальным авторитетом. Но есть и другая жизнь — свободная и с музыкой, жизнь мальчика Бананана. И две эти жизни трагически сталкиваются.',
  array['драма', 'мелодрама', 'криминал', 'музыка']::text[],
  array['СССР']::text[],
  47985,
  43545,
  'tt0094683',
  7.9,
  7.6,
  null,
  null,
  null,
  '2026-06-21T12:09:48.000Z',
  false,
  false,
  false
),
(
  'media-лесная-братва-589',
  'Лесная братва',
  'Over the Hedge',
  'cartoon',
  2006,
  './posters/лесная-братва-101cd61f.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000172c6cef61295075993746d9afd33/1344x756',
  'Медведь Винсент, угрожая съесть енота Эрджея, даёт ему всего неделю на то, чтобы вернуть уничтоженные им съестные припасы. Во время поиска еды Эрджей находит коттеджный посёлок и уговаривает других животных отправиться с ним к людям. Но первая же вылазка за едой в посёлок заканчивается неприятностями.',
  array['мультфильм', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Канада']::text[],
  null,
  6036,
  'tt0327084',
  6.8,
  6.7,
  null,
  null,
  null,
  '2026-06-21T12:09:49.000Z',
  false,
  false,
  false
),
(
  'media-смурфики-2-590',
  'Смурфики 2',
  'The Smurfs 2',
  'cartoon',
  2013,
  './posters/смурфики-2-cbb66034.jpg',
  'https://avatars.mds.yandex.net/get-ott/1672343/2a00000169e80fb121fbb77e9c25731d477c/1344x756',
  'Гаргамель создаёт Крепыша и Занозу, как две капли воды похожую на смурфиков злобную парочку, надеясь всё-таки обрести магическую силу. Когда он узнаёт, что лишь настоящий смурф может дать ему то, что он хочет, и лишь Смурфетта способна превратить Крепыша и Занозу в настоящих смурфов, Гаргамель похищает Смурфетту и берёт её с собой в Париж, планируя сделать её своей прислужницей. Но смурфики — Папа, Растяпа, Ворчун, Красавчик  — и их друзья из Нью-Йорка этого так не оставят.',
  array['мультфильм', 'фэнтези', 'комедия', 'семейный']::text[],
  array['США']::text[],
  77931,
  603191,
  'tt2017020',
  6,
  5.3,
  null,
  null,
  null,
  '2026-06-21T12:09:50.000Z',
  false,
  false,
  false
),
(
  'media-тимон-и-пумба-591',
  'Тимон и Пумба',
  'Timon & Pumbaa',
  'series',
  1995,
  './posters/тимон-и-пумба-43c8427a.jpg',
  './posters/тимон-и-пумба-43c8427a.jpg',
  'Приключения суриката Тимона и бородавочника Пумбы.',
  array['мультфильм', 'мюзикл', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Корея Южная', 'Филиппины', 'Австралия', 'Тайвань', 'Канада']::text[],
  4429,
  94672,
  'tt0112197',
  7.6,
  7,
  null,
  null,
  null,
  '2026-06-21T12:09:51.000Z',
  false,
  false,
  false
),
(
  'media-моана-2-592',
  'Моана 2',
  'Moana 2',
  'cartoon',
  2024,
  './posters/моана-2-482e56fe.jpg',
  'https://avatars.mds.yandex.net/get-ott/1652588/2a00000196d37cb40c8b71eede5b47509892/1344x756',
  'Получив вызов от предков-искателей, Моана и Мауи отправляются в далёкие и опасные воды Океании.',
  array['мультфильм', 'мюзикл', 'фэнтези', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Канада']::text[],
  1241982,
  4324228,
  'tt13622970',
  6.8,
  6.3,
  null,
  null,
  null,
  '2026-06-21T12:09:52.000Z',
  false,
  false,
  false
),
(
  'media-мастера-меча-онлайн-593',
  'Мастера меча онлайн',
  'Sword Art Online',
  'anime',
  2012,
  './posters/мастера-меча-онлайн-3c9627b2.jpg',
  './posters/мастера-меча-онлайн-3c9627b2.jpg',
  'Опытному геймеру Кирито повезло поучаствовать в бета-тестировании самой ожидаемой компьютерной игры нового поколения - Sword Art Online. Когда наконец на прилавках появились диски с финальной версией, тысячи геймеров устремились в совершенный виртуальный мир MMORPG. Там их ждал неприятный сюрприз - гейм-мастер объявил, что выйти из игры по собственной воле невозможно. Единственный шанс это сделать - пройти все сто уровней до конца. А смерть в игре означает смерть и в реальной жизни.',
  array['аниме', 'мультфильм', 'фантастика', 'фэнтези', 'боевик', 'приключения']::text[],
  array['Япония', 'Польша', 'США']::text[],
  45782,
  691223,
  'tt2250192',
  7.9,
  7.5,
  null,
  null,
  null,
  '2026-06-21T12:09:53.000Z',
  false,
  false,
  false
),
(
  'media-черепашки-ниндзя-2003-594',
  'Мутанты черепашки ниндзя. Новые приключения!',
  'Teenage Mutant Ninja Turtles',
  'cartoon',
  2003,
  './posters/мутанты-черепашки-ниндзя-новые-приключения-8f556cc0.jpg',
  './posters/мутанты-черепашки-ниндзя-новые-приключения-8f556cc0.jpg',
  'Черепашки-ниндзя мешают планам таинственного клана, главой которого является их заклятый враг Шреддер. Новые приключения приведут героев в космос, феодальную Японию и киберпространство.',
  array['мультфильм', 'фантастика', 'боевик', 'комедия', 'приключения', 'семейный']::text[],
  array['США', 'Корея Южная']::text[],
  null,
  421422,
  null,
  8.1,
  7.9,
  null,
  null,
  null,
  '2026-06-21T12:09:54.000Z',
  false,
  false,
  false
),
(
  'media-мученицы-595',
  'Мученицы',
  'Martyrs',
  'movie',
  2008,
  './posters/мученицы-eb614a75.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a00000161288ab5a99bdc9d1b1fc9ac4c0d/1344x756',
  'Франция, начало 70-х. Маленькая Люси, почти год считавшаяся пропавшей без вести, обнаружена бредущей по проселочной дороге. Ребёнок находится в состоянии сильного шока и не может рассказать о случившемся. Несколько лет спустя девушка с сильным постравматическим синдромом в компании подруги вламывается в загородный дом респектабельной пары среднего возраста, живущей с двумя детьми-подростками. Кажется, у девушек с этим семейством давние счёты.',
  array['ужасы']::text[],
  array['Франция', 'Канада']::text[],
  null,
  313524,
  'tt1029234',
  6.4,
  7,
  null,
  null,
  null,
  '2026-06-21T12:09:55.000Z',
  false,
  false,
  false
),
(
  'media-к-9-собачья-работа-596',
  'К-9: Собачья работа',
  'K-9',
  'movie',
  1989,
  './posters/к-9-собачья-работа-1295b952.jpg',
  'https://avatars.mds.yandex.net/get-ott/224348/2a00000162251dae69a66b7331dbf83f2191/1344x756',
  'Веселый полицейский-выдумщик Майкл Дули попал в беду: его пытается убить известный наркодилер за то, что тот все время вмешивается в бизнес. Друг полицейского дает ему собаку по имени Джерри Ли, специально натасканную на запах наркотиков. С помощью пса Майкл надеется поймать преступника и разоблачить его, но всё не так просто. 

У Джерри Ли свой характер и он работает только когда пожелает. К тому же собака принимает радостное участие в поломке машины Майкла, разгроме дома и его личной жизни.',
  array['боевик', 'комедия', 'криминал']::text[],
  array['США']::text[],
  10345,
  7376,
  'tt0097637',
  7.7,
  6.1,
  null,
  null,
  null,
  '2026-06-21T12:09:56.000Z',
  false,
  false,
  false
),
(
  'media-турбо-597',
  'Турбо',
  'Turbo',
  'cartoon',
  2013,
  './posters/турбо-c8e002b8.jpg',
  'https://avatars.mds.yandex.net/get-ott/239697/2a0000016b930c79b2b108016a74719da553/1344x756',
  'Ничто так не раздражает темпераментную, мечтающую о скорости улитку с говорящим именем Турбо, как неспешная жизнь, медленные разговоры, заторможенные соседи. Пусть наш герой выглядит большим чудаком в этом вялом, неторопливом мире, пусть он идёт вопреки закону природы о «рождённом ползать», но если в сердце горит «пламенный мотор», а душа верит в чудо, оно просто не имеет права не произойти!',
  array['мультфильм', 'комедия', 'приключения', 'семейный', 'спорт']::text[],
  array['США', 'Индия']::text[],
  null,
  581651,
  'tt1860353',
  6.8,
  6.4,
  null,
  null,
  null,
  '2026-06-21T12:09:57.000Z',
  false,
  false,
  false
),
(
  'media-куми-куми-598',
  'Куми-Куми',
  'Куми-Куми',
  'series',
  2012,
  './posters/куми-куми-87faeb3f.jpg',
  'https://avatars.mds.yandex.net/get-ott/1648503/2a0000016b832936b9db12fa87882b07d038/1344x756',
  'Куми-Куми — это причудливые острова с красочными ландшафтами, на которых обитают фантастические существа — племена Юми, Шуми и Джуми. Каждое племя считает своих соседей чудаками, поэтому они не ищут контактов друг с другом, хоть и говорят на одном «тарабарском» языке, интуитивно понятном каждому.  

Герои сериала — Джуга, Юси и Шумадан — представители разных племён. Несмотря на разные характеры и сложные отношения между их племенами, они очень дружны. В каждой серии кто-то из друзей попадает в переделку, выпутываться из которой приходится всем вместе. Часто ситуация осложняется ещё и тем, что Джуга и Шумадан влюблены в Юси. ',
  array['мультфильм', 'фэнтези', 'комедия']::text[],
  array['Россия']::text[],
  71968,
  584028,
  'tt2623770',
  8.3,
  8,
  null,
  null,
  null,
  '2026-06-21T12:09:58.000Z',
  false,
  false,
  false
),
(
  'media-спайдервик-хроники-599',
  'Спайдервик: Хроники',
  'The Spiderwick Chronicles',
  'movie',
  2008,
  './posters/спайдервик-хроники-23c7cad6.jpg',
  'https://avatars.mds.yandex.net/get-ott/1534341/2a0000019658fff61e39c7edd7e1cd719da6/1344x756',
  'Когда семейство Грейс переезжает в старый дом, стоящий на окраине, начинают твориться странные вещи. Пытаясь понять, в чем дело, Джаред обнаруживает дневник Артура Спайдервика и открывает невероятную правду о поместье: здесь находится скрытая от глаз таинственная страна, населенная фантастическими созданиями! Стремясь защитить свой секрет, Джаред, его сестра и брат-близнец переживают незабываемые приключения.',
  array['фэнтези', 'боевик', 'триллер', 'драма', 'приключения', 'семейный']::text[],
  array['США', 'Канада']::text[],
  null,
  81926,
  'tt0416236',
  6.8,
  6.5,
  null,
  null,
  null,
  '2026-06-21T12:09:59.000Z',
  false,
  false,
  false
),
(
  'media-закулисье-реальности-600',
  'Закулисье реальности',
  'Backrooms',
  'movie',
  2026,
  './posters/закулисье-реальности-fddfa8b4.jpg',
  './posters/закулисье-реальности-fddfa8b4.jpg',
  'Когда продавец мебели обнаруживает скрытый портал в другое измерение в подвале магазина, он оказывается в бесконечном лабиринте желтых коридоров, где пространство не подчиняется логике.',
  array['ужасы']::text[],
  array['США', 'Канада']::text[],
  null,
  5452841,
  null,
  6.8,
  null,
  null,
  null,
  null,
  '2026-06-21T12:10:00.000Z',
  false,
  false,
  false
)
on conflict (id) do update set
  title_ru = excluded.title_ru,
  title_original = excluded.title_original,
  type = excluded.type,
  year = excluded.year,
  poster_url = excluded.poster_url,
  backdrop_url = excluded.backdrop_url,
  description = excluded.description,
  genres = excluded.genres,
  countries = excluded.countries,
  tmdb_id = excluded.tmdb_id,
  kinopoisk_id = excluded.kinopoisk_id,
  imdb_id = excluded.imdb_id,
  kinopoisk_rating = excluded.kinopoisk_rating,
  imdb_rating = excluded.imdb_rating,
  my_rating = excluded.my_rating,
  my_review = excluded.my_review,
  watched_at = excluded.watched_at,
  added_at = excluded.added_at,
  is_favorite = excluded.is_favorite,
  is_top = excluded.is_top,
  rewatch = excluded.rewatch,
  updated_at = now();
